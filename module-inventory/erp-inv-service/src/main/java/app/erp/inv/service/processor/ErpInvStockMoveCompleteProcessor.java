...
/**
 * ErpInvStockMove complete per-mutation Processor（R6.4，{@code processor-extension-pattern.md} 每 mutation 一 Processor）。
 * 自包含完成编排：require → loadLines → doComplete（释放预留 + 记账 + 翻 DONE + 过账派发）→ reload。共享 protected helper
 * 单一真相源在 {@link ErpInvStockMoveProcessor}（delete-after-extract facade）。下游可经 Delta beans.xml 同名 bean id 覆盖本类。
 */
public class ErpInvStockMoveCompleteProcessor {
    ...
    public ErpInvStockMove complete(Long moveId, IServiceContext context) {
        ...
        return facade.requireMove(move.getId(), context);
    }
}
