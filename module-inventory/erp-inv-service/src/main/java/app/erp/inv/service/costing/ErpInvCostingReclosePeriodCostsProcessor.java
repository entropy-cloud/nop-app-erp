...
// 族 A/U20 豁免登记：本类为非 BizModel 服务组件（processor-extension-pattern 惯例）；daoFor 目标（ErpInvCostLayer、ErpInvStockBalance、ErpInvStockLedger、ErpInvStockMove、ErpInvStockMoveLine）=同域实体批量聚合，批量读写，写路径经编排层 Facade 事务边界承接。
/**
 * ErpInvCosting reclosePeriodCosts per-mutation Processor（R6.9，{@code processor-extension-pattern.md} 每 mutation 一 Processor）。
 *
 * <p>自包含期末成本兜底重算编排（{@code period-close.md §步骤2}）。扫描本期 DONE 的 FIFO 移动单：
 * 对成本层缺失的入库补建 {@link ErpInvCostLayer}、对 COGS 异常的出库按 FIFO 重算并刷新
 * {@link ErpInvStockLedger}。下游可经 Delta beans.xml 同名 bean id 覆盖本类。
 */
public class ErpInvCostingReclosePeriodCostsProcessor {
     ...
     */
    protected boolean recomputeIncomingLayerIfMissing(ErpInvStockMove move, ErpInvStockMoveLine line,
     ...
     */
    protected boolean recomputeOutgoingCogs(ErpInvStockMove move, ErpInvStockMoveLine line,
                                            ErpInvStockLedger ledger, String costMethod) {
     ...
     */
    protected boolean recomputeWeightedAverageOutgoing(ErpInvStockMove move, ErpInvStockMoveLine line,
    ...
    protected List<ErpInvStockMove> findDoneMovesInPeriod(LocalDate startDate, LocalDate endDate) {
        ormTemplate.flushSession();
    ...
    protected void appendLayer(ErpInvStockMove move, ErpInvStockMoveLine line, ErpInvStockLedger ledger,
