...
public interface IErpInvStockTakeBiz extends ICrudBiz<ErpInvStockTake>{
     ...
     */
    @BizMutation
    ErpInvStockTake startTake(@Name("takeId") String takeId, IServiceContext context);

    /**
     * 完成盘点：CONFIRMED→DONE。根据盘点结果生成盘盈/盘亏移动单（如配置启用）。
     */
    @BizMutation
    ErpInvStockTake completeTake(@Name("takeId") String takeId, IServiceContext context);

    /**
     * 作废盘点：DRAFT/CONFIRMED→CANCELLED。
     */
    @BizMutation
    ErpInvStockTake cancelTake(@Name("takeId") String takeId, IServiceContext context);
