 ...
 *   <li>{@link #confirm(String, IServiceContext)} / {@link #complete(String, IServiceContext)} / {@link #cancel(String, IServiceContext)}：状态机迁移。</li>
 *   <li>{@link #reverse(String, IServiceContext)}：DONE 的纠错路径——生成反向冲销移动单（非反审核）。</li>
 * </ul>
 *
...
public interface IErpInvStockMoveBiz extends ICrudBiz<ErpInvStockMove> {
     ...
     */
    @BizMutation
    ErpInvStockMove confirm(@Name("moveId") String moveId, IServiceContext context);

    /**
     * CONFIRMED → DONE。写不可变库存流水、更新余额（移动加权平均成本）、释放预留、发存货过账事件。
     */
    @BizMutation
    ErpInvStockMove complete(@Name("moveId") String moveId, IServiceContext context);

    /**
     * DRAFT/CONFIRMED → CANCELLED。若已确认占预留则释放。终态。
     */
    @BizMutation
    ErpInvStockMove cancel(@Name("moveId") String moveId, IServiceContext context);

    /**
     * 已完成移动单的纠错路径：不改原单，生成反向冲销移动单（新 DRAFT，数量取负），走正常流程到 DONE。
     */
    @BizMutation
    ErpInvStockMove reverse(@Name("moveId") String moveId, IServiceContext context);
     ...
     */
    @BizQuery
    TraceChainResult forwardTrace(@Name("moveId") String moveId, IServiceContext context);

    /**
     * 反向追溯：从指定移动单出发，沿 {@code originMoveId} 逐层上溯至根（dest→origin）。
     */
    @BizQuery
    TraceChainResult backwardTrace(@Name("moveId") String moveId, IServiceContext context);
     ...
     */
    @BizQuery
    TraceChainResult returnTrace(@Name("moveId") String moveId, IServiceContext context);
