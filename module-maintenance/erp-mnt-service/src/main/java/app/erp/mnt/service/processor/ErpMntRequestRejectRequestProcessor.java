 ...
 * 自包含 OPEN/ACCEPTED→REJECTED 编排：双源状态守卫（经 {@code ErpMntRequestStateMachine} 矩阵）+ 状态翻转 + 落库。
...
public class ErpMntRequestRejectRequestProcessor extends AbstractErpMntRequestProcessor {
    ...
    public ErpMntRequest rejectRequest(String requestId, IServiceContext context) {
    ...
    protected void doReject(ErpMntRequest request, IServiceContext context) {
        request.setStatus(stateMachine.rejectRequestTargetStatus());
