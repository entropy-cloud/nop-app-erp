...
public class ErpHrAttendanceBizModel extends CrudBizModel<ErpHrAttendance> implements IErpHrAttendanceBiz {
    ...
    public ErpHrAttendance clockIn(@Name("employeeId") Long employeeId, IServiceContext context) {
        ...
        attendance.setClockIn(CoreMetrics.currentDateTime());
    ...
    public ErpHrAttendance clockOut(@Name("employeeId") Long employeeId, IServiceContext context) {
        ...
        attendance.setClockOut(CoreMetrics.currentDateTime());
        attendance.setWorkHours(computeWorkHours(attendance.getClockIn(), attendance.getClockOut()));
        saveOrUpdateAttendance(attendance, context);
        return attendance;
    }
    ...
    @BizLoader(forType = ErpHrAttendance.class)
    public List<String> employeeDisplayName(@ContextSource List<ErpHrAttendance> rows) {
        orm().batchLoadProps(rows, Collections.singleton("employee"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpHrAttendance row : rows) {
    ...
    @BizLoader(forType = ErpHrAttendance.class)
    public List<String> leaveRequestCode(@ContextSource List<ErpHrAttendance> rows) {
    ...
    @BizLoader(forType = ErpHrAttendance.class)
    public List<String> orgName(@ContextSource List<ErpHrAttendance> rows) {
