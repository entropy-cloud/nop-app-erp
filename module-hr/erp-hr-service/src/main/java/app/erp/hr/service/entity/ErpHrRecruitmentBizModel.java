...
public class ErpHrRecruitmentBizModel extends CrudBizModel<ErpHrRecruitment> implements IErpHrRecruitmentBiz {
    ...
    @BizLoader(forType = ErpHrRecruitment.class)
    public List<String> positionName(@ContextSource List<ErpHrRecruitment> rows) {
        orm().batchLoadProps(rows, Collections.singleton("position"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpHrRecruitment row : rows) {
    ...
    @BizLoader(forType = ErpHrRecruitment.class)
    public List<String> departmentName(@ContextSource List<ErpHrRecruitment> rows) {
    ...
    @BizLoader(forType = ErpHrRecruitment.class)
    public List<String> employeeDisplayName(@ContextSource List<ErpHrRecruitment> rows) {
    ...
    @BizLoader(forType = ErpHrRecruitment.class)
    public List<String> interviewerDisplayName(@ContextSource List<ErpHrRecruitment> rows) {
        orm().batchLoadProps(rows, Collections.singleton("interviewer"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpHrRecruitment row : rows) {
            result.add(row.orm_attached() && row.getInterviewer() != null ? row.getInterviewer().getFullName() : null);
        }
        return result;
    }

    @BizLoader(forType = ErpHrRecruitment.class)
    public List<String> orgName(@ContextSource List<ErpHrRecruitment> rows) {
