...
/**
 * 冻结 {@link CoreMetrics} 时钟到 HR 日期敏感型 auto-test 快照的参考日 {@link #REFERENCE_DATE}，
 * 使依赖 {@code CoreMetrics.today()} 的快照（合同到期/考勤/胜任力发展计划/薪酬/薪酬模拟）跨日稳定，
 * 不再随系统日历翻页而漂移。
 *
 * <p><b>仅冻结日期</b>（{@code currentDate}/{@code currentDateTime}），保留 {@code currentTimeMillis}/
 * {@code nanoTime} 走真实系统时钟——这样 {@code ContextProvider} 等依赖时间单调推进的设施（上下文生命周期、
 * 审计拦截器）不受影响；快照中的时间戳列已被 auto-test 框架通配（{@code *}），日期列均由
 * {@code CoreMetrics.today()} 派生，故只需钉住日期。</p>
 *
 * <p>作用域为单个测试类：beforeAll 注册冻结时钟，afterAll 恢复系统默认时钟，不影响其它测试类。</p>
 */
public final class HrFrozenClockExtension implements BeforeAllCallback, AfterAllCallback {
