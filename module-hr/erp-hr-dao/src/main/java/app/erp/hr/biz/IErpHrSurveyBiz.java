...
public interface IErpHrSurveyBiz extends ICrudBiz<ErpHrSurvey>{
     ...
     */
    @BizMutation
    ErpHrSurvey publish(@Name("surveyId") String surveyId, IServiceContext context);

    /**
     * 截止问卷：OPEN→CLOSED，触发 ErpHrSurveyResult 自动聚合（UC-HR-11 基本流程 8，P1-MA2-041 + P1-RC-016）。
     */
    @BizMutation
    ErpHrSurvey close(@Name("surveyId") String surveyId, IServiceContext context);

    /**
     * 归档问卷：CLOSED→ARCHIVED（UC-HR-11，P1-MA2-041）。
     */
    @BizMutation
    ErpHrSurvey archive(@Name("surveyId") String surveyId, IServiceContext context);
}
