
package app.erp.log.service.entity;

import java.util.List;
import io.nop.api.core.annotations.biz.BizLoader;
import io.nop.api.core.annotations.biz.ContextSource;
import java.util.ArrayList;
import java.util.Collections;
import io.nop.api.core.annotations.biz.BizModel;
import io.nop.biz.crud.CrudBizModel;

import app.erp.log.biz.IErpLogShipmentLogBiz;
import app.erp.log.dao.entity.ErpLogShipmentLog;

@BizModel("ErpLogShipmentLog")
public class ErpLogShipmentLogBizModel extends CrudBizModel<ErpLogShipmentLog> implements IErpLogShipmentLogBiz{
    public ErpLogShipmentLogBizModel(){
        setEntityName(ErpLogShipmentLog.class.getName());
    }

    // ---------- 高价值外键名称解析（机制 D：xmeta 派生 *Name + @BizLoader 批量加载防 N+1）----------
    @BizLoader(forType = ErpLogShipmentLog.class)
    public List<String> shipmentName(@ContextSource List<ErpLogShipmentLog> rows) {
        orm().batchLoadProps(rows, Collections.singleton("shipment"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpLogShipmentLog row : rows) {
            result.add(row.orm_attached() && row.getShipment() != null ? row.getShipment().getCode() : null);
        }
        return result;
    }

    @BizLoader(forType = ErpLogShipmentLog.class)
    public List<String> orgName(@ContextSource List<ErpLogShipmentLog> rows) {
        orm().batchLoadProps(rows, Collections.singleton("org"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpLogShipmentLog row : rows) {
            result.add(row.orm_attached() && row.getOrg() != null ? row.getOrg().getName() : null);
        }
        return result;
    }

}
