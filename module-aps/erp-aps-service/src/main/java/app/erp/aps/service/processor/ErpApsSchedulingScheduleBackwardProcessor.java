...
/**
 * ErpApsScheduling scheduleBackward per-mutation Processor（R6.7，{@code processor-extension-pattern.md} 每 mutation 一 Processor）。
 * 自包含后向排产编排（拉取方案 + 调用 run）；共享 protected helper 单一真相源在
 * {@link ErpApsSchedulingProcessor}（delete-after-extract facade，类保留为 helper 持有者）。
 * 下游可经 Delta beans.xml 同名 bean id 覆盖本类。
 */
public class ErpApsSchedulingScheduleBackwardProcessor {
