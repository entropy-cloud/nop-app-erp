#!/usr/bin/env node
/**
 * sys-snapshot.mjs — System resource snapshot for mission-driver diagnostics.
 *
 * Records a point-in-time snapshot of system resource consumption to a CSV-like
 * log file, designed to be called between mission-driver steps to build a timeline
 ...
 *
 * Usage:
 *   node tools/mission-driver/src/sys-snapshot.mjs <runDir> [label]
 *
...
// Cohorts relevant to a pnpm/turbo/Vite workspace (vs the Java/Maven set in
// the upstream nop-entropy variant). Used only for monitoring labels.
const COHORT_PATTERNS = {
  opencode: /opencode/i,
  node: /(^|[\\/])node/i,
  turbo: /\bturbo\b|turbo-runner/i,
  tsc: /\btsc\b|typescript|tsgo/i,
  vite: /\bvite\b|esbuild|rollup/i,
  vitest: /\bvitest\b/i,
  playwright: /playwright|chromium|chrome/i,
...
function snapshot(runDir, label = "") {
  ...
  const csvLine = [
    ...
    snap.cohorts.turbo?.rss_mb ?? "",
    snap.cohorts.turbo?.count ?? "",
    snap.cohorts.tsc?.rss_mb ?? "",
    snap.cohorts.vite?.rss_mb ?? "",
    snap.cohorts.vitest?.rss_mb ?? "",
    snap.cohorts.playwright?.rss_mb ?? "",
  ...
  const csvHeader = "#ts\tlabel\tload1\tload5\tload15\tfreeGB\tusedGB\tappFreeGB\tcompressGB\tswapins\tswapouts\ttotalRSS_GB\tnprocs\topencodeMB\topencodeN\tnodeMB\tnodeN\tturboMB\tturboN\ttscMB\tviteMB\tvitestMB\tplaywrightMB\tideaMB\tdockerMB\tdiskAvailGB\tmemPressure\n";
  ...
  const node = snap.cohorts.node;
  const turbo = snap.cohorts.turbo;
  const kz = snap.kernelZone;
  process.stderr.write(
    `  [sysmon] ${tsLocal} load=${snap.loadAvg["1min"]} free=${snap.vm.free_GB}GB/${snap.vm.apparentFree_GB}GB ` +
    `totalRSS=${snap.totalRSS_GB}GB oc=${oc?.rss_mb}MB(${oc?.count}) node=${node?.rss_mb}MB(${node?.count}) ` +
    `turbo=${turbo?.rss_mb}MB(${turbo?.count}) ` +
