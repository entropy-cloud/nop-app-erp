
drop table erp_md_organization;
drop table erp_md_location;
drop table erp_md_employee;
drop table erp_md_currency;
drop table erp_md_subject;
drop table erp_md_material_category;
drop table erp_ast_asset_category;
drop table erp_ast_asset_model;
drop table erp_ast_asset_capitalization;
drop table erp_ast_inventory;
drop table erp_ast_asset;
drop table erp_ast_asset_action_log;
drop table erp_ast_depreciation_schedule;
drop table erp_ast_movement;
drop table erp_ast_value_adjustment;
drop table erp_ast_disposal;
drop table erp_ast_cip;
drop table erp_ast_split;
drop table erp_ast_merge;
drop table erp_ast_maintenance;
drop table erp_ast_inventory_line;
drop table erp_ast_cip_cost_item;
drop table erp_ast_cip_progress_billing;
drop table erp_ast_split_line;
drop table erp_ast_merge_line;
drop table erp_ast_maintenance_cost;

