...
// 族 A/U20 豁免登记：本类为非 BizModel 服务组件（processor-extension-pattern 惯例）；daoFor 目标（ErpAstMaintenance）=同域实体批量聚合，只读批量聚合，逐条 I*Biz 管道不适用批量场景。
/**
...
public class ErpAstMaintenanceApproveProcessor extends AbstractApproveProcessor<ErpAstMaintenance> {
     ...
     * ERR_AST_MAINTENANCE_ILLEGAL_STATUS_TRANSITION（参数形态与 facade 组装一致）。
     */
    @Override
    protected NopException illegalStatusException(ErpAstMaintenance entity, String current, String... expected) {
        return new NopException(ErpAstErrors.ERR_AST_MAINTENANCE_ILLEGAL_STATUS_TRANSITION)
                .param(ErpAstErrors.ARG_MAINTENANCE_CODE, entity.getCode())
