...
/**
 * 资产处置单（{@code ErpAstDisposal}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 * <p>权威契约：{@code docs/architecture/entity-state-machine-bean.md}；
 * 业务语义：{@code docs/design/assets/state-machine.md §适用对象三：资产价值调整文档双轴}（Disposal 双轴节）。
 *
 * <p>严格无状态（契约 §2）：不注入 DAO/IBiz/IServiceContext/事务，只接收状态值。
 *
 * <p><strong>非退化轴（layer-2 四方对照裁定，plan Phase 1 Decision (B)）</strong>：dict {@code erp/doc-status} 含 3 值
 * （DRAFT/ACTIVE/CANCELLED）。Disposal 的 docStatus 有 1 个生产命名动作 writer：
 * {@code ErpAstDisposalProcessor.executeApprove:93} 写 {@code ACTIVE}（approve 是唯一命名 writer，非 Movement 式退化轴）。
 * 故 {@link #transitions()} 含 1 条边：approve(DRAFT→ACTIVE)。{@code CANCELLED} 经 {@code useLogicalDelete}
 * 既有路径承载（实体 {@code useLogicalDelete="true"} 实仓核实，owner doc「实现模式与守卫边界」已声明），无独立 cancel mutation。
 *
 * <p><strong>守卫接线</strong>：{@code ErpAstDisposalProcessor.validateTransitionForCancel}（doc-cancelled 守卫，
 * 供 approve 前置 {@code validateNotCancelled} 与 5 个审批动作共用）改调 {@link #isCancelled(String)} 只读守卫；
 ...
 * {@code useLogicalDelete} 可达）纳入 {@link #terminalStatuses()}；初始态 {@code DRAFT}（CRUD 创建写入）。
 *
 * <p><strong>reverseApprove 不写 docStatus</strong>：Disposal {@code executeReverseApprove} 仅写
 * approveStatus=REJECTED，docStatus 保持 ACTIVE（与 Capitalization 特例边 reverseApprove→CANCELLED 不同——本 Bean 无此边）。
 */
public class ErpAstDisposalDocumentStateMachine {
    ...
    /**
     * 业务终态判定：{@code ACTIVE}（approve 后无出边）或 {@code CANCELLED}（useLogicalDelete 逻辑删除终态）为终态。
     * {@code DRAFT} 为初始态非终态。
     */
    ...
    /**
     * 迁移矩阵只读快照（1 条命名边）：approve(DRAFT→ACTIVE)。CANCELLED 经 {@code useLogicalDelete} 可达
     * （非命名动作边，不编码入 transitions()）；ACTIVE 无出边（reverseApprove 不写 docStatus）。
     */
    ...
    /**
     * 终态集合：{ACTIVE, CANCELLED}（approve 后业务终态 + useLogicalDelete 逻辑删除终态）。
     */
