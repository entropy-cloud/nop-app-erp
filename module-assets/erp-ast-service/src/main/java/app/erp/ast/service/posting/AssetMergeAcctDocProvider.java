...
/**
 * 资产合并业财过账 Provider（assets 域）。支持 {@link ErpFinBusinessType#ASSET_MERGE}。
 *
 * <p>从 billData 读取结构化借/贷明细（DEBIT_LINES / CREDIT_LINES），物化 VoucherFact：
 * 借 = 固定资产-新卡片合计（单行）；贷 = 固定资产-各源卡片合计（按 MergeLine 拆 N 行）。
 */
public class AssetMergeAcctDocProvider implements IErpFinAcctDocProvider {
    ...
    public Set<ErpFinBusinessType> getSupportedBusinessTypes() {
        return EnumSet.of(ErpFinBusinessType.ASSET_MERGE);
    }
