...
/**
 * 资产合并单（{@code ErpAstMerge}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 *
 * <p><strong>不可逆契约（owner doc {@code split-merge.md} §关键业务规则 5）</strong>：Merge approve 触发
 * 结构性资产合并过账 + 卡片重组，执行后不可撤销。reverseApprove 运行时路径不经本 Bean——
 * per-mutation {@code ErpAstMergeReverseApproveProcessor} 在 require 后**无条件抛
 * {@code ERR_AST_MERGE_REVERSE_NOT_SUPPORTED}**（无 posted 判定、短路在 facade validateTransition 之前）。
 ...
 */
public class ErpAstMergeApprovalStateMachine {
     ...
     *
     * <p><strong>名义边（nominal edge，运行时不可达）</strong>：Merge reverseApprove 是无条件抛错动作
     * （per-mutation {@code ErpAstMergeReverseApproveProcessor} require 后直接抛
     * {@code ERR_AST_MERGE_REVERSE_NOT_SUPPORTED}，无 posted 判定、短路在 facade validateTransition 之前），
     ...
     *
     * <p><strong>名义目标态（运行时不可达）</strong>——Merge reverseApprove 无条件抛
     * {@code ERR_AST_MERGE_REVERSE_NOT_SUPPORTED}，从不产生状态迁移；此值仅供元数据对齐消费。
     */
