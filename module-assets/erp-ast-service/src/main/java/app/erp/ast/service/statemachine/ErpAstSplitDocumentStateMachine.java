...
/**
 * 资产拆分单（{@code ErpAstSplit}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 * （DRAFT/ACTIVE/CANCELLED）。Split 的 docStatus 有 1 个生产命名动作 writer：
 * {@code ErpAstSplitProcessor.executeApprove:117} 写 {@code ACTIVE}（approve 是唯一命名 writer）。
 * 故 {@link #transitions()} 含 1 条边：approve(DRAFT→ACTIVE)。{@code CANCELLED} 经独立 cancel mutation
 * （{@code ErpAstSplitCancelProcessor:26} 写 CANCELLED）+ {@code useLogicalDelete} 可达，无独立命名边
 * （cancel 守卫的 ACTIVE/posted 动态条件保留原位，见 Phase 1 Decision (C)）。
 *
 * <p><strong>守卫接线</strong>：{@code ErpAstSplitProcessor.validateTransitionForCancel}（doc-cancelled 守卫，
 ...
 * （CRUD 创建写入）。
 */
public class ErpAstSplitDocumentStateMachine {
     ...
     * 接线方 {@code ErpAstSplitProcessor.validateTransitionForCancel}（doc-cancelled 守卫）映射为领域码
     * {@code ERR_AST_SPLIT_ILLEGAL_DOC_TRANSITION}（common 码作 cause 保留）。
     */
