...
/**
 * 资产资本化单（{@code ErpAstAssetCapitalization}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@link ErpAstAssetCapitalizationDocumentStateMachine} docStatus 轴分离）。
 *
 * <p>接线方（plan 2026-08-14-1931-2，M4.47 facade 范式，镜像 Phase 1 Disposal）：{@code ErpAstAssetCapitalizationProcessor}
 * 注入 2 Bean（非 private）；{@code validateTransitionForXxx} 各改调 {@code assertCanXxx}（try/catch common 码作 cause →
 * 领域码 {@code ERR_CAPITALIZATION_ILLEGAL_STATUS_TRANSITION}，契约 §7）；{@code executeApprove}/{@code executeReverseApprove}
 * 目标态改调 {@code *TargetStatus()}。per-mutation 5 Processor 经 facade 透传自动生效。
 * 折旧计划生成、库存转固 stock move、Asset.status side-effect（→IN_SERVICE/→DRAFT，计划 1 M4.40 AssetStateMachine 守卫）、
 * 过账、posted 置位全部保留原位。
 *
 ...
 */
public class ErpAstAssetCapitalizationApprovalStateMachine {
     ...
     * 接线方 {@code ErpAstAssetCapitalizationProcessor.validateTransitionForSubmit} 映射为领域码
     * {@code ERR_CAPITALIZATION_ILLEGAL_STATUS_TRANSITION}（common 码作 cause 保留）。
     */
