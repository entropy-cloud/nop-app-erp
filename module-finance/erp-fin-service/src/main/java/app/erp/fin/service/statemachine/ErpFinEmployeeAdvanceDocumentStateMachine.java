 ...
 *
 * <p>非法边直抛领域码 {@link ErpFinErrors#ERR_EMPLOYEE_ADVANCE_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code currentDocStatus}/
 * {@code expectedDocStatus}/{@code action}；plan 2026-09-07-2200-1）；实体元数据（advanceCode）经调用点同码补参。
 */
public class ErpFinEmployeeAdvanceDocumentStateMachine {
     ...
     *
     * <p>非法来源态（CANCELLED）直抛领域码 {@code ERR_EMPLOYEE_ADVANCE_ILLEGAL_DOC_STATUS_TRANSITION}（携带
     * {@code action=cancel}）。接线方 {@code ErpFinEmployeeAdvanceProcessor.validateTransitionForCancel} 同码补参 advanceCode。
     */
