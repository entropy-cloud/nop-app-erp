...
/**
 * 费用报销单（{@code ErpFinExpenseClaim}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus}
 * 业务生命周期轴，字典 {@code erp-fin/expense-claim-status} 5 值：DRAFT/SUBMITTED/APPROVED/REJECTED/CANCELLED）。
 *
 * <p>权威契约：{@code docs/architecture/entity-state-machine-bean.md}；
 * 业务语义：{@code docs/design/finance/state-machine.md} §对象五 + {@code docs/design/finance/expense-claim.md}。
 *
 * <p><b>治理裁定（§11.2 M4 plan-first，plan 2026-08-13-1146-2）</b>：报销 cancel 在已过账时触发红冲
 * （{@code offsetOrchestrator.reverseOffset} + {@code postingDispatcher.reverse} 红字冲销闭环）。
 ...
 *
 * <p>命名带 {@code Document} 后缀（契约 §1 双轴约定，与 {@code ErpFinExpenseClaimApprovalStateMachine} approveStatus
 * 轴分离）。
 *
 ...
 *
 * <p>非法边直抛领域码 {@link ErpFinErrors#ERR_EXPENSE_CLAIM_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code currentDocStatus}/
 * {@code expectedDocStatus}/{@code action}；plan 2026-09-07-2200-1）；实体元数据（claimCode）经调用点同码补参。
 */
public class ErpFinExpenseClaimDocumentStateMachine {
     ...
     *
     * <p>非法来源态（CANCELLED）直抛领域码 {@code ERR_EXPENSE_CLAIM_ILLEGAL_DOC_STATUS_TRANSITION}（携带
     * {@code action=cancel}）。接线方 {@code ErpFinExpenseClaimProcessor.validateTransitionForCancel} 同码补参 claimCode。
     */
