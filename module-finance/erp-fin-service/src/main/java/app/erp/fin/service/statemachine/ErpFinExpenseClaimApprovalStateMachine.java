...
/**
 * 费用报销单（{@code ErpFinExpenseClaim}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus}
 ...
 *
 * <p><b>治理裁定（§11.2 M4 plan-first，plan 2026-08-13-1146-2）</b>：报销 approve 触发业财过账
 * （{@code ExpenseClaimPostingDispatcher}→{@code FinPostingExecutor} 生成 EXPENSE_CLAIM 凭证 + {@code ErpFinArApItem}
 * PAYABLE + {@code AdvanceOffsetOrchestrator} 员工借款冲抵），reverseApprove/cancel 触发红冲。依契约 §11.2 M4 硬约束
 * (i)–(v)：过账/冲抵时序/编排/失败兜底继续由 {@code ExpenseClaimPostingDispatcher}→{@code FinPostingExecutor} +
 * {@code AdvanceOffsetOrchestrator} + {@code posted} 标志契约管理（§11.2 M4 (ii)/(v)），Bean 不触碰；跨域副作用
 * （ArApItem 生成、员工借款冲抵、红冲）保留原 Processor 路径（§11.2 M4 (iv)）；{@code posted} 不入轴（§11.2 M4 (iii)）；
 * SoD（approver-is-creator）为动态业务守卫保留原位（非 Bean 范畴，架构契约 {@code entity-state-machine-bean.md:274}）。
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@code ErpFinExpenseClaimDocumentStateMachine} docStatus
