...
public class ErpSysNotificationMarkReadProcessor extends AbstractErpSysNotificationProcessor {
    ...
    public ErpSysNotification markRead(String notificationId, IServiceContext ctx) {
        ...
        if (n == null) {
            // P2-CK-notify-009-r3 接线面：INSTANCE_NOT_FOUND 已定义未接线——不存在通知不得静默写孤儿已读行
            throw new NopException(ErpNotifyErrors.ERR_NOTIFY_INSTANCE_NOT_FOUND)
                    .param(ErpNotifyErrors.ARG_NOTIFICATION_ID, notificationId);
        }
        // 通知按 recipientUserId 投递，已读记录以接收人为准（与 findUnread/countUnread 口径一致）；
        // recipientUserId 缺失时回退当前登录用户（与 resolveUserId 模式一致）
        String userId = n.getRecipientUserId() != null ? n.getRecipientUserId() : ctx.getUserId();
        // 越权校验（P2-CK-notify-010-r3）：ctx 登录用户与目标接收人不一致时拒绝
        assertActorAllowed(userId, ctx);
