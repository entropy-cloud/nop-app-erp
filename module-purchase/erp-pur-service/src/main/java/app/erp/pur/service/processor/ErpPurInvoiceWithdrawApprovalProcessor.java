...
/**
 * ErpPurInvoice withdrawApproval per-mutation Processor（审批轴 Bean 接线 plan 2026-08-13-1950-1 M4.16，skeleton 路径）。
 *
...
public class ErpPurInvoiceWithdrawApprovalProcessor extends AbstractWithdrawApprovalProcessor<ErpPurInvoice> {
    ...
    @Override
    protected void validateTransitionForWithdraw(ErpPurInvoice entity, IServiceContext context) {
        try {
            stateMachine.assertCanWithdraw(getApproveStatus(entity));
        } catch (NopException e) {
            throw e.param(ErpPurErrors.ARG_INVOICE_CODE, entity.getCode());
        }
    }
