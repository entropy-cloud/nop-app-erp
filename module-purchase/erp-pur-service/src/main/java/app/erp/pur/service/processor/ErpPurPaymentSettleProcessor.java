...
/**
 * ErpPurPayment settle per-mutation Processor（R6.5，{@code processor-extension-pattern.md} 每 mutation 一 Processor）。
 * 自包含付款核销编排（加载 → 委派 {@link PaymentSettler}）。共享 protected helper（{@code requirePayment}）
 * 单一真相源在 {@link ErpPurPaymentProcessor}，经 {@code @Inject facade} 同包 protected 可达。
 * 下游可经 Delta beans.xml 同名 bean id 覆盖本类。
 */
public class ErpPurPaymentSettleProcessor {
    ...
    public ErpPurPayment settle(String id, List<SettlementAllocation> allocations, IServiceContext context) {
        ...
        return doSettle(payment, allocations, context);
    }
    ...
    protected ErpPurPayment doSettle(ErpPurPayment payment, List<SettlementAllocation> allocations,
