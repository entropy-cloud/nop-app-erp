...
/**
 * 采购订单（{@code ErpPurOrder}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 *
 * <p>命名带 {@code Document} 后缀（契约 §1 双轴约定，为 M3.5 approveStatus Bean
 * {@code ErpPurOrderApprovalStateMachine} 预留命名空间）。
 *
 * <p>非法边直抛领域码 {@link ErpPurErrors#ERR_ORDER_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code currentDocStatus}/
 ...
 * 收敛前行为一致（行为不变 Non-Goal）。
 */
public class ErpPurOrderDocumentStateMachine {
