...
/**
 * 采购入库单（{@code ErpPurReceive}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 * {@code ErpPurReceiveApprovalStateMachine} 预留命名空间）。
 *
 * <p>非法边直抛领域码 {@link ErpPurErrors#ERR_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code currentDocStatus}/
 ...
 * 不编码入边）。
 *
 * <p>接线方 {@code ErpPurReceiveCancelProcessor} 同码补参补齐 {@code receiveCode} 元数据
 * （泛型命名漂移，路线图 Non-Goal 不重命名，successor 对齐）。
 */
public class ErpPurReceiveDocumentStateMachine {
