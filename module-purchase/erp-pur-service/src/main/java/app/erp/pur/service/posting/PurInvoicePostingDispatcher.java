 ...
 *
 * <p>对齐 {@code InvPostingDispatcher} 的失败语义：过账失败吞异常记日志、保持 APPROVED+{@code posted=false}
 * （由 Deferred 兜底扫描重试），不阻塞终态。本类为 Facade 编排层，**不持久化源单据**——源单据 {@code posted}
 * 标志由调用方 BizModel 在主事务内统一持久化（避免跨 REQUIRES_NEW 边界操作被挂起事务的托管实体）。
 */
public class PurInvoicePostingDispatcher {

    private static final Logger LOG = LoggerFactory.getLogger(PurInvoicePostingDispatcher.class);
     ...
     */
    public void reverse(ErpPurInvoice invoice) {
        try {
    ...
    private PostingEvent buildEvent(ErpPurInvoice invoice) {
        ...
        LocalDate voucherDate = invoice.getBusinessDate() != null ? invoice.getBusinessDate() : io.nop.api.core.time.CoreMetrics.today();
        event.setVoucherDate(voucherDate);

        Map<String, Object> billData = new LinkedHashMap<>();
        billData.put(PurAcctDocProvider.KEY_TOTAL_AMOUNT, nz(invoice.getTotalAmount()));
    ...
    private Long resolveAcctSchemaId(Long orgId) {
        ...
        q.setLimit(1);
        java.util.List<ErpMdAcctSchema> schemas = dao.findAllByQuery(q);
