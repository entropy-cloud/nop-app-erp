...
/**
 * 采购发票（{@code ErpPurInvoice}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@code ErpPurInvoiceDocumentStateMachine} docStatus 轴分离）。
 *
 * <p>非法边抛 common 层 {@link ErpCommonErrors#ERR_ILLEGAL_STATUS_TRANSITION}（参数 {@code currentStatus}/
 * {@code expectedStatus}），并附 {@code action} 补充诊断参数；领域 ErrorCode 映射归 Processor/facade（契约 §7）。
 * Invoice 领域码为 {@code ERR_INVOICE_ILLEGAL_STATUS_TRANSITION}（approve/reverseApprove 经 facade 映射；
 ...
 * {@code ErpPurInvoiceProcessor.validateTransitionForApprove/ReverseApprove} + {@code doApprove/doReverseApprove}
 * 改调 Bean（facade 路径）。PostingDispatcher 过账编排 + commitment-restore 保留原位。
 *
 * <p><b>reverseApprove 目标态裁定（plan Phase 1 Decision）</b>：实仓核实 {@code ErpPurInvoiceProcessor.doReverseApprove}
 * 写入 REJECTED（非骨架默认 SUBMITTED）——即 Invoice 当前行为已合规 §16.4。故本 Bean {@code reverseApproveTargetStatus()}=REJECTED。
 */
public class ErpPurInvoiceApprovalStateMachine {
