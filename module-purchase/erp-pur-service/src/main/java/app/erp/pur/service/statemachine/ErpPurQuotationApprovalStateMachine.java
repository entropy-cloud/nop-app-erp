...
/**
 * 供应商报价单（{@code ErpPurQuotation}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 * 命名带 {@code Approval} 后缀（与 {@code ErpPurQuotationDocumentStateMachine} docStatus 轴分离）。
 *
 * <p>接线范式（INLINE 路径）：经 {@code ErpPurQuotationBizModel} 注入本 Bean + 暴露 helper，
 * {@code ErpPurQuotation.xbiz} 内联 {@code isCancelled} + 来源态守卫改调 BizModel helper 委托本 Bean（plan Phase 3）。
 *
