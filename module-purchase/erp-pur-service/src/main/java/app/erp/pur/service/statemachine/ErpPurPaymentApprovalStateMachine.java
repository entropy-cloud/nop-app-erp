...
/**
 * 付款单（{@code ErpPurPayment}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@code ErpPurPaymentDocumentStateMachine} docStatus 轴分离）。
 *
 * <p>非法边抛 common 层 {@link ErpCommonErrors#ERR_ILLEGAL_STATUS_TRANSITION}（参数 {@code currentStatus}/
 * {@code expectedStatus}），并附 {@code action} 补充诊断参数；领域 ErrorCode 映射归 Processor/facade（契约 §7）。
 * Payment 领域码为 {@code ERR_PAYMENT_ILLEGAL_STATUS_TRANSITION}（approve/reverseApprove 经 facade 映射；
 * submit/reject/withdraw 经 per-mutation Processor 映射）。
 *
 * <p><b>接线路径（plan Phase 1 Decision 双路径分化）</b>：submit/reject/withdraw 经 per-mutation Processor 覆写
 * {@code validateTransitionForXxx} 委托 Bean（skeleton 路径）；approve/reverseApprove 经 facade
 * {@code ErpPurPaymentProcessor.validateTransitionForApprove/ReverseApprove} + {@code doApprove/doReverseApprove}
 * 改调 Bean（facade 路径）。PostingDispatcher 过账编排 + 核销 + budget-check + workflow（{@code nopFlowId}）保留原位。
 *
 * <p><b>reverseApprove 目标态裁定（plan Phase 1 Decision）</b>：实仓核实 {@code ErpPurPaymentProcessor.doReverseApprove}
 * 写入 REJECTED（非骨架默认 SUBMITTED）——即 Payment 当前行为已合规 §16.4。故本 Bean {@code reverseApproveTargetStatus()}=REJECTED。
 */
public class ErpPurPaymentApprovalStateMachine {
