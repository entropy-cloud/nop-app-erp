...
/**
 * 询价单（{@code ErpPurRfq}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 *
 * <p>命名带 {@code Document} 后缀（契约 §1 双轴约定，为 M3.3 approveStatus Bean
 * {@code ErpPurRfqApprovalStateMachine} 预留命名空间）。
 *
 * <p>非法边直抛领域码 {@link ErpPurErrors#ERR_RFQ_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code currentDocStatus}/
 * {@code expectedDocStatus}，附 {@code action} 补充诊断参数；plan 2026-09-07-2200-1）；
 * 实体元数据（{@code rfqCode}）经调用点同码补参补齐。
 *
 * <p>迁移矩阵（1 条边）：cancel(DRAFT→CANCELLED)。
 *
 * <p><b>层 2 漂移裁定（plan Phase 3 Decision）</b>：owner doc §2 要求 cancel 守卫「非已作废」，但
 * {@code ErpPurRfqBizModel.cancel} 迁移前<b>无任何 docStatus 守卫</b>（允许幂等 CANCELLED→CANCELLED）。
 * 裁定为 <b>implementation drift → Fix</b>（owner doc 为权威）：本 Bean 矩阵含「CANCELLED 非法」守卫，
 * BizModel 接线后 cancel 对已作废询价单抛领域码 {@code ERR_RFQ_ILLEGAL_DOC_STATUS_TRANSITION}
 * （行为变化：原幂等 no-op → 抛错，按路线图规则 5 Fix 登记）。
 */
public class ErpPurRfqDocumentStateMachine {
    ...
    private static NopException illegal(String action, String currentStatus, String expectedStatus) {
        return new NopException(ErpPurErrors.ERR_RFQ_ILLEGAL_DOC_STATUS_TRANSITION)
