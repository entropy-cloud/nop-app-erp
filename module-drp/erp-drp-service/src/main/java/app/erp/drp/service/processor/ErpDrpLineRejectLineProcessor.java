...
// 族 A/U20 豁免登记：本类为非 BizModel 服务组件（processor-extension-pattern 惯例）；daoFor 目标（ErpDrpLine）=同域实体批量聚合，只读批量聚合，逐条 I*Biz 管道不适用批量场景。
/**
 * ErpDrpLine rejectLine per-mutation Processor（R6.7，{@code processor-extension-pattern.md} 每 mutation 一 Processor）。
 * 自包含行驳回编排（SUGGESTED/APPROVED→CANCELLED）：回读行 + 终态门（ORDERED/CANCELLED 不可再驳回）+ 置 CANCELLED。
 * 与 {@link ErpDrpLineCancelLineProcessor} 同语义（原 BizModel rejectLine/cancelLine 共用 doCancel），按自包含要求各自内联副本。
 * 下游可经 Delta beans.xml 同名 bean id 覆盖本类。
 *
 * <p>接线（plan 2026-08-12-1841-1 Phase 2）：固定来源态/终态守卫经 {@link ErpDrpLineStateMachine}（cancel 多源 SUGGESTED/APPROVED）。
 */
public class ErpDrpLineRejectLineProcessor {
