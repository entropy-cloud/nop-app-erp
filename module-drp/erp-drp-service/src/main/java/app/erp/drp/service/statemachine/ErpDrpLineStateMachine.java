 ...
 *
 * <p>非法边直抛领域码 {@link ErpDrpErrors#ERR_DRP_LINE_ILLEGAL_TRANSITION}（参数 {@code currentStatus}/
 * {@code expectedStatus}/{@code action}；plan 2026-09-07-2200-1 直抛领域码）；实体元数据（drpLineId）
 * 经调用点同码补参补齐。例外：{@code DrpReleaseService.releaseLine} 接线方保留误名 pre-existing 码
 * {@code ERR_DRP_LINE_NOT_SUGGESTED} 语义映射（本重构不重命名）。
 *
...
public class ErpDrpLineStateMachine {
    ...
    private static NopException illegal(String action, String currentStatus, String expectedStatus) {
        return new NopException(ErpDrpErrors.ERR_DRP_LINE_ILLEGAL_TRANSITION)
                .param(ErpDrpErrors.ARG_CURRENT_STATUS, currentStatus)
