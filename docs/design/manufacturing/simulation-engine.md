...
> **转正后重算语义注记（2026-08-25，plan 0330-3 分诊确认）**：promoted 计划的设计用途是
> **释放其复制的仿真结果行**（走既有单次释放路径）；若对 promoted DRAFT 计划调用 `runDrp`/
> 单次引擎重算，净需求口径**回落基线参数表**（`ErpDrpParameter`/MRP 主数据）——仿真覆盖值
> 按隔离原则只在场景版本作用域内生效、不写回任何主数据/参数表，重算不携带仿真值属设计内
> 行为而非缺陷。需要参数生效的正式重算应修改基线参数后走标准单次引擎。
