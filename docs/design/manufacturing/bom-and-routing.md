...
## 单 BOM 模型与三视图（EBOM/MBOM/PBOM）

### 决策声明

全仓只有一张 BOM（`ErpMfgBom` + 三个子表），**不分离制造 BOM 与采购 BOM**，也不为设计结构另建工程 BOM。行业常说的「三种 BOM」在本项目折叠为**一个权威 BOM + 按需派生视图**，避免多份 BOM 主数据对不齐（对齐 Odoo `mrp.bom` / ERPNext `BOM` 的单 BOM 范式）。

### 三视图映射

| 视图 | 谁是消费方 | 在本项目中的落位 |
|------|-----------|------------------|
| 工程/设计视图（EBOM） | 研发/工艺 | 同一 BOM 的多版本并存：`isActive`+`isDefault` 区分激活/默认 + `versionLabel` 版本号；BOM 头仅允许一个产出物料（`productId`） |
| 制造视图（MBOM） | 车间/工单 | BOM 头/行/工艺/联副产品本身 + 工单审核按 BOM 展开领料移动单与 JobCard 工序（见上文「多级 BOM 展开」「齐套校验」） |
| 采购视图（PBOM） | 采购/计划 | 无独立采购 BOM 实体；MRP 展开 BOM 后对采购件产出 `PURCHASE_REQUEST` 请购建议 → 释放落 `ErpPurOrder`；价源取 `ErpPurSupplierPriceList` / 默认 SKU `ErpMdMaterialSku.purchasePrice` |

### 为什么不拆

1. **单真相源**：一张 BOM 同时喂给工单展开、MRP 净需求、标准成本滚算，消除「制造 BOM 与采购 BOM 对不上」的主数据漂移。
2. **采购侧不需要独立 BOM**：采购对象是 MRP 展开后的叶子采购件（BOM 展开最终层），其需求结构已由 BOM 表达；采购单据（请购/订单/入库）只消费物料/数量/交期，无需再持有结构。
3. **自制/外购判定已可表达**：按「物料是否存在默认且有效 BOM」判定制造件/采购件（见 §实现注记），无需独立类型字段或独立实体；委外加工件走 `ErpMfgSubcontractOrder`，同样不建独立 BOM。

### 演化触发条件（successor）

当下列任一需求上线时，才考虑拆出独立视图/实体：

- **设计结构 ≠ 装配结构**：研发 EBOM 的层级组织与车间 MBOM 的装配顺序不一致，需 EBOM→MBOM 工艺转换；
- **展开结果独立持久化**：需要独立维护 phantom 展开缓存或工装/辅料清单（当前 `PHANTOM` 展开即时计算，展开结果缓存表为 Non-Goal，见 §实现注记）；
- **采购/委外拆分视图**：需要按俘虏/委外件单独出采购 BOM 视图，供供应商协同或外协结算专用。
...
- **卷算状态机缺失登记（P2-CK-mfg2-008 Deferred，plan 2026-09-17-0800-1 补登记）**：`erp-mfg/cost-rollup-status` 字典 DRAFT/CALCULATED/FIRMED/CANCELLED 四值中，本期仅 MANUAL rollup 写入 DRAFT→CALCULATED 与人工 FIRMED 直改可达——**FIRMED 无命名 writer mutation、CANCELLED 为零 writer 死值、无迁移守卫**；而标准成本读侧（findLatestFirmedStandardCost/生产差异计算）以 FIRMED 为消费前提。修复需状态机 + writer + 迁移守卫的模型设计（不在简单修 complex 范畴）。**重开触发**：标准成本 FIRMED 消费链实际受阻时立项（备选范式：Forecast approve/cancel 的 purpose-built firm/cancel mutation）。
...
- **消耗控制 consumption 运行时接入（P2-CK-mfg-008，plan 2026-09-17-0800-1）**：上方「消耗控制」表（`consumption`：flexible/warning/strict）已由领料确认链消费——`ErpMfgMaterialIssueConfirmProcessor.enforceConsumptionControl` 在任何副作用（预留消耗/出库移动/回写）之前按工单材料行需求量累计判定：行已入账 `actualQuantity` + 本次确认行 issued 之和 > 行 `plannedQuantity` 时，STRICT 抛错中止确认、WARNING warn 放行、FLEXIBLE（列空缺省）放行；`ErpMfgBom.consumption` 列为 per-BOM 级别（非 per-行），无 BOM 关联/工单行缺失/计划量非正的领料行不设限。UC-MFG-04「STRICT 或主管权限」的主管权限旁路面归 successor。
- **useMultiLevelBom 展开深度接入（P2-CK-mfg2-009，plan 2026-09-17-0800-1）**：`ErpMfgBom.useMultiLevelBom` 列已由 `KitAvailabilityChecker.resolveUseMultiLevel` 消费控制齐套/预留展开深度（`false` = 单级展开，仅直接子件；列空缺省 `true` = 多级）；MRP 引擎展开不受该列治理（维持多级，成本口径不变）。
