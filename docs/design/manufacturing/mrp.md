...
- **低层码（F2.6 修正声明，P1-CK-mfg2-002）**：上方「低层编码」本期实现为 **run 内 per-material 已消耗可用量累计**（`MrpEngine`/`SimulationMrpEngine` 的 `availableConsumed` map：同一物料在整个 MRP run 内被多次访问时（多个父件共享子件 / 独立需求+子件），可用量仅扣减一次——首次访问按 `remaining = max(0, total − consumed)` 净额并累计消耗，后续访问 `consumed` 已含此前扣减，`net = gross − remaining` 使跨分支总净需求 = Σgross − available），**不预计算物化 lowLevelCode 列（ORM 无此列）**，也未消费 `BomExplosionNode.level`（原声明「DFS 层级标记实现（同物料取最低层级展开基准）」与实现不符，已按 F2.6 修正）。SAFETY_STOCK 来源需求行已是净缺口（safety − available），跳过 available 扣减且不消耗（P1-CK-mfg2-001）。pegging 保留逐父行结构（同物料多父行，每行 parentLineId 指向各自父行）。
...
- **委外建议释放**：释放已支持——`MrpReleaseService.releaseSubcontractRequest` + `ErpMfgMrpPlanLineReleaseSubcontractRequestProcessor` + `IErpMfgMrpPlanLineBiz.releaseSubcontractRequest` @BizMutation 已落地（config-gated `erp-mfg.subcontract-release-enabled` 默认 false；引擎侧 MRP 零产出 SUBCONTRACT_REQUEST 类型行是另一独立事项，释放动作对手工行可达）。触发条件：委外加工批量投产时翻 config。（P3-CK-mfg2-025-r3 修订：原「本期不支持释放」与实仓 API 面不符。）
...
- **释放状态措辞更正：RELEASED → FIRMED/isFirmed**：上方「建议单释放」ASCII 图原文声明「释放后建议单状态标记为 RELEASED」——此为设计与实现的措辞漂移，**`erp-mfg/mrp-status` 字典无 RELEASED 值**，RELEASED 为幻影状态。实际机制：释放后行级 `ErpMfgMrpPlanLine.isFirmed=true`（`MrpReleaseService.markFirmed:134-138` 同步写 `convertedBillCode`），当该计划全部行均 `isFirmed=true` 时，`advancePlanToFirmedIfComplete:226-250` 将计划头 `ErpMfgMrpPlan.status` 置为 `FIRMED`。即「释放 = 行布尔 isFirmed + 计划头状态 FIRMED」两级，非单态 RELEASED。已按此修正上方 ASCII 图措辞。
