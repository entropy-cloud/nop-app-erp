...
> **1606 固定资产清理中间科目腿实现注记（RC-R1.53，P1-RC-030 收敛）**：实现（`DisposalAcctDocProvider.createFacts`）按上表两步流结构单张凭证多腿落地——Step1 结转：借 1602 累计折旧 / 借 1606 固定资产清理（净值 = 原值−累计折旧）/ 贷 1601 固定资产；Step2 结转：借 1002 银行存款（处置收入 >0 时）/ 贷 1606 + 损益从 1606 结转（收益：借 1606 / 贷 6301 营业外收入；损失：借 6711 营业外支出 / 贷 1606）。GL 恒等式 Dr Σ = Cr Σ 保持，1606 网为零（中间科目腿不残留余额，对 1601/1602/1002/6711/6301 的最终余额影响与合并凭证完全相同）。科目编码常量 `SUBJECT_DISPOSAL_CLEARING="1606"` + GL 映射键 `DISPOSAL_CLEARING`（无映射规则时 resolver 空匹配回退 Provider 科目编码）；种子科目表 `erp_md_subject.csv` 纯加性补 1606 行（ID 46）。凭证级拆分持久化为 Non-Goal（单张凭证内多腿结构，对齐 L1「生成处置凭证」单数语义）。
