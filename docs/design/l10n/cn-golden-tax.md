...
对照 `docs/analysis/erp-survey/2026-06-22-0000-odoo-l10n-china.md` §5 的 OCA 模式(本地化做成独立可拔模块)。**独立工程 `module-l10n-cn`**(独立 Maven 工程 + 独立 appName `erp-l10n-cn`),不内建在核心 ERP 实体里。金税接口/发票字段/银行对账规则可单独演进,不污染通用进销存模型。本仓已有 10 个业务域工程的同等粒度,新增一个本地化工程符合既有结构。
