...
> **行级过滤落地状态（plan 2026-07-31-1023-3 R3.4，P1-MA6-002；翻启 plan 2026-08-10-2059-1 / E2.1，2026-08-10）**：角色侧行级过滤已落地为 `erp-*.data-auth.xml` 规则 + config-gated `ErpRoleDataAuthChecker`。**%test profile 双开关已翻启 ON**（`nop.auth.enable-data-auth=true` + `erp.data-auth.role-row-filter-enabled=true`），`%dev`/`%prod` 保持 OFF（安全姿态，prod 翻转 successor）。E2.1 翻启实证：(1) sal/qa 行级过滤真正生效（销售员仅见 `createdBy == userId` 单据）；(2) sal roleId 实时缺陷修正（6 处 `roleIds="业务员"` → `销售员`，对齐 P1.5a 冻结词表 + `nop_auth_role.csv` 种子）；(3) admin 经平台 `user` 兜底 role-auth 全见（不补 admin role-auth，避免双命名空间混淆）；(4) 单组织基线零回归；(5) 连带加翻 `use-user-id-for-audit-fields=true`（row-filter 运行时必要 enabler：`AuthHttpServerFilter.initUserContext` 仅当此 flag=true 时 `ctx.setUserRefNo(userId)`，否则 createdBy stamp 为 sys-user-name，filter `eq(createdBy,userId)` 永不命中）。
>
