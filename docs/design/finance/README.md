...
3. **业财回链双向闭环**：每张业务生成的凭证通过回链表关联源单据。作废源单据时红冲凭证（方向一）；财务侧红冲凭证时通过 `VoucherReversedEvent` 通知域回退源单据状态（方向二）。两方向缺一不可，详见 `posting.md` §冲销机制。
4. **反写契约（域自治）**：过账引擎只持有 `PostingEvent` 快照，不持有源业务实体 ORM 引用；源单据 `posted` 标志由域调用方在 `post()` / `reverse()` 成功后自行置位/回退。不引入独立反写记录表（与 iDempiere/Metasfresh/Odoo/ERPnext 一致）。详见 `posting.md` §反写契约。
5. **过账全程可观测**：过账全链路留痕——规则命中追溯、模板/规则变更审计、过账异常工作台。失败不静默丢弃，未处置异常记录是期末结账前置门控。详见 `posting-log.md`。
6. **多币种**：凭证分录行同时记录源币种金额与本位币金额，按业务日期汇率转换。
7. **多套科目表**：支持多套会计科目表并行（管理账/税务账），同一业务在多套下各出一组凭证。
8. **辅助核算**：凭证分录行可挂业务维度（往来单位/部门/项目/仓库），支持多维报表。
9. **期间控制**：已结账的会计期间不允许新增/修改凭证。
10. **坏账准备（Allowance 法）**：坏账损失（P&L）与坏账准备（BS 抵减资产）分离；计提基于账龄分桶 × 历史损失率；核销只动 Allowance ↔ AR 不进 P&L；期末 Allowance 充足性是结账前置门控。详见 `bad-debt.md`。
...
| `posting.md` | 业财打通机制、凭证模板、过账引擎、科目映射、冲销双向闭环、反写契约 |
| `posting-log.md` | 业财过账日志与可观测性：规则命中追溯、变更审计、异常工作台、运行监控 |
...
| `document-driven-ap-automation.md` | 文档驱动 AP 自动化管道设计（E1.2 设计，2026-08-12 erp-survey 批次；已实现 E3.5，2026-08-27，端到端管道 + SPI 双引擎 + config-gate 默认关闭） |
