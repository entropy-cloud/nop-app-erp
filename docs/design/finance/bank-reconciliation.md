...
- **自动红冲接线（实现现状补注，plan 2026-08-07-1932-3 RC-R1.2）**：未达调整凭证自动红冲经 nop-job `erp-fin-bank-recon-adj-reverse` 接线（`job.yaml` → `nopBatchTaskRunner` → `bank-recon-auto-reverse.batch.xml` → `ErpFinBankReconAutoReverseHelper`，仅调用既有 `BankReconciliationBuilder.reverse` 入口）。双层门控：job `enabled` 默认 false（部署 opt-in，cron 默认每月 1 日 01:30）+ 业务 config `erp-fin.bank-recon-auto-reverse-next-month` 默认 true（机制开关）。扫描语义 = `docStatus=POSTED` 且 `reconciliationDate < 当月第一天`（跨期未达收敛）。CLOSED 期间候选（启用期末结账部署）逐条失败隔离 + WARN 日志，保持 POSTED 下月重试——完整「跨期还原」（红冲凭证记入次月 OPEN 期间）为 successor（须改 PostingProcessor 期间解析，会计核心路径）。
- **对方账号维度实现注记（§业务规则 2，plan 2026-08-15-1838-2 RC-R1.43，P1-RC-004）**：`ErpFinBankStatementLine` 新增 3 可空列 `counterpartyAccount`（对方账号，VARCHAR 50）/`counterpartyName`（对方户名，VARCHAR 200）/`counterpartyBank`（对方开户行，VARCHAR 200），导入链路 `BankStatementLineInput` DTO + `BankStatementImporter.importStatement` 承接（null 透传，外部文件解析仍属集成层 Non-Goal）。**过滤语义**：`BankLedgerQuery.findCandidates` 新增对方账号维度——对账单行 `counterpartyName` 非空时，候选 `ErpFinVoucherLine` 须经既有 `partner` 关系（`partnerId`→`ErpMdPartner.name`，C1 选项 A）解析户名做**非空精确匹配**（C2 选项 A：两侧均非空才比较，任一为空放行——seed 无 counterparty 数据零回归）；候选侧不新增列（2026-08-12 裁决仅授权 BankStatementLine 3 列）。**残留风险**：partner（ERP 往来单位）≠ 银行流水对方账号字面；户名精确匹配不覆盖「户名差一」SUSPENSE 语义，LIKE 模糊增强留待实际误配暴露后按需追加（Successor Required: no）。
