...
> **错误传播分级**（`posting-log.md §错误传播分级策略`）：步骤2 存货成本兜底重算、步骤3 折旧计提、反结账折旧冲销均为 **G3 编排层跨域集成步骤**。「impl 未就绪」（assets/inventory 域未部署，`IBizObjectManager` 解析失败）容错跳过（告警不阻断）；「配置错误/真实故障」（`NopException` + `ErrorCode`，如 `ERR_DEPRECIATION_RATE_MISSING`）**阻断结账**（rethrow），避免期间带病关闭（GL 缺折旧/成本调整凭证）。期末前置检查（§结账前置检查）扩展覆盖 assets 折旧 posted=false / inventory 到岸成本 posted=false 悬挂，作为 G4 域 dispatcher 静默悬挂的统一兜底入口。

| 异常场景 | 处理 |
|----------|------|
| 过账失败 | 列出失败单据，阻止结账 |
| 成本核算失败 | G3：impl 未就绪告警跳过；配置错误/真实故障阻断结账（rethrow） |
| 折旧执行失败 | G3：impl 未就绪告警跳过；配置错误/真实故障阻断结账（rethrow） |
