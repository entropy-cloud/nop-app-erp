...
各实体特殊约束：
- 物料停用后其下所有 SKU 一并不可被新单据引用；SKU 可独立于物料停用（`ErpMdMaterialSku.status` 列承载，null=ACTIVE 派生；停用守卫 + 跨域引用检查 `IErpMdSkuReferenceChecker` 四域生产实现经 List 收集器聚合，见 `sku-multi-unit.md §SKU 状态管理 §实现注记`，RC-R1.72）。
