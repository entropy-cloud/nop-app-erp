...
> **priceValidationLevel 默认值收敛注记（RC-R1.40，2026-08-15）**：`ErpMdMaterialCategory.priceValidationLevel` ORM `defaultValue` 已由历史孤儿非字典值 `"20"` 收敛为字典合法值 `"WARN"`（P2-RC-057 方案 A，2026-08-08 §7 A5 人工裁决预授权）——**新创建分类未显式赋值时物化字典合法值 WARN**，孤儿值从模型消除（dict `erp-md/price-validation` OFF/WARN/HARD 契约收敛）。**行为不变性**：修复前后新分类默认语义均为 WARN（旧 "20" 经 `resolvePriceValidationLevel` 非字典值→WARN 兜底亦归 WARN），`resolvePriceValidationLevel`/`validatePrice` 行为零变更（非字典值→WARN 兜底保留，防历史遗留 "20" 值行行为翻转）；既有 "20" 值行运行时语义不变（数据卫生 UPDATE 迁移属数据变更须 ask-first，登记 successor）。测试证据：`TestErpMdSkuPriceValidation` 新增 3 组（defaultValue 物化 WARN 断言 + 显式 OFF/WARN/HARD 不受影响 + 非字典值兜底回归断言），erp-md-service 146 tests 全绿。
