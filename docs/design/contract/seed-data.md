...
- **预存 ORM quirk 登记**：`erp-ct/sign-status` / `erp-ct/sign-provider` 字典 option 值为数字串（'10'~'60'/'10'~'99'）而运行时常量（`ErpCtConstants:49-57`）写 `PENDING_SIGNATURE`/`REJECTED`/`MOCK` 字符串码——种子按运行时词汇取值（状态机/查询的实际消费面），字典陈旧修正归 successor。
