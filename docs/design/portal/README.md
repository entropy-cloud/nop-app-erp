...
- **客户门户**：客户登录→查看订单状态/历史、查看/下载发票、在线付款、提交售后工单、更新联系信息/地址簿。
- **供应商门户**：供应商登录→查看/确认/回复采购订单（PO）、提交 ASN（提前发货通知）、提交发票、查看绩效评分。
- **与 nop-app-mall 的关系**：如果部署了电商商城（`nop-app-mall`），客户门户应复用其前端 UI（统一登录/统一地址簿/统一订单查询接口），B2B 客户门户侧重企业级功能（信用额度/批量订单/合同价格/对账），与商城的 B2C 客户门户互补。
- 本模块不负责：B2C 商城前端（`nop-app-mall`）；EDI 自动对接（`b2b/README.md`）。
...
| 在线付款 | 集成第三方支付（银行/支付宝/微信） | `ErpFinPayment` 创建 |
...
2. **SSO 统一认证**——门户用户通过 `nop-auth` OAuth2/JWT 登录，使用与内部系统一致的认证体系。客户和供应商作为 `ErpMdPartner` 的关联用户，通过 `partner.userId` 关联内部账号。
3. **数据权限隔离**——客户门户只看到本客户的数据（`partnerId` 过滤），供应商门户只看到本供应商的数据。通过 `orgId` + `partnerId` 行级权限实现。
4. **复用 nop-app-mall 前端**——若同时部署了商城，客户门户优先复用商城的前端 UI 框架和组件库（统一品牌/导航/登录状态）。
