...
| `ai-native-interface.md` | AI 原生接口层设计（GraphQL 类型定义即 API + REST/GraphQL 双通道，否决 MCP；AI 护栏/human-approval 门；2026-08-12 批次设计；已实现 E3.6，2026-08-27，最小落地集 6 项 + actorType 裁决不落地） |
| `dashboard-semantic-layer.md` | 看板 KPI 语义层设计（KPI 度量目录单一定义源、嵌入式 API-first、行级安全核实；2026-08-12 批次设计；已实现 E3.1+E3.1b，2026-08-26，度量目录全量登记 + 行级安全核实与缺口登记） |
