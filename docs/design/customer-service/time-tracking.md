...
> **实现更正（RC-R1.66）**：「工单.responsibleId」实体不存在该列，实际工单负责人列为 `assignedToId`（orm.xml 权威）；「Admin 手动指定」经 config 键 `erp-cs.time-entry-approver-id` 显式化。落地语义见 §十。
...
## 十、实现约定（RC-R1.66 落地注记，2026-08-18）

> 实现载体：`ErpCsTicketTimerSession` 实体（orm.xml 真相源）+ `ErpCsTicketTimerSessionBizModel` 四 mutation（per-mutation Processor 编排）+ `ErpCsTimeEntryBizModel` submit/approve/reject + `ErpCsTicketBizModel` 三聚合 `@BizQuery`。裁决全文见 `docs/plans/2026-08-17-2125-2-rc-mr1-r1-66-cs-timer-session.md` Phase 1 Decision Record。

- **实体形态（D1）**：`agentId` 为 `stdDomain=userId` VARCHAR(36)（对齐 assignedToId/operatorId/teamLeaderId 全域载体）；单计时器约束 = ORM `UK_CS_TIMER_SESSION_AGENT_ACTIVE(agentId, activeFlag)`——进行中（RUNNING **与** PAUSED，均占槽位）`activeFlag='Y'`、停止置 NULL（NULL-UK 可重复放行历史行）。运行时强制机制 = mutation 管道应用级守卫（守卫复查 DB 状态，含直插行兜底）；本项目 DDL 生成链（H2 init + deploy 三方言）仅物化 PK（全实体既有特征），UK 保留于 ORM 真相源作部署加固元数据。
- **二次启动语义（D1-②，§2.2 实现注记）**：§2.2「启动新计时器前自动停止当前计时器（提示确认）」的「确认」属操作者决策——服务端实现为**拒绝 + 专属错误码 `erp.err.cs.timer.already-active`**；「自动停止+确认」由客户端编排 stop→start 组合实现（前端提示属产品化 successor）。超时（>12h）旧会话在启动入口先惰性结算释放槽位后允许新启动。
- **12h 自动停止 = 惰性结算（D3）**：start/pause/resume/stop/findActiveTimer 各入口先结算——有效运行分钟（墙钟 − Σ暂停）> `erp-cs.time-entry-timer-max-hours`×60 时置 STOPPED + stopTime = startTime + 上限 + 总暂停（反推封顶时刻）+ 生成 duration=720 封顶条目。§2.2「每分钟更新 duration」呈现语义经读取时惰性计算等价实现（实时刷新 job 归 Deferred）。「任一入口触碰时点观察到的会话永不超 12h」：pause/resume 拒绝路径与 findActiveTimer 读取路径的结算经 **REQUIRES_NEW 独立事务物化**（外层异常回滚/只读会话不吞结算）。已知残留（Deferred）：永不再被操作的 RUNNING 会话不物化封顶条目（下次触碰补物化；无人值守扫描 job 归 successor）。
- **停止生成条目（D2/D6）**：`duration = 墙钟 − Σ暂停`（分钟）、`source=TIMER_IMPORT`、`approvalStatus` 留 **NULL 即 DRAFT**（字典无 DRAFT 码，空值承载未提交语义）、startTime=会话开始。`agentId` 映射：数字 userId 直写 `ErpCsTimeEntry.agentId`(BIGINT)；非数字 → 0 哨兵 + WARN（BIGINT agentId 为零取值空间孤儿列，id 空间统一归 successor）。（**2026-08-22 注记**：cs 域主键/外键 Java 层已 String 化（plan `2026-08-22-0002-3`）——`agentId` 现为 String（DB 列保持 BIGINT），`toEntryAgentId` 改恒等直写，本条「0 哨兵 + WARN」机制已随 String 化退役。）费率：`isBillable` 且费率缺失且 `erp-cs.default-billing-rate > 0` 时填充 + `billableAmount = duration/60 × rate`（本计划仅全局默认级；§1.2 优先级 2-3 级归 Deferred）。
- **审批链（D4，§3.2/§3.3 实现注记）**：submit 触发判定 = `isBillable=true` **或** `duration ≥ erp-cs.time-entry-approval-threshold` → PENDING；`erp-cs.time-entry-auto-approve=true` 或（不可计费且未超阈值）直通 APPROVED；REJECTED 可修改后重新 submit。reject 原因经 description 前缀 `[驳回] reason; ` 承载（无独立列）。**命名漂移更正**：§3.3「工单.responsibleId」实体不存在，实际列为 `assignedToId`——审批人链 = 工单 assignedToId → SLA 策略团队 teamLeaderId → config `erp-cs.time-entry-approver-id` 主管兜底；resolved approver 为 advisory 审计日志（不持久化，`approvedById` 记录实际决策人；身份强校验归 successor）。§3.1 第三触发（按组织配置）与 §3.2 PENDING→DRAFT 退回路径超出 UC-CS-11 验收标准（Deferred But Adjudicated）。
- **聚合口径（D5，§四不变）**：三聚合全量经 `ErpCsTicketBizModel` `@BizQuery` 实现（`totalTimeSpent` 含 APPROVED+PENDING——**不滤 isBillable**；`totalBillableTime`/`totalBilledAmount` 仅 isBillable+APPROVED），经 `IErpCsTimeEntryBiz` 查询 + Java 侧 SUM，零 ticket 加列。
- **config 七键（D6，§七 + 兜底键）**：六键默认值对齐 §七；`erp-cs.time-entry-approver-id`（空）为审批人链兜底显式化。`time-tracking-enabled=false` 仅门控计时器 mutation（`erp.err.cs.time-tracking.disabled`），MANUAL 条目 CRUD/submit/approve 不受影响（L1 ① 前置仅约束计时器模式）。
