...
- **停止生成条目（D2/D6）**：`duration = 墙钟 − Σ暂停`（分钟）、`source=TIMER_IMPORT`、`approvalStatus` 留 **NULL 即 DRAFT**（字典无 DRAFT 码，空值承载未提交语义）、startTime=会话开始。`agentId` 映射：数字 userId 直写 `ErpCsTimeEntry.agentId`(BIGINT)；非数字 → 0 哨兵 + WARN（BIGINT agentId 为零取值空间孤儿列，id 空间统一归 successor）。费率：`isBillable` 且费率缺失且 `erp-cs.default-billing-rate > 0` 时填充 + `billableAmount = duration/60 × rate`（本计划仅全局默认级；§1.2 优先级 2-3 级归 Deferred）。
