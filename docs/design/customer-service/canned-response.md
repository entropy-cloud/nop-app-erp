...
> §一~§三：CannedResponseRenderer 纯函数式工具 + ErpCsCannedResponseBizModel（renderTemplate/suggestForTicket/applyCannedResponse）+ 3 ErrorCode + 3 配置键。§四（管理功能 Excel 导入/导出 + §五 分类树校验）归 Deferred successor。
