...
> **最后更新**：2026-08-25（V.1 执行登记：§4 增耗时实测注记——套件墙钟 104s / 全量 ≈15:45，模型保留为上界参考）
...
### 3.5 勘误↔缺陷路由通道绑定纪律（三绑定义务，2026-08-25 立法）

> 立法来源：开放式审计 `docs/audits/2026-08-24-2233-open-audit-integration-test.md` 结论 #4 / P1 发现 OA-01/02/03——集成测试 mission 的「勘误」口径未与缺陷路由通道接通，发现实现-设计实质差距时仅勘误落盘，黄金路径套件会系统性把生产缺陷钉死为期望行为（C13 FX 断言反向看护、C19 fixture 补 orgId 遮蔽、C16 自包含模板绕开种子缺口均为实例）。立法 plan：`docs/plans/2026-08-25-0330-3-adjudicate-integration-test-defect-routing.md` Phase 3。

**规则（适用于本 mission 及后续同类集成/E2E mission 的实施期勘误登记）**：当实施期发现**实现-设计实质差距**（实仓行为与 owner doc 语义冲突，而非动作名/旧式码/表述类漂移）时，勘误登记不再是终点，必须履行**三绑定义务**：

1. **①勘误落盘**（既有义务）：在本文档对应用例的「实施期勘误登记」块记录实仓行为与差距。
2. **②bugs/backlog 路由**（新增）：判定差距性质——
   - **设计为准的实现缺口**（owner doc 语义正确、实仓缺失/错误）→ `docs/bugs/YYYY-MM-DD-slug.md` 登记（机制/证据/影响面/修复/successor 触发条件）或路由修复 plan；不能立即修复的须具名 successor 触发条件，禁止匿名延迟；
   - **实现为准的设计漂移**（实仓行为合理/既定架构裁决、owner doc 需修正确认）→ owner doc 修正或显式确认注记，勘误条目升级为正式裁决记录（含证据回链）。
3. **③测试遮蔽处 `workaround-for` 标注**（新增）：当用例以 fixture/断言/自包含建数**吸收了生产缺口**才能通过（即移除后测试将暴露生产行为差异）时，该遮蔽处必须标注 `workaround-for: <缺陷/差距标识>`（bugs 文件名或裁决记录指针），说明遮蔽仅为快照/流程稳定性、非生产必需。零标注的遮蔽 = 把生产缺陷钉死为期望行为（禁止）。

**首个应用实例（2026-08-25，本批 remediation）**：OA-03（C19 orgId 缺 writer）= 义务①②③全履约范例——勘误升级为已修复记录 + bugs 登记 + 去 fixture 遮蔽（遮蔽解除为最优形态，强于保留标注）；OA-02（C13 FX flush）= 义务①②范例（断言/快照随修复翻转，无遮蔽残留）；C18 勘误(4)/C20b 勘误(3) = 义务②「实现为准的设计漂移」分诊范例（见 §6 B9/B10 对应勘误块的裁决注记）。
...
> **V.1 耗时实测注记（2026-08-25，roadmap V.1 执行登记）**：22 用例套件（`mvn test -pl app-erp-all`，含 23 集成类（22 用例 + 试点）+ 基建类）墙钟实测 **104s**——远低于本节模型下界（8 分钟）；模型高估主因 = 启动/seed 装载假设 ~12s/类，实测摊薄后约 3-4s/类（§3.4 风险 ⑤「单用例 6s 低于模型」结论的规模化复认）。全量回归（`mvn clean install -DskipTests` + 全 reactor `mvn test`）墙钟 ≈ **15:45**（01:47 + 13:58）。本节模型保留作为上界参考，后继执行预期以 V.1 实测为准。
...
> **实施期勘误登记（2026-08-24，B10）**：(1) **仿真版本生成形态**——`runSimulation` 返回 `ErpDrpScenarioVersion`（versionNo 递增、status=COMPLETED、computedDrpPlanId 回链**新建 COMPUTED 计划** `{baseCode}-SIM-V{n}`）；场景状态 DRAFT→COMPLETED，二次运行须重置场景 DRAFT（无重置动作，DAO fixture + `paramResolver.invalidateCache()`——进程内 ParamResolver 缓存 computeIfAbsent 不刷新，TestErpDrpSimulation 先例）。(2) **compareVersions 断言粒度**——JUnit 层可精确 delta 断言（B−A 符号约定：totalReplenishmentQtyDelta/totalSafetyStockDelta/lineDiffs；实施锚点 SS 20→30 → **+10/+10/1 行**）；ParamResolver 缓存约束仅浏览器层（webServer 单实例无 invalidate mutation 可达）降级为结构非空，JUnit 层不受限（设计原文复认）。(3) **promote 后正式计划状态与 runDrp 口径**——promoted 计划 = **DRAFT**（非 COMPUTED 直通），复制仿真行为 SUGGESTED（保留仿真口径 SS）；后续 `runDrp` 守卫 DRAFT 通过 → DRAFT→COMPUTED 重算，但净需求口径**回落基线 ErpDrpParameter**（SS=10/suggested=10）——仿真覆盖仅经 SimulationDrpEngine 生效不写回参数表，「提升后可直接运行」成立而重算值非仿真覆盖值。**分诊裁决（2026-08-25，plan 0330-3 Phase 3）：实现为准的设计漂移（owner doc 语义确认注记），非实现缺口**——「重算回落基线参数」是仿真隔离原则的直接推论而非违背：`manufacturing/simulation-engine.md` §参数变体覆盖语义明文「**在不修改主数据的前提下**…safety stock 覆盖值承载于场景参数（master-data 列虽存在但不修改）」+「DRP 仓库补货参数…经场景参数覆盖；**不修改 `ErpDrpParameter` 主数据**」；§转正路径（Decision D）定义 promote = 复制 computed 行到新 DRAFT plan（保留 netRequirement/plannedQuantity）+「转正后的 DRAFT plan **走既有单次引擎释放路径**（用户手动 release…）」——设计路径是**释放复制的仿真结果行**，从未定义「携带仿真参数重算」；若重算携带仿真值反而要求参数落库，与隔离原则矛盾。实仓与设计逐项一致（复制行为 SUGGESTED 保留仿真口径 SS ✓ / DRAFT ✓ / ARCHIVED 防重复转正 ✓）。owner doc 补显式语义注记（见 simulation-engine.md §转正路径 2026-08-25 注）消除「提升后重算应得仿真值」的误读。无 bugs 登记义务；测试断言为设计内行为、无遮蔽，无 workaround-for 义务。B10 实施证据：`TestErpC20bDrpSimulationPromote`。
