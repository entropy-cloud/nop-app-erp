...
| A4.2.59 | A1.20 SP-5 — P1-RC-025 换货功能完全缺失，product-scope 是否裁剪（须运行时确认真相源） | done ✅ | 同上 §7 | A4.2 done | 同上验证报告（**维持 P1-RC-025**：`product-scope.md:18` 未显式裁剪换货 + L1 UC-SAL-06 显式含换货 + ORM 无 returnType 列 + grep 换货 0 命中 → P1 强制实现；须人工确认裁剪方向，按 §4(iii)+§9 冻结不直改真相源；ORM 结构变更须 ask-first） |
...
| A4.2.79 | A1.26 SP-4 — MR1 P1-RC-031 修复落地后 reserved/available 一致性（expiry check 拦截点选择 doConfirm vs doComplete） | todo | 同上 §7 | A4.2 done | **MR1 阻塞**——显式 MR1 P1-RC-031 修复落地前置依赖，MR1 R1.0 尚未启动（同 A4.2.3 MR1-P1-RC-008 阻塞先例），保留 todo 待 MR1 落地后回队（见 plan Non-Goals + Deferred But Adjudicated） |
