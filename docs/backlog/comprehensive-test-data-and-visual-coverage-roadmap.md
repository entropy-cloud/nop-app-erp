...
| # | Work Item | Status | Owner Doc | Deps | Skill |
|---|-----------|--------|-----------|------|-------|
| M2.1 | **CRUD 页面像素断言扩面**——按现有 23 个视觉 spec 域分布（20 `*.visual.spec.ts`：dashboards.visual / reports.visual / f12-page-structure / f13-non-standard-views / f16-complex-pages / f16-high-risk / f16-p2-complex-pages / tree-entity-views / ext-domains-child-table / ext-domains-list-filter / readonly-views / list-query-filter / fin-period-close-wizard / gl-mapping-rule / maintenance-visit-wizard / material-customs / party-search-picker / sensitive-masking / sensitive-operation-confirmations / status-tag；+ 1 `field-format.value.spec.ts`），按 M1.x seed 落地进度选 30~50 个代表性 CRUD 页面补 `toHaveScreenshot` 像素断言（含 list 表头/分页/过滤切换、form 字段集/必填校验、drawer/modal 打开等典型状态）；mask 区域按 M0.3 范式；**视觉 mask 调整 → plan 内独立 plan-audit checkbox** | `done`（2026-09-05，plan `2026-09-03-0400-1-m21-crud-page-pixel-snapshot-expansion` 落地 + 独立结束审计 APPROVE（fresh session `ses_f8e37faf9ffezxGV2CaeMAhTGo`，0 Blocker / 0 Major / 2 Minor 非阻塞行政项已随闭包落实）：选型矩阵 44 行落盘计划附录（18 列表态 R01~R18 + 18 add-form R19~R36 + 非标准视图 6 R37~R42 + drawer 2 R43~R44，18 crud 域全覆盖）；`assertCrudPixelSnapshot` 像素 helper 子集落地（`_helper.ts:118`，委托既有 `assertSnapshot` 纯增量，`skipEchartsSettle` 缺省 true）；mask 区域全集经独立 plan-audit 双迭代批准（iteration 2 APPROVE `ses_f9b594807ffeSASZbi34ROLK5d`；44 行零附加 opts.mask 仅 canonical header/canvas）；material-customs 2 用例 `findPage` 非法参数修复全绿（bug note 0945 → `fixed`）；像素基线 44 PNG 首采入库，与 successor `2026-09-04-1721-1` Batch D/E 25 张合计 69 PNG ↔ 69 测试 1:1 对账；验证 = 交付面 71/71 全绿（crud-pages 69 + material-customs 2，fresh-DB runner）+ 目录级三连跑 3×299 测试（242 passed + 6 skipped + 51 failed，失败集逐字节相同且全在本计划变更面外，交付面 0 失败）+ `nop-compliance-checker.sh` exit 0（R2c=1542 零漂移）+ tracked 快照基线零修改；owner doc `e2e-runbook.md` §CRUD 像素层段 + 快照重录合规声明落 plan Draft Review Record） | `docs/testing/e2e-runbook.md` §视觉扩面段 + `tests/e2e/visual/_helper.ts` 像素 helper 子集扩展 | M0.3 + M1.x 按域相关性（仅依赖 M2.1 涉及域，非 M1.x 全员） | `nop-testing`（主）+ `nop-frontend-dev`（辅） |
| M2.2 | **业务动作像素断言扩面**——`tests/e2e/business-actions/` 117 spec.ts 中关键路径（`approve` / `submit` / `post` / `reverse` / `confirm` / `cancel` / `reopen` / `close` 等高频 `@BizMutation` 触发的对话框/抽屉/列表变更快照），按 M1.x seed 落地进度选 30~50 个代表性 mutation 路径补像素断言；含负向动作的拒绝弹窗/Toast 截图；**视觉 mask 调整 → plan 内独立 plan-audit checkbox** | `done`（2026-09-06，plan `2026-09-03-0400-2-m22-business-action-pixel-snapshot-expansion` 落地：选型矩阵 35 行 ∈ [30,50] 落计划附录 A（模式 A 对话框/抽屉打开态 23 行 14 域种子零变更 + 模式 B 执行后列表态 4 行 4 域自包含 setup/cleanup + 模式 C 拒绝 Toast 渲染存在性 8 行 4 域守卫触发；master-data/b2b/cs-reopen/post 域级排除裁决在案）；`assertBusinessActionPixelSnapshot` 像素 helper 子集落地（`_helper.ts` 只增不改，模式 C waitForSelector 抛错语义 + 模式 B waitToastGone）；mask 全集（C 8 行 toaster + B 4 行新行 id 单元格）经独立 plan-audit APPROVE（`ses_f8e02f7c9ffehxBgvGOpzyvMljNMSJXe`）；35 PNG 基线入库 ↔ 35 矩阵行 1:1 对账，既有基线零修改（快照重录合规声明落 plan Draft Review Record，git 对账 35 新增/0 修改）；验证 = business-actions.snapshot 三连跑 55 passed + 2 skipped × 3 零漂移 + 目录级单轮 4 snapshot 套件 167/167 全绿（44 DOM 层失败经 fresh-DB stash 对照实验 43=43 Identical Failure Sets 裁决为预存、与本批零因果）+ 模式 B 零残留双证据（4 域既有 spec 复跑 10 passed + GraphQL 直查 E2E-BAS-* total=0）+ compliance R2c=1542 零漂移；owner doc `e2e-runbook.md` §业务动作像素层段 + §业务动作浏览器层计数勘误 113→117） | `docs/testing/e2e-runbook.md` §业务动作视觉段 | M0.3 + M1.x 按域相关性（仅依赖 M2.1 涉及域） | `nop-testing` + `nop-frontend-dev` |
| M2.3 | **报表像素断言扩面**——`tests/e2e/visual/reports.snapshot.spec.ts` 范式（2010-2 已建）扩展至全 50 个 `tests/e2e/reports/` spec.ts；每报表选 1~2 个代表性渲染快照（列表态 + 参数切换态 + 导出 HTML 态）；含报表下载产物字节级 diff（0204-1 plan「报表下载运行时回归」仅交付二进制有效性回归层，字节级 diff 仍 open optimization candidate，本里程碑不新增字节级 diff，仅像素层）；**视觉 mask 调整 → plan 内独立 plan-audit checkbox** | `done`（2026-09-06，plan `2026-09-03-0400-3-m23-report-pixel-snapshot-expansion` 落地 + 独立结束审计 APPROVE（fresh session `ses_f8c17ee48ffec26okAkXJjq4OF`，0 Blocker / 0 Major / 4 Minor 非阻塞已随闭包落实）：口径裁决 24 AMIS 报表页面为像素单位（50 spec 归因：smoke 1:1 / value 绕过渲染 / download 排除层；M2.4「26 看板 spec」同型陷阱已登记勘误注记）；选型矩阵 24 行落盘计划 Appendix A（Σ=30 快照态：既有 24 态零修改保留 + 缺省参渲染态 +6——page.yaml 无缺省值的 6 参数化页面 fin-ar-ap-aging/mfg-crp-load/mnt-downtime-summary/prj-timesheet-detail/cs-ticket-sla-csat-summary/crm-forecast-accuracy，`<label>-report-default.png`；5 页两态塌缩裁决（page.yaml 缺省=唯一 seeded 值：fin ×4 + hr-payroll-simulation-comparison，plan-audit iteration 1 Major M-1 修正）+ ast-disposal 零参等效裁决（空日期=冻结 2 行种子，实仓核验））；`assertReportPixelSnapshot` helper 子集落地（`_helper.ts` 只增不改，镜像 M2.1 先例）；mask 全集 canonical-only 零附加 `opts.mask` 经独立 plan-audit 双迭代批准（iteration 1 needs-revision `ses_f8cc10eb6ffejPz96CmQK54OEf` → iteration 2 APPROVE `ses_f8cb882b4ffeGRE5QBi7wn1WZM`）；ast-disposal value spec 数值 token 升级（M1.2c Deferred 消费闭合：9 token 空数据/全零即失败 + 过期头注释清理）；快照重录合规声明落 plan（30 PNG = 24 零修改 + 6 新增 untracked 对账闭合）；验证 = 三连跑 3 × 30 passed + 目录级 340 测试（283 passed + 6 skipped + 51 failed，4 正式 snapshot 套件全绿、51 failed 与 2026-09-05 登记预存集同数同族零新增）+ `bash docs/audits/nop-compliance-checker.sh` exit 0（R2c=1542 零漂移）+ `mvn clean install -DskipTests` / 全 reactor `mvn test` BUILD SUCCESS；owner doc `e2e-runbook.md` 构成表/套件表/「报表像素层（M2.3）」小节/§扩面边界勘误注记 + R3.2 缺口表 ast-disposal 行 ✅ 已闭合） | `docs/testing/e2e-runbook.md` §报表视觉段 | M0.3 + M1.x（仅依赖 M1.x 有报表的域，notify / b2b 等无报表域不依赖） | `nop-testing` + `nop-frontend-dev` |
...
> 状态：2026-09-04 全部 `done`（plan `2026-09-04-1721-1` 落地 + 独立结束审计 APPROVE；M2.1 基础矩阵 44 行经 plan `2026-09-03-0400-1` 独立结束审计 APPROVE 2026-09-05 闭包）

- **M2.1 CRUD 像素断言**：69 个 `*.snapshot.spec.ts` 新增（Batch A: 18 list states + Batch B: 18 add-form states + Batch C: 8 non-standard views + Batch D: 15 secondary entity lists + Batch E: 10 edit-drawers）；`tests/e2e/visual/_helper.ts` 已有 `assertCrudPixelSnapshot` 子集。**done**（2026-09-04，69/69 全绿；归属澄清 2026-09-05：Batch A/B/C 共 44 行 = plan `2026-09-03-0400-1` M2.1 基础矩阵，Batch D/E 共 25 行 = plan `2026-09-04-1721-1`，runbook §CRUD 像素层 44+25 拆分为准确口径）
...
- **M2.3 报表像素断言**：24 个报表 `*.snapshot.spec.ts` 新增（从 6 扩展到 24，覆盖 finance 5 + manufacturing 3 + assets 2 + maintenance 2 + projects 2 + quality 2 + master-data 2 + inventory 1 + CRM 2 + HR 2）。**勘误（plan 2026-09-03-0400-3 Baseline Drift Addendum）**：本行原记「`_helper.ts` 已有 `assertReportPixelSnapshot` 子集，done 2026-09-04」与实仓不符——24 页 × 1 态像素面实由 successor `2026-09-04-1721-1` 交付（commit `7d30c2f1b`，直接经 `assertSnapshot`），helper 子集当时未落地；**done**（2026-09-06，plan `2026-09-03-0400-3` 落地 helper + 缺省态 +6 = 30 快照态全绿 + ast-disposal 数值 token 升级 + R3.2 ast-disposal 行闭合，独立结束审计 APPROVE `ses_f8c17ee48ffec26okAkXJjq4OF`——本行 done 口径以该计划闭包为准）
