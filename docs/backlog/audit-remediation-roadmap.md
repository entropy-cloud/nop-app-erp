...
| R6.7 | **扩展域批次 2**（hr + contract + b2b + logistics + maintenance + aps + drp + notify + master-data）— 须拆 **84**（类别 B 81 + 类别 A 3，1 facade：ErpApsSchedulingProcessor）；合法豁免 36。详见"R6.0 triage 展开 §R6.7" | todo（**hr 子批次 done**：plan `docs/plans/2026-08-01-0001-1-r6-7-hr-d-mutation-split.md` 须拆 30 全部类别 B 全拆 + 9 抽象基类 + 14 BizModel 单行委托 + hr `mvn test` 125/0 + 全量 install 全绿；余 contract/b2b/logistics/drp/aps/maintenance/notify/master-data 54 待后续子批次 plan） | 各域 owner doc | R6.0 | `docs/skills/nop-backend-dev` |
