...
| Q4 | **故障注入测试**：过账悬挂 / 期间结账 / 并发更新故障注入 harness——系统性验证 tryPost 吞异常同型根因族（P1-MA2-032/048/060/068/074/080 跨 6 域）的可恢复性 | todo (Phase 1 done) | `docs/architecture/quality-engineering/fault-injection.md`（Phase 1 done——设计文档经 2 轮独立审查收敛，裁决路径 A 应用层 stub/override 各域具体 `*PostingDispatcher`/`*PostingExecutor` + `IErpFinVoucherBiz`/`IErpSysNotificationBiz` Facade（无通用接口 `IPostingDispatcher`，实仓核验不存在）+ 可恢复性断言契约 A1/A2/A3-unit/A4-alert 对齐 posting-log.md G1-G4 + 6 域覆盖 finance/hr/assets/qa/projects/maintenance + CI 经 maven.yml 自动包含；Phase 2 实现 plan 待起草） | Q0 | none |
