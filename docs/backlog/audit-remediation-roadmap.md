...
| R2.12 | **[代码]** assets 折旧引擎/Processor 链路测试有效性 — P1-MA4-014（异常路径零覆盖 + 残值边界仅 residual=0）；修复=posted=false 窗口 reverseApprove 不对称测试 + 并发首次折旧重复 + 批量部分失败隔离 + 折旧过账悬挂 + 非零残值算术测试 | ready | `docs/design/assets/` | R2.0 | none |
