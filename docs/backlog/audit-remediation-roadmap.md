...
| R6.9 | **MR6 successor：R6.8 核验暴露的完成判据遗漏补拆 + finance 测试日期硬化**：类别 A 补拆（`ErpFinBudgetScenarioRollForwardProcessor` + `ErpFinBudgetScenarioCarryForwardProcessor`；`ErpFinPostingProcessor` process/reverseProcess 裁决=engine 豁免登记 OR 拆两 Processor）+ 类别 B 补拆（`ErpInvCostingReclosePeriodCostsProcessor`）+ `ErpMdSupplierApproval` 6 :46 豁免补登记 + `suspendByPartner` 复议；finance 测试日期硬化（NotesPayable 按 voucher date 预置 OPEN 期间 + Dashboard `YearMonth.now()` 相对 seed）；完成后 MR6 milestone 可闭合 | todo | `docs/architecture/processor-extension-pattern.md` + `docs/architecture/processor-per-mutation-exemption-registry.md §E` | R6.8 done | `docs/skills/nop-backend-dev` |
...
- R6.9：**MR6 successor（todo）**——R6.8 核验暴露的 4 处完成判据遗漏补拆 + finance 测试日期硬化。完成后 MR6 milestone 可闭合。
- **MR6 milestone 状态：OPEN**（完成判据"所有模块所有 BizModel 的 @BizMutation 满足 processor-extension-pattern.md"未完全满足——R6.8 核验发现 4 处遗漏 + 5 处测试日期脆弱，Rule 13 不可降级；R6.9 解决后闭合）。
