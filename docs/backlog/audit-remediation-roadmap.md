...
| R5.8 | **Facade S-mutation 方法清理 + BizModel 配线变更 + 全量验证**：在 R5.1-R5.7 全部完成后，将 **30 个** facade Processor（实测，roadmap 原"36"为陈旧值，与 line 234 一致）中的 S-mutation 业务方法替换为单行委托（`return processor.method(id, svcCtx)`）；BizModel 从 `@Inject` 单 facade 改为 `@Inject` 各 per-mutation Processor；更新 `app-service.beans.xml` 注册；全量 `mvn clean install -DskipTests` + `mvn test` + compliance checker 基线更新；更新 arm-index 中 P1-MA3-048 状态为 "MR5 已填充，孤儿状态已清除" | ready | — | R5.1-R5.7 done | `docs/skills/closure-audit-prompt.md` |
