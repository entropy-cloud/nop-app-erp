...
| # | Work Item | Status | Owner Doc | Deps | Skill |
|---|-----------|--------|-----------|------|-------|
| A5.1 | finance 测试覆盖深度（64 测试 / 137 mutation，比 0.47） | done | `docs/design/finance/` | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A5.2 | manufacturing 测试覆盖深度（30 测试 / 74 mutation，比 0.41） | done | `docs/design/manufacturing/` | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A5.3 | hr 测试覆盖深度（15 测试 / 92 mutation，比 0.16 — 全域最低！） | done | `docs/design/human-resource/` | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A5.4 | assets 测试覆盖深度（14 测试 / 61 mutation，比 0.23） | done | `docs/design/assets/` | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
...
| # | Work Item | Status | Owner Doc | Deps | Skill |
|---|-----------|--------|-----------|------|-------|
| A6.1 | 全域 @BizMutation/@BizQuery 权限注解完整性 grep | done | `docs/design/roles-and-permissions.md` | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A6.2 | finance+mfg+pur+sal 权限深度抽样 | done | `docs/design/roles-and-permissions.md` | A6.1 | `docs/skills/multi-dimensional-audit-prompt.md` |
| A6.3 | 数据权限运行验证（orgId/角色隔离抽样） | done | `docs/design/roles-and-permissions.md` | 0.3 | `docs/skills/multi-dimensional-audit-prompt.md` |
| A6.4 | **保护区域纪律审计**（accounting 过账 / data deletion 是否有 owner doc + 测试 + plan-audit 证据） | done | `docs/context/ai-autonomy-policy.md` §保护区域 | 0.3 | `docs/skills/multi-dimensional-audit-prompt.md` |

### Milestone MA7 — 运维与性能层审计

| # | Work Item | Status | Owner Doc | Deps | Skill |
|---|-----------|--------|-----------|------|-------|
| A7.1 | 错误码完整性（全域 throw new 核对 ErrorCode） | done | `docs/design/domain-design-guidelines.md §7.1` | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A7.2 | 索引完整性（ORM index vs 查询模式，S+A 级域） | done | 各域 orm.xml | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A7.3 | N+1 查询抽样（S 级域列表查询） | done | 各域 BizModel | 0.3 | `docs/skills/open-ended-audit-prompt.md` |
| A7.4 | CI/guard 持续激活验证（compliance checker 基线漂移 + 19 web 测试 @Tag） | done | `docs/audits/compliance-baseline.md` | 0.3 | compliance-checker |
...
| R3.5 | 保护区域过程纪律 + closure-pending 补审计批次（同结果表面合并，P1-MA6-005 超集含 003/004 高优先级切片）— P1-MA6-003（ORM ask-first 5 计划缺独立 plan-audit/closure）+ P1-MA6-004（deployment+auth 2 计划缺独立 plan-audit/closure）+ P1-MA6-005（第三波 ~16 份 closure-pending「completed」计划超集）；修复=Round 3 独立子代理 closure-audit 批次回填 Closure Audit Evidence（含新浮现 closure-pending：MA5 A5.1-A5.4 经 plan 1430-1 + MA7 A7.4 经 plan 1708-2，不静默降级）。**R3.5 done（2026-07-31，plan 2026-07-31-1439-1）**：Phase 1 确定清单=14 份 closure-pending（20 候选 - 6 份已被 Round 1 清理误报剔除）+ 处置策略全部方案 A（方案 B=0）；Phase 2 独立子代理 fresh session closure-audit 14 份**全 PASS**（5 ORM ask-first 保护区域[含 2 P0 hotfix 无 Draft Review Record 历史 gap] + 2 deployment/auth 保护区域[1206-3 假勾选 Gate 经审计 PASS 正当化保留、1351-1 诚实 Gate 勾选] + 7 非保护区域[含 MA3 A3.8 / MA4 A4.8 / MA5 A5.1-A5.4 / MA7 A7.4 四审计计划]），每份回填 Independent Closure Audit (R3.5 Round 3) Auditor 指针 + 五点一致性 + anti-hollow + deferred honesty + 实时仓库复核要点；Phase 3 MA5 A5.1-A5.4 + MA7 A7.4 ready→done + arm-index P1-MA6-003/004/005 回填 MR3 done (R3.5)。零代码/ORM/view 变更（纯审计轨迹回填 + 状态 bookkeeping）。successor：OPEN_AUDIT 轮次形式化（option B，Deferred）| done | `docs/plans/`（14 份计划补 closure）+ `docs/audits/arm-index.md` | R3.0 | none |
| R3.6 | **[ORM ask-first]** billR 缺 (billCode, businessType) 非唯一索引（与 P0-MA2-018 deferred UK 互补不重复 + P2-MA7-005 有界 N+1 批量加载协同）— P1-MA7-001（`ErpFinPostingProcessor.findBillLinks/alreadyPosted/reverseVoucher` 全表扫描，随凭证累积过账延迟线性增长）；修复=`app-erp-finance.orm.xml` ErpFinVoucherBillR `<indexes>` 增 `<index name="IDX_FIN_VOUCHER_BILL_R_BILL_CODE_BIZ_TYPE" unique="false">` on (billCode, businessType) + codegen 增量再生；触及 ORM ask-first 须独立 plan-audit + 人工确认。**R3.6 done（2026-07-31，plan 2026-07-31-1439-2）**：3 Phase 全 done——Phase 1 grep 实测三查询点（`ErpFinPostingProcessor.findBillLinks:895-901` + `BankReconAdjustmentVoucherBuilder.countAdjustmentLinks:109-116` + `BadDebtProvisionService.findUnreversedProvisionVouchers:161-168`）均按 (billCode, businessType) 前缀 + 核实既有索引仅 voucherId 无重复风险 + Decision 列序 (billCode, businessType) + `unique="false"`（P0-MA2-018 边界区分）；Phase 2 ORM `<indexes>` 落地（xmllint well-formed）+ `mvn clean install -DskipTests` BUILD SUCCESS codegen 增量再生（生成物 `_app.orm.xml:859` 含新索引 `unique="false"` grep 确认）+ owner doc `docs/design/finance/posting-log.md` 新增「ErpFinVoucherBillR 索引与过账性能」节；Phase 3 arm-index P1-MA7-001 `todo (R3.6)`→`done (R3.6)` + 闭合回填写 + 一致性复核（ORM 新索引存在 + 生成物含新索引 + `unique="false"` 非 P0-MA2-018 冲突 + arm-index 非裸 todo + roadmap done）。**验证**：xmllint well-formed + finance `mvn test` 306 测试全绿 + compliance checker 无新增命中（纯 ORM 索引元数据零 Java 逻辑变更）。successor：P0-MA2-018 字面 UK 任一方向落地后评估升级为 acctSchemaId 维度复合唯一约束；P2-MA7-005 批量加载化 | done | `module-finance/model/app-erp-finance.orm.xml`+`docs/design/finance/posting-log.md` | R3.0 | none |
| R3.7 | F15 i18n-coverage-checker.sh 接入 CI workflow — P1-MA7-007（checker 仅可手动运行，`.github/workflows/` 零引用，基线干净 0/0 不触发 CI red）；修复=`.github/workflows/compliance.yml` 新增 job 执行 `bash docs/audits/i18n-coverage-checker.sh --strict` + 解析双模式基线（defects=0/gaps=0）单向收紧门控（对齐 F8 激活范式，关闭 F15「dead armor→live guard」最后一公里）；触及 CI 工作流修复须独立 plan。**R3.7 done（2026-07-31，plan 2026-07-31-1439-3）**：3 Phase 全 done——Phase 1 Decision（F8 option b 范式可复用 + 基线来源方案 X 硬编码 0/0 指针 + gate 形态方案 a 独立 job）；Phase 2 `compliance.yml` 新增独立 job `i18n`（inline python3 解析 DEFECTS `... : N`/GAPS `...): N in M files` 行 → 对照 §F15 基线 0/0 单向收紧，actual>baseline => exit 1，解析失败 exit 2 fail-loud）+ `compliance-baseline.md §F15` 门控状态翻转「未接入」→「已接入 live guard」+ §关联工具清单标注；Phase 3 arm-index P1-MA7-007 `todo (R3.7)`→`done (R3.7)` + 闭合回填 + 一致性复核。**验证**：`yaml.safe_load` 合法 + grep workflow 含 i18n 引用 + 本地复跑 `--strict` exit 0（0/0 零回归）+ 解析逻辑实测捕获 + grep §F15 翻转。零 Java/ORM/test 代码变更（CI YAML + 基线 markdown）。successor：F15 基线调高机制（触发=合法 PR 使 gaps>0，须独立计划裁决） | done | `.github/workflows/compliance.yml`+`docs/audits/compliance-baseline.md §F15` | R3.0 | none |
...
| # | Work Item | Status | Owner Doc | Deps | Skill |
|---|-----------|--------|-----------|------|-------|
| V.1 | 全量 mvn clean install -DskipTests + mvn test 绿色验证 | done | — | MR4 done | none |
| V.2 | compliance checker 基线对比（不得高于 M0 基线） | done | `docs/audits/compliance-baseline.md` | V.1 | compliance-checker |
| V.3 | 抽样 E2E 回归 | done | `tests/e2e/` | V.1 | none |
| V.4 | 独立子代理 closure audit（全部 P0 + 关键 P1 修复） | done | `docs/audits/arm-index.md` | V.1-V.3 | `docs/skills/closure-audit-prompt.md` |
| V.5 | 审计报告索引完整性校验（所有 P0/P1 可追溯到修复或 deferred） | done | `docs/audits/arm-index.md` | V.4 | none |
...
>
> **D-mutation**：MR5 **不**处理。**注意：原 v24 及之前此处将 D-mutation 描述为"保留在 facade 中作为已知简化 / deferred"是错误表述——它违反真相源 `processor-extension-pattern.md:7,29,42`（任何 ≥3 步 mutation 必须拆到 per-mutation Processor，例外清单无 S/D 之分）。roadmap 无权给架构违规发豁免。该缺口已登记为 P1-MA3-062 并由新建的 MR6 milestone 承接（见下）。** 此前列举的"纯 D-mutation facade"清单亦不完整——实仓复核显示 **26 个**裸 `<Entity>Processor` facade 持有 D-mutation（非仅 11 个），另有 89 个 BizModel 含 244 个内联 `@BizMutation` 零 Processor 引用，跨全部 19 域。
>
...
### Milestone MR6 — D-mutation + 内联多步 mutation per-mutation 架构合规修复（P1 结构性债务）

> **触发条件**：P1-MA3-062 登记（2026-07-31）。本 milestone 纠正 MR5 的范围遗漏——MR5 仅处理 S-mutation（标准审批六动作），用 owner doc 不承认的"S/D 之分"伪概念把所有 D-mutation + BizModel 内联多步 mutation 打包豁免，违反真相源 `processor-extension-pattern.md`。
>
> **真相源裁决**：`processor-extension-pattern.md:7`（"任何 ≥3 步的方法都应拆到 Processor"）+ `:29`（"每个 `@BizMutation` 方法对应一个独立的 Processor 类"）+ `:42`（"不允许**多个 mutation 共用同一个 Processor 类。这是强制架构纪律"）+ `:44-47` 例外清单（仅"纯查询 ≤2 步 / 单步状态翻转 / 标准 CRUD"豁免，**无 S/D 之分**）。roadmap 横切注记不能凌驾于架构 owner doc（AGENTS.md §真相源优先级）。
>
> **完成判据（用户明确要求）**：所有模块的所有 BizModel 中的 `@BizMutation` 调用都满足 `processor-extension-pattern.md`——即不存在"多 mutation 共用一个 Processor"，不存在"≥3 步 mutation 内联在 BizModel 无 Processor"，不存在"裸 facade 持有 D-mutation"。
>
> **范围（实仓扫描 P1-MA3-062）**：
> - **类别 A**：26 个裸 `<Entity>Processor` facade 持有 ~88 个 D-mutation 入口方法（违反 :42 多 mutation 共用 Processor）。高密度代表：`ErpFinPostingProcessor`(955 行)、`ErpFinAccountingPeriodProcessor`(916)、`ErpMfgSubcontractOrderProcessor`(574)、`ErpMfgWorkOrderProcessor`(552)、`ErpAstSplitProcessor`(546)、`ErpInvLandedCostProcessor`(532)、`ErpAstCipProcessor`(487)、`ErpAstMergeProcessor`(486)、`ErpAstInventoryProcessor`(487)。
> - **类别 B**：89 个 BizModel 含 244 个内联 `@BizMutation` 方法零 Processor 引用（违反 :5/:7），跨全部 19 域。高密度代表：`ErpCsTicketBizModel`(9)、`ErpB2bEdiDocBizModel`(8)、`ErpQaNonConformanceBizModel`(7)、`ErpCtSignatureRequestBizModel`(7)、`ErpPrjProjectBizModel`(7)、`ErpHrSalarySimulationBizModel`(7)。
>
> **R6.0 triage 机制**（同 R*.0 展开器）：读 P1-MA3-062 + 实仓扫描，对类别 B 244 个内联 mutation **逐方法判定**——≥3 步（须拆 Processor）vs ≤2 步/单步状态翻转（合法豁免，登记豁免清单），然后向本 milestone 表追加 per-domain 工作项行 R6.1, R6.2...。类别 A 26 facade 按域归类到对应 R6.x。
...
| R6.7 | **扩展域批次 2**（hr + contract + b2b + logistics + maintenance + aps + drp + notify + master-data）— 须拆 **84**（类别 B 81 + 类别 A 3，1 facade：ErpApsSchedulingProcessor）；合法豁免 36。详见"R6.0 triage 展开 §R6.7" | done（**hr 子批次 done**：plan `docs/plans/2026-08-01-0001-1-r6-7-hr-d-mutation-split.md` 须拆 30 全部类别 B 全拆 + 9 抽象基类 + 14 BizModel 单行委托 + hr `mvn test` 125/0 + 全量 install 全绿；**contract+b2b+logistics+drp+aps 子批次 done**：plan `docs/plans/2026-08-01-0001-2-r6-7-contract-b2b-logistics-drp-aps-d-mutation-split.md` 须拆 36（catA 3 + catB 33）全拆为独立 per-mutation Processor + 1 facade delete-after-extract[类保留] + 14 BizModel 单行委托 + beans.xml 注册一致 + 独立 closure audit PASS（5 域 `mvn test` 171/0 failures + 全量 `mvn clean install -DskipTests` 156 模块全绿）；**maintenance+notify+master-data 子批次 done**：plan `docs/plans/2026-08-01-0001-3-r6-7-maintenance-notify-masterdata-d-mutation-split.md` 须拆 18 全部类别 B 全拆为独立 per-mutation Processor + 5 抽象基类 + 7 BizModel 单行委托 + beans.xml 注册一致（maintenance 14 + notify 3 + master-data 1）+ 会计保护区域 GL 过账 4 mutation 逐字搬运语义不变 + 3 域 `mvn test` 197/0 failures + 全量 `mvn clean install -DskipTests` 156 模块全绿。R6.7 全部 84 须拆完成。R6.x per-mutation 系列合规漂移[R8/R2c/R2d]归 R6.8 全量验证集中裁决） | 各域 owner doc | R6.0 | `docs/skills/nop-backend-dev` |
| R6.8 | **全量验证 + 完成判据核验**：R6.1-R6.7 完成后跑全量 `mvn clean install -DskipTests` + `mvn test` + compliance checker；核验"所有模块所有 BizModel 的 @BizMutation 调用满足 `processor-extension-pattern.md`"（grep 校验零裸 facade 多 mutation + 零 ≥3 步内联 mutation）；更新 arm-index P1-MA3-062 状态 + 重录快照（类名变） | done（核验完成 + successor 已开，**MR6 milestone 保持 OPEN**）[plan `docs/plans/2026-08-01-0656-1`：全量 `mvn clean install -DskipTests` 156 模块 BUILD SUCCESS + compliance checker R8 三次校准（option a 排除被消费 Processor + 域级 abstract）→ R8=0 + R2c 裁决上调 1250→1380 / R2d 28→32 + R1d·R2a·R2b 改善回写 + checker exit 0 + i18n exit 0；完成判据核验**发现 4 处遗漏**登记 R6.9 successor（类别 A `ErpFinBudgetScenarioProcessor` rollForward/carryForward `:42` + `ErpFinPostingProcessor` process/reverseProcess 边界 + 类别 B `ErpInvCostingBizModel.reclosePeriodCosts` `:5/:7` + `ErpMdSupplierApproval` 6 :46 补登记/suspendByPartner 复议，详见 exemption-registry §E + plan §Phase 1 Evidence）；全量 `mvn test` 0 R6.x/R6.8 回归（6 finance 日期 value drift 重录 8 月基线 + 5 pre-existing date-fragility 登记 R6.9 test-hardening successor，非 R6.x 回归——R6.1 2026-07-31 306 全绿）；**MR6 milestone 保持 OPEN**（完成判据未完全满足——Phase 1 条件分支 2，Rule 13 不可降级）] | `docs/architecture/processor-extension-pattern.md` | R6.1-R6.7 done | `docs/skills/nop-backend-dev` |
| R6.9 | **MR6 successor：R6.8 核验暴露的完成判据遗漏补拆 + finance 测试日期硬化**：类别 A 补拆（`ErpFinBudgetScenarioRollForwardProcessor` + `ErpFinBudgetScenarioCarryForwardProcessor`；`ErpFinPostingProcessor` process/reverseProcess 裁决=engine 豁免登记 OR 拆两 Processor）+ 类别 B 补拆（`ErpInvCostingReclosePeriodCostsProcessor`）+ `ErpMdSupplierApproval` 6 :46 豁免补登记 + `suspendByPartner` 复议；finance 测试日期硬化（NotesPayable 按 voucher date 预置 OPEN 期间 + Dashboard `YearMonth.now()` 相对 seed）；完成后 MR6 milestone 可闭合 | done（plan `docs/plans/2026-08-01-0803-1`：4 处遗漏全部消解[BudgetScenario 2 per-mutation Processor + Posting §Engine 豁免 + InvCosting Processor + suspendByPartner Processor + 6 :46 补登记] + 5 处 finance 测试日期硬化 + 执行期修复 1 处生产缺陷[reversal businessDate]；全量 `mvn test -fae` BUILD SUCCESS 0 failures/0 errors + compliance exit 0 + i18n exit 0 + R6.8 grep 复验零违规。**MR6 milestone CLOSED**） | `docs/architecture/processor-extension-pattern.md` + `docs/architecture/processor-per-mutation-exemption-registry.md §E` | R6.8 done | `docs/skills/nop-backend-dev` |

> **迁移模式**（与 MR5 一致 + D-mutation 适配）：
>
> - **类别 A**：从 facade `<Entity>Processor` 复制每个 D-mutation 实现到新建 `<Entity><Method>Processor`（如 `ErpFinPostingProcessProcessor` / `ErpFinPostingReverseProcessProcessor` / `ErpAstDepreciationScheduleExecuteDepreciationProcessor`）。facade 被多 mutation 共享的辅助方法上提到域专属基类或保留 `@Inject` 独立 Service。facade 瘦身为单行委托（如已无 S-mutation 委托则删除 facade）。
> - **类别 B**：对每个经 R6.0 triage 判定为 ≥3 步的内联 `@BizMutation`，新建 `<Entity><Method>Processor`，BizModel 改为 `@Inject` Processor + 单行委托；≤2 步/单步状态翻转 mutation 经 triage 登记豁免，保留 BizModel。
> - **抽象基类 hook 缺失**：D-mutation 无 S-mutation 的 `AbstractApproveProcessor` 等抽象基类 hook（MR5 line 236 原"D-mutation 无抽象基类 hook，迁移路径未定型"是真实约束，但**不构成豁免理由**——D-mutation 不需要审批 hook，直接写 `process()` 主流程 + protected step 即可，对齐 `processor-extension-pattern.md:80-97` 范式）。
>
...
### Milestone MQ — 质量深化（测试有效性 / 安全 / 性能 / 韧性 / 可观测性）

> **新建背景**：audit-remediation MR1-MR6 全部 done + MR6 milestone CLOSED 后，系统性 gap analysis 确认 6 个质量维度完全空白（变异测试 / 属性测试 / 安全扫描 / 故障注入 / 性能基线 / 可观测性——均经实仓核验为零基础设施或仅有声明未落地）+ 1 个已知反复痛点（时钟测试基础设施并行不安全，bug `2026-08-01-finance-period-resolution-month-rollover.md`）。本里程碑以**文档先行**方式推进。
>
> **文档先行工作流（本里程碑每个工作项强制）**：
> 1. **Phase 1 — 设计文档编写 + 独立子代理反复审查**：为该质量维度编写设计/策略文档（落盘 `docs/architecture/quality-engineering/<dimension>.md`），覆盖现状评估（引用实仓证据）/ 目标与非目标 / 技术选型（含替代方案与裁决理由）/ 实施步骤 / 验收判据 / CI 门控设计（如适用）。文档须经**独立子代理反复审查**至收敛（≥2 轮：第 1 轮规范合规审查 + 第 2 轮覆盖面/可执行性审查），每轮输出修改意见，作者修订后重审。审查记录持久化在文档 `## Review Record` 节。**审查通过后方可编写实现 plan。**
> 2. **Phase 2 — 按文档实现**：以审查通过的文档为实施契约，编写 plan 并执行（plan 中引用文档作为范围与验收依据）。
> 3. **Phase 3 — 独立 closure audit**：由独立子代理验证实现与设计文档一致 + 验证命令全绿。
>
> **为何文档先行**：这 7 个维度是审计-修复未触及的新领域，缺乏既有 owner doc 和实施先例。文档先行确保技术选型（pitest vs 其他 / jqwik vs junit-quickcheck / OWASP vs Snyk / nop-entropy 改造 vs 测试侧参数化 / JMH vs 简单 timing）在实施前经过独立审查，避免实施后返工。独立子代理审查消除单点认知偏差。
...
| Q1 | **变异测试有效性**（pitest）：finance/mfg/inv 三域 mutation score 基线 + 存活变异体分析 + CI 门控设计 | done | `docs/architecture/quality-engineering/mutation-testing.md`（Phase 1 done——设计文档经 3 轮独立审查收敛）；`docs/architecture/quality-engineering/mutation-baseline.md`（Phase 2 done——pitest 1.25.8 接入实测三域基线：finance 61% / mfg 60% / inv 59%，generated 实测 fin 4826/mfg ~2580/inv 1807 远大于 MA5 估算；per-module profile 接入 R4 复核确认根 pom reactor-only 继承失效退候选 P1；targetTests `*Test`→`*Test*` 校正本项目 Test-prefix 命名；excludedClasses 双控三域生成噪声 0；存活变异体三分类真实盲区 3152 供 Q4 消费——fin 顶盲区 ErpFinPostingProcessor 92 等过账 Processor 正是 Q4 tryPost 同型根因；CI `.github/workflows/mutation.yml` nightly 软门控 + 单向收紧；successor：末段 re-fork loop 规避 / **mfg 测试隔离 successor 经 plan `2026-08-02-1500-2` 根因裁决为平台级 adjudicated residual**（保留 `excludedTestClasses`，根因三要素均在 nop-entropy：`OrmTimestampHelper` 用 `currentTimeMillis` + `ThreadLocalFrozenClock` 不冻结 `currentTimeMillis` + `AutoTestVars` 易挥发时间戳索引非确定，实证 3 次复跑 1 pass/2 fail；移除须待平台 successor (a)/(b)/(c) 任一落地）） | Q0 | none |
| Q2 | **安全扫描流水线**：OWASP Dependency-Check（transitive CVE）+ SpotBugs FindSecBugs（静态安全规则）接入 CI + 基线建立 + 单向收紧门控 | done | `docs/architecture/quality-engineering/security-scanning.md`（Phase 1 done——设计文档经 2 轮独立审查收敛，裁决 OWASP Dependency-Check（vs Snyk/Dependabot 否决）+ SpotBugs+FindSecBugs（vs SonarQube 否决）+ 路径 A nightly aggregate 调度（NVD 限速硬约束，per-commit 否决）+ 基线格式 CVE set-allowlist + SpotBugs per-type count 双语义（§5.4）+ 独立载体 `security-baseline.md` + §6 插件位置裁决对齐 Q1 R4 根 pom 继承失效先例 + MA6 RBAC 正交边界）；Phase 2 done——plan `2026-08-02-1121-1` 实施完成：根 pom `<profile id="security">` dependency-check-maven 12.1.0 aggregate（reactor-root goal）+ 19 域 erp-*-service per-module spotbugs-maven-plugin 4.9.8.3 + findsecbugs-plugin 1.13.0（R4 同型 per-module）+ `spotbugs-include.xml`（139 FindSecBugs BugPattern，从 JAR 程序化提取）+ `spotbugs-exclude.xml`（4 类生成包排除 _gen/api.beans/api.crud/codegen）+ `dependency-check-suppression.xml`（B 类骨架）+ `security-baseline.md`（19 域首跑 0 FindSecBugs 命中 + CVE 待 NVD key，空 allowlist = 最严格基线）+ `.github/workflows/security.yml`（nightly 02:00 + workflow_dispatch + NVD cache + 双语义 Python gate，4 falsifiability 测试通过）；实施期机制澄清：FindSecBugs 探测器经 JAR 依赖自动注册，includeFilterFile 作用为 scope 报告到 139 安全 BugPattern（非「启用探测器」）；NVD API key 为 infra prereq（CVE 全量首跑 CI nightly 执行）；successor：Critical/High CVE 修复 / per-commit 静态规则 / Snyk/SonarQube） | Q0 | none |
| Q3 | **属性测试**（jqwik）：财务/库存核心不变量形式化验证——借贷平衡 / 期间结账余额归零 / 成本层累加 = 余额表 / costMethod 切换总成本不变 | done | `docs/architecture/quality-engineering/property-based-testing.md`（Phase 1 done——设计文档经 2 轮独立审查收敛，裁决 jqwik（vs junit-quickcheck）+ 路径 C 混合（纯函数不变量用纯 jqwik 绕过 JunitAutoTestCase 快照冲突 line 785 关键风险，端到端用 JunitAutoTestCase 单次）+ 策略 F2 纯内存状态重置 + 首批 P1 借贷平衡/P2 成本层累加/P3 承付释放（三通道 available≥0 对齐 budget.md）+ CI 经 maven.yml 自动包含（C-1）+ 种子固定消除 flaky + Q1 协同接口明确 published baseline 是 posting-filtered 视图）；Phase 2 done——plan `2026-08-02-1400-1` 实施完成：jqwik 1.10.1 test scope 接入 finance+inventory service pom（实施期裁决：设计文档 §3.1.1 原 1.8.x 指南基于 JUnit Platform 1.x，本项目测试栈实测 JUnit Jupiter/Platform 6.0.3 → 升 1.10.1 兼容，回填设计文档 Review Record）+ 3 核心不变量属性 test 落地（P1 `PropertyErpFinDebitCreditBalance` 直调生产 `balanceTotals`/`assertBalanced`；P2 `PropertyErpInvCostLayerAccumulation` 内存 FIFO 模型 + golden 交叉校验锚定 `TestErpInvFifoCosting` 生产实测数字；P3 `PropertyErpFinBudgetCommitmentRelease` 三通道预算模型镜像 `ErpFinBudgetControlBiz:81` available 公式）保真度硬约束满足（P2/P3 Decision (b)）+ tautology 自检（每域注入变异反向证明敏感）+ `compliance.yml` `property-test-coverage` job 2 域门控（C-3 裁决引入）+ 种子固化；实施期 P1 包位置裁决（同包访问 protected 避免 `@SingleSession` 子类触发 gen-aop-proxy-for-test NoClassDefFoundError，回填设计文档 Review Record）；mvn clean install -DskipTests 156 模块全绿，mvn test 全 reactor 绿（除 1 预存 master-data 汇率日期漂移 R6.9 successor 零因果）；closure audit done——独立子代理冷重播 closure 契约逐条对齐 LIVE 仓库（plan-check.mjs --strict passed: true 26/26 checked 0 unchecked，3 属性 test 类 + JqwikSmokeTest 7+ `@Property`×100 tries 种子固化，jqwik 1.10.1 test scope ×2 pom，compliance.yml property-test-coverage job，docs/logs/2026/08-02.md 条目），Closure Audit Evidence 已落盘；**successor done**——plan `2026-08-02-0650-1` 实施 Q3 §9 纯算术不变量扩展（P5 STANDARD 重估红冲恢复 / P6 多币种折算借贷平衡 / P7 折旧残值守恒 / P8 合并抵消归零，4 类一纯 jqwik 属性 test + golden + tautology + 种子固化；P5/P8 Decision (b) 内存模型镜像，P6/P7 Decision (a) 直调生产纯函数 balanceTotals/DepreciationCalculator；assets service pom 新增 jqwik 1.10.1；compliance.yml property-test-coverage 域门控 2→3 域含 assets option (X)；实施期发现并修复 surefire 默认 include 漏拾 `Property*` 类——`{finance,inventory,assets}/erp-*-service/pom.xml` 显式 `<includes>` 增 `**/Property*.java` 使 P1-P8 全在 mvn test 运行，补全 §8.1/C-1 contract，回填设计文档 Review Record；i18n locale 仍 successor） | Q0 | none |
| Q4 | **故障注入测试**：过账悬挂 / 期间结账 / 并发更新故障注入 harness——系统性验证 tryPost 吞异常同型根因族（P1-MA2-032/048/060/068/074/080 跨 6 域）的可恢复性 | done | `docs/architecture/quality-engineering/fault-injection.md`（Phase 1 done——设计文档经 2 轮独立审查收敛，裁决路径 A 应用层 stub/override 各域具体 `*PostingDispatcher`/`*PostingExecutor` + `IErpFinVoucherBiz`/`IErpSysNotificationBiz` Facade（无通用接口 `IPostingDispatcher`，实仓核验不存在）+ 可恢复性断言契约 A1/A2/A3-unit/A4-alert 对齐 posting-log.md G1-G4 + 6 域覆盖 finance/hr/assets/qa/projects/maintenance + CI 经 maven.yml 自动包含；Phase 2 done——plan `2026-08-01-1357-3` 实施完成：`FaultInjectionStubs` 通用 harness 落盘 `module-common-test`（optional 依赖隔离 ORM 泄漏）+ 6 域 9 个故障注入测试（finance A3-unit / assets+hr+qa+projects+maintenance A4-alert）+ `compliance.yml` `fault-injection-coverage` job 6 域门控；mvn test 1929/0/0） | Q0 | none |
| Q5 | **性能基线与回归门控**：关键路径性能基线（1000 凭证过账 / 期间结账 1 万行 / 库存核算 reclose / 报表渲染）+ CI 软门控（不允许退化 > 阈值） | done | `docs/architecture/quality-engineering/performance-baseline.md`（Phase 1 done——设计文档经 2 轮独立审查收敛，裁决路径 B 简单 `@Test` timing（JMH 微基准否决——4 路径中 3 条 DB-bound 集成路径，JMH fork 隔离放大 DB 冷启动噪声；路径 C 混合作 successor hook）+ 路径 C nightly 非阻塞测量趋势（绝对阈值否决——CI runner 硬件方差致假阳性；相对退化比否决——首基线冷启动；nightly 先积累 N=14 后晋升 path B 阻塞门控）+ 4 关键路径（凭证过账 1000/期间结账 1 万 GL/reclose 5000 移动单×50 物料/报表渲染 8 份）数据规模×方差比复现性判据（K=2 untimed warmup + N=10 timed，方差比=(max−min)/median）+ harness 复用 module-common-test + `@Tag("perf")` per-module `<excludedGroups>` 隔离（full-app 先例）+ 独立 `perf-baselines/*.json` 载体 + Q6 ThreadLocalFrozenClock 仅冻结日期（数据确定性非计时确定性，计时 GC/JIT 噪声须 warmup+多轮收敛）+ db-index-design.md stale reference 标注替换）；Phase 2 done——plan `2026-08-02-1121-2` 实施完成：PerfTiming harness + 4 perf 测试类（TestErpFinVoucherPostingPerf / TestErpFinPeriodClosePerf / TestErpInvCostingReclosePerf / TestErpFinReportRenderPerf）+ erp-fin-service / erp-inv-service `<excludedGroups>perf</excludedGroups>` + perf profile 翻转（surefire 3.x CLI 不覆盖 pom 硬编码，须 profile）+ `perf-baselines/baseline-2026-08-02.json` + `LATEST.json` + `perf-baseline.yml` nightly `0 5`（跨计划 cron 协调：sibling Q2 占 02:00，非设计文档原 02:00）；**全部 4 路径方差超阈值**（165.9%/117.2/43.7/32-110），全部登记路径 C 升级（JMH）候选 + 理由（H2 dataset growth + sub-ms jitter，设计文档 §3.4 已预期 successor hook 触发）；mvn clean install -DskipTests 156 模块全绿，mvn test 全绿（perf 被 excluded）；**方差稳定化 successor done**——plan `2026-08-02-0650-2` 实施完成：路径 2 scale increase（2000→6000 GL 行）稳定化至阈值内（variance 43.7%→**5.39% WITHIN <20%**）；路径 1/3/4 残留方差 inherent（GC/H2-jitter/sub-ms）裁决 absolute-tolerance fallback（median 稳定，nightly relative-median-diff 20% 有效）；JMH 经逐路径裁决**不引入**（4 路径均 DB-bound/sub-ms，§3.4 line 125 + §4.3「reclose 最可能 JMH 候选」假设经 Phase 2 实测反转）；`baseline-2026-08-02.json` schemaVersion 2 + `LATEST.json` perPathMetricAdjudication + `performance-baseline.md` Review Record 实施期裁决回填；基线对 per-path 回归门控可用（退化检测基于稳定 median）；**closure audit PASS**——独立子代理 fresh-session 冷重播 closure 契约逐条对齐 LIVE 仓库（plan-check.mjs passed: true 25/25 checked 0 unchecked；4 perf 测试类改造 + baseline-2026-08-02.json/LATEST.json schemaVersion 2 + performance-baseline.md Review Record + perf-baseline.yml 落地与计划逐字符一致），Closure Audit Evidence 已落盘 | Q0, Q6 | none |
| Q6 | **时钟测试基础设施硬化**：CoreMetrics 全局静态 registerClock 并行安全根治 + 日期参数化（消灭月初翻车重录快照税） | done | `docs/architecture/quality-engineering/clock-test-infrastructure.md`（Phase 1 done——设计文档经 3 轮独立审查收敛，裁决路径 C 应用层 thread-local delegating clock；Phase 2 done——plan `2026-08-01-1357-1` 实施完成：ThreadLocalFrozenClock + 15 域子类迁移 + assets 补建 + finance 4 月初税类冻结根治 + TestThreadLocalFrozenClockParallel 并行隔离证明 + TestClockRolloverFinance 跨月模拟 + clock-rollover.yml nightly；mvn test 1920/0/0/1；**生产侧 successor done**——plan `2026-08-02-0650-1` 消解 §1.1 登记的 2 处生产 `LocalDate.now()` 残留[finance `ErpFinVoucherTemplateRenderTemplateProcessor:75` + master-data `ErpMdCurrencyRefreshRatesFromApiProcessor`]→`CoreMetrics.today()` + 2 聚焦日期边界测试[`TestErpFinVoucherTemplateActiveByDate` + `TestErpMdCurrencyRefreshRatesDate`] + 既有 `TestErpMdExchangeRateApiClient` 日期快照硬化（域无关 ThreadLocalFrozenClock，未新建 MdFrozenClockExtension）+ §1.1/§7 successor 闭合回填；mvn clean install -DskipTests 156 模块全绿 + mvn test 3966/0/0 + compliance exit 0 + i18n exit 0） | Q0 | none |
| Q7 | **可观测性补全评估**：nop-platform 无 Micrometer/Prometheus/OTel 的可行性确认 + Spring Boot Actuator 引入评估 + 业务自定义指标落地评估 | done | `docs/architecture/quality-engineering/observability.md`（Phase 1 done——设计文档经 R1+R2 两轮独立子代理审查收敛，R1 `ses_0410de8f4…` accept 0/0/0 + R2 `ses_0410acae…` accept-after-revision 4 MAJOR+2 MINOR 全修订不触及核心 Decision；**关键证伪校正**：roadmap line 697/789 + Q0 README §Q7「nop-platform 无 Micrometer/Prometheus/metrics API」「app-erp 零可观测性依赖」经 `mvn -pl app-erp-all dependency:list -o` 实仓复核——平台 `nop-commons` 声明 `micrometer-core`+`micrometer-registry-prometheus`，`nop-quarkus-web-starter` 声明 `quarkus-micrometer-registry-prometheus`+`quarkus-smallrye-health`，**全传递到 app-erp-all compile 类路径**（compile scope）；`GlobalMeterRegistry` 单例 + `DaoMetricsImpl`/`TaskFlowMetricsImpl`/`MetricsGraphQLHook` 范式 + `QuarkusIntegration.start()` 自动桥接 MeterRegistry bean→`GlobalMeterRegistry` 已落地；§3 裁决路径 A（Quarkus 原生 + 复用平台 SPI，基础设施接入成本接近零）；§6 裁决路径 A（Prometheus+Grafana 部署侧 successor，app 仅 `/q/metrics` 端点契约）；§5 六项业务指标（过账成功率/耗时/期间结账耗时/并发冲突/posting-exception backlog/吞吐）；Spring Boot Actuator 校正为 Quarkus 原生——`app-erp-all/pom.xml:27 → nop-quarkus-web-orm-starter`）；Phase 2 done——plan `2026-08-02-1400-2` 实施完成：(1) 端点验证 `/q/metrics` HTTP 200 `application/openmetrics-text` 含 `nop_dao_*`/`nop_graphql_*`/`nop_orm_*` 平台族 + `erp_*` 业务族共存（`erp_fin_posting_exception_backlog{biz_type="all"}` @PostConstruct 自注册佐证 `QuarkusIntegration.start():48-51` 桥接 + `GlobalMeterRegistry` 单例工作）+ `/q/health/ready` `{"status":"UP"}`；application.yaml 裁决=仅需验证不添加显式配置（Quarkus 默认 `enabled=true`，三端点实测无歧义）；(2) `ErpFinPostingMetrics` SPI 迁移——删 `volatile long[] samples` ring-buffer → Micrometer `Timer`，`p99LatencyMillis()`/`sampleCount()` 签名保留转发 `Timer.takeSnapshot()`（`publishPercentiles(0.99)` 客户端计算 + `ValueAtPercentile.value(MILLISECONDS)` 单位化校正），`IErpFinPostingExceptionBiz.getRuntimeMetrics` 跨层契约保留；(3) 6 业务指标埋点（指标 1 `erp_fin_posting_total` Counter tag=result+biz_type / 指标 2 `erp_fin_posting_duration_seconds` Timer tag=biz_type / 指标 3 `erp_fin_period_close_duration_seconds` Timer tag=fiscal_year+period_no / 指标 4 `erp_concurrency_optimistic_lock_failure_total` Counter repo-wide grep 定位仅 `StockMoveBookkeeper.updateBalanceWithRetry` 一处 tryLock+retry 范式 tag=domain+operation / 指标 5 `erp_fin_posting_exception_backlog` Gauge 复用 `countUnresolved` 5min 后台刷新 / 指标 6 `erp_business_path_throughput_total` Counter 四路径 posting+period_close+costing_reclose+report_render tag=path）；tag 基数复核全 ≤3 有限枚举（biz_type 来自 erp-fin/business-type 字典 ~50 值，fiscal_year/period_no 日历有界低基数，无 voucher_id 高基数）；(4) 单测——3 指标类各补 `GlobalMeterRegistry.instance().find("erp_...").meter()` 非空生产路径单测（delta capture 确定性）+ 既有 SimpleMeterRegistry 单测；smoke CI 裁决=deferred（Quarkus 启动开销大归 e2e.yml successor，非 per-commit maven.yml）；mvn clean install -DskipTests 156 模块全绿，mvn test 全 reactor 绿（除 1 预存 master-data 汇率日期漂移 R6.9 successor 零因果，同 Q3 closure 处理）；successor：部署侧 Prometheus+Grafana+告警 / OTel 分布式追踪 / 高基数业务指标（多租户/多仓）/ smoke CI 归 e2e） | Q0 | none |
...
- **验证基线**：`mvn clean install -DskipTests` 全绿（156 模块）；`mvn test` 全绿（1903 测试，0 failures）
- **Compliance 基线**：见 `docs/audits/compliance-baseline.md`（19 规则）
- **已有审计**：MA1-MA7 七大层审计全部 done；MR1-MR6 P1 修复全部 done + MR6 CLOSED；MV 全量验证收口
- **残留风险**：见范围文档 §3.2
- **质量深化空白（MQ 里程碑范围）**：经实仓核验，以下 6 维度完全空白 + 1 维度有已知痛点——变异测试 pitest（零配置，审计文档反复写"需 pitest 运行"但从未执行）/ 属性测试（零 jqwik/quickcheck 依赖）/ 安全扫描（CI 零 OWASP/Snyk/SpotBugs/Sonar）/ 故障注入（零基础设施，tryPost 吞异常同型根因跨 6 域无回归保护）/ 性能基线（零 `*PerfTest*`/`*Benchmark*`/JMH/Gatling，性能测试被显式 defer）/ 可观测性（nop-platform 无 Micrometer/Prometheus/OTel，finance-posting 仅进程内 ring-buffer 重启即失）/ 时钟基础设施（`CoreMetrics.registerClock` 全局静态并行不安全，bug `2026-08-01-finance-period-resolution-month-rollover.md`，每月初翻车重录快照）
...
### MA5（测试审计，6 工作项）
- A5.1-A5.4：S 级域测试覆盖深度（finance/mfg/hr/assets）— **done**（plan 2026-07-29-1430-1 四域报告产出 + arm-index 登记 11 P1[5 独立 + 6 归并] + 4 P2；0.16 全域最低比根因裁决：缺口为主 + 口径为辅；test-depth-classification.md 计数口径系统性过时 finance 46→64/mfg 19→29/hr 10→15 登记 MR3；独立 closure audit **PASS 2026-07-31 R3.5 Round 3 独立子代理新会话**[plan 1430-1 补审计，Auditor 指针 = independent closure audit subagent fresh session cold-context]——经 R3.5 转 done）— hr 测试/mutation 比 0.16 全域最低
- A5.5-A5.6：测试隔离性 + E2E 有效性 — **done**（plan 2026-07-29-1430-2 两报告产出：A5.5 PASS[6 项已知污染物全 RESOLVED + JUnit 层平台级 localDb 结构性隔离 + E2E 层 cleanup 纪律收敛 + 零新活跃污染物] + A5.6 ⚠️(P1)[258 spec 断言强度矩阵 强66.3%/中8.5%/弱21.3%/工具2.7% + AMIS `$var` bug 已修复 successor 完整]；arm-index 登记 P1-MA5-012 + P2-MA5-005/006/007 已去重；独立 closure audit 通过 2026-07-29）— **MA5 测试层审计全部 6 工作项（A5.1-A5.6）现已全部 done（A5.1-A5.4 经 R3.5 plan 2026-07-31-1439-1 补独立 closure 转 done），MA5 里程碑完成**
...
- A7.4：CI/guard 持续激活验证 — **done**（plan `2026-07-29-1708-2` 报告产出 + arm-index 登记 P1-MA7-007[F15 i18n-coverage-checker.sh 未接入 CI workflow，裁决接入对齐 F8 模式]；零 P0 + 零 P2；19 规则基线精确 0 漂移[M0.3 锚点后 62 commits 含 4 P0 fix 零回归] + CI 工作流激活性 PASS + checker↔基线块同步(19=19) + 19 模块 web 测试 @Tag 100% 一致；P1-MA7-007 是 A4.9 line 165 委托唯一裁决产出无重复；独立 closure audit **PASS 2026-07-31 R3.5 Round 3 独立子代理新会话**[plan 1708-2 补审计，Auditor 指针 = independent closure audit subagent fresh session cold-context]——经 R3.5 转 done；⚠️ 残留风险：审计快照 HEAD `b933e2403`(07-29) 0 漂移为真，当前 HEAD 已浮现 5 项 post-audit compliance 回归[R2a/R2b/R2c/R5/R12c]，属 MV V.2 / 下一 compliance cycle 范围非本审计计划缺陷）——**MA7 里程碑（A7.1-A7.4）全部 done，MA7 完成**
...
- **范围**：仅 S-mutation（标准审批六动作）。**D-mutation 不在 MR5 范围——但"不在本里程碑范围"不等于"豁免"**：原表述"D-mutation 保留在 facade 中……迁移路径未定型"违反真相源 `processor-extension-pattern.md`，已纠正。D-mutation + BizModel 内联多步 mutation 由新建 MR6 milestone 承接（P1-MA3-062）。
...
### MR6（D-mutation + 内联多步 mutation 架构合规）
- **触发条件**：P1-MA3-062 登记（2026-07-31）。纠正 MR5 范围遗漏——MR5 仅处理 S-mutation，把 D-mutation + BizModel 内联多步用"S/D 之分"伪概念打包豁免，违反真相源 `processor-extension-pattern.md:7,29,42`。
- **范围**：类别 A 26 个裸 facade 持有 ~88 D-mutation 入口（多 mutation 共用 Processor，违反 :42）+ 类别 B 89 个 BizModel 含 244 个内联 `@BizMutation`（零 Processor 引用，违反 :5/:7），跨全部 19 域。**完成判据（用户明确要求）= 所有模块所有 BizModel 的 `@BizMutation` 调用满足 `processor-extension-pattern.md`**。
- R6.0：triage 展开器。对类别 B 244 个内联 mutation 逐方法判定 ≥3 步（须拆）vs ≤2 步/单步翻转（合法豁免），类别 A 26 facade 按域归类，展开为 per-domain 工作项行 R6.1-R6.7。
- R6.1-R6.7：per-domain 拆分。新建 `<Entity><Method>Processor`（无需审批 hook，直接 `process()` + protected step，对齐 :80-97）；facade 瘦身为单行委托；BizModel `@Inject` per-mutation Processor；beans.xml 重注册。
- R6.8：全量验证 + 完成判据核验（grep 校验零裸 facade 多 mutation + 零 ≥3 步内联 mutation）+ 快照重录。**done（核验完成 + successor 已开）**——全量构建 156 模块 BUILD SUCCESS + compliance checker R8 三次校准→R8=0 + R2c/R2d 裁决上调 + checker exit 0；完成判据核验**发现 4 处遗漏**（BudgetScenario/Posting facade + InvCosting/MdSupplierApproval）登记 R6.9 successor；全量 `mvn test` 0 R6.x 回归（6 finance 日期 value drift 重录 + 5 pre-existing date-fragility 登记 successor）。
- R6.9：**done**——R6.8 核验暴露的 4 处完成判据遗漏补拆 + finance 测试日期硬化。**MR6 milestone CLOSED**。
- **MR6 milestone 状态：CLOSED**（完成判据"所有模块所有 BizModel 的 @BizMutation 满足 processor-extension-pattern.md"已满足——R6.9 消解 R6.8 核验的全部 4 处遗漏 + 5 处测试日期硬化 + R6.8 grep 复验零违规 + 全量 mvn test 0 failures/0 errors）。
- **与 MR5 的关系**：MR5 状态保持 done（S-mutation 拆分确实完成且正确），MR6 不重开 MR5，仅承接范围遗漏。
- **依赖**：MR5 done（类别 A 部分 facade 同时承载 S-mutation 委托，须 MR5 完成后再动）。


### MQ（质量深化，8 工作项）

> **文档先行工作流**：每个工作项 Phase 1 编写设计文档 → 独立子代理审查（≥2 轮：规范合规 + 覆盖面/可执行性）→ 修订至收敛 → Phase 2 按文档实现 → Phase 3 独立 closure audit。文档落盘 `docs/architecture/quality-engineering/`。详见横切关注点 §文档先行工作流。

- Q0：gap analysis 正式化——将「当前基线」段已列明的 7 维度 NOT FOUND 证据（变异测试 pitest 零配置 / 属性测试零依赖 / 安全扫描零 CI / 故障注入零基础设施 / 性能基线零测量 / 可观测性仅进程内 ring-buffer）+ 时钟基础设施已知痛点引用为输入（**不重新推导，避免双真相源漂移**），产出稳定 `docs/architecture/quality-engineering/README.md`（维度矩阵 + 复杂度分级 + 实施顺序裁决 + 候选维度排除理由记录）。**候选排除维度**（Q0 须显式记录纳入/排除裁决）：契约测试（19 api 模块 + GraphQL——A3.6 已提供静态基线，运行时 Pact 作为 successor 候选）/ 前端可访问性（前端工程域，独立 successor）/ 数据迁移测试（schema 经 MR1-MR6 已稳定，低优先级）/ Nop 升级兼容性（发布流程域）/ i18n 深度（locale 正确性可部分折入 Q3 属性测试，其余 successor）。
- Q1：变异测试（pitest）——当前 1903 个测试的 mutation score 完全未知（审计文档 MA5 反复写"需 pitest 运行"但从未执行）。Phase 1 文档覆盖：pitest 插件配置 + 目标域选择（finance/mfg/inv 高复杂度优先）+ mutation score 基线定义 + CI 门控设计 + 存活变异体分类策略。**关键风险**：Nop 代码生成产物在 `_gen/` 包下，pitest 须配置 `excludedClasses`/`targetClasses` 排除 `*_gen` 包，否则存活变异体被生成代码噪声主导。Phase 2 接入 pitest 跑首次基线 + 分析存活变异体集中的类（测试盲区）。与 Q4 协同：Q1 发现的盲区类正是 Q4 应优先覆盖的可恢复性路径。
- Q2：安全扫描——CI 当前零安全扫描（无 OWASP/Snyk/SpotBugs/Sonar/FindSecBugs）。MA6 仅做 RBAC 注解审计，依赖链 CVE 和静态安全规则完全无人看。Phase 1 文档覆盖：工具选型（**Dependency-Check 为默认**——开源本地可跑；Snyk 需联网+商业许可）+ FindSecBugs 规则集 + CI job 设计 + 基线建立策略（首次扫描必然有发现）+ 单向收紧门控（对齐 compliance-baseline 模式）+ 发现分类工作流。**关键风险**：156 模块传递依赖 CVE 查询受 NVD API 限速，per-commit 全量扫描不现实，Phase 1 须裁决 `aggregate` 聚合模式 + 调度策略（nightly vs per-commit）。Phase 2 接入 CI + 首次扫描基线 + 分类发现。
- Q3：属性测试——ERP 有大量强不变量（借贷必相等 / 期间结账后余额归零 / 成本层累加 = 余额表 / costMethod 切换前后总成本不变 / 承付释放不超余量），当前测试均为黄金路径具体断言，无法证明不变量在任意操作序列下恒成立。Phase 1 文档覆盖：jqwik 选型（vs junit-quickcheck）+ 不变量枚举与优先级 + 属性 test 设计模式（随机操作序列生成器 + 每步不变量断言）+ 与现有 JUnit 5 集成方案。**关键风险**：Nop 测试栈 `JunitAutoTestCase` + `RECORDING`→`CHECKING` 快照录制与 jqwik 多迭代语义冲突（录制一次 vs 回放多次），Phase 1 须裁决属性测试是否绕过 JunitAutoTestCase（用纯 JUnit 5 + jqwik）及夹具重置策略。Phase 2 实现 3-5 个核心不变量属性 test。
- Q4：故障注入——审计反复发现 tryPost 吞异常 → posted=false 静默悬挂（finance P1-MA2-032 / hr P1-MA2-048 / assets P1-MA2-060 / qa P1-MA2-064 / projects P1-MA2-068 / maintenance P1-MA2-074 同型根因跨 6 域），MR1.16 已修单点但无系统性回归保护。Phase 1 文档覆盖：故障注入 harness 设计（受控异常 / 超时 / 事务回滚注入点 + mock dispatcher）+ 可恢复性断言契约（故障后系统进入可恢复状态而非静默悬挂）+ 与现有测试框架集成。**关键风险**：故障注入需明确 nop-entropy 改造 vs 应用层边界——Phase 1 须评估应用层 test-scope bean 覆盖 `IPostingDispatcher`（首选，避免跨仓库依赖）vs 平台层字节码插桩，首选应用层方案。Phase 2 实现 harness + 覆盖 6 域过账悬挂路径的故障注入测试。与 Q1 协同：Q1 发现的盲区类正是 Q4 应优先覆盖的可恢复性路径。
- Q5：性能基线——性能测试被显式 defer 到"首次生产数据规模"（`db-index-design.md`、`posting.md` 等），但无任何基础设施（零 `*PerfTest*`/`*Benchmark*`/JMH/Gatling）。Phase 1 文档覆盖：关键路径选择（凭证过账 / 期间结账 / 库存核算 reclose / 报表渲染）+ 基线定义（数据规模 × 时间阈值）+ 测量方法（JMH vs 简单 `@Test` timing）+ CI 软门控设计（退化阈值 + nightly vs per-commit）。Phase 2 实现基线测量 + 首次基线数据落盘。
- Q6：时钟测试基础设施——`AbstractFrozenClockExtension` + 15 域子类已存在，但 `CoreMetrics.registerClock` 全局静态并行不安全（bug `2026-08-01-finance-period-resolution-month-rollover.md`），导致每月初翻车重录快照。Phase 1 文档覆盖：根因分析（全局静态 vs thread-local）+ 两种修复路径评估（路径 A：nop-entropy `CoreMetrics` 加 thread-local clock 支持；路径 B：测试侧日期参数化 `@ParameterizedTest` 多日期而非钉死一个）+ 优先级裁决 + 与 15 域子类的兼容方案。Phase 2 实现选定的修复方案 + 消除月初翻车税。
- Q7：可观测性——nop-platform 确认无 Micrometer/Prometheus/OTel/metrics API；finance-posting 仅有进程内 ring-buffer metrics（重启即失）+ 进程内 traceId。Phase 1 文档覆盖：Spring Boot Actuator 引入可行性评估（nop-entropy 是否 Spring-based 可直接注入）+ 业务指标定义（过账成功率 / 期间结账耗时 / 并发冲突率 / posting-exception 堆积量）+ 仪表盘设计 + 持久化方案。Phase 2（视评估结论）实现或标注 deferred。
...
graph LR
    ...
    MR5 --> MR2
    MR5 --> MR6[MR6 D-mutation+内联多步架构合规修复]
    MR6 --> MV2b[MV2b MR6后全量验证]
    MG --> Q0[Q0 质量深化范围矩阵]
    Q0 --> Q1[Q1 变异测试]
    Q0 --> Q2[Q2 安全扫描]
    Q0 --> Q3[Q3 属性测试]
...
Mission Driver 的 closed loop 按**文档顺序**取第一个 `todo` 工作项。本 roadmap 中 MA1-MA7 按文档顺序排列，MR1-MR3 排在 MA 之后。因此**实际执行轨迹是串行的**：M0 → MA1 → MA2 → … → MA7 → MR1 → **MR5** → MR2 → MR3 → MR4 → MV → MG。**MR6（D-mutation + 内联多步架构合规，P1-MA3-062）是 MV/MG 收口后新登记的债务里程碑**，依赖 MR5 done，与 MV/MG 时序无关，可独立推进至 R6.8 全量验证（MV2b）。
...
> **MQ 里程碑执行模式**：MQ（质量深化）是 MG 收口后新登记的**主动深化**里程碑，区别于 MR1-MR6 的"审计发现→直接修复"被动模式。MQ 每个工作项遵循**文档先行工作流**（见横切关注点 §文档先行工作流）：Phase 1 设计文档 + 独立审查 → Phase 2 按文档实现 → Phase 3 closure audit。MQ 工作项之间可按依赖图并行（Q1-Q4 + Q6-Q7 仅依赖 Q0；Q5 依赖 Q0 + Q6）。MQ→MG 依赖为项目约定（"收口后深化"序），非技术硬依赖——MQ 的 7 个质量维度技术上不依赖 MG 的 guard/知识沉淀。

### 文档先行工作流（MQ 里程碑专用）

MQ 里程碑的每个工作项（Q1-Q7）强制遵循文档先行流程，区别于 MR1-MR6 的"审计发现→直接修复"模式。原因：MQ 覆盖的 7 个维度是审计-修复未触及的新领域，缺乏既有 owner doc 和实施先例，技术选型需要实施前独立审查。

**Phase 1 — 设计文档编写 + 独立子代理反复审查循环**：

1. 编写该质量维度的设计/策略文档，落盘 `docs/architecture/quality-engineering/<dimension>.md`
2. 文档必须覆盖：现状评估（引用实仓证据）/ 目标与非目标 / 技术选型（含替代方案与裁决理由）/ 实施步骤 / 验收判据 / CI 门控设计（如适用）
3. **独立子代理审查（≥2 轮，由独立子代理执行，不重用作者上下文）**：
   - 第 1 轮：**规范合规审查** — 文档结构完整性 / 与项目约定一致性（AGENTS.md + roadmap authoring guide + plan authoring guide）/ 反模式检查 / 是否引用正确的 owner doc
   - 第 2 轮：**覆盖面与可执行性审查** — 目标是否可达 / 步骤是否可执行 / 验收判据是否可验证 / 是否遗漏关键风险 / 技术选型替代方案是否充分评估 / 是否与现有基础设施冲突
4. 每轮审查输出修改意见（BLOCKER / MAJOR / MINOR 分级），作者修订后重审，直至两轮均无 BLOCKER/MAJOR
5. 审查记录持久化在文档的 `## Review Record` 节（含审查者 task id + 轮次 + 结论 + 修改摘要）
6. **审查通过后方可编写实现 plan**（plan 的 Goals/Non-Goals/Execution Plan 引用文档作为范围与验收依据）

**Phase 2 — 按文档实现**：以审查通过的文档为实施契约，编写 plan 并执行。plan 的 closure gate 增加一条："实现与设计文档一致（无未经文档批准的范围偏离）"。

**Phase 3 — 独立 closure audit**：由独立子代理验证：(a) 实现与设计文档一致 + (b) 验证命令全绿 + (c) 设计文档 Review Record 完整。

> **审查者多样性**：同一文档的 2 轮审查应由不同子代理会话执行（不同 task id），避免单一审查者盲区。审查者不可与作者为同一会话。
