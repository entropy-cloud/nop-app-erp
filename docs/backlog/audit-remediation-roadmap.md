...
| R5.5 | **manufacturing 域** S-mutation 逻辑下沉：ErpMfgWorkOrder（5：submitForApproval/approve/reject/reverseApprove/withdrawApproval）+ SubcontractOrder（5：同上）= 10 文件。**JobCard、ScheduleToJobCard 纯 D-mutation，不在本里程碑范围** | todo | `docs/design/manufacturing/` | MR1 done | `docs/skills/nop-backend-dev` |
| R5.6 | **inventory 域** S-mutation 逻辑下沉：ErpInvCostAdjust（5：submitForApproval/approve/reject/reverseApprove/withdrawApproval）+ LandedCost（2：approve/reverseApprove）= 7 文件。**StockMove、OwnershipTransfer 纯 D-mutation，不在本里程碑范围** | todo | `docs/design/inventory/` | MR1 done | `docs/skills/nop-backend-dev` |
| R5.7 | **剩余域** S-mutation 逻辑下沉：ErpQaRecall（5）+ ErpPrjProjectSettlement（4：submit/approve/reject/cancel）+ ErpCrmLead（1：cancel）= 10 文件。**ErpCrmConversion、ErpApsScheduling 纯 D-mutation，不在本里程碑范围** | todo | 各域 owner doc | MR1 done | `docs/skills/nop-backend-dev` |
| R5.8 | **Facade S-mutation 方法清理 + BizModel 配线变更 + 全量验证**：在 R5.1-R5.7 全部完成后，将 36 个 facade Processor 中的 S-mutation 业务方法替换为单行委托（`return processor.method(id, svcCtx)`）；BizModel 从 `@Inject` 单 facade 改为 `@Inject` 各 per-mutation Processor；更新 `app-service.beans.xml` 注册；全量 `mvn clean install -DskipTests` + `mvn test` + compliance checker 基线更新；更新 arm-index 中 P1-MA3-048 状态为 "MR5 已填充，孤儿状态已清除" | todo | — | R5.1-R5.7 done | `docs/skills/closure-audit-prompt.md` |
