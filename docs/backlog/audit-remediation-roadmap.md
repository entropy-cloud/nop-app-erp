...
| Q2 | **安全扫描流水线**：OWASP Dependency-Check（transitive CVE）+ SpotBugs FindSecBugs（静态安全规则）接入 CI + 基线建立 + 单向收紧门控 | todo | `docs/architecture/quality-engineering/security-scanning.md`（Phase 1 done——设计文档经 2 轮独立审查收敛，裁决 OWASP Dependency-Check（vs Snyk/Dependabot 否决）+ SpotBugs+FindSecBugs（vs SonarQube 否决）+ 路径 A nightly aggregate 调度（NVD 限速硬约束，per-commit 否决）+ 基线格式 CVE set-allowlist + SpotBugs per-type count 双语义（§5.4）+ 独立载体 `security-baseline.md` + §6 插件位置裁决对齐 Q1 R4 根 pom 继承失效先例 + MA6 RBAC 正交边界）；Phase 2 待实现（Dependency-Check + FindSecBugs 接入 CI + 首次扫描 + 基线落盘，设计文档收敛后起草） | Q0 | none |
