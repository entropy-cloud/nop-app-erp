...
- **ORM 变更已获批准（2026-08-12）**：本 roadmap E3 工作项涉及的 ORM 变更已获人工授权，授权记录见 §8.1；实施时不再逐项 ask-first，在整体计划中显式列出即可。
...
- **ORM 授权**：E3.5（文档引用/解析字段）、E3.6（actorType 字段）、E3.2（如物化快照表）、E3.8（独立审计实体）涉及的 ORM 变更**已获人工批准**（§8.1 授权记录），实施时在整体计划中显式列出变更清单，不再逐项 ask-first；E3.3 首选 ext 字段规避 ORM。
...
3. 涉及 ORM 变更的字段/实体在整体计划中显式列出（授权见 §8.1），不再逐项 ask-first。
4. 发现新的平台能力复用机会时更新 §3。
