...
- [x] E3.5 `getInventorySnapshot` 绕过 role-row-filter 未登记入 G1 缺口族（`dashboard-semantic-layer.md` §4 清单补录）（来源：multi-audit P2-2；归 permissions-enforcement G1 族同机制收口） ✅ plan `2026-08-28-0219-3`（G1 行补录 3 个 E3 批同型 API：`getInventorySnapshot`/`checkStockBalanceConsistency`/`getAssetAuditTrail`（逐一核查确认直连绕过）；排除 2 写路径 action 带理由；修复本体仍归 G1 族——本项只补登记）
...
- [x] E3.3 资产 UPDATE 审计白名单过窄：`depreciationMethod/depreciationRate/acquisitionDate/categoryId` 等财务敏感字段变更零审计事件（来源：multi-audit P2-4） ✅ plan `2026-08-28-0219-2`（财务敏感 7 字段入 UPDATE 事件白名单 + remark 变更字段名清单；清单定稿与 VALUATION(currentValue) 边界登记 owner doc `audit-trail-and-custom-fieldsets.md` §1；Proof = `TestErpAstExtFieldsAndAuditTrail.testFinancialFieldChangeAuditedAsUpdateWithFieldNames`）
- [x] 逻辑删除资产型号可被绑定：`ErpAstAssetModel` `useLogicalDelete=true` 但校验钩子不拒已删型号（来源：multi-audit P2-5） ✅ plan `2026-08-28-0219-2`（双路径守卫：新绑定拒绝（钩子层 `ERR_AST_ASSET_MODEL_DELETED` + 执行发现平台 `ObjMetaBasedValidator` deleted-ref 预检先行，标准入口以 `nop.err.dao.unknown-entity` 观察——审计「可被绑定」对标准入口实测不成立，双层登记 owner doc §2）+ 存量豁免（已删 defs 不强制、既有值零拒绝）；Proof = `TestErpAstExtFieldsAndAuditTrail` 双用例）
...
- [x] E3 批新增 5 个对外 action 零 `@Description`（违反 `ai-native-interface.md:72` 自登记约定）（来源：multi-audit P2-10） ✅ plan `2026-08-28-0219-3`（5 action = `uploadApDocument`/`getInventorySnapshot`/`checkStockBalanceConsistency`/`getAssetAuditTrail`/`scheduleToc`，BizModel + IBiz 接口双面补 `@Description`（中文，进 GraphQL schema description）；编译产物注解存在性证明 + 4 域 1179 测试 + introspection 测试回归零失败）
...
- [x] 悬空引用 ×2：`TestErpAiIntrospectionEnabled` 注释指 `ai-native-interface.md` 不存在的实现注记；E3 计划「IBiz 同步」空指（来源：multi-audit P2-12） ✅ plan `2026-08-28-0219-3`（①`ai-native-interface.md` 落「实现注记（AST 单父约束 workaround）」节，测试注释零改动即指向真实内容；②plan 0735-2 Phase 7 ② 补 post-hoc 注记——空指证据 + 归因修正不改结论，引用本计划 plan-id）
- [x] `data-dependency-matrix.md` 缺登记：fin→md `ErpMdPartner` 只读边 + aps→mfg `IErpMfgCapacityProvider` SPI 边（§2.4 矩阵行）（来源：multi-audit P2-13；与开放审计 P2-C fin→pur 行合并一次收口） ✅ plan `2026-08-28-0219-3`（§2.4 补 fin-service→md-dao 只读直访行 + aps-service→mfg-dao SPI 消费行（含缺省空列表兜底降级表述，活仓核实）；与 P2-C 同批收口见下）
- [x] E3 批测试充分性缺口：`ap-document.batch.xml` + job yaml 接线零覆盖；MAINTENANCE/DISPOSAL 审计事件零 JUnit；TOC 阈值等值边界未测；`TestErpInvSnapshotAndStockCheck` 零值等价断言空转；`TestErpFinApDocumentPipeline` `enableActionAuth=FALSE`（来源：multi-audit P2-14）——**fin 部分已收口** ✅ plan `2026-08-28-0219-1`（`TestErpFinApDocBatchWiring` 落地时发现并修复 batch.xml 两处运行时缺陷：接口名 inject unknown-bean + 恒非空 loader 无限 chunk 循环，改 orm-reader 触发扫描 + bean id 注入；job yaml 注册形态断言；enableActionAuth=FALSE 理由注记 + 鉴权拒绝路径由 P2-E E2E 承载）；**非 fin 部分已收口** ✅ plan `2026-08-28-0219-2`（MAINTENANCE/DISPOSAL 审计事件 JUnit = `TestErpAstMaintenanceDisposalAuditEvents`，7 事件类型全覆盖收口；TOC 阈值等值边界 = `TestErpApsSchedulingToc.testTocBottleneckThresholdEqualityBoundary`（== 不触发、> 触发语义锁定）；inv 零值等价断言实化 = `TestErpInvSnapshotAndStockCheck.testStockCheckDetectsAllMismatchTypes`（补零值余额行断言不出现在差异集））
...
- [x] `job-scheduling.md` 漏登记生产 nop-job `erp-fin-ap-doc-processing`（含 `nop.job.erp-fin-ap-doc-processing.enabled/.cron-expr` 两部署键；对齐 :131 inv 行格式补 §3.x）（来源：`docs/audits/2026-08-26-2226-open-audit-erp-enhancement.md` P2-A；文档注册批） ✅ plan `2026-08-28-0219-3`（§3.1 补 WIRED(E3.5, 2026-08-27) 行：batch xml + job.yaml 三件套 / 两部署键 / owner doc 引用；目录汇总 52→53；§7 归档表仅收 batch-candidate 故不入（qa-spc-sampling 同型先例））
- [x] 设计路由层 README「暂不编码」标注陈旧 ×6：`docs/design/README.md:40-41` + finance/aps/inventory/assets 各 README（→「已实现（E3.x，2026-08-26/27）」）（来源：open-audit P2-B；文档注册批） ✅ plan `2026-08-28-0219-3`（6 行按 roadmap §5 E3 归属更新：design README 两行 → E3.6 / E3.1+E3.1b、finance → E3.5、aps → E3.4、inventory → E3.2、assets → E3.3+E3.8；grep 零残留）
- [x] `data-dependency-matrix.md` finance 行 R-only 特征化失实：E3.5 fin→pur 经 `IErpPurInvoiceBiz.save()` command 写边未登记（§2.4 补边 + 行特征修正）（来源：open-audit P2-C；与 multi P2-13 合并收口） ✅ plan `2026-08-28-0219-3`（finance 行 R 列补「唯一写边例外」表述：§4.4 认可的 I*Biz command 编排形态、期末结账同型、非 ORM/S 层反向边；§2.4 两新行见 P2-13 条）
- [x] `known-good-baselines.md` 未落 E3 批全绿行（3889 锚落后于 +23 新测试；P1 修复批全量重跑时落新行并做差量归因）（来源：open-audit P2-D；随 mission VERIFY 批收口） ✅ mission VERIFY 批已落 2026-08-28 行链（3925→3930→3941→3947/0/0/1，最新 669 报告文件）：3925 行对照 3889 锚差量 +36/+7 全额归因（E3.1b/E3.6 看板测试 + F2.2 fin 增量 + E3.5 app-erp-all +3），后续行链逐计划差量归因（2006-2 +5 / 0219-1 +11 / 0219-2 +6），证据见各 plan Closure 与 `docs/logs/2026/08-28.md`
