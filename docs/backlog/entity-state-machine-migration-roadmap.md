...
| M4.63 | ErpHrSalary.paymentStatus 薪酬发放生命周期 Bean（plan-first） | ready | `human-resource/state-machine.md` §Salary | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.64 | ErpHrSalary.approveStatus 薪酬审批轴 Bean（plan-first） | ready | `human-resource/state-machine.md` §Salary | M1.3 + M4.63 | `nop-backend-dev` + `nop-testing` |
