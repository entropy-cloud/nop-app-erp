...
| M4.3 | ErpFinReconciliation.docStatus 对账单生命周期 Bean（plan-first） | ready | `finance/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.10 | ErpFinBadDebt.approveStatus 坏账审批轴 Bean（plan-first） | ready | `finance/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
