...
> 最后更新：2026-08-31-2300（**M5.3 done ✅ mission 完结**——2026-08-31 收官：CG1-CG5 新鲜复核全过 + CG6 独立子代理 closure-audit ACCEPT，全部工作项 done；证据 `docs/audits/check/2026-08-28-2103-entity-state-machine-m5-3/`）（上一轮 2026-08-28-2103：**M5.3 CG1-CG5 主会话全通过**——M5.1 工具 strict mode 0 finding（105 Bean）、CG2 stub 检测能力验证（1 UNREACHABLE_STATE 触发 + strict exit 1）、CG3 多次执行隔离目录 3 子目录 + LATEST 链接 + 历史只读、CG4 9 章节齐全、CG5 compliance 零漂移；**CG6 独立子代理 closure-audit-prompt 验收——本会话子代理通道结构性不可用**——登记 successor 触发条件：A. 子代理通道恢复；B. 人工裁决。证据：`docs/audits/check/2026-08-28-2103-entity-state-machine-m5-3/audit.md`）（**M4.65 done ✅（contract 返利结算单 ErpCtRebateSettlement.status 结算过账轴 Bean，plan `2026-08-14-2000-1-erpct-rebate-settlement-state-machine-bean.md`，§11.2 M4 plan-first）**：全 2 Phase 执行完成——1 Bean（`ErpCtRebateSettlementStateMachine`：1 实现边 postSettlement(DRAFT→POSTED) + assertCanPostSettlement + postSettlementTargetStatus + isTerminal/initial[DRAFT]/terminal[POSTED] + transitions 1 边元数据；CANCELLED **intentional reserved 死状态裁定**（dict 含值 + 全域零 setStatus(CANCELLED) writer + 无 cancel mutation，不入任一集合，dict 值保留；Successor：PM 要求 settlement cancel 工作流时）+ `posted` 布尔列不对称 **watch-only residual 登记**（Processor 从不 setPosted(true)，credit-memo 发票 posted=false 经 pur/sal 管道后续翻转，Bean 不入轴）+ PostSettlementProcessor 接线（@Inject 非 private，内联 `Objects.equals` 守卫改委托 assertCanPostSettlement，try/catch common 码作 cause → 领域 `ERR_CT_SETTLEMENT_ILLEGAL_TRANSITION` 仅传 settlementId+currentStatus；目标态改调 postSettlementTargetStatus；credit-memo AP/AR 负额发票生成 + accrual isSettled 回写 + postedAt/postedBy 全部保留原位））+ 层 1 矩阵 `TestErpCtRebateSettlementStateMachineMatrix` 7 tests（含 CANCELLED 不在任一集合断言）+ 层 2 四方对照 pass（dict↔owner doc §适用对象四补章节↔Bean↔writer：唯一生产 writer PostSettlementProcessor:94 + DRAFT CRUD seed + CANCELLED 零 writer）+ 层 3 回归 82/82 全绿零回归。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。**；上一批：（assets 资产拆分/合并文档双轴 4 Bean 2 实体）**：plan `2026-08-14-1931-3-erpast-structural-documents-state-machine-beans.md` 全 3 Phase 执行完成——4 Bean（Split/Merge 各 Approval 审批轴 5 动作 6 边含 reverseApprove **名义边**（运行时不可达——per-mutation 无条件抛 `ERR_AST_{SPLIT,MERGE}_REVERSE_NOT_SUPPORTED`，无 posted 窗口，不可逆契约 owner doc `split-merge.md` §关键业务规则 5）+ Document docStatus 轴各 1 边 approve(DRAFT→ACTIVE) + isCancelled 守卫）+ 2 facade 接线（validateTransitionForXxx 委托 assertCanXxx + executeApprove 目标态委托 *TargetStatus()，try/catch common 码作 cause → 领域码 4 参重载；cancel 守卫 CANCELLED 判定委托 Document Bean；reverseApprove 不接线——per-mutation 无条件抛错保持；动态守卫/资产卡片结构性重组/AssetSplit/MergePostingDispatcher 仅 post 无 reverse 全部保留原位；per-mutation 12 个 Processor 经 facade 透传自动生效）+ 层 1 矩阵 4 新测试类 44 tests + 层 2 四方对照（dict↔split-merge.md §关键业务规则 5↔Bean↔writer 全对齐；owner doc 补「适用对象四」章节）+ 层 3 回归 299 tests 全绿零回归。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。**；上一批：M4.42–M4.47 done ✅（assets 资产价值调整文档双轴 6 Bean 3 实体）：plan `2026-08-14-1931-2-erpast-value-change-documents-state-machine-beans.md` 全 4 Phase 执行完成——6 Bean（Disposal/Capitalization/ValueAdjustment 各 Approval 审批轴 5 动作 6 边 + Document docStatus 轴：Disposal 1 边 approve(DRAFT→ACTIVE) / Capitalization 2 边含 reverseApprove(ACTIVE→CANCELLED) 特例边 / ValueAdjustment 2 边含 cancel(DRAFT→CANCELLED) 唯一 cancel mutation）+ 3 facade 接线（validateTransitionForXxx 委托 assertCanXxx + executeApprove/executeReverseApprove/doAutoApprove 目标态委托 *TargetStatus()，try/catch common 码作 cause → 领域码 4 参重载；动态守卫/过账/Asset.status side-effect/schedule/stock move 全部保留原位；per-mutation 16 个 Processor 经 facade 透传自动生效）+ 层 1 矩阵 6 新测试类 68 tests + 层 2 四方对照（dict↔owner doc §适用对象三新增节↔Bean↔writer 全对齐；Capitalization Document 轴特例边 + posted=false 不对称窗口 + 动态守卫登记）+ 层 3 回归 255 tests 全绿零回归。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。**；上一批：M4.40+M4.41+M4.52+M4.53 done ✅（assets 域核心生命周期/跟踪实体 status 单轴 4 实体，plan `2026-08-14-1931-1-erpast-core-lifecycle-state-machine-beans.md`：4 Bean + 16 Processor 接线 + 层 1 矩阵 53 tests + 层 2 四方对照 + 层 3 回归 185 tests 全绿；验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（全 19 规则 actual = baseline）。）**；上一批：M4.35–M4.39 done ✅（manufacturing M4 5 轴 3 实体，plan `2026-08-14-0930-1-manufacturing-m4-state-machine-beans.md`：5 Bean + 三路径接线 + 层 1 矩阵 59 tests + 层 2 四方对照 + 层 3 回归 260 tests 全绿；验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0，R12c 40≤40）。）
...
| M4.35 | ErpMfgWorkOrder.docStatus 工单生命周期 Bean（plan-first） | done | `manufacturing/state-machine.md` §WorkOrder | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.36 | ErpMfgWorkOrder.approveStatus 工单审批轴 Bean（plan-first） | done | `manufacturing/state-machine.md` §WorkOrder | M1.3 + M4.35 | `nop-backend-dev` + `nop-testing` |
...
| M4.42 | ErpAstValueAdjustment.docStatus 减值/增值单据生命周期 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.43 | ErpAstValueAdjustment.approveStatus 减值/增值审批轴 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 + M4.42 | `nop-backend-dev` + `nop-testing` |
| M4.44 | ErpAstDisposal.docStatus 处置单据生命周期 Bean（plan-first） | done | `assets/state-machine.md` §Disposal | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.45 | ErpAstDisposal.approveStatus 处置审批轴 Bean（plan-first） | done | `assets/state-machine.md` §Disposal | M1.3 + M4.44 | `nop-backend-dev` + `nop-testing` |
| M4.46 | ErpAstAssetCapitalization.docStatus 资本化单据生命周期 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.47 | ErpAstAssetCapitalization.approveStatus 资本化审批轴 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 + M4.46 | `nop-backend-dev` + `nop-testing` |
| M4.48 | ErpAstSplit.docStatus 拆分单据生命周期 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.49 | ErpAstSplit.approveStatus 拆分审批轴 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 + M4.48 | `nop-backend-dev` + `nop-testing` |
| M4.50 | ErpAstMerge.docStatus 合并单据生命周期 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.51 | ErpAstMerge.approveStatus 合并审批轴 Bean（plan-first） | done | `assets/state-machine.md` | M1.3 + M4.50 | `nop-backend-dev` + `nop-testing` |
...
| M4.65 | ErpCtRebateSettlement.status 返利结算生命周期 Bean（plan-first） | done | `contract/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M5.3 | 最终跨域回归、Delta 覆盖回归、owner doc 对齐及独立 closure audit | **done**（2026-08-31 收官：CG1-CG5 新鲜复核全过（strict 105 Bean/0 finding、5 stub 5 findings/4 类型全覆盖+exit 1、隔离目录、9 章节、compliance R2c=1542 零漂移）+ CG6 独立子代理 closure-audit-prompt **ACCEPT**（触发条件 A：子代理通道恢复）。证据落 `docs/audits/check/2026-08-28-2103-entity-state-machine-m5-3/`（audit.md 2026-08-31 复核段 + closure-audit.md）；plan `2026-08-28-2054-1` completed）| 本路线图所有 owner docs | M5.2 | `nop-testing` + `state-machine-business-review-prompt.md` |
