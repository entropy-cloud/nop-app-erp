...
> 最后更新：2026-08-14（**M4.57 done ✅（logistics 发运单 ErpLogShipment status 单轴实体级状态机 Bean）**：plan `2026-08-14-0456-3-erplog-shipment-state-machine-bean.md` 全 2 Phase 执行完成——1 Bean（5 命名动作 10 边：advise DRAFT→ADVISED / completeShipment ADVISED→DISPATCHED / advanceToInTransit DISPATCHED→IN_TRANSIT / advanceToDelivered {ADVISED,DISPATCHED,IN_TRANSIT}→DELIVERED（**Decision (C) 刻意收紧**，排除 DRAFT/CANCELLED——code 原无来源态守卫，收紧为安全改善）+ cancelShipment 4 源→CANCELLED；initial={DRAFT}、terminal={DELIVERED,CANCELLED}）+ 注册（beans.xml 紧邻 GatewayDispatcher）+ Facade 接线（GatewayDispatcher 注入 Bean 非 private；advise/completeShipment/cancelShipment/advanceTracking 前向守卫全部委托 `assertCanXxx` + `*TargetStatus()` 写回，try/catch common 码→cause-chain `ERR_LOG_SHIPMENT_ILLEGAL_TRANSITION`；幂等短路 4 处 + 网关重试/死信/告警 + DELIVERED path-1 FREIGHT 过账 + path-2 config-gated 到岸成本 + freightSettlementStatus 保留原位）+ 层 1 矩阵 12 测试 + 层 2 四方对照（dict 6 值 ↔ owner-doc §2 ↔ Bean 元数据 ↔ 5 writer call-site + CRUD 排除全对齐；DELIVERED drift 分类 `intentional narrowing` 登记）+ 层 3 回归全绿（39/39，含 8 个既有集成测试零回归）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- > 最后更新：2026-08-14（**M4.63 + M4.64 done ✅（hr 薪酬 ErpHrSalary 双轴 2 实体）**：plan `2026-08-14-0456-2-erphr-salary-state-machine-beans.md` 全 3 Phase 执行完成——2 Bean（Payment 支付轴 2 边 markPaid/voidSalary / Approval 审批轴 5 动作 6 边 + markPaid 交叉守卫）+ 接线（MarkPaid/GenerateBankFile Processor 守卫+目标态委托 Bean、voidSalary BizModel §11.4 终态码保留、xbiz 5 动作经 `ErpHrSalaryApprovalGuard` 委托 Bean——机制替代注记：XLang 不支持 try/catch，common 码→领域码映射下沉 Java Guard）+ 层 1 矩阵 18 测试 + 层 3 回归全绿（232）+ 层 2 四方对照（payment/approval 双轴全对齐；dict 漂移 `erp-hr/salary-approval-status` 6 值 legacy 未引用登记 successor）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- > 最后更新：2026-08-14（**M4.3 + M4.10 done ✅（finance 核销单 docStatus 单轴 + 坏账单 approveStatus 单轴 2 实体）**：plan `2026-08-14-0456-1-erpfin-reconciliation-baddebt-state-machine-beans.md` 全 3 Phase 执行完成——2 Bean（Reconciliation Document 2 边 post/reverse / BadDebt Approval 4 动作 6 边）+ 7 守卫接线（行为保持，ArApItem 核销联动/往来余额刷新/FX 凭证/BAD_DEBT_WRITE_OFF·RECOVERY 凭证/红冲对称回滚/config-gate/SoD/终态守卫原序保留）+ 层 1 矩阵 17 测试 + 层 3 回归全绿（463）+ owner doc `finance/state-machine.md` 补 §对象七（核销单 docStatus 单轴 + posted 不入轴 + CRUD 排除）+ §对象八（坏账 approveStatus 单轴 + auto-approve 旁路 §9.2 + per-mutation stubs 旁路架构事实 + 不可逆 REJECTED 终态）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- > 最后更新：2026-08-14（**M4.6/M4.7 done ✅（finance 员工借款单 ErpFinEmployeeAdvance 双轴 2 实体）**：plan `2026-08-13-1146-3-erpfin-employee-advance-state-machine-beans.md` 全 3 Phase 执行完成——2 Bean（Approval 审批轴 5 边 / Document 生命周期轴 1 边 cancel）+ 6 守卫接线（行为保持，过账/红冲/SoD/markPosted/clearPosted 原序保留）+ 层 1 矩阵 19 测试 + 层 3 回归全绿（454）+ owner doc `finance/state-machine.md` 补 §对象六（双轴 + docStatus 残余值 intentional reserved + SoD/过账边界 + cashRepay 非状态轴 + 被冲抵方引用关系）+ NOT_REVERSED_BEFORE_CANCEL 死码裁定（dead error code，清理归 successor）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- 原 2026-08-14 记录（**M4.4/M4.5 done ✅（finance 费用报销单 ErpFinExpenseClaim 双轴 2 实体）**：plan `2026-08-13-1146-2-erpfin-expense-claim-state-machine-beans.md` 全 3 Phase 执行完成——2 Bean（Approval 审批轴 5 边 / Document 生命周期轴 1 边）+ 6 守卫接线（行为保持，过账/红冲/员工借款冲抵/SoD 原序保留）+ 层 1 矩阵 19 测试 + 层 3 回归全绿（435）+ owner doc `finance/state-machine.md` 补 §对象五（双轴 + docStatus 残余值 intentional reserved + SoD/过账边界）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- 原 2026-08-14 记录（**M4.8/M4.9 done ✅（finance 应收/应付票据 status 2 实体）**：plan `2026-08-13-1146-1-erpfin-notes-receivable-payable-state-machine-beans.md` 全 4 Phase 执行完成——2 Bean（NotesReceivable 7 态 / NotesPayable 4 态）+ 11 守卫接线（行为保持，过账/红冲/授信占用释放/ArApItem/FX 原序保留）+ 层 1 矩阵 26 测试 + 层 3 回归全绿（416）+ owner doc `finance/state-machine.md` 补 §对象三/四（ENDORSED 非终态仅 writeOff / COLLECTION_PENDING·DISHONORED 不过账 / 授信占用释放边界）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- **M4.31/M4.32/M4.33/M4.34 done ✅（inventory docStatus 4 实体）**：plan `2026-08-13-0810-3-inventory-docstatus-m4-state-machine-bean.md` 全 3 Phase 执行完成——4 Bean（TransferOrder 单边 / OwnershipTransfer 3 动作独立 dict / CostAdjust 2 动作 approveStatus-gated / LandedCost 2 动作双轴联动）+ 4 实体 Processor 接线（行为保持，过账/余额重分类/成本层/双轴写/红冲告警原序保留）+ 层 1 矩阵 34 测试 + 层 3 回归全绿（208）+ owner doc `inventory/state-machine.md` 补 4 章节 + TransferOrder 错误码缺陷 Decision（行为保持 + successor Fix）。验证：`mvn clean install -DskipTests` 全仓 BUILD SUCCESS + compliance 零漂移（R5=0/R11=0）。--- **【2026-08-14 人工门控确认（M4 大批次）】**：**【2026-08-14 人工门控确认（M4 大批次）】**：M4.3/M4.4/M4.5/M4.6/M4.7/M4.8/M4.9/M4.10/M4.13/M4.15/M4.17/M4.19/M4.31/M4.32/M4.33/M4.34/M4.35/M4.36/M4.37/M4.38/M4.39/M4.40/M4.41/M4.42/M4.43/M4.44/M4.45/M4.46/M4.47/M4.48/M4.49/M4.50/M4.51/M4.52/M4.53/M4.57/M4.63/M4.64/M4.65 共 39 项的 §11.2 M4 (i) plan-first 人工/owner-doc 门控已于 2026-08-14 经人工确认解除，各 `Plan Status: draft → active`，工作项 Status `todo → ready`（规则 1）。涉及 13 计划：`2026-08-13-0810-1`（purchase docStatus 4 实体）+ `2026-08-13-0810-3`（inventory docStatus 4 实体）+ `2026-08-13-1146-{1,2,3}`（notes receivable/payable + expense-claim 双轴 + employee-advance 双轴）+ `2026-08-14-0456-{1,2,3}`（reconciliation+baddebt + hr-salary 双轴 + logistics shipment）+ `2026-08-14-0930-1`（manufacturing M4 5 实体）+ `2026-08-14-1931-{1,2,3}`（assets core-lifecycle + value-change + structural）+ `2026-08-14-2000-1`（contract rebate-settlement）。门控确认范围 = 各计划以行为保持的矩阵集中化方式迁移、过账/红冲/reversal-listener/授信/ArApItem/stock move 等受保护路径完整保留（各计划 Review Hold + Draft Review Record 记录）。--- 原 2026-08-14 记录（**reverseApprove-§16.4-compliance-fix done ✅**：跨域共享骨架 `AbstractReverseApproveProcessor` §16.4 合规化修复完成，plan `2026-08-14-2000-2-reverse-approve-skeleton-section16-4-compliance.md` 全 2 阶段绿。Work Item `reverseApprove-§16.4-compliance-fix` = 5+ 状态机迁移计划（0945-1/1950-1/1950-2/0930-1/0930-2）共有 Deferred successor，本计划关闭。修复：(a) `doReverseApprove:39` 改调 `rejectedStatus()`（原 `submittedStatus()`）；(b) 新增 default `rejectedStatus() { return "REJECTED"; }`（方案 B，对齐全 25 子类 `wf/approve-status` 标准 dict）；(c) javadoc 更新。零行为回归——独立子代理零信任盘点全 25 子类（Category A 4 覆写 doReverseApprove→Bean REJECTED + Category B 19 覆写公共 reverseApprove + Category C 2 抛不可逆异常），无一依赖骨架默认 body，缺陷运行时惰性。验证：`mvn clean install -DskipTests` 156 模块 BUILD SUCCESS + 跨域 5 service `mvn test` 全绿（0 failures/0 errors）+ compliance R5=0/R11=0 零漂移。`submittedStatus()` 在骨架内成 dead code 保留 abstract 以最小化跨域 churn。--- 原 2026-08-14 记录：**M4 资产域批次草案**：M4.40 ErpAstAsset.status + M4.41 ErpAstDepreciationSchedule.status + M4.52 ErpAstInventory.status + M4.53 ErpAstMaintenance.status（计划 1 `docs/plans/2026-08-14-1931-1-*.md`，status 单轴 4 实体，2 轮草案审查 acceptable-as-draft）+ M4.42/M4.43 ErpAstValueAdjustment docStatus+approveStatus + M4.44/M4.45 ErpAstDisposal + M4.46/M4.47 ErpAstAssetCapitalization（计划 2 `2026-08-14-1931-2-*.md`，文档双轴 facade 范式 3 实体，1 轮 acceptable-as-is）+ M4.48/M4.49 ErpAstSplit + M4.50/M4.51 ErpAstMerge（计划 3 `2026-08-14-1931-3-*.md`，结构性不可逆双轴 2 实体，3 轮 accept）。三计划均触及业财过账保护域（CAPITALIZATION/DISPOSAL/DEPRECIATION/VALUE_ADJUSTMENT + Split/Merge 结构性过账 post-only），§11.2 M4 (i) plan-first 人工/owner-doc 门控**未确认**——各保持 `Plan Status: draft` + `Review Hold`，工作项 Status 维持 `todo`（门控非起草者/审查者可自主解除，project-context 会计/财务硬停止；同 0930-1 制造 / 0810-x / 1146-x batch-consistent hold）。审查零信任实证修正：(1) Asset.status 是**跨实体 writer 轴**——5 处 side-effect writer（资本化 executeApprove 写 IN_SERVICE + executeReverseApprove:102 posted 窗口写 DRAFT[逆资本化]、处置 executeApprove:76 写 SCRAPPED/SOLD + executeReverseApprove:116 写回 IN_SERVICE、Inventory handleShortageDisposal:270 写现有资产 SCRAPPED），无 Asset 自有 status Processor，Bean 矩阵 6 边含跨阶段 Inventory 接线依赖；(2) 资产域无共享 Abstract*Processor 骨架，全 facade `validateTransitionForXxx` + per-mutation 双路径（与维护 Visit skeleton 分化）；(3) Capitalization reverseApprove 额外写 docStatus=CANCELLED（:112，Disposal/ValueAdjustment 无，Document Bean 须特例边）；(4) **Split/Merge reverseApprove 无条件抛 `ERR_*_REVERSE_NOT_SUPPORTED`**（per-mutation :22-26，无 posted 窗口、短路在 facade validateTransitionForReverseApprove 之前），Bean reverseApprove 边为名义元数据（运行时不可达，不接线）。--- 原 2026-08-13 记录：**M4 第二批 plan-first 草案**：M4.14/M4.16/M4.18/M4.20 采购 Receive/Invoice/Payment/Return approveStatus + M4.22/M4.24/M4.26/M4.28 销售 Delivery/Invoice/Receipt/Return approveStatus + M4.11/M4.12 ErpFinBudgetScenario docStatus+approveStatus 计划草案创建（`docs/plans/2026-08-13-1950-{1,2,3}-*.md`），各经独立子代理草案审查 2 轮收敛 acceptable-as-draft；全部触及业财过账保护域。**【2026-08-13 人工门控确认】**：M4.11/M4.12/M4.14/M4.16/M4.18/M4.20/M4.22/M4.24/M4.26/M4.28 的 §11.2 M4 (i) plan-first 人工/owner-doc 门控已于 2026-08-13 经人工确认解除，各 `Plan Status: draft → active`，工作项 Status `todo → ready`（规则 1）。审查零信任实证修正：(1) 采购 Invoice/Payment approve/reverseApprove 经 **facade** `ErpPurInvoice/PaymentProcessor.validateTransitionForXxx`（非 skeleton），与 Receive/Return 的 skeleton 路径分化，Bean 接线须双路径；(2) BudgetScenario rollForward 非 source docStatus 迁移（spawn-new-entity，源保持 APPROVED），仅 carryForward 是 APPROVED→CLOSED source 迁移；(3) carryForward/rollForward 绕过 facade `validateTransition`（使用自身守卫 + 不同错误码），Bean 守卫范围仅 4 动作；(4) BudgetScenario REJECTED 重提三方 drift（owner doc/javadoc=REJECTED→DRAFT vs live code=REJECTED→SUBMITTED）+ owner doc 实际为 `budget.md` 非 `state-machine.md`。**M4 首批 plan-first 草案**：M4.2 ErpFinAccountingPeriod.status + M4.29/M4.30 ErpInvStockMove/StockTake.docStatus + M4.1 ErpFinVoucher.docStatus 计划草案创建（`docs/plans/2026-08-13-2045-{1,2,3}-*.md`），各经独立子代理草案审查 2 轮收敛 acceptable-as-draft。**【2026-08-13 人工门控确认】**：M4.1/M4.2/M4.29+M4.30（+ M3.10/M3.11/M3.12 经 §11.2 M3(iii)→M4 升级）的 §11.2 M4 (i) plan-first 人工/owner-doc 门控已于 2026-08-13 经人工确认解除，各 `Plan Status: draft → active`，工作项 Status `todo → ready`（规则 1）；M3.10-12 表内 M3→M4 reconcile 仍为 Phase 3 Fix 登记项（非阻塞）。审查零信任实证修正：M0.2 §3.5 finance/inventory 测试名引用漂移（`TestErpFinPeriodStateMachine` 实存 / inventory「既有测试：无」实有具名测试）+ StockTake COUNTING↔CONFIRMED doc 标签漂移（保留 CONFIRMED 不改绑）+ ErpFinVoucher 恰 7 生成路径 writer（含首轮漏列的 `CommitmentVoucherGenerator`）+ ErpFinVoucher 无独立 `posted` 字段（过账状态即 docStatus=POSTED）+ StockMove 错误码实为 `ErpInvErrors.ERR_ILLEGAL_STATUS_TRANSITION`（无 `ERR_INV_STOCK_MOVE_ILLEGAL_TRANSITION`）。M3.15/M3.16 done：资产移动单 docStatus 退化轴 + approveStatus 审批轴双 Bean 迁移完成，计划 `2026-08-13-0805-2-erpast-movement-state-machine-beans.md`，首个 INLINE→Bean 迁移范式 + owner doc §适用对象二补章节 + ACTIVE 死状态/退化轴 Decision。M3.17 done：维护请求 `ErpMntRequest.status` 状态机 Bean 迁移完成，计划 `2026-08-13-0805-1-erpmnt-request-state-machine-bean.md`，owner doc §维护请求补 `IN_PROGRESS` doc-drift 修正 + 双源 reject/cancel Decision；M3.10/M3.11/M3.12 计划草案 `2026-08-13-0805-3-erpprj-timesheet-settlement-state-machine-beans.md` 发现分类漂移：Timesheet/Settlement approve 触发会计过账，依 §11.2 M3(iii) 应升级 M4 plan-first，M0.2 清单 §3.4 标`过账=无`与 live code 冲突；计划保持 draft 待人工/owner-doc plan-first 门控，并建议 M0.2 reconcile 表内分类 M3→M4。M3.18/M3.19 done：合同版本 + 返利协议状态轴 Bean 迁移完成，计划 `docs/plans/2026-08-13-1430-3-erpct-version-rebate-state-machine-beans.md`，owner doc 补 §适用对象二/三 + 退化轴/死状态 Decision 登记。M3.2/M3.3/M3.4/M3.5 done：采购四实体 approveStatus 审批轴 Bean 迁移完成，计划 `docs/plans/2026-08-13-0945-1-purchase-approvestatus-state-machine-bean.md`。M1.3 完成：客服试点四方对照审计 PASS + go 裁定 + 批量迁移模板裁定落地至 `docs/architecture/entity-state-machine-bean.md §11`；M2/M3 Deps 门控解除，各迁移项可启动独立 plan。M0.2 展开：M2=19 + M3=19 + M4=65 = 103 纳入轴）
...
| M2.19 | ErpMfgForecast.status Forecast 生命周期 Bean | done | `manufacturing/state-machine.md` §Forecast | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M3.8 | ErpHrEmployee.employmentStatus 雇佣生命周期 Bean | done | `human-resource/state-machine.md` §Employee | M1.3 | `nop-backend-dev` + `nop-testing` |
| M3.9 | ErpHrTimesheet.status Timesheet 审批生命周期 Bean | done | `human-resource/state-machine.md` §Timesheet | M1.3 | `nop-backend-dev` + `nop-testing` |
| M3.10 | ErpPrjTimesheet.status Timesheet 审批轴 Bean | done | `projects/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M3.11 | ErpPrjProjectSettlement.docStatus 结算单据生命周期 Bean | done | `projects/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M3.12 | ErpPrjProjectSettlement.approveStatus 结算审批轴 Bean | done | `projects/state-machine.md` | M1.3 + M3.11 | `nop-backend-dev` + `nop-testing` |
...
| M3.15 | ErpAstMovement.docStatus 资产移动单据生命周期 Bean | done | `assets/state-machine.md` §适用对象二 | M1.3 | `nop-backend-dev` + `nop-testing` |
| M3.16 | ErpAstMovement.approveStatus 资产移动审批轴 Bean | done | `assets/state-machine.md` §适用对象二 | M1.3 + M3.15 | `nop-backend-dev` + `nop-testing` |
| M3.17 | ErpMntRequest.status Request 生命周期 Bean | done | `maintenance/state-machine.md` §Request | M1.3 | `nop-backend-dev` + `nop-testing` |
| M3.18 | ErpCtContractVersion.status 版本生命周期 Bean | done | `contract/state-machine.md` §适用对象二 | M1.3 + M2.18 | `nop-backend-dev` + `nop-testing` |
| M3.19 | ErpCtRebateAgreement.status 返利协议生命周期 Bean | done | `contract/state-machine.md` §适用对象三 | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.3 | ErpFinReconciliation.docStatus 对账单生命周期 Bean（plan-first） | done | `finance/state-machine.md` §对象七 | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.4 | ErpFinExpenseClaim.docStatus 报销单生命周期 Bean（plan-first） | done | `finance/state-machine.md` §ExpenseClaim | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.5 | ErpFinExpenseClaim.approveStatus 报销审批轴 Bean（plan-first） | done | `finance/state-machine.md` §ExpenseClaim | M1.3 + M4.4 | `nop-backend-dev` + `nop-testing` |
| M4.6 | ErpFinEmployeeAdvance.docStatus 预付款生命周期 Bean（plan-first） | done | `finance/state-machine.md` §EmployeeAdvance | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.7 | ErpFinEmployeeAdvance.approveStatus 预付款审批轴 Bean（plan-first） | done | `finance/state-machine.md` §EmployeeAdvance | M1.3 + M4.6 | `nop-backend-dev` + `nop-testing` |
| M4.8 | ErpFinNotesReceivable.status 应收票据生命周期 Bean（plan-first） | done | `finance/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.9 | ErpFinNotesPayable.status 应付票据生命周期 Bean（plan-first） | done | `finance/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.10 | ErpFinBadDebt.approveStatus 坏账审批轴 Bean（plan-first） | done | `finance/state-machine.md` §对象八 | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.13 | ErpPurReceive.docStatus 入库单生命周期 Bean（plan-first） | done | `purchase/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.15 | ErpPurInvoice.docStatus 采购发票生命周期 Bean（plan-first） | done | `purchase/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.16 | ErpPurInvoice.approveStatus 采购发票审批轴 Bean（plan-first） | done | `purchase/state-machine.md` | M1.3 + M4.15 | `nop-backend-dev` + `nop-testing` |
| M4.17 | ErpPurPayment.docStatus 付款单生命周期 Bean（plan-first） | done | `purchase/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.18 | ErpPurPayment.approveStatus 付款审批轴 Bean（plan-first） | done | `purchase/state-machine.md` | M1.3 + M4.17 | `nop-backend-dev` + `nop-testing` |
| M4.19 | ErpPurReturn.docStatus 退货单生命周期 Bean（plan-first） | done | `purchase/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.21 | ErpSalDelivery.docStatus 出库单生命周期 Bean（plan-first） | done | `sales/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.23 | ErpSalInvoice.docStatus 销售发票生命周期 Bean（plan-first） | done | `sales/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.25 | ErpSalReceipt.docStatus 收款单生命周期 Bean（plan-first） | done | `sales/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.27 | ErpSalReturn.docStatus 销退单生命周期 Bean（plan-first） | done | `sales/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.31 | ErpInvTransferOrder.docStatus 调拨单生命周期 Bean（plan-first） | done | `inventory/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.32 | ErpInvOwnershipTransfer.docStatus 所有权转移生命周期 Bean（plan-first） | done | `inventory/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.33 | ErpInvCostAdjust.docStatus 成本调整生命周期 Bean（plan-first） | done | `inventory/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.34 | ErpInvLandedCost.docStatus 到岸成本生命周期 Bean（plan-first） | done | `inventory/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.35 | ErpMfgWorkOrder.docStatus 工单生命周期 Bean（plan-first） | ready | `manufacturing/state-machine.md` §WorkOrder | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.36 | ErpMfgWorkOrder.approveStatus 工单审批轴 Bean（plan-first） | ready | `manufacturing/state-machine.md` §WorkOrder | M1.3 + M4.35 | `nop-backend-dev` + `nop-testing` |
| M4.37 | ErpMfgSubcontractOrder.docStatus 委外单生命周期 Bean（plan-first） | ready | `manufacturing/state-machine.md` §Subcontract | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.38 | ErpMfgSubcontractOrder.approveStatus 委外审批轴 Bean（plan-first） | ready | `manufacturing/state-machine.md` §Subcontract | M1.3 + M4.37 | `nop-backend-dev` + `nop-testing` |
| M4.39 | ErpMfgMaterialIssue.docStatus 领料单生命周期 Bean（plan-first） | ready | `manufacturing/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.45 | ErpAstDisposal.approveStatus 处置审批轴 Bean（plan-first） | ready | `assets/state-machine.md` §Disposal | M1.3 + M4.44 | `nop-backend-dev` + `nop-testing` |
| M4.46 | ErpAstAssetCapitalization.docStatus 资本化单据生命周期 Bean（plan-first） | ready | `assets/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.47 | ErpAstAssetCapitalization.approveStatus 资本化审批轴 Bean（plan-first） | ready | `assets/state-machine.md` | M1.3 + M4.46 | `nop-backend-dev` + `nop-testing` |
| M4.48 | ErpAstSplit.docStatus 拆分单据生命周期 Bean（plan-first） | ready | `assets/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.57 | ErpLogShipment.status 发运单生命周期 Bean（plan-first） | done | `logistics/state-machine.md` §Shipment | M1.3 | `nop-backend-dev` + `nop-testing` |
...
| M4.63 | ErpHrSalary.paymentStatus 薪酬发放生命周期 Bean（plan-first） | done | `human-resource/state-machine.md` §Salary | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.64 | ErpHrSalary.approveStatus 薪酬审批轴 Bean（plan-first） | done | `human-resource/state-machine.md` §Salary | M1.3 + M4.63 | `nop-backend-dev` + `nop-testing` |
| M4.65 | ErpCtRebateSettlement.status 返利结算生命周期 Bean（plan-first） | ready | `contract/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
