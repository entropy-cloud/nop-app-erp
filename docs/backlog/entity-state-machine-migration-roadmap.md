...
| M4.4 | ErpFinExpenseClaim.docStatus 报销单生命周期 Bean（plan-first） | ready | `finance/state-machine.md` | M1.3 | `nop-backend-dev` + `nop-testing` |
| M4.5 | ErpFinExpenseClaim.approveStatus 报销审批轴 Bean（plan-first） | ready | `finance/state-machine.md` | M1.3 + M4.4 | `nop-backend-dev` + `nop-testing` |
