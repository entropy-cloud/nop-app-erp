...
- 2.5b 资产盘点（UC-AST-09：固定资产盘点流程）：`done`（`ErpAstInventory`/`ErpAstInventoryLine` 头行实体 + 3 字典物化经 model→codegen（inventory-status DRAFT/COUNTING/RECONCILING/POSTED/CANCELLED + variance-type SURPLUS/SHORTAGE/MATCHED + inventory-line-disposition）+ `IErpAstInventoryBiz` 8 动作 + `ErpAstInventoryProcessor` protected step 范式（expandAssetsToLines 范围展开 + calculateVariance 差异引擎 + handleSurplusCreateCard 盘盈建新卡 + handleShortageTriggerDisposal 盘亏 SCRAPPED）+ `ErpFinBusinessType.ASSET_INVENTORY_ADJUSTMENT`(460) 新增 + `AssetInventoryAcctDocProvider` 按差异分支科目分解（盘盈 借1601/贷6301，盘亏 借6711/贷1601）+ config-gated（inventory-require-approval/inventory-negative-shortage-blocks）+ reverse 红冲；实现偏离（盘盈/盘亏处置链复用收窄为直接建卡/SCRAPPED，避免双重过账）见 owner doc `inventory.md` §四/§八；owner doc `assets/inventory.md` 创建 + 5 测试全绿，2026-07-07，`docs/plans/2026-07-07-0842-1-assets-physical-inventory-count.md`）
- 2.5c 资产维修管理（UC-AST-10：资产维修工单/费用归集）：`done`（`ErpAstMaintenance`/`ErpAstMaintenanceCost` 头行实体 + 3 字典物化经 model→codegen（maintenance-status DRAFT/SUBMITTED/IN_PROGRESS/COMPLETED/POSTED/CANCELLED + maintenance-treatment CAPITALIZE/EXPENSE + maintenance-cost-type LABOR/SPARE_PART/SUBCONTRACT）+ `IErpAstMaintenanceBiz` 9 动作 + `ErpAstMaintenanceProcessor` protected step 范式（费用归集 + CAPITALIZE/EXPENSE 裁决 + applyTreatmentCapitalize 原值增量+折旧重算 + 防双重扣减分支）+ `ErpFinBusinessType.MAINTENANCE_EXPENSE`(470)/`MAINTENANCE_CAPITALIZATION`(480) 双业务类型新增 + `MaintenanceExpenseAcctDocProvider`（linkedVisit 贷中转科目防双重扣减，独立贷存货/银行）+ `MaintenanceCapitalizationAcctDocProvider`（借固定资产/贷银行）+ `IErpAstDepreciationScheduleBiz.recalculateForCapitalizationMaintenance` 加性扩展折旧重算 + config-gated（maintenance-require-approval/maintenance-capitalize-threshold/maintenance-cap-adjust-depreciation-base/maintenance-linked-credit-clearing）+ reverse 红冲（资本化路径回退原值+折旧，费用化路径仅凭证）+ owner doc `assets/maintenance.md` 创建 + 12 测试全绿，2026-07-07，`docs/plans/2026-07-07-0842-2-assets-maintenance-cost-collection.md`）
...
> - crm（3.1~3.4 之外）：UC-CRM-05（领地管理）、UC-CRM-06（配额管理）✅ done（plan `2026-07-07-1100-1-crm-territory-quota`：ErpCrmLead 加 territoryId 加性列 + TerritoryAssignmentEngine 四 conditionType 匹配 + MANUAL 降级（团队成员表缺失触发条件）+ 区域树维护 createChild/moveTerritory/有子节点禁删 + QuotaRollupCalculator 显式值优先层级聚合 + 定稿/解冻 + 年度均分 + getTerritoryPipeline 三段对比入口）；UC-CRM-07（CPQ 配置定价报价）✅ done（plan `2026-07-07-1430-2-crm-cpq-configure-price-quote`：ProductConfigRuleEngine 纯函数式（四 ruleType + conditionExpression XLang 评估 + EXCLUDED 禁用优先）+ PriceRuleEngine（CUSTOMER_SPECIFIC>PROMOTIONAL>VOLUME + priority + 期间 + 数量区间 + 币种）+ BundlePricingCalculator（PERCENTAGE/FIXED/bundleAmount 覆盖）+ IErpCrmProductConfiguratorBiz.generateQuote 跨域经 IErpSalQuotationBiz.save 建单+lead 回写 + 三 BizModel 维护钩子（规则数上限/discount 一致性/qty 区间/生效日期）+ 6 ErrorCode + 3 配置键 + 4 测试类 34 cases）；UC-CRM-08（序列管理）、UC-CRM-09（CRM 漏斗分析）✅ done（plan `2026-07-07-1430-3-crm-sales-sequence-funnel-analysis`：SequenceAssignmentEngine 纯函数式（四 conditionType LEAD_SOURCE/TERRITORY/PRODUCT_LINE/CUSTOM_FIELD + default 兜底 + priority）+ SequenceStepAdvancer（completionCondition 五值 CALL_COMPLETED/EMAIL_OPENED/EMAIL_REPLIED/MEETING_HELD/TASK_DONE + EMAIL_* 降级 eventType 匹配 + TASK 字典口径映射 event-type + autoCreateEvent 建下一步 + 末步 completedAt）+ 序列进度关联表既定方案（Lead 无序列字段）+ IErpCrmLeadSequenceProgressBiz assign/advance/switch/scanOverdue/getSequencePerformance + ErpCrmSequenceOverdueJob 每日逾期检查 + FunnelAggregationEngine 纯函数式（头度量 + 阶段明细进入/流出/转化率/流失率/停留天数/丢失原因 TOP N JSON + stageName 快照）+ IErpCrmLeadFunnelBiz.refreshFunnel 清旧重建 + getFunnelView 可视化 + ErpCrmFunnelAggregationJob 每日聚合 + 5 ErrorCode + 8 配置键 + 4 测试类 39 cases）
> - customer-service（3.5~3.6 之外）：UC-CS-07（预设应答变量渲染）、UC-CS-10（客服质量监控）；UC-CS-05（知识库搜索与建议）✅ done（plan `2026-07-08-0056-2-cs-knowledge-base-search-suggestion`：`searchKnowledge`/`suggestForTicket` `@BizQuery` LIKE 关键词匹配 title+content（isPublished=true，title 命中优先排序，config-gated limit 钳制）+ `adoptKnowledge` `@BizMutation` 采纳登记 TicketAction actionType=NOTE + knowledgeBaseId 引用（零 ORM/零字典变更）+ AMIS 工单 add/edit 表单 gen-control service 挂接 sendOn subject + Top 5 list 展示 + 采纳按钮 + 7 测试 cases）；UC-CS-09（客户权益）+ UC-CS-10（服务目录）+ UC-CS-12（履行首步）✅ done（plan `2026-07-07-1430-1-cs-entitlement-service-catalog`：EntitlementMatcher 纯函数式匹配引擎 + consume/release/scanExpiring/getUsage + 工单建单权益集成（matchAndAttachSla 内 matchAndConsumeEntitlement，config-gated）+ nop-job 到期扫描自动停用 + 目录分类树维护（成环/深度/禁删）+ createFromCatalog 目录驱动建单（ticketType/slaPolicy 自动填充 + catalogItemId 回写 + formData 字段映射 urgency→priority）+ 履行首步 CREATE_TICKET 落地（ASSIGN_TEAM/NOTIFY_CUSTOMER 占位登记，INVOKE_WORKFLOW/CREATE_CHILD_TICKET SKIPPED 归 successor））。**UC 编号口径以权威源 `docs/design/customer-service/use-cases.md` 为准（UC-CS-09=权益/UC-CS-10=目录/UC-CS-12=履行）；本行历史 UC-CS-08/09 编号待人工勘误。**
> - human-resource（3.7~3.9 之外）：~~UC-HR-04（休假管理）、UC-HR-05（考勤管理）、UC-HR-06（招聘管理）、UC-HR-07（合同管理）~~ ✅ done 胜任力段（plan `2026-07-07-1100-2-hr-competency-management`：七 BizModel 扩展 + AssessmentAggregator 360 多源加权聚合（缺类型重归一化）+ GapAnalysisCalculator 差距严重程度映射 + 字典/矩阵 CRUD 钩子（parentId 成环 + requiredLevel 1-5）+ completeAssessment 触发差距快照清旧重建 + generateDevelopmentPlan 针对 CRITICAL/MODERATE 排序生成 + 计划项状态机）；UC-HR-08 员工调动段 ✅ done（plan `2026-07-08-0517-2-hr-employee-transfer-uc-hr-08`：`ErpHrEmployeeBizModel.transferEmployee` `@BizMutation` 单步直接更新（无审批/状态机）+ `IErpHrDepartmentBiz`/`IErpHrPositionBiz`/`IErpHrEmploymentContractBiz`/`IErpHrLeaveRequestBiz` 跨实体校验/联动 + 合同处理三态 `handleContract` AUTO/YES/NO（config-gated，原 ACTIVE→TERMINATED + 新建 ACTIVE 承袭 contractType）+ 休假冲突告警不阻塞（config-gated）+ 3 ErrorCode + 2 配置键 + AMIS 员工页「调动」drawer 入口 + 9 测试 cases；调动单实体 + 审批工作流归 Deferred）；**UC-HR-02(休假)/05(招聘)/06(考勤)/07(合同到期) `ready` — 计划 `2026-07-10-1100-7`**（erp-survey 对标：Odoo/ERPNext/Axelor/AureusERP 4/4 核心内置；CRUD 骨架已就位，本期补审批状态机+余额+打卡+招聘联动+到期Job；原 Non-Goal 为纯优先级排序无技术约束）
