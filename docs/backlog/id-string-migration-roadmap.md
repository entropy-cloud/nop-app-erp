...
> 最后更新：2026-08-22（**M2.4 done**：assets 域迁移四 Phase 完成，plan `2026-08-22-0002-2`，见 M2/M3 表位次 7 行证据摘要；冻结序位次 8（cs）解锁。**mission 首例晚域退役早域桥接点落地**（B 义务：ast 翻转 → fin 侧 bridge-main-061/063/065 退役 + fin 链重建 + fin 测试复跑全绿）；FQN 盲区复扫补登 bridge-test-134（A3' 先例第二例）。此前同日：M2.1 done（finance，plan `2026-08-22-0002-1`，位次 7/8 解锁 + IoC delta 缺 `x:extends="super"` 潜伏缺陷发现（bug `docs/bugs/2026-08-22-ioc-delta-missing-extends-super.md`，fin 侧已修正，M2.4 起新 delta 直接采用修正版））。更早：2026-08-21 M3.6/M3.8/M3.9/M1.2/M0.2/M1.1/M0.1/M1.3 done）
