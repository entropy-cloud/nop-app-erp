...
Status: `partial completed`（plan `docs/plans/2026-07-20-0629-2-f8-f2-search-filter-and-readonly-views.md` 核心 6 只读实体全绿：inventory 4 + finance 2；CRUD 按钮移除 + edit/add form abstract 切断继承 + asideFilter + query 双筛选面 + dialog 详情）。长尾只读实体（aps/logistics/maintenance 等域流水/日志）按需逐域补齐归 Deferred。
...
Status: `partial completed`（plan `docs/plans/2026-07-20-0629-2-f8-f2-search-filter-and-readonly-views.md` 8 核心列表页 query + asideFilter 双筛选面落地：inventory 4 只读 + finance 2 只读 + purchase/sales/finance 4 主域列表；每域 3-6 业务字段 + filterOp）。扩展域（crm/cs/hr/aps/logistics/b2b/contract/drp）列表页筛选独立 plan 待启动。
