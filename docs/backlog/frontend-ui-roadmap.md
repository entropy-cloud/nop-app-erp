...
**AMIS 退役路径**（P1 Phase 3 Follow-up 触发条件）：移除 `host-amis-adapter` 依赖与 AMIS 资产的触发条件 = 全部 P1-P5 完成（1232-2/3/4 重写 29 手写页 + picker 前端渲染器扩展）**AND** flux E2E 连续 N=5 天全绿 **AND** 混合期豁免清单清零。当前 AMIS schema 页经 flux amis-compat 通路仍可渲染（dashboard smoke 绿），AMIS 运行时保留直至退役条件满足。
