...
> **picker 字段级契约修复完成（2026-08-24，plan `2026-08-24-1147-1` 全 5 阶段 done ✅）**：`nop.app-erp` 项目内 `_vfs/_delta/default/nop/web/xlib/flux-control.xlib` delta 覆盖 nop-entropy 同名基线文件（仅重写 `edit-relation` / `edit-roleId` / `edit-userId` / `edit-ref-id` / `edit-ref-ids` 五个 picker tag 的 `<source>` 输出，从 AMIS 风格 `source`/`joinValues`/`extractValue` 改为 flux 风格 `pickerDialog` + `loadAction` + `columns` + `valueField` + `labelField` + `multiple`）；同时清理 27 个 view.xml 中约 36 处冗余 `<gen-control>` AMIS picker schema 块（仅保留含 `onEvent` 跨字段联动等自定义逻辑的），由 `DefaultControl` 自动接管 picker schema 生成。**核心机制**：依赖平台 `view-gen.xlib:DefaultViewPostExtends` 在 flux 模式下自动重写 `<controlLib>` 子节点为 `flux-control.xlib`（`docs-for-ai/02-core-guides/flux-rendering.md §自动切换机制`）。`ErpPickerSchemaContractTest` 验证 146 picker schema 全部满足 flux PickerSchema 契约（0 违反）。`mvn test -pl app-erp-all` 52 测试类 0 失败。
