...
> 最后更新：2026-08-23（**B2 done——plan `docs/plans/2026-08-23-2133-2-b2-c03-c04-sales-o2c-return.md` 三 Phase 完成**：C03 `TestErpC03O2cGoldenPath`（O2C 销售到收款黄金路径：B2B EDI 入站接收→CRM 商机转化来源核对→Order→Delivery（SALES_OUTPUT COGS=1200）→Invoice（AR_INVOICE + OPEN AR 项）→**Receipt xwf setUserId("0") 复核成立**→settle 域级核销→reconciliation 财务核销 AR openAmount→0，GL 应收 1131 VoucherLine 聚合双处一致）+ C04 `TestErpC04SalReturnWithCs`（销售退货与客服联动：CS 工单六态状态机→自包含已过账销售链→退货红字凭证+负 AR −45.2+余额回减 67.8）RECORDING→CHECKING 往返全绿；全量回归 3813/0/0/1（+2 全额归因 B2，零新增失败）+ app-erp-all 33/0/0/1 + `mvn clean install -DskipTests` 156 模块 BUILD SUCCESS；执行期处置：drp/logistics 日期漂移快照 `*` 通配（M4.1 先例，非 B2 引入）；roadmap B2 → done。下一步 B3 分批执行（plan `docs/plans/2026-08-23-2133-3-b3-c05-c06-inventory-cost.md`））
