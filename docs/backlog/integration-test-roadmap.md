...
> 最后更新：2026-08-24（**B5 done——plan `docs/plans/2026-08-24-0159-2-b5-c09-c10-quality-maintenance.md` 三 Phase 完成**：C09 `TestErpC09QaNcrCapaScrap`（质检门控与 NCR/CAPA/SCRAP 闭环：自包含 MOVING_AVERAGE 产成品 P + BOM inspectionRequired=true → P 入库 10@5 → 工单 DIRECT 审批 → reportCompletion(1) 被完工门控阻断 `erp.err.mfg.work-order.inspection-required`（BOM 门控，close 结案放行证明门控点）→ close CLOSED → 质检单 recordResult REJECTED（自动生成 NCR quantity=lotQuantity=2 + isInspectionCleared=false）→ NCR submitReview → resolve 门控双侧（no-capa / capa-not-completed）→ CAPA 三步 startAction/completeAction/verifyAction → RESOLVED → dispositionType=SCRAP → postNcr（NCR_SCRAP 凭证 Dr 6711/Cr 1401=2×5=10 posted）+ notify 域可达断言路径（自包含模板 + notify → markRead → countUnread 归零））+ C10 `TestErpC10MntRequestSparePart`（维护工单与备件消耗过账：自包含请求 save（EQ-2026-001）→ 非法迁移守卫（OPEN startRepair 拒）→ accept（DRAFT RESPONSIVE Visit + requestId 回填）→ startRepair → complete（六态终态）→ visit schedule/start/complete（设备 UNDER_MAINTENANCE↔RUNNING 联动）→ 自包含备件物料 M + 入库 10@5 → 消耗单 confirm（spare-part-posting-enabled 门控开启 → OUTGOING + MAINTENANCE_ISSUE 凭证 Dr 6602/Cr 1403=2×5=10 + 余额 10→8）→ `ErpAstAsset__get`（AST-2026-002 核对））两用例 RECORDING→CHECKING 往返全绿；执行期裁决：C09 完工门控阻断点 = reportCompletion（非 close）、NCR 通知无自动触发机制（notify 域既有机制作可达断言路径）、SCRAP 不扣物理库存；C10 六态状态机（非 5 态）、visit_task 仅 PLANNED 访问、备件库存自包含建数（seed MAT-003 无 location 不匹配）；全量回归 **3820/0/0/1**（对照 B4 3818/0/0/1 差量 +2 tests / +2 报告文件全额归因 B5，零新增失败）+ app-erp-all **40/0/0/1** + `mvn clean install -DskipTests` 156 模块 BUILD SUCCESS；日志 + 基线差量登记 + 设计文档 §6 C09/C10 勘误登记。下一步 B6（C11/C12，projects+assets））
