...
- **浏览器层验证**（plan 2026-07-26-1500-2，✅ done）：`ErpPartyBizModel` 3 `@BizQuery`（`findParties`/`getParty`/`findReferences`）经 Playwright 浏览器层 E2E 全栈验证（`tests/e2e/business-actions/md-party-query.action.spec.ts` 1 用例 6 场景全绿，镜像 JUnit `TestErpPartyBiz` 8 场景 + GraphQL `/graphql` query 路径 selection set 显式声明）：(1) findParties keyword 命中跨 PARTNER/EMPLOYEE/ORGANIZATION 三实体 ≥3 + PartyRef 字段投影（partyType+code+name+status）断言；(2) findParties partyTypes:[EMPLOYEE] 过滤排除 PARTNER/ORGANIZATION；(3) findParties keyword < 2 字符返回空 List（MIN_KEYWORD_LENGTH=2 守卫）；(4) findParties limit:2 截断 ≤2 可观测；(5) getParty 三类型 + Organization phone/email=null 容忍（实体无对应列）；(6) findReferences Map 结构可达（JSON 对象形态——实测 schema 漂移：原 `[{k,v}]` 数组描述已过时，Map 不支持 selection set，原 `party-search-picker.visual.spec.ts:88` `{ k v }` selection 现触发预存错误属 schema 漂移非本 spec 引入）。**自包含 setup 默认**（对齐 1407-3 Decision 3）——经 `__save` 建测试专用 partner/employee/org（code=`E2E-PARTY-{PN,EMP,ORG}-{ts}` + 三实体 name 共享独占 keyword `E2E-PARTY-KEY-{ts}`），断言针对自包含实体非种子中文 name；cleanup 按反依赖链 __delete（employee→partner FK + organization 独立）不污染共享 DB 基线。owner doc `unified-party-identity.md` §6.6 增「浏览器层验证」实现注记 + e2e-runbook 业务动作表 +master-data 统一 Party 查询行；`mvn clean install -DskipTests` 154 模块 BUILD SUCCESS + master-data 回归抽样 0 新增失败。
- **Deferred successor**：Employee/Organization 引用扫描下游域 SPI 实现 + ErpMdUserAccount 接入统一 Party + 物化视图/反向索引 + Party 合并去重 + 全文索引 + C2 跨境贸易字段扩展 + 业务单据 FK 通用化 + `party-search-picker.visual.spec.ts` findReferences 用例 schema 漂移修复（平台 GraphQL Map 序列化演进为 JSON 对象，原 `{ k v }` selection 已失效）
...
- **浏览器层验证**（plan 2026-07-26-1500-1，✅ done）：`ErpMdMaterialCustomsBizModel` 3 保存校验钩子（declarationNo 唯一 / partnerType CUSTOMS_BROKER / sourceBill 必填）经 GraphQL `__save`/`__update` 写路径 Playwright 浏览器层 E2E 全栈验证（`tests/e2e/business-actions/md-material-customs-validation.action.spec.ts` 5 用例全绿，镜像 JUnit 4 场景 + `__update` 自身排除）：(1) 正路径合法 `__save` 持久化 + `__get` 反查；(2) declarationNo 重复守卫拒绝（`ERR_CUSTOMS_DECLARATION_NO_DUPLICATE`，errors message 含「报关单号」token）；(3) partnerType 非 CUSTOMS_BROKER 守卫拒绝（`ERR_PARTNER_NOT_CUSTOMS_BROKER`，含「报关行」token）；(4) sourceBill 均空守卫拒绝（`ERR_CUSTOMS_SOURCE_BILL_REQUIRED`，含「业务单据」token）；(5) `__update` 自身排除（`enforceDeclarationNoUnique` 经 `entity.getId()` 排除自身，对齐 0500-3 `enforceMutex selfId` 范式）。自包含经 `ErpMdPartner__save` 建 CUSTOMS_BROKER partner（种子 0 行须自建）+ materialId 复用种子 MAT_1=1；拒绝路径经 `saveRaw` 裸 mutation 断言。owner doc `cross-border-trade.md` §3 增「浏览器层验证」实现注记 + e2e-runbook 业务动作表 +master-data 物料报关记录行；`mvn clean install -DskipTests` 154 模块 BUILD SUCCESS + master-data dashboard 回归 0 新增失败。
...
- **浏览器层验证**（plan 2026-07-26-1407-2，✅ done）：预算滚动复制 `rollForward` 3 策略 + 结转 `carryForward` 4 规则两入口经 Playwright 浏览器层 E2E 全栈验证（`tests/e2e/business-actions/fin-budget-rollforward-carryforward.action.spec.ts` 7 用例，镜像 JUnit 3 策略 + 4 规则场景，config 双门控 `erp-fin.budget-roll-forward-enabled=true` + `erp-fin.budget-carry-forward-enabled=true` playwright.config.ts webServer JVM arg 启用）。**自包含 setup 隔离**（经 `ErpMdSubject__save` 唯一 code EXPENSE/DEBIT 使 actual 聚合按 subjectId 隔离 + `ErpFinAccountingPeriod__save` source/target 年度期间 + `ErpFinBudgetScenario__save` 直置 APPROVED/DRAFT 绕状态机避免 BUDGET 影子凭证污染 + actual voucher 直置 postingType=NORMAL/POSTED/isReversed=false）；rollForward 3 策略行金额确定性派生断言（FIXED_PERCENTAGE 1000→1000 / ZERO_BASED 1000→0 / INCREMENTAL 1000→1050 ×(1+0.05)）+ 目标 fiscalYear/parentScenarioId/docStatus=DRAFT 经 `__get` 独立反查 + RollforwardLog 写入；carryForward 4 规则结转金额确定性派生断言（REMAINING_FULL 1000−400=600 / REMAINING_RATIO (1000−400)×0.5=300 / USED_FULL=400 / NONE=0）+ 源方案 docStatus=CLOSED + CarryForwardLog 写入。owner doc `budget.md` §滚动预算自动复制引擎 + §结转规则引擎 各增「浏览器层验证」实现注记 + e2e-runbook 业务动作表 +finance 预算滚动复制 + 结转行；`mvn clean install -DskipTests` 154 模块 BUILD SUCCESS + 既有预算 spec 回归 0 新增失败。
...
- **浏览器层验证（plan 2026-07-26-1407-3）**：`refreshRatesFromApi` `@BizMutation` 经 GraphQL `/graphql` 全栈可达性补全栈覆盖，收口「JUnit 单层验证但零浏览器层 E2E」缺口。`md-exchange-rate-api.action.spec.ts`（1 用例）双层断言：(1) mutation 返回 List + Mock 确定性 rate 值（USD→CNY 7.20 / USD→EUR 0.92，rateType=MIDDLE）；(2) `ErpMdExchangeRate__findPage` 反查持久化 fromCurrency/toCurrency code + rate；(3) 幂等重写 count 稳定（2→2 不累积）。config-gate webServer JVM arg 全局启用 5 项 config（关闭路径守卫 `ERR_EXCHANGE_RATE_API_UNAVAILABLE` 经 JUnit 已覆盖，浏览器层不重复）；自包含 EUR setup（种子仅 CNY+USD）；MIDDLE/SPOT rateType 隔离 cleanup 不触碰种子 SPOT 行。rate limiting RATE_LIMITED 浏览器层不复现归 watch-only residual（JUnit 覆盖）。
...
- **浏览器层验证**（plan 2026-07-27-0930-1，✅ done）：`ErpModuleMetaBizModel` 4 `@BizQuery`（`listModules`/`getModule`/`checkDependencyIntegrity`/`listOptionalFeatures`）经 Playwright 浏览器层 GraphQL `/graphql` 全栈验证（`tests/e2e/business-actions/all-module-meta-diagnostic.action.spec.ts` 1 用例 6 场景全绿，镜像 JUnit `TestModuleMetaReader` 7 场景 + 1500-2 C1 同型非实体 `@BizQuery` 读路径范式）：(1) listModules 返回 List 经 `erp/*` 前缀过滤后精确 == 19（收紧 JUnit `>= 19`）+ 全部 version="1.0.0" + 含 erp/md（DAG 根 businessDependencies=null 向后兼容）+ 含 erp/pur（6 businessDependencies + 1 optionalFeature 样本）；(2)-(4) getModule 三路径——erp/md（businessDependencies=null DAG 根）+ erp/pur（6 deps 含 {erp/md,1.0.0} + supplier-scorecard-red-gate）+ erp/nonexistent（返回 null 容忍，对齐 `ModuleMetaReader.getModule:42-45`）；(5) checkDependencyIntegrity ok=true + missing=[] + mismatches=[]（fresh-DB 19 域一致版本 + DAG 闭合）；(6) listOptionalFeatures 含 `{supplier-scorecard-red-gate, erp-pur.scorecard-prevent-on-red, defaultValue:true}`（defaultValue Object scalar 序列化为 JSON 布尔）。**无 setup/cleanup**（模块元数据为运行时启动期 codegen 产物非 DB 行，非实体 BizModel 无 `__save` 入口）+ **无 config-gate**（4 `@BizQuery` 读路径无 config 门控）；selection set 经 `GraphQLClient.raw` 内联完整 query + selection set（4 `@BizQuery` 全返回复杂类型，镜像 cs-canned-response/fin-reconciliation 范式）。owner doc `business-module-metadata.md` §4.4 增「浏览器层验证」实现注记 + e2e-runbook 业务动作表 +app-erp-all 模块元数据诊断行；`mvn install -DskipTests` 154 模块 BUILD SUCCESS + finance 抽样回归 0 新增失败。**负路径归 JUnit 单层**：checkDependencyIntegrity 缺失依赖/版本不匹配负路径经 `testMissingDependency`/`testVersionMismatch` 已覆盖，浏览器层非实体 BizModel 无 `__save` 入口无法制造缺失/不匹配场景（watch-only residual，对齐 1407-3 RATE_LIMITED 范式）。
...
- **浏览器层验证**（plan 2026-07-26-1407-1，✅ done）：公司间配对 + 合并抵消四入口（`runMatching` MATCHED/DIFF + `checkDualSideConsistency` + `generateEliminationCandidates` AR_AP/REVENUE_COST + `postElimination` DRAFT 凭证+状态翻转）经 Playwright 浏览器层 E2E 全栈验证（`tests/e2e/business-actions/fin-intercompany-matching-elimination.action.spec.ts` 5 用例，镜像 JUnit 5 场景，config-gated `erp-fin.consolidation-elimination-enabled=true` playwright.config.ts webServer JVM arg 启用）。**自包含直置配对凭证 setup**（经 `ErpFinAccountingPeriod__save` 建测试专用 OPEN 期间 + `ErpFinVoucher__save` 直置 SALE/PURCHASE 配对凭证 + `ErpFinVoucherBillR__save` 挂 billCode）使 `runMatching(myPeriodId)` 仅扫描本 spec 凭证零跨 spec 干扰；scalar-returning @BizMutation（int/Long）经 `gql.raw` 无选择集范式（对齐 finance-voucher-post）；MATCHED(sale=purchase=1000→MATCHED+matchedAmount=1000+diffAmount=0) / DIFF(sale=1000/purchase=800→DIFF+matchedAmount=800+diffAmount=200) 两态精确数值断言；AR_AP+REVENUE_COST 两类 CANDIDATE（INVENTORY_PROFIT config-gated 默认 off Non-Goal）+ eliminationAmount=matchedAmount；postElimination 生成 ErpFinVoucher（voucherType=TRANSFER + docStatus=DRAFT + totalDebit=totalCredit=amount）+ candidate.draftVoucherId 回写 + status 翻转 CANDIDATE→DRAFT_VOUCHER（__get 权威查库）。
...
- **浏览器层验证**（plan 2026-07-26-0500-1，✅ done）：跨公司 PO/SO 配对凭证 + 红冲 + 同法人控制对照经 Playwright 浏览器层 E2E 全栈验证（`tests/e2e/business-actions/fin-intercompany-cross-company.action.spec.ts` 4 用例，config-gated `erp-fin.intercompany-posting-enabled=true` webServer JVM arg 启用）。自包含跨法人 setup（`ErpMdOrganization__save` 建 2 COMPANY 法人根 + 2 DEPARTMENT 子公司 + 转移定价规则）使 `resolveLegalEntityRoot` walk-up 产出不同法人；helper `findIntercompanyVoucherIdByBillCode` 按 `ErpFinVoucherBillR.billType`（INTERCOMPANY_SALE/PURCHASE）反查（intercompany 凭证无独立 postingType + 同 orderCode 含 COMMITMENT 凭证须按 billType 区分）；既有 `runP2pChain`/`runO2cChain` 链路零回归（固定 orgId=2 + 无 transfer_price 种子 → counterparty=null skip 零凭证）。
