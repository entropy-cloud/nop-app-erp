...
### 6.1 短期（P0：id 字段隐藏）✅ 已完成（2026-07-13）

~~全域批量在定制层 view.xml 重写 `<form>` layout，移除 `id` 字段。~~
实际实现：修改 codegen 模板（`view-gen.xlib:GenForm` + `web-gen.xlib:GenLayout`）跳过 `seq-default` id，全域 338 实体一次覆盖，无需逐实体定制。

### 6.2 中期（P1：核心实体分组 + 查询表单）✅ 已完成（2026-07-13）

按域分批处理高频核心实体（39 实体），每个实体：
1. ✅ 设计字段分组方案（基本信息 / 金额 / 业务关联 / 审批 / 审计）
2. ✅ 折叠低频字段组（审批、审计信息缺省收缩）
3. ✅ 配置查询表单（选择 4-7 个关键查询字段 + filterOp 运算符）

### 6.3 长期（P2：全量覆盖 + codegen 模板增强）⏳ successor
