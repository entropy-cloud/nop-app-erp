...
### 3.8 checker 校准范式矩阵（plan 2026-07-27-0823-1 先例登记）

**背景**：合规 checker 是 grep 启发式工具（非 AST），规则原始 grep 不区分代码行与 javadoc/注释行时会产生大量注释虚假命中，使 baseline 语义模糊、未来注释增减触发虚假漂移。当某规则的注释虚假命中主导 actual 时，标准修复手段是**行级 grep 注释排除校准**（content-based 排除），而非上调 baseline 吸收虚假命中。

**checker 校准范式矩阵**（四先例，按校准手段分类）：

| 先例计划 | 规则 | 校准手段 | 校准类型 | 校准效果 |
|----------|------|----------|----------|----------|
| `2026-07-24-0941-2` | R3 | 交叉引用 `*.orm.xml` `<entity className>` 构建已注册实体白名单，仅对 `new <RegisteredEntity>()` 计数 | 实体声明交叉引用 | 19→5（排除 14 非实体 Erp* 前缀类） |
| `2026-07-25-1057-1` | R8 | 排除 `module-common-service/` 目录（抽象基类非领域 Processor） | 目录排除集 | 49→42 |
| `2026-07-25-1057-2` | R8 | 循环内跳过类体含 `extends Abstract*Processor` 的文件（per-mutation 子类经 BizModel 路由） | content-based 类体排除 | 191→42 |
| `2026-07-27-0823-1` | R1d/R6/R10 | grep 管道后追加 `grep -vE ':[0-9]+:[[:space:]]*(\*|//|/\*|\*/)' | grep -vE '\{@code|\{@link'` 排除 javadoc/行注释/块注释行 | 行级注释排除 | R1d 28→17 / R6 7→2 / R10 51→6 |

**适用判定**：当某规则 actual 中注释命中（javadoc `*` 续行 + 行注释 `//` + `{@code}`/`{@link}` 引用）占比显著（>30%）且真实代码站点已逐项确认合法时，采用行级注释排除校准。校准=per-rule 实施（option b，不动公共 helper，避免影响已稳定规则），最小爆破半径。

**与基线裁决的关系**：注释排除校准是**测量口径修正**（actual 下降至真实代码计数），可与基线裁决性下调（吸收既有合法真实站点）同步进行。校准后 baseline 语义 = 真实代码站点计数，未来注释增减不再触发虚假漂移，仅真实代码新增才触发门控。

**残留风险与升级路径**：行级 grep 可能漏排除块注释 `/* ... */` 内的跨行命中（无 `*` 续行前缀）。缓解：实测三规则块注释跨行命中为 0；若未来出现，升级为 AST 解析（开独立 successor，Non-Goal 显式排除语义级解析以保持 checker 一致性）。
