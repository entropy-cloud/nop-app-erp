...
- `2026-08-19-1948-odoo-recon-gap-scenarios-research-note.md` — **外部文章未实现场景联网调研深化笔记（web 检索证据收集）**：为 1925 差异分析报告判定的 3 未实现场景（PSP 批量 1:N / 模糊匹配 / 数电发票联动）+ 1 部分实现场景（银行对账小额容差写销）补充行业落地模式证据。结论：① PSP=成熟两段式对账（毛额↔销售 / 净额↔银行，手续费为桥；Stripe po_XXX/matching_key/reconciliation_status + settlement.batch/line）；② 模糊匹配=Jaro-Winkler/Levenshtein + 分级阈值 + blocking 分桶，零 ORM 变更最高性价比增强；③ 数电发票=乐企直连 vs 第三方服务商双路线成熟生态，建议维持 Non-Goal；④ 容差写销支持比例/绝对值+非对称。按 evidence 诚实性命服务🔵联网实测/⚪领域常识 + 来源列表，未修改任何 owner doc（rule 6）。已登记 backlog 3 行 watch-only 候选。
- `2026-08-19-1925-odoo-auto-reconciliation-vs-nop-app-erp-design.md` — **外部文章《拆解 Odoo 自动对账引擎》与 nop-app-erp 对账设计差异分析**：核心对账工作流已实现（对齐度高，无需改造）；文章四大二开场景=1 部分实现（场景 3 汇率差额 AR/AP 侧已有、银行侧缺小额容差强制结平）+ 3 未实现（PSP 批量/模糊匹配/数电发票，均增强/定制层）。避坑三坑点坑1/坑3 架构天然满足、坑2 自动勾对可接 nop-batch。全场景可实现，落地路径+保护区域门控表格见 §5；模糊匹配 successor 已在 `bank-reconciliation.md:152` 登记、税务对接对齐 `treasury.md:208` Non-Goal 立场。
- `2026-08-08-2000-skill-coverage-gap-analysis-rc-mr1.md` — **技能覆盖缺口分析（RC-R1.x 计划执行期发现 vs 既有技能库）**：抽取 RC-R1.5~1.13 + expander + MA4 计划「执行期发现」段 18 项，与 docs/skills README、lessons 01-11、.opencode/skills 三库逐项比对。结论：7 项 P1 操作技能补充（冻结时钟仅冻结日期与镜像断言 / GraphQL 时间戳秒级精度 E2E 跨秒 / IUserContext roleId 语义 / afterEntityChange 2-arg 重载陷阱 / BeanCopier Map 键 / _cases 孤儿与 0 字节占位 / seedRole 范式）+ 2 项 P2 审计层（lesson 09 精确容错正范式、预授权枚举不可类推）；业务知识（承付对称性/超收容差/可用性预检/匿名聚合）不提升，归 owner docs。
- `2026-08-04-0000-velpos-vs-mission-driver.md` — **外部 Agent 驱动器两种形态对比**：Velpos（交互式 Web 产品，人在环，驱动 agent）vs Mission-Driver（自主 Headless 状态机，驱动 opencode）。结论：同赛道（均非 Agent 引擎）的两个正交解——mission-driver 做"自治"（状态机/契约/审计门控），Velpos 做"交互"（流式/权限回传/IM/分支）。与自研 nop-ai-agent 不同层，不竞争。
