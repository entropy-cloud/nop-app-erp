...
| 4 | **既有 `nop-compliance-checker.sh` 接入 CI**（非新写），在 README 记录当前基线计数（如 R2c=965, R3=21, R11=0），后续每次增量不得超过基线 | F8 | P1 | CI 配置文件存在；checker 在 PR 检查中实际运行；基线日志可查 |
...
| 6 | 19 个 `Erp*WebPagesTest` 改为 `@Tag("full-app")` + 在 CI `app-erp-all` 阶段强制运行；或在 `-web` 模块用 `<optional>true</optional>` 解耦 | F9 | P1 | `app-erp-all` CI 阶段跑 19 个测试应全绿 |
