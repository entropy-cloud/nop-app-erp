...
- MR0/MR1 修复阶段：fix plan 起草（分级修复义务 + 保护区域门控，§5）、越界项批量裁决（§11.1）、闭包基线证据（§11.2）、修复行生命周期回写（§11.3）
...
## §11 MR1 修复阶段方法（批量裁决 + per-site 基线证据 + 修复行生命周期）

> 本节是**修复阶段**（MR0/MR1 执行期）方法的权威落点，2026-08-20 由 plan `2026-08-20-1255-2` G.2 从 MR1 89 份修复计划的实践中回流沉淀。审计阶段方法见 §1-§10；本节与 §5（保护区域门控）、§10（展开器机制）互补。

### §11.1 越界项 A/B/C 批量裁决范式

**触发**：R1.0 展开后，MR1 表中出现**批量越界项**（超出预授权类目、按规则逐项须双独立子 agent 批准的行，如 2026-08-08 展开初期的 49 项）。

**机制**（2026-08-12 裁决先例，批准人：用户；原裁决仅存 roadmap 头部，本节回流为方法论）：

1. **多路并行独立 agent 核实**：对全部越界项逐项做 ORM 必要性研究（需求文档 + ORM 模型 + Java 代码三源核实，5 路并行独立 agent 分担）。
2. **三分类裁决**（每项归入且仅归入一类）：

| 类 | 判据 | 后续门控 |
|----|------|---------|
| **A. ORM 变更批准** | 经核实确实需要 ORM 结构变更（加列/加 UK/新增实体），且纯加性（对齐 §5 Q3 边界：不改既有语义、无 NOT NULL 无默认列、无既有数据 UK 增设、无删除/迁移/索引改造） | 批量批准 ORM 修改授权；越界回落双独立子 agent 批准 |
| **B. 降级为预授权** | 经核实不需要 ORM 结构变更，纯代码逻辑/跨域契约可解决（含 useLogicalDelete 软删等规范解法） | 预授权自动执行（跨域契约项仍须协调确认） |
| **C. 已实现确认** | 经核实当前代码已完整实现 | 直接标 done，附实仓证据（file:line） |

3. **登记义务**：裁决落 roadmap 头部裁决段（分域列 A 类项 + B 类清单 + C 类证据）；**对应工作项行标签同步改写**消除「越界项…checkbox」歧义（对齐 RC-R1.48/50/52-54/59/61/62 先例——行标签与批量裁决一致，避免后续审计误判漏批准）。
4. **替代方案**：逐项独立裁决（被否决——49 项逐项人工/双 agent 成本不可行）；全部维持 ask-first（被否决——批量核实已提供等效保障，C 类已实现项无须任何批准）。

### §11.2 baseline-raise per-site 证据范式（修复计划闭包纪律）

**触发**：任何修复计划新增了 checker 可计数的站点（daoFor / import / 状态字面量等）——MR1 修复的主要漂移来源。

**机制**（RC-R1.48/49/51/56/57/60/78/81/86/89 等先例固化为义务）：

1. **闭包前必跑 checker** 并对照 `docs/audits/compliance-baseline.md` §BASELINE 机器可读块（§8 教训同样适用于修复阶段）。
2. **actual > baseline 时逐站点分类裁决**（baseline-raise vs Fix），**严禁整批 raise**——裁决方法主体见 lesson `docs/lessons/07-compliance-baseline-drift-on-plan-changes.md` + skill `docs/skills/compliance-baseline-drift-adjudication-prompt.md`（本节不重复，仅声明修复阶段同样强制）。
3. **per-site 证据落盘**：每处上调站点在 compliance-baseline.md 登记（规则编号 旧值→新值 + 站点清单 + 分类依据），修复计划 Closure 节引用该登记（标准表述如「checker R2c 1413→1415 baseline-raise 登记，per-mutation Processor 2 处同域 daoFor per-site 证据落 compliance-baseline.md」）。
4. **零漂移同样留痕**：actual == baseline 时在计划 Closure 记录实测值（如「R2c=1422 不变——经同实体 getter，零新增 daoFor 面」），供 MV V.2 对账。

### §11.3 修复行生命周期实践约定（散落实践聚合）

以下实践在 MR1 各修复计划中反复出现但此前散落，此节聚合为可引用约定（机制本体在 §5/§10 者仅引用）：

1. **分批启动裁决**：R1.0 展开后可经人工裁决分批启动（先纯预授权类、后越界项逐项双批准——2026-08-08 A1 先例），非触及行不阻塞。
2. **同域同控制点合并 owner plan**：同域同 owner doc 同结果表面的多个修复行按计划指南规则 14 合并为一个 owner plan 的多个 Phase（RC-R1.18/19、RC-R1.52/53/54 先例），但每行仍独立保留 finding ID 交叉引用与独立 done 语义。
3. **双批准记录落盘位置**：触及保护区域的修复，双独立子 agent 批准记录（两个 fresh session 的 task id + APPROVE 结论）落盘于**计划文件的对应 Phase**（§5 checkbox 机制），非独立文档。
4. **修复完成四点回写**（缺一即触发 lesson 11 反模式）：① arm-index finding 行 `修复状态` → done（附修复摘要）② roadmap 工作项行 → done ✅（附 plan id + 验证摘要）③ owner doc 实现注记（Deferred→已实现收敛）④ `docs/logs/` 当日条目。
5. **产品范围确认闭环**：修复涉及「product-scope 是否裁剪」疑义时（§4 (iii)），在计划内显式复核 product-scope 未裁剪 → Q4=(a) 强制实现，并声明「审计不复开」（RC-R1.51/58/60/78 先例）。
...
- [x] §11 MR1 修复阶段方法（A/B/C 批量裁决范式 + baseline-raise per-site 证据 + 修复行生命周期实践约定，2026-08-20 G.2 回流）
