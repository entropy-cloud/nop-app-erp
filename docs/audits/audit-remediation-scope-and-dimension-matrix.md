...
| 设计完整性扫描 | 全域 vs product-scope。**终态：`⚠️(P1)`**——A3.2 已于 2026-07-28 完成（`docs/audits/2026-07-28-1510-arm-ma3-design-completeness-scan.md`）。Verdict GAPS（有差距）：零 BLOCKER（18 product-scope 域 + notify 全部有 design owner；71 开源标配功能逐项有对应 owner doc；功能覆盖度维度 PASS 113/118=95.8% 经第二轮纵深核查有效；无范围要求域已有代码但零设计需升级 A4）；2 项 MAJOR → P1-MA3-022/023（目标 MR2，文档类：flow-overview §3 L2 状态映射缺 12 扩展域引用 dim 4 / domain-glossary 缺 8 第二批扩展域词汇节 dim 5——与 A3.1 P1-MA3-009/010 同根因"8 第二批扩展域全局视图系统性缺位"在 dim 4+5+6+9 四联投影）；1 项 P2 watch-only（P2-MA3-022 subscription billing 深度未拆解，InvoicePlan 已覆盖广度）；与 A3.1 13 项 P1 + 8 项 P2 经交叉去重无重复登记 | design-completeness-scan |
| owner doc vs 代码 drift | S+A 级域抽样。**终态：`⚠️(P1)`**——A3.3+A3.4+A3.5 已于 2026-07-28 完成（`docs/audits/2026-07-28-1953-arm-ma3-owner-doc-vs-code-drift.md`）。Verdict FAIL（有 drift）：零 BLOCKER（所有 blocker 级 drift 均为"实施者按错误基线实现"风险非活跃数据破坏）；22 项 NEW MAJOR → P1-MA3-024~045（目标 MR2 文档类 [20 项] / MR1 代码侧 [2 项]）+ 13 项 NEW MINOR → P2-MA3-023~035 watch-only。finance 域 drift 密度最高（16 P1 + 9 P2 = 25 项，配置/门控维度 8 项 + 字段语义维度 5 项）；mfg 6 P1 + 2 P2（material-reservation.md 整个子系统未实现 blocker）；pur+sal+inv 0 P1 + 2 P2 minor（核心机制文档与代码高度一致）。MA2 owner-doc drift 复核全部确认一致无升级 | multi-dimensional-audit |
