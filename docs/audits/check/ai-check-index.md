...
| `ck-quality-r3.md`（r3 轮，目录 `2026-09-06-1645-ai-check-r3/`） | M1.12 | quality（全格） | 0 | 2 | 0 | 6 | done（新立 8：P2-qa-026/027-r3 severity 字典污染 + 跨域 daoFor 三站点、P3-qa-028..033-r3；复用 8 / 归并 17；五维矩阵 B=finding、F/S/T/I=pass；SPC 静态行裁决不重开） |
| `ck-purchase-r3.md`（r3 轮，目录 `2026-09-06-1645-ai-check-r3/`） | M1.13 | purchase（全格） | 0 | 1 | 0 | 2 | done（新立 3：P2-pur-015-r3 settle/reverseSettlement 不拒已作废单据（PaymentSettler 零 docStatus 守卫，与 pur-005 cancel 侧镜像分立）、P3-pur-016-r3 Dashboard javadoc 口径漂移、P3-pur-017-r3 三单匹配页 ACTIVE 死状态样式分支；复用 3 / 归并 11（pur-007 容差量纲 UI 面加重证据）；五维矩阵 B=finding、F=finding(minor)、S/T/I=pass；审计时点 HEAD `e0eda4d23`） |
...
| `ck-inventory-r3.md`（r3 轮，目录 `2026-09-06-1645-ai-check-r3/`） | M1.13 | inventory（全格） | 0 | 1 | 0 | 0 | done（新立 1：P2-inv-012-r3 序列号「已售拒绝出库」状态校验与 IN_STOCK→OUT 翻转 writer 缺失（inv-004 fixed 之 Deferred 残留承接，与 P3-inv-017 死字典联动）；复用 6 / 归并 15；五维矩阵 B=finding、F/S/T/I=pass；审计时点 HEAD `e0eda4d23`） |
...
| P3-CK-fin3-017-r3 | P3 | DIM-B（②） | ck-finance-budget-costing-r3.md | fin-3 预算族 7 文件 IDaoProvider daoFor 直查命中处无注释理由——跨域站点 5 处（ErpMdSubject getEntityById ×4 + ErpMdAcctSchema ×1，IErpMdSubjectBiz 在位未注入）+ 同域 daoFor 站点零豁免登记；同切片 Classifier/CarryForwardProcessor 已合规登记范式不对称 | 新增（r1 fin3 族 16 条无此形态；同型家族跨单元先例 P2-CK-fin-007 / P3-CK-ast-028-r3 / P2-CK-qa-027-r3，按 qa-027/ast-028 跨单元另立先例自立 ID） | open | 修复归 M2.x：md 直查站点注入 I*Biz 或按 fin-2 先例补豁免注释 + 同域站点补一行豁免理由（对齐 CarryForwardProcessor L132-133 措辞）；与 fin-007/ast-028-r3/qa-027-r3 同族一并收口，全仓同型扫描归 U20/M1.15 |
| P3-CK-fin3-018-r3 | P3 | DIM-B（⑮owner-doc） | ck-finance-budget-costing-r3.md | budget.md §浏览器层验证 L257「结转凭证 code = CARRY-FORWARD-…precision 50 约束」stale——F2.3（fin3-001 修复）已移除结转凭证段（writeCarryForwardVoucher no-op）且全仓零 CARRY-FORWARD- 写点，doc 描述的凭证与其约束已不存在 | 新增（同型先例 P3-CK-pur-016-r3 / P3-CK-prj-025-r3 归 doc 维护批；⑮抽样漂移 1 < 2 未触发扩样） | open | 修复归 doc 维护批：L257 移除结转凭证 code 约束句改注 F2.3 后 BudgetLine 承载裁决；与 E2E spec 头注释同步 |
| P3-CK-fin3-019-r3 | P3 | DIM-T（覆盖缺口） | ck-finance-budget-costing-r3.md | F2.3 修复体 fin3-004 TOCTOU per-维度串行锁与 fin3-005 org/账套/币种解析链零专属回归断言——测试树 grep CHECK_LOCKS/TOCTOU/CountDownLatch = 0，Commitment/EndToEnd 种子恒用 orgId=1/acctSchemaId=1 身份值且无凭证三元组输出断言，解析实现与旧硬编码在测试矩阵下不可区分 | 新增（r1 fin3-004/005 终态证据列未登记修复测试；先例 P2-CK-fin2-018-r3 / P2-CK-inv-012-r3「已修复残留无回归承接」；行为结局路径已覆盖故 P3 非 P2） | open | 修复归 M2.x 测试批：commit 用例加非身份 orgId 种子 + 断言凭证 orgId/acctSchemaId/functionalCurrencyId 三元组；check 并发子路径双线程 latch 断言（或登记 watch-only）；config 投产启用时升优先级 |
...
| P3-CK-ast-028-r3 | P3 | DIM-B（维度②） | ck-assets-lifecycle-r3.md | `ErpAstAssetBizModel.getAssetAuditTrail` 跨实体（ErpAstAssetActionLog）读取经 `daoProvider().daoFor` 直查（L100）+ IOrmTemplate runInSession，`IErpAstAssetActionLogBiz` 在位未注入、无豁免注释——同型注记 P2-CK-fin-007（fin 格跨域子型，跨单元按 r1 同型另立先例自立 ID） | 新增（r1 ast 27 条无此形态；r2 无同型独立登记；冻结清单 §1.1 程式②「命中处须有注释理由」命中） | open | 修复归 M2.x：注入 `IErpAstAssetActionLogBiz` 改经 `findList`（镜像 Cap `countExecutedDepreciation` I*Biz 范式），或补豁免注释说明 session 语义依赖；与 fin-007 同族一并收口，全仓同型扫描面归 U20/M1.15 裁决 |
| P3-CK-prj-022-r3 | P3 | DIM-B（⑨） | ck-projects-r3.md | Settlement/Timesheet/Project/Task 四实体 Java 审批-冲销-生命周期动作零 auth 声明（保留层 xbiz 空 `<actions/>`、无 @Auth、全域 `ErpPrj*:approve` 权限引用 0）——同单元 Billing/Budget/CostCollection xbiz 脚本与姊妹域 ErpSalOrder.xbiz 均声明 auth，范式不对称 | 新增（r1 同族 b2b-011/drp-018 缺 FNPT 注册均 P3 校准；主守卫=状态机+F1.3 基类在位） | open | 修复归 M2.x：4 实体保留层 xbiz 补 `<mutation><auth/>` 增量声明（镜像 ErpSalOrder 范式）或 ADRAM 豁免裁决；与 qa-029-r3 同批跨域对齐 |
| P3-CK-prj-023-r3 | P3 | DIM-B（②） | ck-projects-r3.md | `ProjectCostAggregator.rollbackFromTimesheet` I*Biz 回退链五站点全程 `new ServiceContextImpl()` 丢弃调用方 Processor 真实 context（用户身份/数据权限不进管道）——对照同模块 ExpenseCostAggregator:182-185 getCtx() 兜底范式 | 新增（r1 prj 21 条无此形态） | open | 修复归 M2.x：rollbackFromTimesheet 增加 context 参数透传（镜像兜底范式）；全域 `new ServiceContextImpl()` 族扫描归 U20/M1.15 |
| P3-CK-prj-024-r3 | P3 | DIM-B（⑧dict 治理） | ck-projects-r3.md | `erp-prj/timesheet-status` 字典零列绑定（status 实绑 wf/approve-status，常量注释自认合并）残留死字典 + `pnl-calc-status` PENDING 零 writer 死值（常量 ：86 零消费） | 新增（r1 prj 无 dict 死值条目；lesson 10 静态面） | open | 修复归 M2.x：ORM（ask-first 保护区域）登记清理/复用决策；PENDING 保留初始态语义则补注释登记 |
| P3-CK-prj-025-r3 | P3 | DIM-B（⑮owner-doc） | ck-projects-r3.md | state-machine.md §适用对象三「ErpPrjBilling 18 行 CRUD 桩零 writer」断言过期——ErpPrjBilling.xbiz 现含 5 个审批 script mutation 写 approveStatus，doc 未随实现同步 | 新增 | open | 修复归 doc 维护批：§适用对象三补实现注记（不修需求契约段语义） |
| P2-CK-qa-026-r3 | P2 | DIM-B（⑧/②） | ck-quality-r3.md | `ErpQaNonConformanceUpgradeToRecallProcessor:52-54` NCR severity=NORMAL 直写 recall.severityLevel（recall-severity 字典 LOW/MEDIUM/HIGH/CRITICAL 无 NORMAL），:52 注释谎称「码值对齐」——非法字典值污染召回单 UI 映射/报表聚合 | 新增（qa-004 同型词表污染主路径 P1 已修，本站为升级路径窄面新立；qa-022 为其报表下游表征） | open | 修复归 M2.x：NORMAL→MEDIUM 显式映射（常量级）+ 修注释 + 升级路径负向测试 |
| P2-CK-qa-027-r3 | P2 | DIM-B（②） | ck-quality-r3.md | 跨域 daoFor 直查无 I*Biz 无豁免三站点：NcrPostingDispatcher:121-131 / NcrReturnOrchestrator:131-141（ErpInvStockBalance）+ ErpQaReportBizModel:361（ErpMdMaterial，缺 Dashboard 同型豁免注释）——同型注记 P2-CK-fin-007 / P3-CK-ast-028-r3（跨域形态对齐 fin-007 P2） | 新增（r1 qa 25 条无此形态；冻结清单 §1.1 程式②命中） | open | 修复归 M2.x：注入 I*Biz（镜像 RecallTargetLocator 范式）或补 E3 豁免注释；与 fin-007/ast-028 同族一并收口，全仓同型扫描归 U20/M1.15 |
| P3-CK-qa-028-r3 | P3 | DIM-B（②） | ck-quality-r3.md | SPC 域服务 I*Biz 声明未用簇：SpcCapabilityCalculator:3-4 import 未用 + :44-47 javadoc 称 I*Biz 回写 vs :287-298 daoFor 直写；SpcOutOfControlHandler:51-53 死注入 vs :99/:116 daoFor；InspectionTemplateMatcher:30-73 同域 daoFor 无豁免注释 | 新增 | open | 修复归 M2.x：改真用 I*Biz 或 javadoc 取齐 + 补豁免 + 删死注入（三站点同批） |
| P3-CK-qa-029-r3 | P3 | DIM-B（⑨） | ck-quality-r3.md | ErpQaNonConformance.xbiz 空 `<actions/>`——postNcr/reverseNcr/resolve 财务过账动作零显式 auth（action-auth 无 FNPT 子节点），对照同单元 Recall.xbiz 五动作全带 auth 不对称 | 新增（prj-022-r3 同族跨域站点） | open | 修复归 M2.x：保留层 xbiz 补三动作 auth 声明（镜像 Recall 范式）；与 prj-022-r3 同批 |
| P3-CK-qa-030-r3 | P3 | DIM-B（⑨作业） | ck-quality-r3.md | spc-sampling.batch.xml collectSamples 无 per-item 容错——parameterId 空/subgroupSize<2 chart 抛错毒化 chunk（失败隔离仅 chunk 级；对照 evaluate 段 per-sample catch 达标） | 新增 | open | 修复归 M2.x：collectSamples 包 per-chart try/catch WARN 隔离 |
| P3-CK-qa-031-r3 | P3 | DIM-B（⑮owner-doc，疑似需求分歧只登记） | ck-quality-r3.md | 强制质检触发机制 3 doc 漂移（README:81/inspection-integration:25/use-cases:21 物料级 inspection_required vs 代码单据类型级 config `erp-qua.mandatory-inspection-bill-types`）——两种机制均提供强制质检能力，分歧归需求侧裁决 | 新增 | open | 需求裁决后改代码或 3 doc 同步；审计不裁决 |
| P3-CK-qa-032-r3 | P3 | DIM-B（⑮owner-doc） | ck-quality-r3.md | owner-doc 命名/状态/契约漂移簇 7 处：reference_* vs relatedBill*（README:40/inspection-integration:40）、erp-qa/approve-status vs wf/（recall.md:46）、IN_EXECUTION 幽灵态（inspection-integration:182）、use-cases:10 NCR 漏第 5 态、spc.md:29 clCenterType 数字注记 vs string 字典、spc.md:168 range/stdDev 契约未实现、recall.md:77 batchId→batchNo | 新增（state-machine.md 已收敛 5 态基线，漂移集中于其余 doc） | open | 修复归 doc 维护批：批量对齐（不改契约段语义） |
| P3-CK-qa-033-r3 | P3 | DIM-B（⑮，疑似需求分歧只登记） | ck-quality-r3.md | 检验模板「物料类别级」优先级 2 doc 设计层级未实现（inspection-integration §5.2 + use-cases UC-QA-07；InspectionTemplateMatcher 仅物料×类型→全局默认两级，模板实体无类别字段） | 新增 | open | 需求裁决：补类别维度（ORM ask-first）或 doc 降两级描述 |
| P2-CK-pur-015-r3 | P2 | DIM-B（⑧/⑮） | ck-purchase-r3.md | `PaymentSettler#settle/#reverseSettlement` 仅守卫 approveStatus（全文零 docStatus/CANCELLED 引用）——已作废发票可被继续核销、已作废付款单可继续核销，AR/AP 派生态与作废语义冲突；owner doc state-machine.md §异常路径 L99「付款核销时发票已作废→拒绝核销」逐字未实现 | 新增（与 P2-CK-pur-005 cancel 侧互为镜像不同控制点，对齐 r1 sal-010/sal-012 分立先例） | open | 修复归 M2.x：settle/reverseSettlement 双侧补 `docStatus≠CANCELLED` 守卫（payment+invoice 两侧）；先写「作废发票核销拒绝」失败测试；与 sal-010/pur-005 统一「核销 docStatus 守卫」批次设计 |
| P3-CK-pur-016-r3 | P3 | DIM-B（⑮） | ck-purchase-r3.md | `ErpPurDashboardBizModel` javadoc L54-55/L176 仍书「docStatus=ACTIVE」旧口径——F2.10 修复后查询已改 APPROVED+≠CANCELLED，注释未随（pur-001 同站点措辞级残留） | 新增 | open | 修复归 M2.x：javadoc 口径措辞同步（零行为变更），与 pur-001 族修复一并收口 |
| P3-CK-pur-017-r3 | P3 | DIM-F | ck-purchase-r3.md | `three-way-match.page.yaml` L129/170/207 三处 `docStatus == "ACTIVE"` 样式分支消费死状态——live 单据恒 DRAFT/CANCELLED，primary 分支不可达（display-only 死代码 + 样式语义误导） | 新增（r1 pur-001 同族死状态消费的页面侧残留站点，控制点不同） | open | 修复归 M2.x：tpl 改按 approveStatus（或 docStatus≠CANCELLED）分支；与 P2-CK-pur-007 归并修复时一并清理 |
| P2-CK-inv-012-r3 | P2 | DIM-B（⑧/⑮） | ck-inventory-r3.md | 序列号「已售拒绝出库」状态校验与 IN_STOCK→OUT 翻转 writer 缺失——validateBatchSerialPresence 仅 presence 校验、ErpInvSerialNumberBizModel 16 行 CRUD 桩无状态翻转；owner doc state-machine.md §异常路径 L67「已售序列号拒绝再次出库」逐字未实现（README §关键业务规则 6 同） | 新增（P1-CK-inv-004 F2.11 修复注记明示「未售/在库状态翻转为独立特性 Deferred」但无独立 ID 登记，本切片按 §2 新立承接，原 ID fixed 状态不动） | open | 修复归 M2.x：SerialNumber 补出库 IN_STOCK→OUT 翻转 writer（Bookkeeper 同事务）+ 出库链 status=IN_STOCK 守卫 + 新 ErrorCode `erp.err.inv.serial-not-in-stock`；与 P3-CK-inv-017 死字典联动（writer 落地字典复活）同批收口；序列管控物料 seed 投产时升 P1 |
| P2-CK-fin2-018-r3 | P2 | DIM-T（覆盖缺口） | ck-finance-arap-r3.md | 核销 FX 汇兑损益路径（`ReconciliationSettler.settleWithFx`/`reverseSettle(lines, fxPath=true)`/`generateReconFxVoucher`/`reverseReconFxVoucher`，config `erp-fin.recon-fx-gain-loss-enabled` 默认 false）全链零测试断言——src/test + `_cases` grep `settleWithFx`/`fxPath`/`reverseSettle` = 0，`TestErpFinReconciliation` 9 用例全走非 FX 路径；testing-strategy P1「多币种过账+汇兑损益」行核销侧子路径无覆盖，P1-CK-fin2-002 修复体（FX 对称回滚）回归将静默穿透 | 新增（r1 交叉验证注记「FX 测试无 reverse 断言」无独立 finding ID 不可归并；r2 无同型；002 本体 fixed 复用——按 inv-012-r3 同款「已登记残留无 ID → 新立 -r3 承接」先例） | open | 修复归 M2.x 测试批：config 开启下 settleWithFx→reverse 断言双侧 open 复原 + fxGainLoss 凭证红冲（r1 报告 §最不确定 2 复现程式）；config 投产启用时升 P1 立即修复 |
