...
| MI.8 批 2/2 | `docs/plans/2026-09-07-1715-2-mi8-handwritten-yaml-i18nen-batch2.md`（2026-09-08） | 其余 17 域 **1318 行 / 82 文件**（maintenance 122/6 + cs 120/7 + quality 114/8 + hr 118/8 + purchase 99/4 + manufacturing 96/7 + inventory 93/4 + b2b 91/4 + assets 87/7 + crm 79/10 + master-data 74/6 + contract 50/2 + notify 42/1 + drp 41/2 + sales 39/2 + aps 35/2 + logistics 18/2；试点 maintenance visit-wizard main.page.yaml 34 行 Phase 1 先行） | 全局 CAT4=1318（批前实跑 2026-09-08：17 域逐域计数与 SNAPSHOT 一致，CAT1/2/3=0/0/0） | 17 域逐域 **0**；全局 CAT4 **0 sites / 0 files**（CAT-4 全域归零收官；CAT1/2/3 持平 0/0/0、CAT5 持平 20992） | 1318 → 0 逐域归零（Phase 2 474 + Phase 3 466 + Phase 4 378 = 1318 对账一致；批 1 修订版单一承载协议：标量兄弟式/块承载式/`: ` 引号化/flow→block 等价重序列化 + 键序等价重排，行为不变式未破坏——各批域文件经 PyYAML parse 剥离 i18nEn 后与 HEAD 语义树逐节点一致抽样复验）；quoted-key labelMap（`'true':`/数值键）等价重序列化为裸布尔/flow + 承载（YAML/JSON 键等价复验）；唯一 literal-block `adaptor: |`（quality dashboard flux）等价重序列化单行引号标量（parse 字节级等价）；新词扩 `docs/design/i18n-glossary.md` 批 2/2 节（17 域业务术语 + 通用 UI 词统一口径登记）；`--strict` PASS exit 0（0 新增违规 vs 冻结快照 170 baseline files，actual 0/0/0/0 全部 improvement）；SNAPSHOT/WHITELIST 冻结块零改动（批中一次子代理误触 `--baseline` 重生成已当场 `git checkout` 还原后复验） | 17 域 web 模块三阶段聚合 `mvn test -am` 全绿零新增失败：Phase 2（mnt 157 + cs 185 + qa 184 + hr 249 = 775/0/0）+ Phase 3（pur 341 + mfg 308 + inv 248 + b2b 80 + ast 337 = 1314/0/0）+ Phase 4（crm 188 + md 160 + ct 168 + notify 23 + drp 98 + sal 316 + aps 82 + log 66 = 1101/0/0），各域计数与 known-good-baselines 持平；`npm run validate:flux` 与全量 build 见计划 Verification 节（本批 step [1/3] 导出 0 error、整体 exit 1 余项 = 325 条既有 variant 外部漂移 successor 在案） |
