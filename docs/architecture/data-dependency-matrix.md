...
| **projects** | master-data | finance（成本归集过账 S 写凭证） | finance（P：成本归集反查源单） |
...
1. **单向 DAG**：业务域 → master-data 单向合法。跨业务域仅 `finance → projects/assets`（finance 是 DAG 顶，单向合法）。**禁止**业务域之间的反向或循环引用。
