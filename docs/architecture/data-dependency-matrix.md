...
> **完整 18 域的物理目录 × 逻辑工程名 × appName 映射见 `domain-module-split-analysis.md §2.0`**。本图按逻辑工程名标注依赖方向。下方 DAG 仅刻画**已稳定的核心依赖**（master-data 根 + 业财一体闭环）；第二批 8 个扩展域（crm/cs/hr/aps/contract/drp/logistics/b2b）在 ORM 层主要引用 master-data（其中 hr 额外只读引用 projects：员工项目分配/工时归集），业务层关联待各域设计深化后补充。
...
> **关于 8 个第二批扩展域的业务层关联**（crm/cs/hr/aps/contract/drp/logistics/b2b）：当前 ORM 层主要引用 master-data，其中 hr 额外只读引用 projects（员工项目分配/工时归集，详见 §2.2 与 §5.6.2）。它们是否参与业财过账（S 写 finance）、是否与其他业务域有弱指针（P）关联，待各域业务设计深化后在 §2.2/§4.2/§5.1 补充。
...
| **finance** | master-data / purchase / sales / inventory / assets / projects（经 I*Biz 查源单，R 为主；**唯一写边例外（E3.5，2026-08-27）**：`ErpFinApDocumentPipelineProcessor.draft()` 经 `IErpPurInvoiceBiz.save()` 创建采购发票草稿——fin→pur command 写边，属 §4.4「禁止反向 S 写」明示认可的 I*Biz command 编排形态（与期末结账同型：finance 仅发起 command、purchase 域自管实体写，fin 不持反向 ORM 引用），非 ORM/S 写层反向边） | （处于 DAG 顶，不被 S 写） | （凭证被业务域 P 反查） |
| **assets** | master-data / inventory | finance（折旧/处置过账 S 写凭证） | finance（P：折旧凭证反查资产；assets→finance 凭证经 `voucherId` 弱指针，无 ORM to-one） |
...
| **maintenance** | master-data / inventory / assets / manufacturing（OEE 性能/计划时长分量，Java 层只读）/ quality（OEE 质量分量，Java 层只读） | manufacturing（停机影响排产）/ finance（维修领料过账） | inventory（P：备件领用→stock_move） |
| **crm** | master-data | （业务层 S/P 待深化） | （待业务设计补充） |
| **cs** | master-data | （业务层 S/P 待深化） | （待业务设计补充） |
| **hr** | master-data / projects（员工项目分配/工时归集 ORM 只读引用，按 `projectId`/`taskId` 对 `ErpPrjProject`/`ErpPrjTask` 建 to-one，机制 B，projects 不反向） | （业务层 S/P 待深化，如薪资过账） | （待业务设计补充） |
...
| projects | 3（被 finance/purchase/sales 查）+ hr 只读引用 | 3 类主数据 | **finance** | 0 | 2 |
...
| hr | 待业务深化 | ~6 类主数据 + projects（实测 19+2 to-one） | 待深化 | 待深化 | 待深化 |
...
| mfg-service → mnt-dao | `IErpMntDowntimeEntryBiz.findOpenDowntimeEquipmentWorkcenters` 开放停机窗口只读拉取（排产门控，拉取消费模型） | RC-R1.76（P1-RC-068） | 单向只读：§2.2 maintenance 行「被 mfg 查」预期方向的 Java 层落地；**RC-R1.78 起 mnt-service→mfg-dao 反向只读边已落地（见下行），「mnt 不依赖 mfg」陈述由本边修订为 mfg↔mnt 双向域耦合（Maven 菱形非环，镜像 R1.77 assets↔mnt 披露先例），DAG 仍无环（dao 层单向，service 层互不依赖）** |
...
| pur-service → drp-dao | `IErpInvDrpCrossDockBiz.markReceivedFromPurchase`（收货审批后置：按采购单号+物料标记 PENDING 越库记录→STAGING + inboundMoveId 回写，D1 裁决选项 A，镜像 R1.61 collectProjectMaterialCost 接线方向）+ `IErpInvDrpLeadTimeRecordBiz.recordFromPurchaseReceive`（收货审批后置：actualLeadTime=DATEDIFF(receiptDate, orderDate) 落提前期记录，D4 裁决选项 A；UC-DRP-08 触发） | RC-R1.81 + RC-R1.82（P1-RC-081/082） | 单向叶依赖：drp 不反向依赖 purchase service（drp-service→pur-dao 为另一 Java 边，两 service 模块互不依赖，Maven 菱形非环，对齐 §2.4 assets↔maintenance 披露先例）；drp 模块缺失时消费方 `@Nullable` 注入容错；失败隔离 try/catch 不阻断 RECEIVED 主迁移与过账；drp 侧 `erp-inv.drp-xdock-enabled` 默认 false 时 CrossDock Facade 零副作用 |
| drp-service → sal-dao | `IErpSalOrderBiz`/`IErpSalOrderLineBiz`（越库 ON_RECEIPT 策略扫描待出库销售订单：物料未出库完行→APPROVED 未作废单，承诺发货日期 ASC + 创建时间 ASC 取首个；只读） | RC-R1.81（P1-RC-081） | 单向只读：sal 零依赖 drp，DAG 无环；sales 模块缺失时消费方 `@Nullable` 注入容错（无候选→拒绝匹配降级语义）；test scope 另挂 `app-erp-sales-service`（测试装配边，不计入生产依赖方向） |
| drp-service → qa-dao | `IErpQaInspectionTemplateBiz`（D2 裁决选项 A：物料存在有效检验模板即视为需质检，越库 match 前置守卫载体）+ `IErpQaInspectionBiz`（暂存区快检通过凭证查询 + R1.82 供应商评分质量合格率维度读来料检验；只读） | RC-R1.81 + RC-R1.82（P1-RC-081/082） | 单向只读：qa 不反向依赖 drp（quality 被业务域引用不反向，§2.2），DAG 无环；quality 模块缺失时消费方 `@Nullable` 注入容错（守卫视为无需质检/评分维度记样本缺失） |
| drp-service → pur-dao | `IErpPurOrderBiz`（供应商评分数量准确率维度：读订单头 supplierId 过滤 + 行 quantity/receivedQuantity 偏差聚合；只读） | RC-R1.82（P1-RC-082） | 单向只读：pur-dao 无反向依赖 drp；与 pur-service→drp-dao 边构成 Maven 菱形非环（service 层互不依赖，dao 层单向，显式披露）；purchase 模块缺失时消费方 `@Nullable` 注入容错（评分维度记样本缺失不静默忽略）；test scope 另挂 `app-erp-purchase-service`（测试装配边） |
...
> **凭证层时序可配**：下图的"凭证生成"段（第②层）按 `(billType, acctSchemaId)` 可切 **SYNC 同步调用（独立事务传播 REQUIRES_NEW，跨域失败 posted 标志隔离 + 兜底扫描）**（默认）或 **ASYNC post-commit**（性能瓶颈时）。库存段（第①层）恒定 SYNC 强一致。事务边界对齐 `flow-overview.md §6.1`。

```
[app-erp-purchase] ErpPurReceiveBiz.confirmReceive()  @BizMutation
    │ 同一事务上下文（@BizMutation 自动 @Transactional REQUIRED）
    │
    ...
    ├─ 方式 A（SYNC，默认）：同步调用，Facade IErpFinVoucherBiz.post() 叠加 @Transactional(REQUIRES_NEW) 独立事务承接
    │   └─ @Inject IErpFinAcctDocProvider → ErpFinAcctDocRegistry → Provider
    │       ├─ S 写 finance：插入 erp_fin_voucher / voucher_line（独立事务）
    │       ├─ S 写 finance：插入 erp_fin_voucher_bill_r（业财回链）
    │       └─ 更新 posted = true（同步调用，跨域失败经 posted=false + 兜底扫描保证最终一致）
    │   事务提交 → 业务单据 / 库存 / 凭证强一致；凭证失败不回滚主事务（posted=false 落盘待兜底）
    │
