...
4. **命名遵守平台规范**：不创建 `*Service`/`*Controller` 类（`../nop-entropy/docs-for-ai/02-core-guides/service-layer.md`）；编排类用 `<Entity><Method>Processor` 命名，复步用 `*Step`。
5. **通用 CRUD mutation 的状态锁（F1.3 立法 2026-08-26）**：实体 BizModel 统一继承 `AbstractErpCrudBizModel`（common-service）——`posted=true` 或 `approveStatus=APPROVED` 的单据拒绝通用 `__update`/`__delete`（硬规则 5 的 CRUD 面对偶：过账后锁定）。内部状态机动作经 `updateEntity`/`deleteEntity` helper 直写不经 prepare 钩子，不受影响；无 posted/approveStatus 列的实体守卫惰性。不可变台账实体（库存流水/余额）继承 `AbstractErpImmutableCrudBizModel` 拒绝全部通用写。config kill-switch：`erp-common.crud-status-lock-enabled`。
6. **REQUIRES_NEW 凭证后无可抛步骤（孤儿凭证不变量，F1.2 立法 2026-08-26）**：凡编排中调用以 REQUIRES_NEW 独立提交凭证的步骤（`IErpFinVoucherBiz.post` 族），其后不得再排任何可抛步骤（守卫/联动 hook/写库）——主事务回滚时凭证已独立提交即成孤儿。守卫与联动一律前移到过账前（同主事务原子性保证）；结构性依赖凭证产出的聚合无法前移时，幂等键（billHeadCode）必须跨重试稳定，使重试经引擎幂等命中返回既有凭证 id 后 posted 收敛（引擎契约：幂等命中返回既有 id 而非 null，F1.1）。反例教训：幂等键取自随主事务回滚消失的中间产物（如新生成移动单号）则收敛失效。另注：主事务内脏实体在后续 REQUIRES_NEW 步骤的内层连接上 flush 不可见（跨连接 update-entity-not-found）——脏实体的回写应推迟到全部 REQUIRES_NEW 凭证提交之后。
