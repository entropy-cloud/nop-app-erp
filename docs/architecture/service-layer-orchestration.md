...
xbiz action 通过 `task:name` 绑定到 task flow。这是一个**编译期标签库转换**，非运行时属性——xbiz 文件通过 `<x:post-extends><biz-gen:TaskFlowSupport xpl:lib="/nop/core/xlib/biz-gen.xlib"/></x:post-extends>` 引入转换器，编译时扫描 `task:name` 属性，加载 `TaskFlowModel`，自动生成 `<arg>`（从 task `<input>` 映射）、`<return>`（从 task `<output>` 映射）和 `<source>`（生成调用 `nopTaskFlowManager` 的执行体代码）：

```xml
<!-- xbiz 声明：task:name 会在编译期被 TagLib 展开为完整 action -->
<mutation name="approveOrder" task:name="ErpSalOrder/approveOrder"/>
<!-- 展开后等价于（自动生成 arg/return/source，开发者看不到） -->
```
...
**注意**：使用 `task:name` 前需确保 xbiz 文件已通过 `<x:post-extends>` 引入了 `biz-gen:TaskFlowSupport` 标签库，否则 `task:name` 属性会被忽略。
