...
1. **触发条件**：任何部署期 seed 资产（97 CSV 基点 + M1.x 新增 CSV + `zz-sequence-advance.sql`；M1.1b 后 = 125 CSV + 1 SQL，M1.1c 后 = 137 CSV + 1 SQL，M1.2a1 后 = 168 CSV + 1 SQL，M1.2a2 后 = 194 CSV + 1 SQL，M1.2b 后 = 222 CSV + 1 SQL，M1.2c 后 = 241 CSV + 1 SQL，M1.3 后 = 273 CSV + 1 SQL，M1.4a 后 = 318 CSV + 1 SQL，M1.4b 后 = 333 CSV + 1 SQL，M1.5 后 = 372 CSV + 1 SQL）新增/修改/删除。
...
- 其他扩展域交易种子（logistics/b2b/contract/drp/aps 后续批次）——无看板无报表（seed 不解除额外阻塞）；触发条件：对应域端到端数值回归需交易数据时。**aps/logistics 子集消费登记（2026-09-02）**：aps 子集（operation_order / schedule / constraint / op_routing / dispatch_rule / dispatch_log / capacity_reservation）+ logistics 子集（carrier / carrier_config / shipment / shipment_line / shipment_log / shipment_parcel / delivery_window / delivery_booking）已由 M1.4b 批次（plan `2026-09-02-1415-1-m14b-aps-logistics-seed-expansion`）按 roadmap M1 全量覆盖口径全量消费落地（owner doc：`docs/design/aps/seed-data.md` + `docs/design/logistics/seed-data.md`）；**b2b/contract/drp 子集消费登记（2026-09-02）**：b2b 子集（asn / asn_line / certification_checklist / code_mapping / edi_doc / edi_format / edi_log / mft_certificate / mft_config / mft_log / partner_credential / partner_profile / test_exchange）+ contract 子集（approval_matrix / approval_record / consumption_line / contract / contract_line / contract_version / document / invoice_plan / rebate_accrual / rebate_agreement / rebate_settlement / rebate_tier / signature_request / template / volume_discount）+ drp 子集（drp_line / drp_parameter / drp_plan / drp_scenario / drp_scenario_param / drp_scenario_version / inv_drp_cross_dock / inv_drp_dock_appointment / inv_drp_lead_time_record / inv_drp_safety_stock_calc / inv_drp_supplier_score）已由 M1.5 批次（plan `2026-09-02-1415-2-m15-b2b-ct-drp-seed-expansion`，b2b EDI 段独立 plan-audit APPROVE 在先）按 roadmap M1 全量覆盖口径全量消费落地（owner doc：`docs/design/b2b/seed-data.md` + `docs/design/contract/seed-data.md` + `docs/design/drp/seed-data.md`）——**本 Deferred 全部子集（aps/logistics 归 M1.4b + b2b/contract/drp 归 M1.5）至此清零，扩展域交易种子无残留**。
...
>
> **M1 全量覆盖闭环（2026-09-02）**：M1 里程碑 11 工作项（M1.1a~M1.5）全部 `done`——有 seed **368 = 363（className 全集）+ 5（finance 缺 className 补充档）**，精确缺 seed **0**，270 规格表全部行已消费；扩展域交易种子 Deferred 全部子集清零（aps/logistics 归 M1.4b、b2b/contract/drp 归 M1.5）。
...
| 有 seed 的 app.erp.* 实体 | **368** | `_init-data/` 368 个 `erp_*` CSV ↔ entity tableName 精确匹配 | 复算 368（93 基线 + M1.1a 批次 12 + M1.1b 批次 16 + M1.1c 批次 12 + M1.2a1 批次 31（26 规格表 + 5 运行时补充）+ M1.2a2 批次 26 + M1.2b 批次 28 + M1.2c 批次 19 + M1.3 批次 32 + M1.4a 批次 45，2026-09-02）；M1.4b 批次后 329（+15）；M1.5 批次后 **368**（+39 = 363 className 全集 + finance 5 缺 className 补充档，**M1 全量覆盖闭环**，见下方批次行） | 权威（当前） |
| 平台 seed CSV | 4 | `nop_auth_user` / `nop_auth_user_role` / `nop_auth_role` / `nop_sys_code_rule` | 复算 4 | 权威（当前） |
| 精确缺 seed | **0** | roadmap §当前基线 + 本节重算（entity tableName ↔ CSV 精确匹配，逐域小计相加） | 复算 0（= 39 − M1.5 批次 39：b2b 13 + contract 15 + drp 11 已落地，规格表 b2b/contract/drp 节已由 M1.5 消费落地）；**M1 里程碑 11 工作项全 `done`，seed 全量覆盖达成**（运行时口径 368 = 363 + finance 5 补充档同样全覆盖） | **权威（当前）** |
...
| M1.5 批次增量 | +39 CSV（app.erp.*） | plan `docs/plans/2026-09-02-1415-2-m15-b2b-ct-drp-seed-expansion.md`（b2b 13 + contract 15 + drp 11 规格表逐行消费，CSV-only 路径，共 81 行；b2b 主子链 asn+line / mft_config+mft_log，contract 主子表 3 组（contract+line / contract+version / rebate_agreement+accrual+tier+settlement），drp scenario+param/version 与 plan+line 链 + dock_appointment→cross_dock 边；用例指示编码以 REMARK 后缀（P）/（N-TERM）/（N-DIS）落地（无 REMARK/NOTES 列实体以 STATUS 状态列承载并登记 owner doc：b2b partner_profile/test_exchange、drp scenario 三表）；干扰面零漂移设计——b2b：ASN 全终态（RECEIVED_TO_STOCK/CANCELLED）+ edi_format code 避开 `UBL_*` 族（C03 `createInbound` by-code 碰撞向量，独立 plan-audit APPROVE 在先）+ 凭据/端点字段全 masked placeholder（`MASKED-CREDENTIAL-SEED`，含 partner_profile 扩展承载）+ partner_profile 非 PRODUCTION 态；contract：rebate_agreement 避 ACTIVE（DRAFT/SETTLED 承载，runAccrual per-agreementId 零交互）+ invoice_plan planDate 静态 2027 不落 due 窗口 + signature 字符串态码；drp：**`erp_drp_parameter.ORG_ID="1"` 载重避让**（`loadParametersInScope` 仅按 orgId 过滤 + C20a/C20b fixture orgId="2"）+ (wh,mat) 组合 belt-and-braces + cross_dock 终态避 STAGING 扫描面；seed-data.md「其他扩展域交易种子」Deferred 之 b2b/contract/drp 子集由本批消费 → **该 Deferred 全部子集清零**） | 复算：有 seed 329→368 / 精确缺 seed 39→**0**（M1 全量覆盖闭环）/ 平台 4 不变；`TestErpSeedDataIntegrity` 基线常量 329→368 随批更新（常量 javadoc 登记） | 权威（当前批次档） |
