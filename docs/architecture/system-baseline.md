...
|----------|---------|--------|----------|----------|
| `app-erp-master-data` | `app-erp-md` | `erp_md_` | `ErpMd*` | `module-master-data/model/app-erp-master-data.orm.xml` |
| `app-erp-inventory` | `app-erp-inv` | `erp_inv_` | `ErpInv*` | `module-inventory/model/app-erp-inventory.orm.xml` |
| `app-erp-purchase` | `app-erp-pur` | `erp_pur_` | `ErpPur*` | `module-purchase/model/app-erp-purchase.orm.xml` |
| `app-erp-sales` | `app-erp-sal` | `erp_sal_` | `ErpSal*` | `module-sales/model/app-erp-sales.orm.xml` |
| `app-erp-finance` | `app-erp-fin` | `erp_fin_` | `ErpFin*` | `module-finance/model/app-erp-finance.orm.xml` |

### 扩展业务域

| 领域工程 | appName | 表前缀 | 类名前缀 | 权威模型 |
|----------|---------|--------|----------|----------|
| `app-erp-assets` | `app-erp-ast` | `erp_ast_` | `ErpAst*` | `module-assets/model/app-erp-assets.orm.xml` |
| `app-erp-projects` | `app-erp-prj` | `erp_prj_` | `ErpPrj*` | `module-projects/model/app-erp-projects.orm.xml` |
| `app-erp-manufacturing` | `app-erp-mfg` | `erp_mfg_` | `ErpMfg*` | `module-manufacturing/model/app-erp-manufacturing.orm.xml` |
| `app-erp-quality` | `app-erp-qa` | `erp_qa_` | `ErpQa*` | `module-quality/model/app-erp-quality.orm.xml` |
| `app-erp-maintenance` | `app-erp-mnt` | `erp_mnt_` | `ErpMnt*` | `module-maintenance/model/app-erp-maintenance.orm.xml` |
...
服务方法实现遵循 **task.xml 编排 + I*Biz 逻辑**的分工原则。详细约定、步骤实现方式选择、映射规则、Delta 定制见 `service-layer-orchestration.md`，此处仅列核心规则：

- **CRUD 方法**使用 CrudBizModel 默认实现，不写 task.xml / Java / xbiz
- **多步骤编排方法**（≥2 步编排逻辑）使用 `task.xml` + xbiz 绑定作为首选模式，简单单步逻辑直接 Java 或 xbiz
...
- 跨业务域引用（finance → projects/assets）单向合法；其他业务域之间走**弱指针（P）**：`relatedBillType`/`sourceBillType` + `billCode` 字符串三元组
- 单向 DAG，禁止循环依赖（已验证：9 业务域 × 10 引用边全部合规）
