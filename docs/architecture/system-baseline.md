...
**项目状态**：bootstrap 阶段未启用租户（`useTenant` 默认 `false`，未配置全局开关）。是否启用 SaaS 多租户待业务确认；启用时在 `app-erp-all/application.yaml` 配置 `nop.orm.enable-tenant-by-default: true` + `auto-add-tenant-col: true`。
...
- **字典 valueType（D1，Deferred+前向指导）**：现有字典统一 `valueType="int"`（值 `10/20/30`），代码用 `ErpXxxConstants` 命名常量映射。**Deferred 全量迁移到 string**（裁决理由见计划 Phase 5：int→string 使列 Java 类型 Integer→String，全量 `== CONSTANT` 比较退化为引用相等，静默回归风险主导成本）。**前向指导（立即生效）**：新域/新字典一律采用 `valueType="string"`（语义编码如 `"APPROVED"`），不再向新模型传播 int 模式。重新评估触发条件：业财一体打通前 / 跨系统集成启动时。
