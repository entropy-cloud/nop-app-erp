...
> 现状：11 域 ~37 审批实体已标 `use-approval`（DIRECT 模式），`_*.xbiz` 已继承 `approval-support.xbiz`，`I*Biz` 已 `extends IApprovableBiz`。WORKFLOW 模式实体（付款/收款/资产处置/HR 薪酬）待 `.xwf` 流程定义后续计划接入。
