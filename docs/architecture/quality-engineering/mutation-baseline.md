...
> ² mfg 同样在末段进入 re-fork loop（≈90%，2324 变异体落盘后受控终止）。mfg 首跑经 `excludedTestClasses` 排除 `TestErpMfgWorkOrderEndToEnd`——**adjudicated residual（plan `2026-08-02-1500-2` 根因裁决）**：根因为平台级非确定性，`TestErpMfgWorkOrderEndToEnd` 在 pitest 单 JVM minion（全 32 类共享 JVM）下其快照 `@var:ErpMfgJobCard/WorkOrder@updateTime_N` 变量解析偶发 `null` → `check-output-fail`。机制链（均在 nop-entropy，受 Non-Goal「不修改 nop-entropy」约束）：(1) `OrmTimestampHelper.onUpdate/onCreate` 用 `new Timestamp(CoreMetrics.currentTimeMillis())` 写审计时间戳（真实墙钟）；(2) app 层 `ThreadLocalFrozenClock` 依设计**不**冻结 `currentTimeMillis`（保留单调时间真实）；(3) `AutoTestVars.addVar` 按捕获计数（`putIfAbsent`）累加 `_N` + `output()` 录制经 `getNameByValue` 按值反查——同类多次 flush（recordWork 使 JobCard/WorkOrder 更新达 `_2`/`_8`）落同毫秒→值碰撞→`_N` 索引与录制基线漂移。实证：surefire `forkCount=1` 复跑同对（CostFlowEndToEnd→EndToEnd）3 次 run1 FAIL/run2 PASS/run3 FAIL，**非确定性/时序敏感，重录基线无法消除**。fin/inv 及 mfg 其余 31 类更新次数少未触达。score 60% 基于 31/32 类（缺 1 类的杀变异能力，整数位不受影响）。平台 successor 触发：nop-entropy 侧任一修复（`OrmTimestampHelper` 改用 `currentDateTime()` / `ThreadLocalFrozenClock` 增冻结 `currentTimeMillis` 选项 / `AutoTestVars` 对时间戳易挥发字段改按名稳定索引）落地后移除排除并重跑全 32 类基线。详见 pom 注记 + plan `2026-08-02-1500-2`。
