...
| —（successor） | 全不变量穷举（多币种折算平衡 / 合并抵消归零 / 资产折旧残值非负 / i18n locale 正确性） | optimization candidate → **部分已落地** | 纯算术 3 项（P5 STANDARD 重估红冲 / P6 多币种折算 / P7 折旧残值 / P8 合并抵消归零）经 plan `2026-08-02-0650-1` 落地（类一纯 jqwik 内存模型/直调纯函数 + golden + tautology + 种子固化）；i18n locale 正确性仍为 successor（触发：i18n locale 格式化须随机化验证时） |
...
### Q3 successor 实施期发现回填（plan `2026-08-02-0650-1` 执行 P5-P8 纯算术不变量扩展，非静默偏离）

> 本计划是 Q3 §9 successor（line 474「全不变量穷举…核心不变量属性 test harness 沉淀后扩展」）的落地——harness（P1-P3 + jqwik + CI 门控）已沉淀且经独立 closure 验证，触发条件满足。沿用 §3.4 已裁决技术选型（jqwik + 路径 C 类一 + 策略 F2），不引入新设计维度。以下 3 项为执行期对设计文档前提假设的实测修正/补全，不改变技术选型。

- **P5-P8 落地 + 保真度裁决**（§4.2/§9）：4 个纯算术不变量属性 test 落地——P5 STANDARD 重估红冲恢复（inventory，Decision (b) 内存模型镜像 `StandardCostingStrategy.onOutgoing:86-99` + `onIncoming:55-56` 红冲路径，golden 锚定 `TestErpInvStandardCosting`）/ P6 多币种折算借贷平衡（finance，Decision (a) 直调生产 `balanceTotals`——`amount` 字段即功能币额 per posting.md:449，包位置同 P1 同包 `app.erp.fin.service.posting` 访问 protected）/ P7 资产折旧残值守恒（assets，Decision (a) 直调生产 `DepreciationCalculator.calculate` public static 纯函数，golden 锚定 `TestDepreciationCalculator`）/ P8 合并抵消归零（finance，Decision (b) 内存模型镜像 `generateEliminationCandidates`+`writeDraftEliminationVoucher`，golden 锚定 `TestErpFinIntercompanyMatchingAndElimination`）。每域含 golden 交叉校验 + tautology 自检（注入变异反向证明敏感）+ 种子固化（@Property seed）。jqwik 1.10.1 test scope 新增 assets service pom（对齐 finance/inventory）。
- **CI 门控域覆盖扩展**（§8.4 C-3）：`compliance.yml` `property-test-coverage` job 域覆盖基线从 finance+inventory（2 域）扩到含 assets（3 域），P7 在 assets 落地触发。裁决选 option (X) 域覆盖扩展（resilient to class rename + 新增属性不触发基线漂移）而非 option (Y) @Property 计数门控（脆性，每新增属性须手调基线）。本地 Python 复跑 3/3 域 PASSED。
- **surefire include 补全（关键契约修正，§8.1/C-1）**：设计文档 §8.1/C-1 假设「@Property 经 mvn test 自动跑」实测**不成立**——surefire 默认 include 模式（`Test*`/`*Test`/`*Tests`/`*TestCase`）不匹配属性 test 类名约定 `Property*`（§6.2），导致 Q3 首批 P1/P3 + 本计划 P5-P8 在补全前**仅 `JqwikSmokeTest`（匹配 `*Test`）在 `mvn test` 运行**，其余 `Property*` 类只在 `-Dtest='Property*'` 显式运行。修复：`{finance,inventory,assets}/erp-*-service/pom.xml` surefire `<configuration>` 显式 `<includes>` 增 `**/Property*.java`（含默认 4 模式）。验证：finance 341（+19）/inventory 143（+9）/assets 111（+5）tests，P1-P8 全部在 `mvn test` 运行绿。**裁决影响**：补全 §8.1/C-1 contract 使 `maven.yml` CI 实际强制属性 test（先前 closure 未察觉此 gap——`-Dtest='Property*'` 验收掩盖了默认 mvn test 漏拾）；零技术选型/范围变更。
