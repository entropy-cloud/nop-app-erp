...
- **我们的设计**：10 域构成 **DAG**，master-data 为根，业务域间**不做 ORM 强引用**（走 `I*Biz` 接口），无循环依赖。每个域可独立 Maven 工程、独立组装裁剪（见 `docs/architecture/domain-module-split-analysis.md`）。
...
| **MixERP** | (1) 技术栈现代（vs 过时 WebForms）；(2) 活跃（vs 停摆）；(3) 制造/财务深度（vs 弱） | 全部 10 域 145 实体 |
...
1. **生态规模与现成垂直行业模块**：Odoo 有数千个 OCA/第三方模块，App Store 生态巨大；ERPNext 有成熟行业方案。nop-app-erp 处于 bootstrap 阶段，垂直行业模块需逐步建设。
