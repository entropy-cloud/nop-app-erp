...
| **多公司** | 是（多 bug） | 是 | 是（Client/Org） | 是（Client/Org） | **优（MultiValue）** | 是（Client/Org） | 多门店 | **结构地基（orgId 物理列 + 组织树）；行为层（数据隔离/权限范围/公司间交易）待建** |
...
- **模型证据**：**19 份** `app-erp-<domain>.orm.xml`（**352 个自有实体**，另 110 个跨域 `notGenCode` 引用桩，合计 462 个 `<entity>` 元素）是权威；生成的 `_gen/`/`_` 前缀文件**永不手改**（AGENTS.md 强制）。
- **平台继承 vs 本项目落地（诚实区分）**："模型驱动 + Delta 分层"能力**继承自 Nop 平台**（见 `../nop-entropy/docs-for-ai/`），非本项目原创；本项目通过 **338 个手写 `.view.xml` 以 `x:extends="_gen/_*.view.xml"` + `x:override="bounded-merge"`** 演示 Delta 定制（定制列/按钮叠加于生成基线），并在 `<domain>/model/*.orm.xml` 沉淀 19 域业务模型。
...
- **当前落地状态（诚实补注）**：当前 `orgId` 为**物理列 + 组织树结构地基**（列 + 字典 + 自引用树 + `code,orgId` 复合唯一键）；自动 org 数据过滤、权限范围、组织树遍历、公司间交易/转移定价/合并抵销等**行为层尚未实现**（`orgId.*filter|scopeByOrg|orgScope` 零命中，零多组织行为测试）。即"多组织结构地基"已就位，"多公司行为系统"待建——相对 iDempiere 25 年验证的 Client/Org 隔离尚有差距。
...
- **我们的设计**：18 域构成 **DAG**，master-data 为根，业务域间**不做 ORM 强引用**（走 `I*Biz` 接口），无循环依赖（`-service→-dao` 单向，Maven reactor 无环，`app-erp-all` 构建成功佐证）。域间无 ORM 强引用 + 接口化调用，**为未来微服务化奠定结构基础**；当前 `-service` 层仍有编译期跨域依赖（如 `inventory-service → finance-service`/`manufacturing-dao`/`purchase-dao`），**尚不可单域独立构建部署**（见 `docs/architecture/domain-module-split-analysis.md`）。
...
- **为何更好**：**结构上可独立演进、独立替换**某域实现（域间走接口而非 ORM 强引用，无循环依赖）。当前 `-service` 层仍有编译期跨域依赖，单域独立部署尚未达成，但已为未来微服务化与多团队并行开发奠定结构基础——单体 ERP（Odoo/ERPNext/Compiere 家族）无此解耦基础。
...
| **ERPNext** | (1) 多套账（vs 无）；(2) 深度定制不丢（Delta vs class override 升级丢）；(3) 生产版本 + 独立 MRP（vs 拼 Production Plan）；(4) 跨域 DAG + 接口化解耦（vs 单体紧耦合，为微服务化奠定结构基础） | `acctSchemaId`、`ErpMfgProductionVersion`、`docs/architecture/domain-module-split-analysis.md` |
...
| **MixERP** | (1) 技术栈现代（vs 过时 WebForms）；(2) 活跃（vs 停摆）；(3) 制造/财务深度（vs 弱） | 全部 19 域，352 个自有实体（+110 跨域引用桩） |
...
6. **多组织行为层（数据隔离/权限范围/公司间交易）**：当前 `orgId` 仅为物理列 + 组织树结构地基，自动 org 数据过滤、权限范围、公司间交易/转移定价/合并抵销等行为层尚未实现（杠杆 C）。相对 iDempiere 25 年验证的 Client/Org 隔离，本项目尚处"多组织地基"阶段。
...
- **更可扩展**：模型驱动（杠杆 A）+ 跨域 DAG（杠杆 G）让系统可按域独立演进、独立替换（域间 ORM 解耦 + 接口化调用，为未来微服务化奠定结构基础）；新增域/新增账套/新增成本方法都是数据而非代码改动。
