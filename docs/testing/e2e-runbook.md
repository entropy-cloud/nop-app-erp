...
`assertReportRendered(cfg)` 编排：`loginAndNavigate` → `page.waitForResponse`（拦截含 `renderHtml` 的 `/graphql` 响应）→ 条件化 `fill`（input-number/input-text 表单字段）→ 条件化 `fillDates`（AMIS input-date 经 label 定位）→ `page.route` 拦截器（空字符串变量 → null、Unix 时间戳 → ISO 日期）→ 点「渲染报表」→ 断言 200 → `expect.poll` 轮询 body textContent 含首个 token → 逐 token 断言全量。token 为 format-independent subject/label（title/header/code/dict-label），数值 ≥1000 因 DOM 千分位格式化（如 120000→"120,000.00"）仅在 value-spec 层断言。
...
`assertAmisDownloadButton({ domain, reportName, renderType, route, formFields, fillDates, expectedTokens })` 编排：`loginAndNavigate` → 条件化 `fill`（input-number/input-text）/`fillDates`（AMIS input-date 经 label 定位）→ `page.route('**/p/*')` 归一化 `/p/` 请求 `data` map（`""→null` / 10 位时间戳→ISO 日期，镜像 renderHtml visual helper 的 AMIS 日期序列化 workaround）→ 点「下载 XLSX/PDF」→ ①`page.waitForRequest('/p/{biz}__download')` 断言按钮命中 `/p/` 端点（URL 回退到 `/graphql` 的回归会令此 promise 超时）→ ②`page.waitForEvent('download')` → `download.path()` 落盘文件 `fs.readFileSync` Buffer → ③长度 > 0 → ④魔数 → ⑤文本提取后断言至少一个 `expectedTokens` 子串。
