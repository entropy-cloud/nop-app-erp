...
| assets | `ErpAstDepreciationSchedule` 折旧引擎（executeDepreciation/reverseDepreciation） | `ast-depreciation.action.spec.ts` | DIRECT `@BizMutation` 折旧引擎生命周期：自包含建 `ErpAstAsset`(IN_SERVICE + STRAIGHT_LINE + 原值/残值/年限) → `executeDepreciation(assetId,period)` 断言 schedule status=EXECUTED + STRAIGHT_LINE 数学 (12000-0)/36=333.3333 + 资产卡片 accumulatedDepreciation/netBookValue 回写 + DEPRECIATION 过账 posted=true → `reverseDepreciation` 断言 REVERSED + posted=false + 资产卡片回退；非法迁移守卫（无 schedule 时 reverse 拒绝 + 不存在资产拒绝）；期间控制须经 ErpFinAccountingPeriod(code=period,status=OPEN)；科目 6602(折旧费用 Dr)/1602(累计折旧 Cr) 经种子补齐（0215-1）（plan 2026-07-14-0215-1） |
| assets | `ErpAstCip` 在建工程转固链（startConstruction→addCostItem→addProgressBilling→transferToAsset→reverseTransfer） | `ast-cip-capitalization.action.spec.ts` | DIRECT `@BizMutation` CIP 转固全链编排：自包含建 `ErpAstCip`(DRAFT) → `startConstruction`(IN_CONSTRUCTION) → `addCostItem`(PURCHASE 500 + OTHER 300 = accumulatedCost 800) → `addProgressBilling`(进度付款 400，不参与转固) → `transferToAsset`(TRANSFERRED + isCompleted + completedAssetId 回链 + 资产原值=成本归集合计 800) → `reverseTransfer`(IN_CONSTRUCTION + isCompleted=false)；资本化单 posted=true（CAPITALIZATION 过账 Dr 1601/Cr 1603，科目经 0215-1 补齐）；cleanup 逐域删除凭证→折旧计划→资产→资本化单→成本行→CIP（plan 2026-07-14-0215-1） |
| assets | `ErpAstInventory` 资产盘点 8 动作生命周期 | `ast-inventory-count.action.spec.ts` | DIRECT `@BizMutation` 盘点状态机：自包含建测试资产类别+资产（范围隔离）→ `createInventory`(DRAFT + 范围展开行) → `submitForCount`(COUNTING) → 物理盘点行更新 actualQuantity=0(SHORTAGE) → `reconcile`(RECONCILING + shortageAmount=bookValue) → `processVariance`(盘亏 SCRAPPED) → `approve`(复核) → `post`(POSTED + ASSET_INVENTORY_ADJUSTMENT 过账 Dr 6711/Cr 1601) → `reverse`(RECONCILING + 红冲)；cancel 异常路径 + 非法迁移守卫（CANCELLED→submitForCount 拒绝）；MATCHED-only 零差异路径 Provider 返回空 facts 不产凭证故须实际差异达 POSTED；科目 1601 经种子补齐（0215-1）（plan 2026-07-14-0215-1） |
| assets | `ErpAstMaintenance` 维修费用化/资本化双路径 | `ast-maintenance.action.spec.ts` | DIRECT `@BizMutation` 维修双路径：(a) 费用化 EXPENSE：createMaintenance→submit→startWork→completeWork→decideTreatment(EXPENSE)→approve→post(POSTED + MAINTENANCE_EXPENSE 凭证 Dr 6602/Cr 1002)→reverse(COMPLETED)；(b) 资本化 CAPITALIZE：同上但 decideTreatment(CAPITALIZE)→post(POSTED + 资产原值增量 + MAINTENANCE_CAPITALIZATION 凭证 Dr 1601/Cr 1002 + 折旧重算)→reverse(资产原值回退)；cancel 异常路径 + IN_PROGRESS→cancel 非法守卫；费用归集行 ErpAstMaintenanceCost(LABOR/SPARE_PART/SUBCONTRACT) aggregateCost>0 为 post 前置；科目 6602/1601 经种子补齐（0215-1）（plan 2026-07-14-0215-1） |
