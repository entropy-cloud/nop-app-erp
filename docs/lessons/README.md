...
- `10-dict-dead-state-unreachable.md` — **状态机 dict 死状态**：dict 声明了无 `setStatus` writer 的状态值（永不出现），owner doc 迁移图却声明进/出迁移。MR1 跨 finance/mfg/hr/inv/qa/prj/contract/aps/logistics 多域同型。含「每个状态值 grep writer」决策树与删/实现/Deferred 三选一裁决。
