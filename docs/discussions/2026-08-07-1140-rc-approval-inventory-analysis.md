...
> **注（2026-08-08 快照修正）**：上述 §1 快照时点为 2026-08-07 11:40。实仓 2026-08-08 已确认：cs `2026-08-08-0015-1` 已 completed（A4.2.124-142 全 done，含 A4.2.130/137 checkbox 已勾选 + Closure 审计通过 + roadmap 回写 done）；**master-data plan `2026-08-08-0015-2` 仍 active**（A4.2.143-146 尚 todo，未开展）。表 G 及 §1 中关于 0015-1 的「草稿/未勾选」描述属历史时点，以此条修正为准。
...
### 表 G：在途 plan 中未勾选 ask-first checkbox（5 处）

| plan | 行 | 内容 |
|---|---|---|
| 2026-08-08-0015-1（cs 草稿） | A4.2.130/137 | 只读确认（不实施修复，无修复义务） |
| 2026-08-08-0015-2（master-data 草稿） | A4.2.144/146 | 只读确认；修复义务归 MR1（P2-RC-058 / P1-RC-062 触 ORM/删除 ask-first） |
| 2026-07-31-0310-2（r2-7） | Phase 4 | `enable-action-auth=true` 翻转须人工批准 + 灰度（config-gated successor，当前保持 false） |

### 表 H：待执行 MA4 只读审计行（无需批准，仅登记）

- A4.2.124-142（cs，19 项只读运行时确认，plan `2026-08-08-0015-1` 已审）
- A4.2.143-146（master-data，4 项只读运行时确认，plan `2026-08-08-0015-2` 已审）
- 其余 65 个 todo 行均为只读审计（预授权类目），不触及保护区域，无需人工批准
...
- **Q3（纯加性 ORM 批量授权）覆盖 ORM ask-first 门**：限于 *加列 / 加 UK / 新增实体，不改既有语义 / 无 NOT NULL 无默认值列 / 无涉及既有数据的 UK 增设（须数据变更评估）*，超出即回落独立 fix plan + plan-audit。
