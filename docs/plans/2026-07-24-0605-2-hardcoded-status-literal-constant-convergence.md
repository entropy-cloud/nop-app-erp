...
- [ ] `Proof`：产出权威字面量清单——grep 全 9 域 service src/main/java 的 7 个状态字面量，逐条标注 file:line + 所在方法 + 上下文（setStatus/setter 比较 / 状态机判断 / 其他）。落权威清单于本计划附录（或 `docs/audits/hardcoded-status-literal-inventory.md`），作为 Phase 2 替换框（对齐 0930-2 Phase 0 stale 清单范式，消除计数漂移风险）。
  - Skill: `nop-backend-dev`
- [ ] `Decision`：逐条语义轴判定——「doc-status 轴（替换为 DOC_STATUS_*）」/「approve-status 轴（替换为 APPROVE_STATUS_*）」/「异语义（排除，如设备 ACTIVE 运行态、合同 ACTIVE 生效态）」/「mfg 域 doc-status（排除，绑域专属字典）」。记录每条的判定 + 依据 + 残留风险（边界判定歧义处显式标注）。
  - Skill: `nop-backend-dev`

Exit Criteria:

- [ ] 权威清单已产出（file:line + 语义轴判定三态/排除标注）
- [ ] 替换框（doc/approve-status 轴子集）与排除集（异语义/mfg doc-status）边界明确
...
- [ ] `Fix`：逐域将替换框内字面量改为 `Erp*DocStatus.<CONST>` 引用——补 `import ...constants.Erp*DocStatus`（若未在），使用点 `"LITERAL"` → `Erp*DocStatus.<CONST>`。按域推进（purchase/sales/finance/inventory/assets/maintenance/quality/cs 8 域 approve+doc 双轴；manufacturing 仅 approve 轴）。每域替换后 `mvn compile -pl <module>/<service> -am` 通过。
  - Skill: `nop-backend-dev`
- [ ] `Add | Decision`：若某字面量对应的常量在 Erp*DocStatus 接口缺失（如某域 doc-status 有非标准值），裁决补接口常量 vs 排除为异语义，不擅自扩展接口语义。
  - Skill: `nop-backend-dev`

Exit Criteria:

- [ ] 替换框内字面量全部改为常量引用（grep 替换框字面量在 service src/main/java 命中数较 Phase 1 清单下降至排除集）
- [ ] 每域 `mvn compile` 通过（解除后续验证阻塞）
...
- [ ] `Proof`：`mvn clean install -DskipTests`（154 模块 BUILD SUCCESS）+ 受影响域 `mvn test` 全绿（常量值与原字面量等价，状态机行为不变）；复跑 `bash docs/audits/nop-compliance-checker.sh` 确认 R3/R11 等基线无回归（字面量→常量不引入 `new Erp*()` 构造）。
  - Skill: none
- [ ] `Proof`：治理审查 §F2(d) 标注 successor RELEASED（D1 接口全域落地后字面量→常量替换完成）；更新 `docs/audits/compliance-baseline.md`（若计数变化说明）。
  - Skill: none
...
- [ ] checker 基线复跑结果记录在案（R3/R11 无回归，若计数变化已说明）
- [ ] 治理审查 §F2(d) successor 标注 RELEASED（D1 接口全域落地后字面量→常量替换完成）
...
- [ ] 范围内行为完成（替换框字面量→Erp*DocStatus 常量）
- [ ] 相关文档对齐（治理审查 F2(d) successor + compliance-baseline）
- [ ] 已运行验证：`mvn clean install -DskipTests` + 受影响域 `mvn test` + checker 复跑（R3/R11 无回归）
- [ ] 无范围内项目降级为 deferred/follow-up（异语义/mfg doc-status 是 Phase 1 明示的排除裁决非范围缩减；ORM ext:dict 统一为 0930-2 既有 Deferred）
- [ ] 独立草案审查已完成并记录
