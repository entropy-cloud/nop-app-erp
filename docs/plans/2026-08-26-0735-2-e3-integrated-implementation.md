...
- [x] Add: AI 工具消费约定最小落地集（按 Phase 1 冻结清单逐项落地：action 元数据/描述补全等——不新建 AI 专用业务 API，AP-1 外挂化裁决）+ REST 与 GraphQL 双通道调用一致性验证。
      ...
      → **④ 证据归因 post-hoc 注记（2026-08-28，plan `2026-08-28-0219-1` P2-E）**：本项「高影响 mutation 被拒」的负路径证据当时实测的是既有 `ErpFinBadDebt__writeOff`（非 E3 批新增 API 面——归因漂移，open-audit P2-E 发现）。E3.5 管道新增 mutation 面（`uploadApDocument`，action-auth 已登记 FNPT）的无权限拒绝路径由该计划补齐于 `ai-interface.value.spec.ts`「uploadApDocument denied for unauthorized role」用例；归因修正不改变原验收结论（enforcement 门径复用范式本身已由 BadDebt 负路径证实）。
      → **② 「IBiz 同步」空指 post-hoc 注记（2026-08-28，plan `2026-08-28-0219-3` P2-12②）**：本项②中「11 个 `getDashboardKpi` 补 `@Description`（IBiz 同步，11 域 grep 核对）」的「IBiz 同步」为**空指**——仓内不存在 `IErp*Dashboard*` 接口（活仓 grep 零命中证实：`getDashboardKpi` 均为各域 Dashboard BizModel 上的方法注解，无 IBiz 接口层可同步）。归因修正不改变原验收结论（11 域 `@Description` 已实际补全， introspection 断言为证）；E3 批新增 5 个对外 action 的 `@Description` 欠账由该计划 Phase 2 收口。
