...
- **坏账凭证科目经 0540-1 设计 + 0413-2 种子补齐**：`ErpFinBadDebt` 经 `CloseVoucherWriter` 直接持久化凭证（核销不进 P&L，owner doc `bad-debt.md`）。四业务类型：BAD_DEBT_WRITE_OFF（核销：Dr 坏账准备 1231 / Cr 应收账款 1122）/ BAD_DEBT_RECOVERY（收回：反向 Dr 1122 / Cr 1231）/ BAD_DEBT_RESERVE（计提：Dr 资产减值损失 6701 / Cr 坏账准备 1231）/ BAD_DEBT_RELEASE（释放：反向 Dr 1231 / Cr 6701）。**精确 Dr/Cr 科目映射须经 Phase 1 Explore 核实 `BadDebtAcctDocProvider` / `CloseVoucherWriter` 源码确认**（0413-2 仅补种子 + 存在性断言，未核实行级科目分解）。
- **自动核销 BizModel 已落地（DIRECT `@BizMutation`，浏览器层可达但 config 门控）**：`ErpFinReconciliationBizModel.runAutoReconciliation(direction, partnerId, strategy, ctx)` `@BizMutation` :166——config 门控 `erp-fin.auto-reconcile`（默认 false，:208 `isAutoReconcileEnabled`），关闭时抛 `ERR_AUTO_RECON_DISABLED`；启用时按 strategy（FIFO/BY_AMOUNT/BY_RATIO，默认 FIFO :212 `resolveStrategy`）逐 partner 匹配 OPEN 对 → 内部 create+post → 返回核销结果。**无 xwf**（DIRECT），浏览器层经 GraphQL mutation `ErpFinReconciliation__runAutoReconciliation` 可达。
- **webServer JVM arg 追加范式已稳定验证（0413-2/0730-1）**：`tests/e2e/playwright.config.ts:18` webServer JVM args 已追加 `-Derp-mfg.variance-auto-calc-enabled=true`/`-Derp-qua.ncr-default-acct-schema=1`/`-Derp-mfg.inspection-gate-enabled=true`/`-Derp-fin.bad-debt-default-acct-schema=1`。追加 `-Derp-fin.auto-reconcile=true` 同范式。
...
- **自动核销对种子基线副作用风险（0204-2/0413-2 Deferred 触发条件核心关切）**：`erp-fin.auto-reconcile=true` 启用后，`runAutoReconciliation` 仅对传入的 `direction+partnerId` 范围生效（:166 签名需 partnerId 入参，非全局扫描），**不对种子全 SETTLED 对产生副作用**（种子 1-4 已 SETTLED，auto-recon 仅匹配 OPEN 对，SETTLED 项不可再核销）。自包含新建 partner+OPEN 对隔离，cleanup 删除使无残留。**故触发条件「对种子基线无副作用」已满足**。
...
- 验证 config 门控负路径（`erp-fin.auto-reconcile=false` 时 `ERR_AUTO_RECON_DISABLED`）——经独立 spec 或 spec 内 config-off 场景断言。
...
- 依赖 webServer 启动参数追加 `-Derp-fin.auto-reconcile=true`（`tests/e2e/playwright.config.ts:18`，Phase 2 追加）
...
- [ ] `Explore`：核实坏账四业务类型凭证行精确 Dr/Cr 科目分解。**核实项**：读 `BadDebtAcctDocProvider` / `CloseVoucherWriter`（坏账核销经 CloseVoucherWriter 直接持久化）源码，确认 BAD_DEBT_WRITE_OFF（Dr 1231 / Cr 1122）/ BAD_DEBT_RECOVERY（反向）/ BAD_DEBT_RESERVE（Dr 6701 / Cr 1231）/ BAD_DEBT_RELEASE（反向）的精确科目码 + dcDirection + 金额来源（writeOff 金额=AR 项 openAmount；reserve 金额=账龄分桶计提引擎输出）。银行对账 BANK_RECON_ADJ（Dr 1002 / Cr 2240OTHER）经 0413-2 Phase 1 裁决已确认。记录于本计划。
      - Skill: `nop-testing`
...
Status: planned
Targets: `tests/e2e/playwright.config.ts`（追加 JVM arg）、`tests/e2e/business-actions/fin-auto-recon.action.spec.ts`（新建）
Skill: `nop-testing`

- Item Types: `Add | Proof`
- Prereqs: 无硬依赖；可与 Phase 1 并行（不同 spec 文件，无前后置阻断）

- [ ] `Add`：`tests/e2e/playwright.config.ts:18` webServer JVM args 追加 `-Derp-fin.auto-reconcile=true`（镜像 0413-2/0730-1 范式）。验证对种子基线无副作用（种子 1-4 全 SETTLED，auto-recon 仅匹配 OPEN 对）。
      - Skill: `nop-testing`
- [ ] `Add`：`fin-auto-recon.action.spec.ts` 新建——三策略各 1 用例：(a) 自包含建 partner `E2E-AUTORECON-PN-{ts}` + OPEN AR 对（AR_INVOICE + RECEIPT，同金额，复用 0204-2 `ErpFinArApItem__save` 范式）→ `ErpFinReconciliation__runAutoReconciliation(direction:"RECEIVABLE", partnerId, strategy:"FIFO")` `callMutation` 断言核销单自动创建 + posted + 双方辅助账 openAmount→0/status=SETTLED（`verifyState` 逐项 `__get`）；(b) 同 setup，strategy="BY_AMOUNT"；(c) 同 setup，strategy="BY_RATIO"。每步 cleanup 删核销单+行+辅助账+partner。config 门控负路径：若可独立验证 config-off 场景（需独立 webServer 实例或 spec 内不依赖该 config 的断言），断言 `ERR_AUTO_RECON_DISABLED`；否则经 0115-1 单测覆盖记录 Non-Goal。
      - Skill: `nop-testing`
...
- Why Not Blocking Closure: webServer JVM arg 启动期固定 `-Derp-fin.auto-reconcile=true`，运行中不可翻转。config-off 场景需独立 webServer 实例（不实际）或 0115-1 单测覆盖（已存在）。本计划覆盖 config-on 三策略正路径。
