...
> Plan Status: draft
> Review Hold: §11.2 M4 (i) plan-first 人工/owner-doc 门控未确认（Reconciliation post/reverse 触发 ArApItem 联动；BadDebt approve 触发 BAD_DEBT_WRITE_OFF/RECOVERY 凭证 + ArApItem 对称回滚）。project-context.md 会计/财务硬停止。门控非起草者/审查者可自主解除——同 `0930-1` / `0810-x` / `1146-x` / `1931-x` batch-consistent hold。
