...
计划文件名遵循**日期时间加序号**的命名约定，不使用纯数字前缀编号（如 `01-`、`02-`）：

```
{YYYY-MM-DD-HHmm}-{N}-{slug}.md
```

- `{YYYY-MM-DD-HHmm}` — 起草时间戳（年-月-日-时分，24 小时制）。
- `{N}` — 一位序号（`1`、`2`、`3`...），反映同一批起草计划的预期执行顺序。先解除其他计划阻塞的计划排在前面。同一时间戳的多个计划按文件名字母顺序排序决定执行顺序，`{N}` 前缀保证此顺序稳定。
- `{slug}` — 简短的小写连字符 slug，描述计划的结果表面。

模板中的 `<plan-id>` 即文件名主干（不含 `.md`）。此命名约定与 mission driver 的起草流程一致（见 AGE 模板 `tools/mission-driver/prompts/draft-from-roadmap.md`，本仓不持有该工具源码）。

示例：

- `2026-06-30-2328-1-phase3-new-domains-app-aggregation.md`
- `2026-06-30-2328-2-phase4-crud-smoke-tests.md`
- `2026-07-01-0215-1-complete-deferred-crud-smoke-tests.md`

例外：本指南本身、`README.md` 等目录级元文件保留 `00-`、`README` 等固定前缀，不参与此命名约定。
