...
- [ ] Fix: md-001 findEffectiveByPartner 补 `le("validFrom", today)` + `ge("validTo", today)` 过滤 + `orderBy(validFrom, DESC)` 确定性排序
...
- [ ] Fix: md-004 validTo 改 `today`（单日窗口）+ 刷新路径补 `ErpDateRanges.contains` 互斥校验
- [ ] Fix: md-005 resolveConversionRate 正向未命中时尝试反向查（取倒数系数 `ONE.divide(rate, scale, HALF_UP)`）
