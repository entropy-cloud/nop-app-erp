...
- [x] **Decision**：七项 finding 处置方案逐项裁决。
...
- [x] Phase 1 Decision 逐项记录；后续 Phase 严格遵循（049/056/058→Phase 2，050/057/059→Phase 3 守卫，051→Phase 3 listener）。
...
- [x] **Fix（049）**：`ErpPurQuotation.xbiz:97` + `ErpPurRfq.xbiz` reverseApprove 改 `entity.approveStatus = 'REJECTED'` + `entity.approvedBy = null; entity.approvedAt = null`（与 pur 大 Processor 对齐）。
      - Skill: `nop-backend-dev`
- [x] **Fix（056）**：`ErpSalContract.xbiz:97` reverseApprove 改 REJECTED + 清 approvedBy/At。
      - Skill: `nop-backend-dev`
- [x] **Fix（058）**：`ErpAstMovement.xbiz:97` reverseApprove 改 REJECTED + 清 approvedBy/At。
      - Skill: `nop-backend-dev`
- [x] **Proof**：测试——三实体 reverseApprove 后断言 approveStatus='REJECTED' + approvedBy/At cleared；submit→approve→reverseApprove 链路主路径通过。
      - Skill: `nop-backend-dev`
      - 证据：`TestErpPurQuotationRfqReverseApprove`（4 测试，Quotation/Rfq 各 2）、`TestErpSalContractReverseApprove`（2 测试）、`TestErpAstMovementReverseApprove`（2 测试）全绿。

Exit Criteria:

- [x] 三实体 xbiz reverseApprove 目标态为 REJECTED（grep `approveStatus = 'SUBMITTED'` 在 reverseApprove 段为零）；与大 Processor 一致。
...
- [x] **Fix（050，方案B）**：pur 全 INLINE 实体前置补 isCancelled 守卫——覆盖 Receive/Invoice/Payment/Return/Requisition/Order 的 INLINE reject + withdrawApproval **以及 Quotation/Rfq 全 INLINE 动作**（submitForApproval/approve/reject/reverseApprove/withdrawApproval，P1-MA2-050 明确含 Quotation/Rfq 全 INLINE；Phase 2 仅改其 reverseApprove 目标态，其余动作仍需守卫）：脚本前置 `if (entity.docStatus === 'CANCELLED') throw NopScriptError(...)`。
      - Skill: `nop-backend-dev`
      - 证据：pur 8 实体共 22 处 isCancelled 守卫（Quotation/Rfq 各 5、Receive/Invoice/Payment/Return 各 2、Requisition 3、Order 1）。
- [x] **Fix（057，方案B）**：sal 6 实体 INLINE withdrawApproval + Contract 全 INLINE 动作前置补 isCancelled 守卫。
      - Skill: `nop-backend-dev`
      - 证据：sal 7 实体共 11 处守卫（Contract 5、Delivery/Quotation/Return/Order/Invoice/Receipt 各 1）。
- [x] **Fix（059，方案B）**：ast Movement 全 5 INLINE 动作前置补 isCancelled 守卫（Movement 经 useLogicalDelete 承载 CANCELLED，守卫为防御性）。
      - Skill: `nop-backend-dev`
      - 证据：Movement 5 处守卫。
- [x] **Fix（051，方案A）**：`PurReversalListener.rollbackReceive` 与 rollbackInvoice/Payment/Return 对齐：`if (approveStatus == APPROVED) setApproveStatus(REJECTED)`；purchase state-machine.md / returns.md 回退目标态表补 receive→REJECTED。
      - Skill: `nop-backend-dev`
      - 证据：`PurReversalListener.java:112` rollbackReceive 对齐；posting.md 裁决4 表 + purchase/state-machine.md 回退表更新。
- [x] **Proof**：测试——CANCELLED 单据调 reject/withdrawApproval/reverseApprove assertThrows（守卫阻断）；PurReversalListener rollbackReceive 后断言 receive approveStatus=REJECTED + posted=false。
      - Skill: `nop-backend-dev`
      - 证据：`TestErpPurQuotationRfqReverseApprove`（3 守卫阻断测试）+ `TestErpSalContractReverseApprove`（2 守卫阻断）+ `TestErpAstMovementReverseApprove`（2 守卫阻断）+ `TestPurReversalListenerReceiveRollback`（2 回退对称）；三域全量回归 130+140+96 测试全绿。
- [x] **Add（owner doc）**：三域 state-machine.md 补「§实现模式」注记（PROC vs INLINE 守卫边界：INLINE 仅 isCancelled + src 状态校验，无 requireSupplier/Customer/Lines 业务守卫）+ reversal listener 回退目标态表（receive→REJECTED）。
      - Skill: `nop-backend-dev`
      - 证据：purchase/sales/assets state-machine.md 各补「实现模式与守卫边界」节。

Exit Criteria:

- [x] 三域 INLINE 动作 isCancelled 守卫可测（CANCELLED 单据阻断）；PurReversalListener.rollbackReceive 对称（REJECTED）；owner doc 实现模式 + 回退表与代码一致。
...
- [x] 范围内行为完成（7 项 finding 全部修复：3 reverseApprove REJECTED + 3 INLINE 守卫 + 1 listener 对称）
- [x] 相关文档对齐（三域 state-machine.md 实现模式 + 回退目标态表）
- [x] 已运行验证（`mvn clean install -DskipTests` 全绿 + `mvn test` 全绿 + compliance checker 基线不高于 M0；grep 验证 reverseApprove 段零 SUBMITTED + INLINE 守卫存在）
      - `mvn clean install -DskipTests` 全 reactor BUILD SUCCESS；pur(130)+sal(140)+ast(96) 全量测试 0 失败；compliance checker R12a/b/c=69/66/38（=基线，无增长），无新增生产代码高严重度违规。
- [x] 无范围内项目降级为 deferred/follow-up（方案B INLINE 守卫是范围内存活实现项；死代码 Processor 处置归 P2-MA2-054 watch-only 不在范围）
- [x] 独立草案审查已完成并记录
...
Status Note: 已完成并通过独立结束审计（PASS）。7/7 finding 修复落地，三域全量回归 + 全 reactor install 全绿。

Closure Audit Evidence:

- 独立结束审计（ses_0503e13d8ffeF3ML5TUGwOJe05，新会话 read-only）裁决 **PASS**：
  - V1 reverseApprove 目标态 REJECTED：4 实体（Quotation/Rfq/Contract/Movement）reverseApprove 块均设 REJECTED + 清 approvedBy/At，零 SUBMITTED 残留。
  - V2 INLINE isCancelled 守卫：38 处守卫计数精确匹配（pur 22 + sal 11 + ast 5），守卫位置正确（requireEntity 后、status 校验前），Processor 委派块无冗余守卫。
  - V3 rollbackReceive 对称：`PurReversalListener.java:122-124` 与 rollbackInvoice/Payment/Return 对齐 APPROVED→REJECTED，Javadoc 已更新。
  - V4 owner docs：三域 state-machine.md + posting.md 裁决4 表均已补实现模式节 + 回退表（receive→REJECTED）。
  - V5 测试：4 测试文件存在且断言覆盖 reverseApprove=REJECTED + CANCELLED 守卫阻断 + rollbackReceive 对称。
  - 无 scope creep：SalReversalListener.rollbackDelivery（P2-MA2-057 watch-only）正确未动；死代码 Processor 处置（P2-MA2-054）排除。
- 验证基线：`mvn clean install -DskipTests` 全 reactor BUILD SUCCESS；pur 130 + sal 140 + ast 96 全量测试 0 失败；compliance checker R12a/b/c=69/66/38（=基线无增长）。

Follow-up:

- 非阻塞；successor 已在 Deferred But Adjudicated 命名触发条件（INLINE→大 Processor 迁移 / rollbackDelivery 对齐）。
