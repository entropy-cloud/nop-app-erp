...
- [ ] `Decision`: 确认跳过条件——`prop.name == 'id' && prop.tagSet != null && prop.tagSet.contains('seq-default')`。替代方案：`prop.primaryKey`（但不确定 codegen context 是否暴露此属性）。Explore：读 `view-gen.xlib:GenForm` 确认 `${prop}` 对象上可用的属性集合（tagSet 是否可访问），确认后选定条件。
- [ ] `Fix`: `view-gen.xlib:44-46` 的跳过条件从 `if(prop.internal) continue;` 扩展为同时跳过 seq-default 主键。修改后条件：`if(prop.internal) continue; if(prop.name == 'id' && prop.tagSet && prop.tagSet.contains('seq-default')) continue;`
- [ ] `Proof`: `mvn clean install -DskipTests` 触发全域 codegen 增量重生成。抽样验证 5+ 域的 _gen view.xml 中 form layout 不再包含 `id[ID]` 行。

Exit Criteria:

- [ ] `view-gen.xlib` GenForm 跳过条件已扩展
- [ ] 重生成后抽样 5+ 域 `_gen/_*.view.xml` 的 form layout 无 `id[ID]` 行
...
- [ ] `Add`: **purchase 6 实体** view.xml 表单改造——ErpPurOrder / ErpPurReceive / ErpPurInvoice / ErpPurPayment / ErpPurReturn / ErpPurRequisition。每实体：
  ...
  - 大表单（≥20 字段）设 `size="lg"`
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **sales 6 实体** view.xml 表单改造——ErpSalOrder / ErpSalDelivery / ErpSalInvoice / ErpSalReceipt / ErpSalReturn / ErpSalQuotation。同 purchase 范式（code like + status + customerId + approveStatus + businessDate date-between）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **inventory 3 实体**——ErpInvStockMove / ErpInvStockBalance / ErpInvTransferOrder。查询字段（code like + moveType/status + materialId + warehouseId）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **finance 3 实体**——ErpFinVoucher / ErpFinVoucherLine / ErpFinReconciliation。查询字段（code like + voucherDate date-between + voucherType + docStatus）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **master-data 4 实体**——ErpMdMaterial / ErpMdPartner / ErpMdSubject / ErpMdOrganization。查询字段（code like + name like + status + category）。
  - Skill: `nop-frontend-dev`
- [ ] `Proof`: 22 实体 view.xml `xmllint --noout` 全 well-formed；抽样 3+ 域启动应用验证表单分组渲染正确（fieldSet 折叠/展开）+ 查询表单 filterOp 生效（查询字段名含 `__like` / `__date-between` 后缀）。
  - Skill: `nop-frontend-dev`

Exit Criteria:

- [ ] 22 实体 view.xml 的 view/edit 表单含分组（`=====`>`/`^` 标记），审计组折叠
- [ ] 22 实体 view.xml 的 query 表单含 5+ 查询字段 + filterOp 配置
- [ ] 22 view.xml `xmllint --noout` 全通过
- [ ] 抽样域运行时验证通过
...
- [ ] `Add`: **manufacturing 3**——ErpMfgWorkOrder（43 字段，最大实体，必须分组）/ ErpMfgBom / ErpMfgJobCard。ErpMfgWorkOrder 分 5 组：基本信息 / BOM 信息 / 计划信息 / 状态信息 / 审计。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **assets 2**——ErpAstAsset / ErpAstDepreciationSchedule。查询（code like + assetCategory + status + costCenterId）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **crm 2**——ErpCrmLead（40 字段，第二大实体）/ ErpCrmOpportunity。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **hr 1**——ErpHrEmployee（38 字段）。分 4 组：基本信息 / 联系方式 / 雇佣信息 / 薪酬。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **projects 2**——ErpPrjProject / ErpPrjProjectSettlement。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **logistics 1**——ErpLogShipment（40 字段）。分 4 组：基本信息 / 承运信息 / 费用 / 状态。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **quality 1 + maintenance 1 + cs 1 + aps 1 + b2b 1 + contract 1**——各域最大实体。查询字段按业务语义选择。
  - Skill: `nop-frontend-dev`
- [ ] `Proof`: 17 实体 view.xml `xmllint --noout` 全 well-formed。
  - Skill: `nop-frontend-dev`

Exit Criteria:

- [ ] 17 实体 view.xml 的 view/edit 表单含分组
- [ ] 17 实体 view.xml 的 query 表单含查询字段 + filterOp
- [ ] 17 view.xml `xmllint --noout` 全通过
...
- [ ] 范围内行为完成：Phase 0 codegen 修复 + Phase 1-2 共 ~40 实体表单分组 + 查询表单
- [ ] 相关文档对齐：`docs/analysis/view-form-layout-gap-analysis.md` §4-6 对应修复项标记完成；Phase 0 codegen 变更记录于 `nop-entropy/ai-dev/logs/`，本项目 view.xml 定制记录于 `docs/logs/`
- [ ] 已运行验证：`mvn clean install -DskipTests` 154 模块 BUILD SUCCESS + `xmllint --noout` 全通过 + 抽样域运行时验证
- [ ] 无范围内项目降级为 deferred/follow-up（全部 ~40 核心实体在范围内）
- [ ] 独立草案审查已完成并记录
- [ ] 文本一致性已验证
- [ ] 结束审计由独立子代理（新会话）执行
- [ ] 结束证据存在于文件中
...
- ~298 低频实体分组 + 查询表单 successor（触发条件见 Deferred）
