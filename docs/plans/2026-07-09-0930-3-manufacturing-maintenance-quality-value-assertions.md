...
- **像素截图子集 RELEASED by 2026-07-17-2010-2**：经 plan `2026-07-17-2010-2` Phase 2 承接——三域看板（manufacturing/maintenance/quality）含于 10 看板 `dashboards.snapshot.spec.ts` 像素基线，三域代表性报表（qa-inspection-summary 类、mnt-maintenance-history 类）含于 6 代表性报表 `reports.snapshot.spec.ts` 基线（pixel-subset RELEASED）。**bundle 内「报表下载产物字节级 diff」子集不在此 RELEASE**：0204-1 仅交付二进制有效性回归层（魔数 + 弱结构 token），字节级 diff 仍 open optimization candidate，触发条件不变。**bundle 内「跨浏览器矩阵」子集不在此 RELEASE**：AMIS 主目标 Chromium，触发条件不变（需支持非 Chromium 时），归独立 successor。
