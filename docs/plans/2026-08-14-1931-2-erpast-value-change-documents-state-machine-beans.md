...
> Plan Status: draft
> Review Hold: §11.2 M4 (i) 人工/owner-doc 门控待确认——本计划触及受保护资产/业财过账行为（ValueAdjustment approve→VALUE_ADJUSTMENT 减值/增值凭证、Disposal approve→DISPOSAL 清理凭证 + Asset 终态 SCRAPPED/SOLD、Capitalization approve→CAPITALIZATION 入账凭证 + Asset DRAFT→IN_SERVICE + 库存转固 stock move；reverseApprove posted=true 窗口红冲上述副作用，posted=false 不对称窗口悬挂经 DeferredPostingSweepJob 兜底）。M4 plan-first 门控成立；该人工裁定非起草者/审查者可自主解除（project-context.md 会计/财务保护域硬停止）。计划格式/完备性/范围/结束证据经 1 轮独立草案审查 acceptable-as-is（零 BLOCKER/MAJOR），保持 `draft` 直至门控确认（同 2026-08-14-0930-1 制造 M4 / 2026-08-13-0810-x / 1146-x batch-consistent hold）。
...
- **ErpAstDisposal**（M4.44 docStatus + M4.45 approveStatus，双轴，facade + 5 per-mutation Processor——**已实仓核实，直接范本**）：
  ...
  - **领域错误码**：`ERR_DISPOSAL_ILLEGAL_STATUS_TRANSITION`（:121-124）+ `ERR_DISPOSAL_ILLEGAL_DOC_TRANSITION`（:125-128）+ `ERR_DISPOSAL_ASSET_NOT_DISPOSABLE`/`ALREADY_DISPOSED`（Asset 来源态动态守卫）。
