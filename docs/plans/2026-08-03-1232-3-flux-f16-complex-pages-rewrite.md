...
- [ ] Decision: links 派生规则——`ErpApsOperationOrder` 无依赖字段，仅 sequence：方案 A 同 workOrder 按 sequence 相邻连边 / 方案 B links 为空（依赖连线显示降级）/ 方案 C 预留（ORM 扩展 ask-first 禁止）。记录理由
      - Skill: `nop-frontend-dev`
- [ ] 数据适配：`ErpApsOperationOrder__findGanttData` 聚合查询返回 `{ tasks:[{id,text,start,end,type,progress,children}], links:[按裁决] }`（task 字段映射：text=工序名/type=task/start,end=plannedStartDateT/plannedEndDateT，对齐 `scheduling.md` §8 契约字段）
      - Skill: `nop-backend-dev`
- [ ] Decision: 拖拽持久化契约——host 与方法名裁决（新增 `ErpApsOperationOrder__updateSchedule` vs 复用 `IEtpApsGanttService` spec 的 dragUpdateOperation 单参），事件 payload（onTaskDragEnd）字段映射；**校验语义**（§8.2 拖拽「需后端校验产能」——新端点是否含校验）
      - Skill: `nop-frontend-dev`
- [ ] 甘特页重写：`{ type:"gantt", tasks, links, defaultZoom:"week", draggable:true, editable:true, linkable:true, onTaskDragEnd → 按裁决端点 }`；只读对比场景 `draggable:false` 配置开关
      - Skill: `nop-frontend-dev`
- [ ] Proof: 甘特 E2E（flux 引擎）——渲染断言 + 拖拽事件触发 mutation 断言（隔离测试数据：拖拽写 plannedStartDateT/plannedEndDateT 无版本保护）
      - Skill: `nop-testing`

Exit Criteria:

- [ ] 甘特以 flux gantt 渲染（非只读 echarts），拖拽/缩放可用，依赖连线按 links 裁决（连边 or 空）
- [ ] 拖拽持久化 E2E 证据通过
- [ ] links 派生 + 拖拽契约 + 校验语义 Decision 已记录
...
- [ ] 2 个向导以 flux wizard 渲染（非手写 page.yaml 步骤指示器），步骤校验与聚合提交行为等价
- [ ] 向导 E2E 证据通过（含 12 月年度结转分支 visible 断言）
- [ ] 响应式 gating Decision 已记录（E2E 实测结论入档）
...
- [ ] 三单匹配：`crud` ×4（差异预警 + 三表并列，与实况一致）+ `mapping` 差异色 + 筛选联动（data-source dependsOn/sendOn）
      - Skill: `nop-frontend-dev`
- [ ] BOM 树：flux `tree`（data 绑定）——**嵌套重建机制裁决**：方案 A 后端聚合返回嵌套（findBomTree 查询）/ 方案 B data-source formula 客户端转换（explode 扁平 + level → 嵌套）。记录理由
      - Skill: `nop-frontend-dev`
- [ ] 薪酬审批：`crud` + `input-table`（可编辑批改）+ data-source 聚合；批量批准复用既有 mutation
      - Skill: `nop-frontend-dev`
- [ ] 发运追踪：`timeline`（shipment + logs 双查询）+ data-source ×2（dependsOn 级联）
      - Skill: `nop-frontend-dev`
- [ ] 净需求报表：`data-source`（分组聚合）+ `loop` + `table`（嵌套分组）
      - Skill: `nop-frontend-dev`
- [ ] 版本对比：`diff-view`（oldContent/newContent + viewType split/unified）——元数据对比表保留，content 双栏经 diff-view（字段级 diff 数据模型不支持，维持降级并注明）
      - Skill: `nop-frontend-dev`
- [ ] ASN 流程：`steps`（items status 驱动，4 值字典 RECEIVED/MATCHED/RECEIVED_TO_STOCK/CANCELLED）
      - Skill: `nop-frontend-dev`
...
- [ ] 8 页以 flux 原生控件渲染（无 AMIS 专属降级残留，除字段级 diff 数据模型限制注明外）
- [ ] 8 页 E2E 证据通过
...
- [ ] F16 全部页面 flux 引擎 E2E 回归通过（与 P1 Phase 2 聚合）
      - Skill: none
- [ ] Follow-up: `page-structure-patterns.md` §5/§8 的 flux 更新交付 P5 输入
      - Skill: none

Exit Criteria:

- [ ] F16 全部重写页 flux E2E 全绿
- [ ] 文档更新需求清单交付 P5
...
- [ ] 范围内行为完成（甘特拖拽 + 2 向导 + 8 B 族页全部 flux 原生）
- [ ] 相关文档对齐（范式文档 flux 更新交付 P5）
- [ ] 已运行验证（`E2E_ENGINE=flux npx playwright test` F16 相关 + 新增聚合查询单测）
