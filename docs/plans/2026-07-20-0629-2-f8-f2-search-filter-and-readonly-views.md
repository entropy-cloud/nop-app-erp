...
- [ ] `Decision`: 决策 8 核心列表页 query + asideFilter 双筛选面字段集（每页 5-10 字段，覆盖业务筛选场景）。基于各域 `ui-patterns.md` + 业务语义：
  ...
  - Skill: `nop-frontend-dev`
- [ ] `Decision`: 决策只读实体 dialog 模式（基于实时仓库抽样）：
  ...
  - Skill: `nop-frontend-dev`
- [ ] `Decision`: 决策只读实体清单（roadmap F2 提到 13+，本计划聚焦 6 实体）：
  ...
  - Skill: `nop-frontend-dev`
- [ ] `Add`: 在 `docs/design/query-filter-patterns.md` 新建文档，固化 query + asideFilter 双筛选面字段集表 + dialog 范式 + 只读实体清单。
  - Skill: `none`

Exit Criteria:

- [ ] 8 列表页 query + asideFilter 双筛选面字段集表格化记录
- [ ] 6 只读实体清单 + dialog 模式决策记录
- [ ] `docs/design/query-filter-patterns.md` 文件落地（≥150 行）
...
- [ ] `Add`: **ErpInvStockLedger** asideFilter + query + dialog 核实：materialId + warehouseId + businessType + dateRange + batchNo 筛选；row-view dialog 显示流水字段（time/material/direction/qty/after-change balance/unitPrice/source）。
  - Skill: `nop-frontend-dev`
- [ ] `Fix | Add`: **ErpInvStockBalance** query 扩展 + dialog 核实：扩展现有 4 字段（`view.xml:44-52`）→ 增 locationId + includeZero 勾选框；row-view dialog 显示余额字段。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpInvBatch** asideFilter + query + dialog 核实：batchNo + materialId + warehouseId + status 筛选；dialog 显示批次台账字段。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpInvSerialNumber** asideFilter + query + dialog 核实：serialNo 全局搜索 + materialId + status；dialog 显示序列号台账。
  - Skill: `nop-frontend-dev`
- [ ] `Fix`: 核实 4 实体 listActions/rowActions 无 add/update/delete 按钮（F1 已落地，本计划抽样验证）；核实 edit/add form 已 abstract 切断继承。
  - Skill: `nop-frontend-dev`
- [ ] `Proof`: 启动 app，逐一打开 4 实体列表页 → asideFilter + query 字段完整 → 多维筛选生效 → row-view 弹 dialog 显示完整字段 → 无 add/edit 入口。证据记录到 plan。
  - Skill: `nop-frontend-dev`

Exit Criteria:

- [ ] 4 inventory 只读实体 asideFilter + query 双面字段集完整（含 filterOp）
- [ ] 4 实体 row-view dialog 显示字段（复用既有 codegen dialog 模式）
- [ ] 4 实体无 add/update/delete 入口（listActions + rowActions + form abstract 切断 _gen 继承）
...
- [ ] `Add`: **ErpFinGlBalance** 只读视图 + asideFilter/query：subjectId + periodId + currencyId + includeUnposted 勾选；dialog 显示余额字段（subject/opening/debit/credit/closing）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpFinTrialBalance** 只读视图 + asideFilter/query：periodId + subjectId + currencyId + includeUnposted；dialog 显示试算平衡表字段。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpFinVoucher** asideFilter/query 扩展：voucherWord + voucherNo + dateRange + subjectId + businessType + docStatus（保持 edit/add form 不变）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpFinArApItem** asideFilter/query 扩展：partnerId + direction + itemType + status + dateRange（保持 edit/add form 不变）。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpPurOrder** asideFilter/query 扩展：code + supplierId + materialId + dateRange + docStatus + approveStatus。
  - Skill: `nop-frontend-dev`
- [ ] `Add`: **ErpSalOrder** asideFilter/query 扩展：code + customerId + materialId + dateRange + docStatus + approveStatus。
  - Skill: `nop-frontend-dev`
- [ ] `Proof`: 启动 app，逐一打开 6 列表页 → asideFilter + query 字段完整 → 多维筛选生效。证据记录到 plan。
  - Skill: `nop-frontend-dev`

Exit Criteria:

- [ ] ErpFinGlBalance + ErpFinTrialBalance 只读视图 + dialog 落地
- [ ] ErpFinVoucher + ErpFinArApItem + ErpPurOrder + ErpSalOrder asideFilter/query 字段扩展
- [ ] 本地化验证：view.xml 经 `mvn clean install -DskipTests` + `ErpAllWebPagesCollectTest` 全绿
...
- [ ] `Add`: 新建 `tests/e2e/visual/readonly-views.visual.spec.ts`，覆盖 6 只读实体（4 inventory + 2 finance）：断言 listActions/rowActions 无 add/update/delete 按钮 + edit/add form abstract 切断继承 + row-view 弹 dialog + dialog 字段集 token。
  - Skill: `nop-testing`
- [ ] `Add`: 新建 `tests/e2e/visual/list-query-filter.visual.spec.ts`，覆盖 8 列表页 asideFilter + query 双面：断言筛选面字段数 ≥ 设计值 + 关键字段 token（如 supplierId / customerId / subjectId）+ 多维筛选生效（搜索返回空集 / 非空集）。
  - Skill: `nop-testing`
- [ ] `Proof`: 运行 `npx playwright test tests/e2e/visual/readonly-views.visual.spec.ts tests/e2e/visual/list-query-filter.visual.spec.ts` 全绿。
  - Skill: `nop-testing`
- [ ] `Add`: 更新 `docs/design/query-filter-patterns.md`：补实施证据 + 反模式自检。
  - Skill: `none`
- [ ] `Add`: 标记 F3 P0 Deferred「query asideFilter」RELEASED，更新 `docs/plans/2026-07-19-1818-2-f3-core-line-and-remaining-main-form-layout.md` 对应段。
  - Skill: `none`
- [ ] `Add`: 更新 `docs/backlog/frontend-ui-roadmap.md` F2 + F8 状态（部分 done，长尾实体 defer 到后续）。
  - Skill: `none`
...
- [ ] readonly-views visual spec 落地（≥4 实体）
- [ ] list-query-filter visual spec 落地（≥8 列表页）
- [ ] playwright 测试全绿
- [ ] 文档对齐：query-filter-patterns.md + F3 P0 Deferred RELEASED + roadmap 状态 + 日志
...
- [ ] 范围内行为完成（inventory 4 只读实体 + finance GlBalance + TrialBalance 只读 + 主域列表 asideFilter/query 扩展）
- [ ] 相关文档对齐（query-filter-patterns.md + F3 P0 Deferred RELEASED + roadmap 状态 + 日志）
- [ ] 已运行验证（`mvn clean install -DskipTests` 154 模块 + `npx playwright test` 新增 visual spec 全绿 + `ErpAllWebPagesCollectTest` PAGE_ERROR_COUNT=0）
