...
- [x] 端到端过账测试 + 内部调拨不过账测试 + 过账失败不阻塞终态测试存在且 `mvn test -pl module-inventory/erp-inv-service -am` 全绿
- [x] 当日日志已记 StockMove BizModel 落地与验证状态
...
- [x] 范围内行为完成：generateMove 契约 + 状态机 + 流水/余额 + 可用量/负库存 + 存货过账接入全部落地，行为测试通过
- [x] 相关文档对齐：`core-business-roadmap.md` P1 工作项 1.3 标注进展；当日日志已记
- [x] 已运行验证：`mvn test -pl module-inventory/erp-inv-service -am` 全绿；根 `mvn test -fae` = BUILD SUCCESS（无回归）
- [x] 无范围内项目降级为 deferred/follow-up（跨法人调拨/记录级预留/盘点/批次序列号全生命周期/两步调拨在途/FIFO成本 均为计划内 Non-Goal，非范围内降级）
- [x] 独立草案审查已完成并记录
...
- Auditor / Agent: 独立结束审计子代理（新会话，无执行者上下文），对实时仓库逐项核实。
- 独立审计核实结论（live repo 核对，非采信自报）：
  - **Phase 1（契约+状态机+幂等）**：`IErpInvStockMoveBiz` 含 `generateMove/confirm/complete/cancel/reverse` 5 方法；`ErpInvStockMoveBizModel`（380 行）`@SingleSession @Transactional`，`doConfirm` 校验前置 DRAFT、`doComplete` 校验前置 CONFIRMED、`cancel` 限 DRAFT/CONFIRMED、`reverse` 校验 DONE 后生成反向新单（`inverseMoveType` + 走 `generateMove`，非反审核）；业务联动（`isBusinessLinked`）自动推进 DRAFT→CONFIRMED→DONE，独立创建停 CONFIRMED；幂等键 `(relatedBillType,relatedBillCode)` 经 `findExisting` 反查命中即返回；非法迁移统一抛 `NopException(ERR_ILLEGAL_STATUS_TRANSITION)`。`TestErpInvStockMoveBizModel` 5 方法名逐一命中。状态机自检结论文件内已记（终态无出边、有向无环、冲销无循环）。
  - **Phase 2（流水+余额+可用量+负库存）**：`StockMoveBookkeeper`（208 行）`bookCompletion` 按行写 `ErpInvStockLedger`（`balanceQuantity`/`balanceTotalCost` 结存快照、quantity 按方向带符号、`unitCost` 快照）+ upsert `ErpInvStockBalance`（移动加权平均：入库 `avgCost=(旧totalCost+入cost)/(旧qty+qty)`、出库 `unitCost=当前avgCost`、内部调拨扣源加目的）；`validateAvailable` 出库/调拨校验 `available≥required`（`BigDecimal compareTo`），不足抛 `ERR_AVAILABLE_INSUFFICIENT`；`isNegativeStockAllowed` 经 `AppConfig.var(CONFIG_ALLOW_NEGATIVE_STOCK, FALSE)` 跳过。`TestErpInvStockMoveBookkeeping` 5 方法名逐一命中。
  - **Phase 3（存货过账端到端）**：`erp-inv-service/pom.xml` 含 compile 依赖 `app-erp-finance-service`；`InvPostingDispatcher.dispatchIfApplicable` DONE 后按 moveType 派生（入库→PURCHASE_INPUT、出库→SALES_OUTPUT、内部调拨 `return null` 跳过），经 `InvPostingExecutor`（**`@Transactional(REQUIRES_NEW)`** 独立新事务）调 `ErpFinPostingService.post`，成功置 `posted=true`、失败 try/catch 吞异常记 WARN/ERROR 日志保持 DONE+`posted=false`（不阻塞终态，符合 cross-domain 合约）；`InvAcctDocProvider implements IErpFinAcctDocProvider`（非默认，`EnumSet.of(PURCHASE_INPUT,SALES_OUTPUT)`，产 2 条 `VoucherFact`：入库借1401/贷2202、出库借6401/贷1401）。4 个组件均注册于 `_vfs/erp/inv/beans/app-service.beans.xml`。`TestErpInvPosting` 3 方法名逐一命中。
  - **反空心**：5 个 BizModel 方法 + bookkeeper/dispatcher 均为实质实现（无 `{}`/`return null` 占位）；过账失败路径有日志非静默吞；`InvAcctDocProvider` 经 beans.xml 注册并由引擎 Registry 聚合，运行时可达。
  - **范围保护**：未改任何 `model/*.orm.xml`/`.api.xml`（保护区域），inventory 侧纯新增 Java+测试+beans.xml；finance 侧跨计划 Fix（`buildVoucherCode` 用 `type.getCode()`）已记于日志。
  - **文档同步**：`docs/logs/2026/07-01.md` 计划 0811-2 条目已记（时间倒序、含验证状态）；`docs/backlog/core-business-roadmap.md` 工作项 1.3 标 ✅ `done`。
  - **Deferred 诚实性**：6 项 Deferred（跨法人调拨内部交易凭证/记录级预留/盘点/批次序列号全生命周期/两步调拨在途/FIFO成本）均为计划内 Non-Goal，非范围内降级；无范围内缺陷隐藏。
  - **五点一致性**：Plan Status `completed` / 各 Phase Status `completed` / 各 Exit Criteria 全 `[x]` / Closure Gates 全 `[x]` / 日志一致。
- Executor 交付证据（执行者自报，非结束审计）：
  - Phase 1-3 全部 `[x]` 且各 `Status: completed`（见各 Phase 节）。
  - 范围内行为落地：`IErpInvStockMoveBiz.generateMove/confirm/complete/cancel/reverse`（`StockMoveRequest`/`StockMoveLineRequest` DTO）；`ErpInvStockMoveBizModel` 状态机 DRAFT→CONFIRMED→DONE/CANCELLED + 幂等 `(relatedBillType,relatedBillCode)` + 冲销反向新单；`StockMoveBookkeeper` DONE 写不可变 `ErpInvStockLedger`（结存快照、移动加权平均 `avgCost`、入库增/出库扣/内部调拨扣源加目的）+ 更新 `ErpInvStockBalance`；可用量校验 `available≥required`（`erp-inv.allow-negative-stock` 跳过）；`InvPostingDispatcher`+`InvPostingExecutor`（`@Transactional(REQUIRES_NEW)`）DONE→`PostingEvent`→`ErpFinPostingService.post`，成功 `posted=true`、失败吞异常保持 DONE+`posted=false`；`InvAcctDocProvider`（非默认，PURCHASE_INPUT/SALES_OUTPUT，借库存商品/贷暂估应付 or 借主营业务成本/贷库存商品）。
  - 文档对齐：`docs/backlog/core-business-roadmap.md` 工作项 1.3 已 ❌→✅ `done`；`docs/logs/2026/07-01.md` 已记。
  - 验证：`mvn test -pl module-inventory/erp-inv-service -am` = BUILD SUCCESS（StockMoveBizModel 6 + Bookkeeping 5 + Posting 3 + CRUD 冒烟 5 = 19 测试，0 失败/0 错误）；根 `mvn test -fae` = BUILD SUCCESS（0 `FAIL!`，含 finance 13 + 全 18 域 CRUD 冒烟，无回归）。
  - 范围保护：未改任何 `model/*.orm.xml`/`.api.xml`。跨计划 Fix：finance `ErpFinPostingService.buildVoucherCode` 用 `type.getCode()` 替代 `type.name()`（修复 `CODE VARCHAR(50)` 超长，blocking bug，已重装 finance-service，其既有测试无回归）。
  - 结束审计门控保留 `[ ]`：按 AGENTS.md Rule 12，结束审计须由独立子代理（新会话）执行，执行者未自我审计。
