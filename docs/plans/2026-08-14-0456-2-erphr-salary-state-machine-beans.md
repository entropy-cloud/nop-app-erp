...
> Plan Status: draft
> Review Hold: §11.2 M4 (i) plan-first 人工/owner-doc 门控未确认（markPaid 触发 SALARY_PAYMENT(280) 发放凭证；approve 触发 SALARY(270)/SOCIAL_INSURANCE_ER(290)/HOUSING_FUND_ER(300) 计提凭证——后三者当前 Deferred/dead-code 但 config 翻转后触发受保护行为）。project-context.md 会计/财务硬停止。门控非起草者/审查者可自主解除——同 `0930-1` / `0810-x` / `1146-x` / `1931-x` / `0456-1` batch-consistent hold。
