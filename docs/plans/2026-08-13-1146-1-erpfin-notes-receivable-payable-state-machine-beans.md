...
- **生产 Bean 注册**：`module-finance/erp-fin-service/src/main/resources/_vfs/erp/fin/beans/app-service.beans.xml`（380 行）已显式注册 Processor（FQN id）：facade helper（`:151-154`）+ 11 per-mutation Processor（`:308-329`，注释 `<!-- R6.1 ... NotesPayable 4 + NotesReceivable 7 -->`）+ `NotesPostingDispatcher`（`:133-134`）。**`statemachine/` 包已存在**：M4.1（Voucher，`completed`，人工门控已于 2026-08-13 解除）已落地 `ErpFinVoucherDocumentStateMachine`，注册于 `app-service.beans.xml:379-380`。本计划向既有包 `module-finance/erp-fin-service/src/main/java/app/erp/fin/service/statemachine/` 新增两 Notes Bean，并在 beans.xml 末尾追加注册（两 Notes Bean 文件本身仍为新建）。注：M4.1 人工门控确认范围仅覆盖凭证过账，**不含本计划票据过账/授信维度**（见治理声明 §11.2 M4 (i) + Draft Review Record 迭代 4 门控独立性裁定），本计划门控独立、未由 M4.1 解除而解除。
