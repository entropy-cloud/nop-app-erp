...
> 按 M0.2 清单 `docs/analysis/2026-08-12-entity-state-axis-inventory.md §1.2 + §3.5 inventory`（444 行段）+ 实仓核实。inv-service statemachine 目录现含姊妹计划落地的 `ErpInvStockMoveStateMachine`/`ErpInvStockTakeStateMachine`（StockMove/StockTake M4.29/M4.30 已 done）；本计划 4 目标实体（TransferOrder/OwnershipTransfer/CostAdjust/LandedCost）仍 greenfield，且可参考上述姊妹 Bean 形状。
...
- **生产 Bean 注册**：`erp-inv-service/.../beans/app-service.beans.xml` 已注册各实体 Processor + 服务 Bean（FQN id）。statemachine 目录现含姊妹 StockMove/StockTake Bean；本计划 4 目标 Bean 仍 greenfield，追加于 beans.xml 末尾（参考姊妹 Bean 注册写法）。
...
- **M4 plan-first 人工/owner-doc 门控状态：confirmed（2026-08-14 人工确认解除）**（§11.2 M4 (i) + 会计/库存强一致保护区）。人工/owner 于 2026-08-14 确认「以行为保持方式迁移此 4 轴、过账/余额/成本层/双轴联动/红冲路径完整保留」可接受，门控解除。据此将 Plan Status 由 `draft` 转 `active`（对齐 M3.10/M4.29-30/M4.1/M4.2 plan-first 先例）。
...
- Mission-driver review iteration 6（2026-08-13，fresh 独立复核 + live code 实证 format/completeness/scope/closure checklist）：独立复验全部四项并经实仓核实——(1) **格式合规**：front matter 完整（Plan Status=draft + Review Hold §11.2 M4(i) + Last Reviewed + Source + Related + Mission + Work Item + Audit=required）；10 必需段齐全（Current Baseline / Goals / Non-Goals / Task Route / Infrastructure And Config Prereqs / Execution Plan / Draft Review Record / Closure Gates / Deferred But Adjudicated / Closure）；3 阶段 Phase 各含 Status=planned / Targets / Skill / Item Types / Prereqs / checkbox+Skill / Exit Criteria，结构合法；Item Types 全部落入合法集合 {Fix,Add,Decision,Proof,Follow-up}（P1=Add|Decision|Proof、P2=Fix|Proof、P3=Proof|Decision|Add）。(2) **完备性达标**：各阶段 Exit Criteria 可测互不重叠（P1=4 Bean 无状态+注册 FQN id+R5 合规+层1 矩阵+mvn compile；P2=既有集成回归全绿含过账失败悬挂断言+grep 证内联矩阵已清除；P3=四方对照无未裁决漂移+owner doc 4 章节与 dict/Bean/代码一致）；执行项完整覆盖全部 5 条 Goals，无 Goal 缺失执行项。(3) **范围清晰**：规则 14 bundling 已显式裁定（同一 owner doc `docs/design/inventory/state-machine.md` + 同一 inventory docStatus 结果表面，4 实体矩阵不同故各为独立 Bean+独立矩阵测试）；Non-Goals 明确穷尽，无 "and also..." scope creep / 松弛词。(4) **结束证据就绪**：Closure Gates 9 项含具体验证命令（mvn test -pl module-inventory/erp-inv-service + mvn clean install -DskipTests + compliance-checker.sh）+ M4 门控确认门控 + 独立结束审计门控；Deferred 5 项全带 Classification + Successor 触发条件；TransferOrder 错误码缺陷正确分类为 `confirmed live defect moved to explicit successor ownership`（规则 13 合规）。**live code 独立实证**：statemachine 目录仅含 StockMove/StockTake 姊妹 Bean，4 目标 Bean greenfield（TransferOrder/OwnershipTransfer/CostAdjust/LandedCost 均无）；`ErpInvTransferOrderConfirmProcessor.java:38-40` 抛 `ERR_INV_STOCK_TAKE_ILLEGAL_TRANSITION`+`ARG_TAKE_ID`（StockTake 码，copy-paste 缺陷 CONFIRMED），intercompany hook `:44-55` config-gated+失败吞掉 CONFIRMED；`docs/design/inventory/state-machine.md`（194 行）仅覆盖 StockMove/StockTake/Picking，4 实体无章节（owner-doc 缺口 CONFIRMED → Phase 3 补 4 章节义务正确）。**结论**：format/completeness/scope/closure 四维无就地可修的 Blocker/Major。唯一阻塞 = §11.2 M4 (i) 人工/owner-doc 门控（OwnershipTransfer done / CostAdjust applyCostAdjust / LandedCost approve 触发存货成本过账事件 + owner-doc 缺章节，属 project-context.md §AI 阻塞条件硬停止域），为审查者不可自主解除的上游人工裁定。Review Hold 已就位（line 4），Plan Status 保持 `draft`。本审查仅 corroboration，不改 hold 状态。

- Mission-driver review iteration 7（2026-08-13，fresh 独立复核 + live code 实证 format/completeness/scope/closure checklist）：独立复验全部四项并经实仓核实——(1) **格式合规**：front matter 完整（Plan Status=draft + Review Hold §11.2 M4(i) + Last Reviewed + Source + Related + Mission + Work Item + Audit=required）；10 必需段齐全；3 阶段 Phase 各含 Status=planned / Targets / Skill / Item Types / Prereqs / checkbox+Skill / Exit Criteria，结构合法；Item Types 全合法（P1=Add|Decision|Proof、P2=Fix|Proof、P3=Proof|Decision|Add）。(2) **完备性达标**：Exit Criteria 可测互不重叠；执行项完整覆盖全部 5 条 Goals，无 Goal 缺失执行项。(3) **范围清晰**：规则 14 bundling 已显式裁定（同一 owner doc `docs/design/inventory/state-machine.md` + 同一 inventory docStatus 结果表面，4 实体矩阵不同故各为独立 Bean+独立矩阵测试）；Non-Goals 明确穷尽，无 scope creep / 松弛词。(4) **结束证据就绪**：Closure Gates 9 项含具体验证命令 + M4 门控确认门控 + 独立结束审计门控；Deferred 5 项全带 Classification + Successor 触发条件；TransferOrder 错误码缺陷正确分类为 `confirmed live defect moved to explicit successor ownership`（规则 13 合规）。**live code 独立实证**：statemachine 目录仅含 StockMove/StockTake 姊妹 Bean（4 目标 Bean greenfield CONFIRMED）；`ErpInvTransferOrderConfirmProcessor.java:38-39` 抛 `ERR_INV_STOCK_TAKE_ILLEGAL_TRANSITION`+`ARG_TAKE_ID`（copy-paste 缺陷 CONFIRMED）；`docs/design/inventory/state-machine.md` 仅覆盖 StockMove(§1-§10)+StockTake(盘点单)+Picking(Deferred)，4 实体无章节（owner-doc 缺口 CONFIRMED → Phase 3 补 4 章节义务正确）。**结论**：format/completeness/scope/closure 四维无就地可修的 Blocker/Major。唯一阻塞 = §11.2 M4 (i) 人工/owner-doc 门控（OwnershipTransfer done / CostAdjust applyCostAdjust / LandedCost approve 触发存货成本过账事件，属 project-context.md §AI 阻塞条件「触及会计/财务保护区域且 owner doc 缺失对应章节」硬停止域），为审查者不可自主解除的上游人工裁定。Review Hold 已就位（line 4），Plan Status 保持 `draft`。本审查仅 corroboration，不改 hold 状态。

- Mission-driver review iteration 5（2026-08-13，fresh 复核 + live code 实证 format/completeness/scope/closure checklist）：独立复验全部四项并经实仓核实——(1) **格式合规**：front matter 完整（Plan Status=draft + Review Hold §11.2 M4(i) + Last Reviewed + Source + Related + Mission + Work Item + Audit=required）；10 必需段齐全（Current Baseline / Goals / Non-Goals / Task Route / Infrastructure And Config Prereqs / Execution Plan / Draft Review Record / Closure Gates / Deferred But Adjudicated / Closure）；3 阶段 Phase 各含 Status=planned / Targets / Skill / Item Types / Prereqs / checkbox+Skill / Exit Criteria，结构合法；Item Types 全部落入合法集合 {Fix,Add,Decision,Proof,Follow-up}（P1=Add|Decision|Proof、P2=Fix|Proof、P3=Proof|Decision|Add）。(2) **完备性达标**：Exit Criteria 可测互不重叠（P1=4 Bean 无状态+注册 FQN id+R5 合规+层1 矩阵+mvn compile；P2=既有集成回归全绿含过账失败悬挂断言+grep 证内联矩阵已清除；P3=四方对照无未裁决漂移+owner doc 4 章节与 dict/Bean/代码一致）；执行项完整覆盖全部 5 条 Goals，无 Goal 缺失执行项。(3) **范围清晰**：规则 14 bundling 已显式裁定（同一 owner doc `docs/design/inventory/state-machine.md` + 同一 inventory docStatus 结果表面，4 实体矩阵不同故各为独立 Bean+独立矩阵测试）；Non-Goals 明确穷尽（posted/approveStatus 不迁移、过账编排/余额/成本层不改、双轴联动写时序不改、reverse 失败告警保留、ORM/字典/API 不改、不静默修正 TransferOrder 缺陷、M4 门控不自主跳过、Delta 证明归 M5.3），无 "and also..." scope creep / 松弛词（optional/maybe/nice-to-have）。(4) **结束证据就绪**：Closure Gates 9 项含具体验证命令（mvn test -pl module-inventory/erp-inv-service + mvn clean install -DskipTests + compliance-checker.sh）+ M4 门控确认门控 + 独立结束审计门控；Deferred 5 项全带 Classification + Successor 触发条件；TransferOrder 错误码缺陷正确分类为 `confirmed live defect moved to explicit successor ownership`（规则 13 合规——已确认+successor，非静默降级）。**live code 实证**：TransferOrder `validateDraft:38-40` 抛 StockTake 的 `ERR_INV_STOCK_TAKE_ILLEGAL_TRANSITION`（copy-paste 缺陷 CONFIRMED）+ `dispatchIntercompanyPosting`→`IErpFinIntercompanyTransferBiz` intercompany hook；OwnershipTransfer `dispatchIfApplicable`+`reclassifyBalances`+`ERR_OWNERSHIP_TRANSFER_ILLEGAL_STATUS`/`ERR_OWNERSHIP_TRACKING_DISABLED` 过账路径 CONFIRMED；owner doc `docs/design/inventory/state-machine.md` 无 4 实体章节（owner-doc 缺口 CONFIRMED）。**MINOR 修正（已就地修复）**：Current Baseline 原述「无 statemachine 目录（greenfield）」已陈旧——姊妹 StockMove/StockTake Bean 已落地（`ErpInvStockMoveStateMachine`/`ErpInvStockTakeStateMachine` 已存在），已修正为反映现状（4 目标实体仍 greenfield，可参考姊妹 Bean）；不影响计划正确性。**结论**：format/completeness/scope/closure 四维无就地可修的 Blocker/Major。唯一阻塞 = §11.2 M4 (i) 人工/owner-doc 门控（3 实体触发存货成本过账事件 = project-context.md §AI 阻塞条件「触及会计/财务保护区域且 owner doc 缺失对应章节」硬停止域），属审查者不可自主解除的上游人工裁定。Review Hold 已就位（line 4），Plan Status 保持 `draft`，与 4 个姊妹 M4 plan-first 计划（StockMove/StockTake、Projects timesheet、Purchase、Sales）一致。本审查 corroboration 不改变 hold 状态。

- Mission-driver review iteration 8（2026-08-13，fresh 独立复核 + live code 实证 format/completeness/scope/closure checklist）：独立复验全部四项并经实仓核实——(1) **格式合规**：front matter 完整（Plan Status=draft + Review Hold §11.2 M4(i) + Last Reviewed=2026-08-13 + Source + Related + Mission + Work Item=M4.31+M4.32+M4.33+M4.34 + Audit=required）；10 必需段齐全（Current Baseline / Goals / Non-Goals / Task Route / Infrastructure And Config Prereqs / Execution Plan / Draft Review Record / Closure Gates / Deferred But Adjudicated / Closure）；3 阶段 Phase 各含 Status=planned / Targets / Skill / Item Types / Prereqs / checkbox+Skill / Exit Criteria，结构合法；Item Types 全部落入合法集合 {Fix,Add,Decision,Proof,Follow-up}（P1=Add|Decision|Proof、P2=Fix|Proof、P3=Proof|Decision|Add），无非法类型。(2) **完备性达标**：各阶段 Exit Criteria 可测互不重叠（P1=4 Bean 无状态+注册 FQN id+R5 合规+层1 矩阵+mvn compile；P2=既有集成回归全绿含过账失败悬挂断言+grep 证内联矩阵已清除；P3=四方对照无未裁决漂移+owner doc 4 章节与 dict/Bean/代码一致）；执行项完整覆盖全部 5 条 Goals，无 Goal 缺失执行项。(3) **范围清晰**：规则 14 bundling 已显式裁定（同一 owner doc `docs/design/inventory/state-machine.md` + 同一 inventory docStatus 结果表面，4 实体矩阵不同故各为独立 Bean+独立矩阵测试，合为四阶段切片）；Non-Goals 明确穷尽（posted/approveStatus 不迁移、过账编排/余额/成本层不改、双轴联动写时序不改、reverse 失败告警保留、ORM/字典/API 不改、不静默修正 TransferOrder 缺陷、M4 门控不自主跳过、Delta 证明归 M5.3），无 "and also..." scope creep / 松弛词（optional/maybe/nice-to-have）。(4) **结束证据就绪**：Closure Gates 9 项含具体验证命令（mvn test -pl module-inventory/erp-inv-service + mvn clean install -DskipTests + compliance-checker.sh）+ M4 门控确认门控 + 独立结束审计门控；Deferred 5 项全带 Classification + Successor 触发条件；TransferOrder 错误码缺陷正确分类为 `confirmed live defect moved to explicit successor ownership`（规则 13 合规——已确认+successor，非静默降级为 follow-up，且位于 Deferred 而非 Follow-up 段）。**live code 独立实证**：statemachine 目录仅含 StockMove/StockTake 姊妹 Bean，4 目标 Bean greenfield CONFIRMED；`ErpInvTransferOrderConfirmProcessor.java:38` 抛 `ERR_INV_STOCK_TAKE_ILLEGAL_TRANSITION`（StockTake 码，copy-paste 缺陷 CONFIRMED）；`docs/design/inventory/state-machine.md` 仅含 StockMove(§1-§10)+StockTake(盘点单)+Picking(Deferred)，4 实体无章节（owner-doc 缺口 CONFIRMED → Phase 3 补 4 章节义务正确）。**结论**：format/completeness/scope/closure 四维无就地可修的 Blocker/Major。唯一阻塞 = §11.2 M4 (i) 人工/owner-doc 门控（OwnershipTransfer done / CostAdjust applyCostAdjust / LandedCost approve 触发存货成本过账事件，属 project-context.md §AI 阻塞条件「触及会计/财务保护区域且 owner doc 缺失对应章节」硬停止域），为审查者不可自主解除的上游人工裁定。Review Hold 已就位（line 4），Plan Status 保持 `draft`。本审查仅 corroboration，不改 hold 状态。
