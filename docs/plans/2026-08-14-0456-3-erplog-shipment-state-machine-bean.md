...
> Plan Status: draft
> Review Hold: §11.2 M4 (i) plan-first 人工/owner-doc 门控未确认（DELIVERED 触发 FREIGHT 运费过账 path-1 + config-gated 到岸成本自动创建 path-2）。project-context.md 会计/财务硬停止。门控非起草者/审查者可自主解除——同 `0930-1` / `0810-x` / `1146-x` / `1931-x` / `0456-1` / `0456-2` batch-consistent hold。
