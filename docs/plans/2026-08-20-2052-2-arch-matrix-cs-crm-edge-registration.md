...
- [ ] Fix: §2.4 表追加行（镜像 :127 cs→qa 行格式与克制注记，措辞以实仓机制为准）：`| cs-service → crm-dao | IErpCrmTeamBiz/IErpCrmTeamMemberBiz（工单创建自动分配候选池：cs team code → crm 同码团队成员 userId 池，只读） | RC-R1.65（P1-RC-054） | 单向叶依赖：crm 对 cs 零反向（ORM + Java 双向均无），DAG 无环；crm 查询失败经 try/catch 失败隔离（池空/解析失败 → 留 NEW + 客服主管升级通知，非 @Nullable 容错路径——节前言 :123 约定的既有偏离先例见 :137-138） |`——表格列序与文字对齐既有行风格，pom 注释为语义真相源
  - Skill: none
- [ ] Fix: plan `2026-08-17-2125-1` 的 `## Closure` 节追加补记行：matrix §2.4 cs→crm 边登记由本 plan 完成（来源 multi 审计 F2，注明原收尾清单漏项）
  - Skill: none
- [ ] Fix: roadmap `docs/backlog/requirement-compliance-roadmap.md:457` RC-R1.65 done 行末追加短语「matrix §2.4 cs→crm 边登记补齐（本 plan，2026-08-20）」——对齐 R1.68/76/77/78/81/85/86 兄弟行内显式「matrix §2.4 登记」的回写对称性
  - Skill: none

Exit Criteria:

- [ ] `grep -n "crm-dao" docs/architecture/data-dependency-matrix.md` ≥ 1 命中且位于 §2.4 表内；表格渲染完整（列数与表头一致）
- [ ] RC-R1.65 plan Closure 补记在盘；roadmap :457 行末登记短语在盘
...
- [ ] 范围内行为完成（矩阵行登记 + 收尾补记）
- [ ] 相关文档对齐（§2.4 与 pom/roadmap 声明一致）
- [ ] 一致性复核完成（grep 证据 + 表格完整性）
- [ ] `docs/logs/2026/08-20.md` 条目在盘（计划级结束步骤）
