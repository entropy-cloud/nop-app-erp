...
- [x] 按 `orm-model-audit-prompt.md` 9 维度审计 finance ORM（S 级整域，机械维度单次会话可完成），对照 `docs/design/finance/` + `domain-design-guidelines.md` 单据标准字段约定。重点关注：业财过账三件套（posted/postedAt/postedBy）、多币种四件套（currencyId/exchangeRate/amountSource/amountFunctional）、GL 映射与科目表、跨模块引用（机制 B notGenCode 外部实体）。
      - Skill: `orm-model-audit-prompt.md`
- [x] 按 9 维度审计 manufacturing ORM（S 级整域）。重点关注：工单/BOM/工艺路线/工作中心、质量集成跨域引用、生产成本回写字段。
      - Skill: `orm-model-audit-prompt.md`
- [x] 按 9 维度审计 hr ORM（S 级整域）。重点关注：员工与组织（双员工表 `erp_hr_employee` vs `erp_md_employee` 已登记 deferred successor）、考勤/工资/排班字段完整性。
      - Skill: `orm-model-audit-prompt.md`
- [x] 产出 S 级域 ORM 审计报告 `docs/audits/2026-07-27-1015-arm-ma1-s-tier-orm.md`（含裁决、各维度通过率、finding 按 P0/P1/P2 分级、残留风险），并更新 `arm-index.md`（报告清单 + P0/P1 汇总）。
      - Skill: none

Exit Criteria:

- [x] finance / mfg / hr 三域 ORM 审计报告产出，9 维度均有结论
- [x] 报告已登记 arm-index（解除 MA2 业务审计对 ORM 完整性的依赖的本地化检查）
...
- [x] 审计 purchase + sales ORM（A 级 2 域合并）。重点关注：双轴状态分离（docStatus + approveStatus）、价税分离字段、退货/退款单据完整性。
      - Skill: `orm-model-audit-prompt.md`
- [x] 审计 assets + inventory ORM（A 级 2 域合并）。重点关注：assets 折旧/处置/资本化字段、inventory 成本方法（7 种 costMethod）、批次/序列号追踪字段、库存移动类型。
      - Skill: `orm-model-audit-prompt.md`
- [x] 审计 crm + quality + projects ORM（A 级 3 域合并）。重点关注：crm 漏斗/预测/配额、quality 检验/NCR/CAPA/SPC、projects 工时/结算/成本归集。
      - Skill: `orm-model-audit-prompt.md`
- [x] 产出 A 级域 ORM 审计报告（按域簇组织），更新 arm-index。
      - Skill: none

Exit Criteria:

- [x] 7 A 级域 ORM 审计完成，报告产出并登记 arm-index
...
- [x] 审计 master-data ORM（B 级，DAG 根域单独）。重点关注：被全域引用的根实体（partner/material/subject/facility/org）字段完整性、`erp-md/*` 字典被全域复用的一致性。
      - Skill: `orm-model-audit-prompt.md`
- [x] 审计 cs + contract + b2b + maintenance + drp ORM（B 级 5 域合并）。重点关注：cs 工单/SLA/满意度、contract 版本/发票计划/返利、b2b EDI 文档、maintenance 设备/巡检/备件、drp 需求计划/安全库存。
      - Skill: `orm-model-audit-prompt.md`
- [x] 审计 aps + logistics + notify ORM（C 级 3 域合并）。重点关注：aps 排产/工序单、logistics 发运/承运商、notify 跨域通知派发子系统。
      - Skill: `orm-model-audit-prompt.md`
- [x] 产出 B+C 级域 ORM 审计报告，更新 arm-index。汇总全域 ORM 维度覆盖率至 scope matrix §2.1（`❓`→`✅/⚠️`）。
      - Skill: none

Exit Criteria:

- [x] 9 B+C 级域 ORM 审计完成，报告产出并登记 arm-index
- [x] scope matrix §2.1 "ORM 模型规范" 行全域标记完成
...
- [x] P0 finding 即时处理：每个 P0 当即就地修复（改 `model/*.orm.xml` 源 + `mvn clean install -DskipTests` + 该修复的独立审计）或异步注入 fix plan（`docs/plans/YYYY-MM-DD-HHmm-arm-fix-*.md`）。P0 永不进入 MR 批量修复。每个 P0 在报告中标注修复路径与状态。
      - Skill: none
      - **执行结论**：MA1 ORM 审计全域 0 P0 blocker，无即时通道触发。前序 D1–D5 系统性偏差修复成果保持。
- [x] P1 finding 汇总：全部 P1 登记至 `arm-index.md` §P1 发现汇总（Finding ID `P1-MA1-NNN`、报告、描述、目标 MR1、修复状态 todo），供 R1.0 展开机制转化为具体修复工作项行。
      - Skill: none
      - **执行结论**：51 项 P1 已全部登记 arm-index §P1 汇总（S 级 1 + A 级 42 + B+C 级 9 - maintenance 重复计 1）。三种根因：(a) propId 缺失 40 项（D3/D4 多币种补字段未重编号，codegen 增量再生自动补全）；(b) crm DECIMAL↔double 7 项（浮点精度损失，需手动改 stdDataType）；(c) drp 命名异常 4 实体（ErpInvDrp* 不符合 §19.1，需裁决重命名或登记例外）。
- [x] 确认 scope matrix §3.2 "daoFor Type 4 设计边界错误" 等进入 MA1 的残留风险项已在审计中被覆盖并给出结论（已确认/已修复/归 successor）。
      - Skill: none
      - **执行结论**：scope matrix §3.2 已更新——"daoFor Type 4 设计边界错误"标记为 **MA1 已覆盖**（ORM dim 7 跨模块引用一致性 0 blocker，daoFor Type 4 在 ORM 层无残留；下游调用层归 MA4 A4.5）。
...
- [x] 范围内行为完成（A1.1–A1.9 全 19 域 ORM 审计报告产出 + arm-index 更新 + scope matrix §2.1 标记完成）
- [x] 相关文档对齐（审计报告、arm-index、scope matrix 已反映审计结论）
- [x] 已运行验证：零 P0 即时修复 → 全量 `mvn clean install -DskipTests`（BUILD SUCCESS，154 模块）+ `mvn test`（BUILD SUCCESS，0 failures/0 errors/1 skipped）作回归基线确认（2026-07-27 11:52 落锚）
...
Status Note: 已完成。MA1 ORM 审计批 A1.1–A1.9 全 19 域（finance/mfg/hr + pur+sal+assets+inv+crm+qa+prj + md+cs+ct+b2b+mnt+drp+aps+log+notify）按 9 维度机械审计完成，全域 0 P0 blocker，51 项 P1 全部登记 arm-index §P1 汇总待 MR1。roadmap MA1 A1.1–A1.9 Status `todo` → `done`，scope matrix §2.1 "ORM 模型规范" 行全域覆盖（10 ✅ + 9 ⚠️ + 0 ❓）。audit-only 计划零生产代码变更，build/test 门控为回归基线确认（154 模块 BUILD SUCCESS + 0 failures/0 errors/1 skipped，2026-07-27 11:43/11:52 实测落锚）。

Closure Audit Evidence:

- Auditor / Agent: opencode 子代理（独立审计会话，自动扫描 + 人工核实三类高信号维度）
- Evidence:
  - 审计报告 3 份：`docs/audits/2026-07-27-1015-arm-ma1-{s,a,bc}-tier-orm.md`
  - arm-index 更新：`docs/audits/arm-index.md`（报告清单 3 行 + P1 汇总 7 ID）
  - scope matrix 更新：`docs/audits/audit-remediation-scope-and-dimension-matrix.md` §2.1（19 列全覆盖）+ §3.2（daoFor Type 4 标 MA1 已覆盖）
  - roadmap 更新：`docs/backlog/audit-remediation-roadmap.md` MA1 A1.1–A1.9 全 done
  - 自动化扫描输出：`/var/folders/lv/yfm8thx903d6bnjjz9c4m_mm0000gn/T/opencode/orm_audit_out/*.json`（19 域 + aggregate + tier summary）
  - build/test 回归基线：154 模块 BUILD SUCCESS + 0 failures/0 errors/1 skipped（2026-07-27 11:43/11:52 落锚）
  - 日志：`docs/logs/2026/07-27.md` 新增 MA1 ORM 审计条目

Follow-up:

- P1 finding 经 R1.0 展开机制进入 MR1（roadmap §R*.0 展开机制）
- 若 P0 即时修复注入 fix plan，该 fix plan 独立 closure（本批 0 P0，无 fix plan 注入）
