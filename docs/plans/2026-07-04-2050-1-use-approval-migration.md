...
> Plan Status: draft (iteration 2 revised, awaiting iteration 3 review)
...
- [ ] `Add`：更新 `../nop-entropy/ai-dev/design/nop-wf/approvable-entity-design.md` 的状态迁移表和反模式表，记录 5 态语义
- [ ] `Add`：更新 `../nop-entropy/docs-for-ai/03-runbooks/enable-approval-on-entity.md` 的常见误区表
- [ ] `Proof`：`mvn -f ../nop-entropy/pom.xml install -pl nop-wf/nop-wf-core -am -DskipTests -q` 成功

Exit Criteria:

- [ ] 平台 `approval-support.xbiz` 已支持 REJECTED 重提 + reverseApprove→REJECTED
- [ ] 平台设计文档和 runbook 已同步更新
- [ ] nop-wf-core 编译安装成功
...
- [ ] `Proof`：确认 `nop-wf-core` 已含 Phase 0 修改并安装到本地 Maven 仓库
- [ ] `Proof`：确认 `nop-codegen` 模板 `I{entityModel.shortName}Biz.java.xgen` 和 `_{entityModel.shortName}.xbiz.xgen` 按 `entityModel.containsTag('use-approval')` 条件生成（`I*Biz extends IApprovableBiz` + `_*.xbiz x:extends approval-support.xbiz`）
- [ ] `Proof`：验证 `approval-support.xbiz` 在 VFS 路径 `/nop/wf/base/approval-support.xbiz` 可访问
- [ ] `Proof`：验证 `wf/approve-status` 字典可解析

Exit Criteria:

- [ ] `mvn -f ../nop-entropy/pom.xml install -pl nop-wf/nop-wf-core -am -DskipTests -q` 成功（含 Phase 0 修改）
- [ ] 本地 `~/.m2/repository/io/nop/nop-wf-core/2.0.0-SNAPSHOT/` 存在 jar
- [ ] codegen 模板条件分支经源码核实存在
...
|----|------|--------|-------------|-------------------|-----------|--------|
| purchase | ErpPurRequisition, Order, Invoice, Return, Payment, Receive, Quotation, Rfq | ✅ | — | ✅ 已存在 | `wf/approve-status` | 无（值相同） |
| sales | ErpSalOrder, Invoice, Receipt, Delivery, Return, Quotation, Contract | ✅ | — | ✅ 已存在 | `wf/approve-status` | 无（值相同） |
| inventory | ErpInvStockMove, StockTake, TransferOrder | **不标（NONE 模式）** | — | — | 保持现状 | 见 Decision |
| finance | ErpFinExpenseClaim, EmployeeAdvance | ✅ | — | 需核实补齐 | `wf/approve-status` | 无（值相同） |
| assets | ErpAstDisposal, AssetCapitalization, ValueAdjustment, Movement, Merge, Split | ✅ | —（`.xwf` 后续计划加） | 需核实补齐 | `wf/approve-status` | 无（值相同） |
| manufacturing | ErpMfgWorkOrder, SubcontractOrder, MaterialIssue | ✅ | — | 需核实补齐 | `wf/approve-status` | 无（值相同） |
| quality | ErpQaInspection, Recall, Review, Calibration | ✅ | — | 需核实补齐 | `wf/approve-status` | 无（值相同） |
| projects | ErpPrjBudget, Billing, CostCollection | ✅ | — | 需核实补齐 | `wf/approve-status` | **DRAFT→UNSUBMITTED** |
| maintenance | ErpMntSparePartUsage, Calibration | ✅ | — | 需核实补齐 | `wf/approve-status` | 无（值相同） |
| cs | ErpCsTicket | ✅ | — | 需核实补齐 | `wf/approve-status` | 无（值相同） |
| hr | ErpHrSalary | ✅ | —（`.xwf` 后续计划加） | 新增 | `wf/approve-status` | **列名 `approvalStatus`→`approveStatus` + 6 态→4 态（见 Phase 6）** |
| contract | — | **不标（NONE 模式）** | — | — | — | 见 Decision |

- [ ] `Fix`：purchase 域所有审批实体加 `tagSet="use-approval"`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Fix`：sales 域所有审批实体加 `tagSet="use-approval"`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Fix`：finance 域 expense claim + employee advance 加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Fix`：assets 域所有审批实体加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 改为 `wf/approve-status`（`useWorkflow="true"` 归 `.xwf` 后续计划，本计划不加）
- [ ] `Fix`：manufacturing 域所有审批实体加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Fix`：quality 域所有审批实体加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Fix`：projects 域审批实体加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 从 `erp-prj/timesheet-status` 改为 `wf/approve-status`，**值迁移：DRAFT→UNSUBMITTED**（含 Java 常量 `TIMESHEET_STATUS_*` → `APPROVE_STATUS_*`）
- [ ] `Fix`：maintenance 域审批实体加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Fix`：cs 域 `ErpCsTicket` 加 `tagSet="use-approval"`，核实并补齐 `approvedBy`/`approvedAt`，`ext:dict` 改为 `wf/approve-status`
- [ ] `Decision`：contract 域处理——**选择** NONE 模式，整体移出范围。实测 `ErpCtContract` 无 `approvalStatus` 列（只有 `status`→`erp-ct/contract-status`）；唯一持有 `approvalStatus` 的是 `ErpCtApprovalRecord`（审批日志表，配合 `ErpCtApprovalMatrix` 构成自建审批台账，非被审批单据）。**替代 A**：给 ErpCtContract 新增审批能力（rejected：属新功能，需独立需求/设计）。**替代 B**：迁移 `ErpCtApprovalRecord` 台账（rejected：日志表非被审批实体，标 use-approval 语义不成立）。**残留风险**：`ErpCtApprovalRecord`+`ErpCtApprovalMatrix` 自建审批台账与"禁止自建审批状态迁移"强制规则冲突，退役归独立计划。Phase 5 中 `ErpCtInvoicePlanBizModel.setApproveStatus()` 操作的是 purchase/sales 发票（跨域），不涉及 contract 自身审批。Skill: `nop-backend-dev`
- [ ] `Fix`：**HR 域 ErpHrSalary**——`approvalStatus` 列改名为 `approveStatus` + `ext:dict` 改为 `wf/approve-status` + 加 `tagSet="use-approval"` + 新增 `approvedBy`/`approvedAt` 列 + `paymentStatus` 保留为独立字段（非派生投影）。**不加 `useWorkflow="true"`**（归 `.xwf` 后续计划）
- [ ] `Decision`：inventory 域 `approveStatus` 列处理——inventory 作业单（StockMove/StockTake/TransferOrder）按 `approval-framework.md` 是 NONE 模式，但 ORM 有 `mandatory=true` 的 `approveStatus` 列且 `ErpInvStockMoveProcessor.java:270` 手工 set。**选择**保留列但不标 `use-approval`（存量逻辑不迁移，归独立清理计划）。**替代**：删除列（rejected：破坏性变更，需数据迁移）。**残留风险**：与"禁止自建审批状态迁移"强制规则冲突，但 inventory 作业单实际不做审批决策，仅保留字段兼容。Skill: `nop-backend-dev`
- [ ] `Proof`：xml well-formed 校验：`xmllint --noout module-*/model/app-erp-*.orm.xml`

Exit Criteria:

- [ ] 所有需审批实体的 ORM 模型已标 `tagSet="use-approval"`，`approvedBy`/`approvedAt` 已补齐
- [ ] HR `ErpHrSalary` 列改名和新增列完成（不含 `useWorkflow` 和 `nopFlowId`）
- [ ] projects/HR 值迁移完成
- [ ] 各审批实体的 `ext:dict` 已切换为 `wf/approve-status`
- [ ] XML well-formed 校验通过
...
- [ ] `Proof`：执行增量生成：`mvn clean install -DskipTests`（触发 codegen 增量链，`use-approval` tag 被识别）
- [ ] `Proof`：验证生成产物：
  - `I*Biz` 接口文件（`dao/src/main/java/app/erp/*/biz/I*.java`）追加 `extends IApprovableBiz<Entity>`
  - `_*.xbiz` 文件追加 `x:extends="/nop/wf/base/approval-support.xbiz"`
- [ ] `Proof`：HR 域 `ErpHrSalary` 的 codegen 生成新字段正确（`approveStatus`/`approvedBy`/`approvedAt`）
- [ ] `Proof`：`mvn compile -DskipTests` 编译全绿（146 reactor 模块）

Exit Criteria:

- [ ] `mvn compile -DskipTests` 不报错
- [ ] I*Biz 接口已追加 IApprovableBiz
- [ ] _*.xbiz 已继承 approval-support.xbiz
...
Status: planned
Targets: `module-purchase/erp-pur-service/`（6 Processors + 4 BizModels + 2 Support + 1 task.xml）
Skill: `nop-backend-dev`
...
```
当前:
  BizModel.@BizMutation submit() → Processor.doSubmit()
    Processor: statusCheck + setApproveStatus(SUBMITTED) + businessLogic

目标:
  approval-support.xbiz 标准 source: statusGuard + setApproveStatus(SUBMITTED)
  自定义 xbiz <source x:override="append">: inject("processor").onSubmit(id, svcCtx)
    Processor.onSubmit(): 仅 businessLogic（无 approveStatus 管理）
```

- [ ] `Fix`：**ErpPurOrderProcessor**——移除 `doSubmit`/`doApprove`/`doReject`/`doReverseApprove` 中的 `setApproveStatus`/`getApproveStatus` 守卫校验；方法重构为纯业务回调（`onSubmit`/`onApproved`/`onRejected`/`onReverseApproved`）；状态守卫由 `approval-support.xbiz` 标准 source 承担
- [ ] `Fix`：**ErpPurRequisitionProcessor** —— 同上模式
- [ ] `Fix`：**ErpPurInvoiceProcessor** —— 同上模式
- [ ] `Fix`：**ErpPurReturnProcessor** —— 同上模式
- [ ] `Fix`：**ErpPurPaymentProcessor** —— 同上模式
- [ ] `Fix`：**ErpPurReceiveProcessor** —— 同上模式
- [ ] `Fix`：**ErpPurOrderBizModel**、**ErpPurRequisitionBizModel**、**ErpPurReceiveBizModel**、**ErpPurPaymentBizModel**——移除手写 `@BizMutation submit`/`approve`/`reject` 方法（它们现在由 `approval-support.xbiz` 标准 action 提供）；保留需要业务联动的 action 通过 xbiz `x:override="append"` 注入
- [ ] `Fix`：**PaymentSettler**——`getApproveStatus` 比较改为判等标准常量（行为不变，字段值已对齐）
- [ ] `Fix`：**RequisitionToOrderConverter**——`setApproveStatus(UNSUBMITTED)` 保留（新建实体设 UNSUBMITTED 是业务初始化，非状态迁移）
- [ ] `Fix`：删除孤立 `ErpPurReceive/approve.task.xml`（未被 xbiz 引用）
- [ ] `Decision`：xbiz 覆盖策略 + `wf-integration-design.md` 矛盾裁决——**选择** purchase 域 xbiz 文件（`module-purchase/erp-pur-service/src/main/resources/_vfs/erp/pur/_xbiz/`）新增 `x:extends` 链继承生成的 `_*.xbiz`，在 `approve`/`reject` action 的 `<source>` 上用 `x:override="append"` 注入 Processor 业务联动。**同时裁决** `wf-integration-design.md:78-86` "推荐模式"示例用整源 `replace` 调 `processor.approve()` 与迁移表 `:152-154` "approveStatus 归标准 source" 矛盾——采纳 `append` 模式为权威，修正该文档示例。**替代**：`task:name` 引用 task.xml（rejected：Phase 4 需删除孤立 task.xml，引入新模式矛盾）。**残留风险**：无。Skill: `nop-backend-dev`
- [ ] `Proof`：`mvn test -pl module-purchase/erp-pur-service -am` 既有套件全部通过（~70 tests）

Exit Criteria:

- [ ] purchase 域 6 个 Processor 不再包含 `setApproveStatus`
- [ ] purchase 域 BizModel 不再包含手写 submit/approve/reject `@BizMutation`
- [ ] 移除 `ErpPurReceive/approve.task.xml`
- [ ] purchase 域全部既有测试（~70 tests）通过
...
Status: planned
Targets: sales/finance/assets/manufacturing/quality/projects/maintenance/cs（~30 Java 文件）
Skill: `nop-backend-dev`

- Item Types: `Fix-heavy`
- Prereqs: Phase 4

- [ ] `Fix`：sales 域（6 Processors + 7 BizModels + 4 Support）—— 同 phase 4 迁移模式
- [ ] `Fix`：finance 域（2 Processors + 2 BizModels）—— 同 phase 4 迁移模式。注意：expense claim + employee advance 的 `currentApproveStatus` helper 方法可删除
- [ ] `Fix`：assets 域（2 Processors + 2 BizModels）—— 同 phase 4 迁移模式
- [ ] `Fix`：manufacturing 域（ErpMfgWorkOrderProcessor + 2 BizModels + MrpReleaseService）—— 同 phase 4 迁移模式。`MrpReleaseService` 的 `setApproveStatus(UNSUBMITTED)` 保留（新建实体初始化）
- [ ] `Fix`：quality 域（ErpQaRecallBizModel + ErpQaInspectionBizModel）—— 已核实使用标准 action 名（`submit:105`/`approve:116`/`reject:136`），无 reverseApprove（由 approval-support.xbiz 标准动作补齐）。同 phase 4 迁移模式
- [ ] `Fix`：projects 域（ProjectCostAggregator + ExpenseCostAggregator）—— `setApproveStatus(APPROVED)` 改为由 `approve` action 的标准 source 处理 + Java 常量 `TIMESHEET_STATUS_*` 重命名 `APPROVE_STATUS_*` + 值迁移 DRAFT→UNSUBMITTED
- [ ] `Fix`：maintenance 域（ErpMntSparePartUsageBizModel）—— 同 phase 4 迁移模式
- [ ] `Fix`：cs 域（无需迁移，无手写 approveStatus 逻辑）
- [ ] `Fix`：`ErpCtInvoicePlanBizModel`——`setApproveStatus("UNSUBMITTED")` 字面量改为引用常量（此文件操作的是 purchase/sales 发票实体，跨域引用，不涉及 contract 自身审批——见 Phase 2 contract Decision）
- [ ] `Proof`：各域按优先顺序测试：`mvn test -pl module-<domain>/erp-<dom>-service -am`

Exit Criteria:

- [ ] 所有域 Processor 不再包含手写 `setApproveStatus` 状态迁移
- [ ] 所有域 BizModel 不再包含手写 submit/approve/reject `@BizMutation`
- [ ] 各域既有测试全部通过
...
- [ ] `Fix`：`IErpHrSalaryBiz`——接口中 `approvalStatus` 引用改为 `approveStatus`
- [ ] `Fix`：`ErpHrSalaryBizModel`——移除手写 6 态审批 action（`review`/`approveFinance`/`approveManager`/`rejectSalary`/`voidSalary`/`markPaid` 中属于审批迁移的部分）；`markPaid`/`voidSalary` 保留（属 `paymentStatus` 独立轴）
- [ ] `Fix`：`PayrollCalculator`——注释/字面量更新
- [ ] `Decision`：`markPaid` 前置条件——**选择**改为 `approveStatus=APPROVED AND paymentStatus=PENDING`（不再检查旧 6 态值 APPROVED_MANAGER）。**替代**：保留旧检查（rejected：6 态值已迁移，无法匹配）。**残留风险**：无。Skill: `nop-backend-dev`
- [ ] `Proof`：`mvn test -pl module-hr/erp-hr-service -am` 既有测试通过（TestErpHrPayrollEngine 等）

Exit Criteria:

- [ ] `ErpHrSalaryBizModel` 不再包含 6 态审批自定义状态迁移逻辑
- [ ] `paymentStatus` 独立运作，`markPaid` 前置条件使用 `approveStatus=APPROVED`
- [ ] HR 全部测试通过
...
- [ ] `Fix`：各域测试文件——`setApproveStatus` 调用改为通过标准 action 入口（`biz.submitForApproval(id, ctx)`→`biz.approve(id, ctx)`），避免跳过 approval-support.xbiz 的守卫检查
- [ ] `Fix`：projects/HR 两域测试中的旧值断言迁移（DRAFT→UNSUBMITTED, 6 态→4 态等）
- [ ] `Fix`：测试中 approveStatus 断言值核对（当前已是 `UNSUBMITTED`/`SUBMITTED`/`APPROVED`/`REJECTED`，类型已对齐，仅确认）
- [ ] `Proof`：全量测试运行：`mvn test`（或按域分组依次验证）
- [ ] `Proof`：`mvn clean install -DskipTests` 全绿（编译+代码生成）
...
- Independent draft review iteration 3: 待修订后复审。

## Closure Gates

- [ ] 范围内行为完成：11 域审批实体统一接入 `use-approval`（inventory 除外，已在 Decision 裁决）
- [ ] 相关文档对齐：
...
- [ ] 已运行验证：`mvn clean install -DskipTests` + `mvn test`
- [ ] 无范围内项目降级为 deferred/follow-up（WORKFLOW `.xwf` / 前端 / dict 清理 / inventory 均为独立结果表面，已登记触发条件）
- [ ] 独立草案审查已完成并记录（iteration 1 needs revision → 修订 → iteration 2 待复审）
