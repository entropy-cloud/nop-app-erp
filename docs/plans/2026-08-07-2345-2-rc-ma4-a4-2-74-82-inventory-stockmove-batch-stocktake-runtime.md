...
- [x] 每项裁决明确：主路径闭合 / 维持分级（reuse / watch-only / 设计输入）+ 运行时证据记录，或升级触发 MR0
...
- [x] `Add` roadmap A4.2.74-A4.2.78、A4.2.80-A4.2.82 `todo → done`（A4.2.79 保留 todo 注记 MR1 阻塞）；`docs/logs/2026/08-07.md` 追加完成条目。

Exit Criteria:

- [x] roadmap 八项状态已更新为 done 且与报告裁决一致；A4.2.79 保留 todo 并注记 MR1 阻塞原因
...
- [x] 范围内行为完成（八项存疑点均有 file:line 运行时证据 + 明确裁决）
- [x] 相关文档对齐（报告落盘 + roadmap/log 同步 + arm-index 衔接裁决记录）
- [x] 已运行验证（checker actual=baseline 确认；无代码变更故无 build/test 回归风险）
- [x] 无范围内项目降级为 deferred/follow-up（A4.2.79 非「降级」——显式 MR1 前置依赖，保留 todo 待 MR1 落地后回队，记录于 Non-Goals）
- [x] 独立草案审查已完成并记录
...
Status Note: <completed — 两阶段执行完成，八项存疑点全数收口（五项主路径闭合/watch-only + 一项 watch-only residual[A4.2.76] + 一项维持 P1 设计输入[A4.2.78 → P1-RC-031 null 语义] + 一项维持 reuse[A4.2.80 → reuse P1-MA2-062 §4 复核倾向重开须人工确认 product-scope]，零新 finding / 不触发 MR0 / 不归 MR1 本审计）；A4.2.79 显式 MR1 P1-RC-031 修复落地前置依赖，保留 todo 待 MR1 落地后回队>

Closure Audit Evidence:

- Phase 1 验证报告落盘 `docs/audits/2026-08-07-2345-rc-ma4-a4-2-74-82-inventory-stockmove-batch-stocktake-runtime.md`（`> Report Status: done`，§0 业财探针纪律前置声明 + §1 八项逐项裁决 + §2 finding 衔接 + §3 过程纪律自检 + §4 裁决摘要），含八项存疑点各自裁决 + file:line 证据 + §2 判据命中分支 + 业财保护区域探针纪律声明（A4.2.74/82 READ-ONLY）。
- Phase 2 `docs/audits/arm-index.md` 追加「A4.2.74-82 inventory 运行时确认」RC 交叉引用注记（P1-RC-031 维持 P1 + reuse P1-MA2-062 维持 §4 复核 + P0-MA2-020/P2-MA2-028/P1-MA4-021/P1-MA4-020/P1-MA4-001 family 维持既有分级不撤销，无新 finding）。
- `docs/backlog/requirement-compliance-roadmap.md` A4.2.74-A4.2.78、A4.2.80-A4.2.82 `todo → done ✅`；A4.2.79 保留 todo 注记 MR1 阻塞。
- `docs/logs/2026/08-07.md` 追加完成条目（含八项 live code 实测核验结论）。
- 零生产代码变更（git status 仅 .md 文件），checker actual == baseline（本审计只读无 build/test 回归风险）。
