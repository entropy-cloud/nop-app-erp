...
- [ ] `Fix`: `ErpFinPostingProcessor`（`module-finance/erp-fin-service/.../posting/ErpFinPostingProcessor.java`）——5 个 `private` 方法改为 `protected`：
...
- [ ] `Fix`: `ErpFinAccountingPeriodProcessor`（`module-finance/erp-fin-service/.../processor/ErpFinAccountingPeriodProcessor.java`）——10 个 `private` 方法改为 `protected` + 1 个内部类改为 `protected`：
...
- [ ] `Fix`: `ErpFinBudgetScenarioProcessor`——3 个 `private` 方法改为 `protected`：`requireScenario` / `save` / `join`
- [ ] `Fix`: `ErpFinEmployeeAdvanceProcessor`——3 个方法：`markPosted` / `clearPosted` → `protected`；`nz` → `protected static`
- [ ] `Fix`: `ErpFinExpenseClaimProcessor`——2 个方法：`nz` → `protected static`；`isBlank` → `protected static`
- [ ] `Fix`: `ErpFinNotesReceivableProcessor`——1 个方法：`nz` → `protected static`
- [ ] `Fix`: `ErpFinNotesPayableProcessor`——1 个方法：`nz` → `protected static`
- [ ] `Fix`: `ErpFinBadDebtProcessor`——1 个方法：`nz` → `protected static`
- [ ] `Fix`: `ErpPrjProjectSettlementProcessor`——9 个 `private` 方法改为 `protected`：`requireSettlement` / `save` / `findBillings` / `findCostCollections` / `loadProject` / `resolveUserId` / `illegalTransition` / `nz`（改为 `protected`，去掉 `static`，因为是实例方法）/ `parseAmount`

Exit Criteria:

- [ ] 9 个 finance + projects Processor 文件中全部 35 个 `private` 方法 + `TbAgg` 内部类改为 `protected`，零 `private` 方法残留（`PostingRun` private 构造器除外，属 Non-Goal）
- [ ] 涉及模块 `mvn compile -DskipTests -pl module-finance/erp-fin-service,module-projects/erp-prj-service -am` 编译通过
...
- [ ] `Fix`: `ErpAstAssetCapitalizationProcessor`——3 方法：`markPosted` / `clearPosted` → `protected`；`nz` → `protected static`
- [ ] `Fix`: `ErpAstMergeProcessor`——2 方法：`nz` → `protected static`；`sum` → `protected static`
- [ ] `Fix`: `ErpAstInventoryProcessor`——2 方法：`nz` → `protected static`；`nzInt` → `protected static`
- [ ] `Fix`: `ErpAstSplitProcessor`——2 方法：`nz` → `protected static`；`lineMap` → `protected static`
- [ ] `Fix`: `ErpAstCipProcessor`——2 方法：`nz(BigDecimal, BigDecimal)` → `protected static`；`nz(BigDecimal)` → `protected static`
- [ ] `Fix`: `ErpAstValueAdjustmentProcessor`——1 方法：`nz` → `protected static`
- [ ] `Fix`: `ErpAstDepreciationScheduleProcessor`——1 方法：`nz` → `protected static`
- [ ] `Fix`: `ErpAstDisposalProcessor`——1 方法：`nz` → `protected static`
- [ ] `Fix`: `ErpAstMaintenanceProcessor`——1 方法：`nz` → `protected static`
- [ ] `Fix`: `ErpInvStockMoveProcessor`——2 方法：`nz` → `protected static`；`newMoveCode` → `protected static`
- [ ] `Fix`: `ErpInvOwnershipTransferProcessor`——2 方法：`transferDao` → `protected`；`nz` → `protected static`
- [ ] `Fix`: `ErpInvLandedCostProcessor`——1 方法：`nz` → `protected static`
- [ ] `Fix`: `ErpPurReceiveProcessor`——1 方法：`addLineQuantities` → `protected`
- [ ] `Fix`: `ErpSalDeliveryProcessor`——1 方法：`addLineQuantities` → `protected`
- [ ] `Fix`: `ErpCrmConversionProcessor`——内部接口 `Setter` 从 `private interface` → `protected interface`
- [ ] `Proof`: 全量扫描确认 24 个 Processor 文件中零 `private` 方法残留（`PostingRun` private 构造器除外），经 `rg 'private\s+(static\s+)?(final\s+)?[\w<>\[\],\s]+\s+\w+\s*\(' --type java -g '*Processor*.java' module-*/` 验证

Exit Criteria:

- [ ] 15 个 assets + inventory + purchase + sales + CRM Processor 文件中全部 22 个 `private` 方法 + `Setter` 内部接口改为 `protected`，零残留
- [ ] 涉及模块编译通过
...
- [ ] 范围内行为完成：全部 24 个 Processor 文件的 57 个 `private` 方法 + 2 个内部类型改为 `protected`
- [ ] 相关文档对齐：`docs/analysis/productization-readiness-analysis.md` §6.1 修复清单全部标记为已修复
- [ ] 已运行验证：`mvn clean install -DskipTests` 全 154 模块 BUILD SUCCESS + `mvn test`（finance + projects + assets + inventory 代表域现有测试全绿，无回归）
