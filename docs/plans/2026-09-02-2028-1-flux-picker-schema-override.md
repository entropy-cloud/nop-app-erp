...
- [ ] **PickerSchema v3 顶层字段定义**（composite-schemas.ts:168-182）：
...
- [ ] **PickerPopupConfig v2 定义**（composite-schemas.ts）：
      ```typescript
...
- [ ] **OverflowConfig 类型新增**（composite-schemas.ts）：
      ```typescript
...
- [ ] **picker-renderer.tsx 移除 CRUD 重复实现**：
...
- [ ] **picker-renderer.tsx popup 类型分支**（dialog/drawer/popover）：
      ```typescript
...
- [ ] **picker-renderer.tsx confirm 逻辑（CRUD pickerSchema）**：
...
- [ ] **picker-renderer.tsx label 反应式解析**：
...
- [ ] **picker-renderer.tsx propContracts 注册新字段**：
      ```typescript
...
- [ ] **picker-helpers.ts createPickerCrudSchema 简化**：
...
- [ ] **`createNormalizedPickerSource` 移除**（picker-helpers.ts:123-126）：
      - 删除该函数（不再需要 source 概念）
      - Skill: `none`（无引用即删）

- [ ] **picker-renderer.tsx 内置 `pick` action 上下文**：
...
- [ ] **picker-dropdown.tsx 重构**（plan-audit 补漏）：
...
Exit Criteria:
- [ ] `PickerSchema` v3 类型已声明并通过 TS 编译（`pnpm --filter @nop-chaos/flux-renderers-form-advanced tsc --noEmit` 无错）
- [ ] 旧字段 `valueKey` / `labelKey` / `options` / `loadAction` / `columns` / `searchable` 已从 schema 移除
- [ ] `pickerSchema` 渲染逻辑支持任意 BaseSchema（不仅 CRUD）
- [ ] `pickerPopup.type` 支持 dialog/drawer/popover 三种 surface
- [ ] confirm 逻辑：CRUD pickerSchema 读取 `selectionStatePath`；非 CRUD 读取 picker 上下文累积
- [ ] label 反应式解析生效：picker mount 时根据 value 拉取对应 label，外部 value 变化时重新解析
...
- [ ] **isPicker 时构造 pickerSchema 子树**（flux-web/grid_crud.xpl line 32-89）：
...
- [ ] **移除顶层 size/modalSize/source**：已移入 pickerPopup/pickerSchema.loadAction
      - Skill: `nop-frontend-dev`

- [ ] **保留向后兼容**：当 view.xml 中无 gridApi 且无 pickerSchema 时，走 pickerSchema 缺省路径（picker 仅显示触发按钮）
      - Skill: `none`

Exit Criteria:
- [ ] `npm run validate:flux` 中 `pickerSchema` 警告从 368 → 0
- [ ] `valueField` / `labelField` 命名替换 `valueKey` / `labelKey` 后无新警告
- [ ] 所有 999 个 picker 页面导出 0 failures
- [ ] 至少 1 个 picker 页面（手动抽样）渲染正确：弹窗打开、CRUD 列表加载、选中回写
...
Exit Criteria:
- [ ] 5 个 view.xml 中 `valueKey` / `labelKey` / `pickerDialog` 字面引用全部替换为 `valueField` / `labelField` / `pickerPopup`
- [ ] `grep -rn 'valueKey\|labelKey\|pickerDialog' module-*/erp-*-web/src/main/resources/_vfs/` 在 view.xml 中无匹配（仅允许 Flux 渲染器 TS 源中出现废弃别名警告）
...
- [ ] **新增 picker-schema-override.test.tsx**（3 用例）：
...
- [ ] **新增 picker-popup-types.test.tsx**（3 用例）：
...
- [ ] **新增 picker-label-parse.test.tsx**（4 用例）：
...
- [ ] **picker-renderer.test.tsx 命名与结构调整**：测试用例按新字段名重命名
      - Skill: `nop-testing`

Exit Criteria:
- [ ] 现有 4 个 picker 测试文件全部通过 `pnpm --filter @nop-chaos/flux-renderers-form-advanced test`
- [ ] 新增 5 个测试文件全部通过（picker-schema-override、picker-popup-types、picker-label-parse、picker-overflow-config、picker-amis-fields）
- [ ] 共新增 18 个测试用例
...
- [ ] **`flux-guide/09-amis-migration.md` Picker 节更新**：
      - 转换规则表格更新：
...
- [ ] **picker-renderer.tsx JSDoc 更新**（line 47-50）：
      ```typescript
      /**
...
- [ ] **composite-schemas.ts JSDoc 更新**（PickerSchema 上方）：
      ```typescript
      /**
...
- [ ] **`flux-guide/design-patterns/picker-transfer.md` Design Status 更新**（plan-audit 补漏）：
...
Exit Criteria:
- [ ] `flux-guide/09-amis-migration.md` Picker 节内容与代码一致
- [ ] `flux-guide/design-patterns/picker-transfer.md` Design Status 更新为 `stable`
- [ ] PickerSchema 与 picker-renderer 的 JSDoc 反映 v3 设计
- [ ] 转换规则表格覆盖所有 AMIS → Flux 字段
...
- [ ] **Flux 包构建**：`pnpm --filter @nop-chaos/flux-renderers-form-advanced build` 无 TS 错误
- [ ] **Flux 工作区类型检查**：`pnpm -w typecheck` 无 TS 错误（plan-audit 补漏：覆盖 `@nop-chaos/flux-renderers-data` 等依赖包对 PickerSchema 类型变化的连锁反应）
- [ ] **Flux 包测试**：`pnpm --filter @nop-chaos/flux-renderers-form-advanced test` 全绿
- [ ] **nop-web 构建**：`mvn -pl nop-frontend-support/nop-web clean install -DskipTests` 无错
- [ ] **nop-app-erp flux 页面导出**：`mvn test -pl app-erp-all -Dtest=ErpAllFluxPagesTest` 全绿，0 failures
- [ ] **validation**：`npm run validate:flux` 警告总数从 13866 → ≤ 13500（pickerSchema 368 + 关联 = -368）；错误数从 325 减少或不变
- [ ] **抽样视觉验证**（手测，至少 5 个 picker 页面，覆盖所有 popup 类型与 pickerSchema 形态）：
      - [ ] dialog + CRUD pickerSchema 弹窗打开正常（ERP 库存 picker 页面）
      - [ ] dialog + CRUD pickerSchema 多选跨页保留正常（ERP 采购 picker 页面）
      - [ ] drawer + CRUD pickerSchema 右侧抽屉正常（ERP 销售 picker 页面）
      - [ ] popover + list pickerSchema 轻量选择正常（状态选择场景）
      - [ ] label 反应式解析正常：编辑表单打开 picker 显示已选 value 的 label

Exit Criteria:
- [ ] Phase 1 + 2 + 3 + 4 Exit Criteria 全部勾选
- [ ] 上述 6 项全量验证全部勾选
- [ ] 抽样 5 个 picker 页面渲染正确（人测通过）

## Closure Gates

- [ ] Plan Status 更新为 `completed`
- [ ] `docs/logs/{year}/{month}-{day}.md` 记录本计划完成（含验证状态 + git commit hash）
- [ ] 独立结束审计通过（独立子代理会话审查）
