...
- **RELEASED by 2026-07-19-0730-1**：触发条件已满足（owner-doc `treasury.md §业财过账` 已明示 FX 语义在生产路径不可达属规则 13 不可降级项）。`docs/plans/2026-07-19-0730-1-fx-notes-receivable-discount-exchange-gain-loss-builder.md` 交付完整修复：(Phase 1) 3 Decisions 落地 cash-at-spot plug 范式公式（`discount.netAmount = netAmountSource × spotRate` + `exchangeGainLoss = amountFunctional − discountInterestFunctional − netAmount` plug 平衡差额，符号语义外币升值 → Cr 6051 汇兑收益 / 外币贬值 → Dr 6051 汇兑损失）+ (Phase 2) Java `ErpFinNotesReceivableProcessor.buildDiscount` 派生公式 + `discount` 公共方法 5 参数签名扩展（`@Optional exchangeRate`）+ IBiz/BizModel 单方法重载 + `ErpFinConstants.CONFIG_NOTES_FX_GAIN_LOSS_ENABLED` 常量 + `ErpFinErrors.ERR_NOTES_DISCOUNT_FX_RATE_REQUIRED` ErrorCode + Processor 内联 `notesFxGainLossEnabled` / `isForeignCurrency` / `resolveFunctionalCurrencyId` private helper（对齐 `ExchangeRevaluationService:280-287` 范式）+ config-gate 默认 false 向后兼容 + 4 JUnit 新测试（state machine 3 + posting 集成 1 含 `Σ Dr ≡ Σ Cr = 669.97` 复式平衡断言）；(Phase 3) `fin-notes-receivable-fx-discount.action.spec.ts` 0120-1 workaround 经 `ErpFinVoucher__post` 直驱迁移至真实 `discount(exchangeRate)` mutation 路径（Cr 6051=3.3000 cash-at-spot plug 4 行凭证，复式平衡 Dr 663.3000+Dr 6.67=669.97 ≡ Cr 3.3000+Cr 666.67=669.97）+ 直驱原语 `postVoucher`/`buildNotesReceivableDiscountedEvent` 一并清理 + 1430-1 单币种 spec 0 回归 + finance 抽样回归 0 新增失败；(Phase 4) `treasury.md §业财过账` 实现注记段 + e2e-runbook JVM arg + 凭证行断言表 + 0120-1/0330-1 RELEASED + backlog/logs。验证：4 JUnit 全绿 + 3 spec 全绿 + 154 模块 BUILD SUCCESS。
