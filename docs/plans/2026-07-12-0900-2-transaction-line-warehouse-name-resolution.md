...
- [x] `Decision | Explore`: 裁决 warehouseName 命名约定与纳入准则——(a) 命名约定：单关系实体沿用 `{relation}Name`（`warehouseName`，对齐头实体批次 1/2 范式）。(b) 纳入准则：仅解析含 `warehouse` to-one 关系且 `getName()` 有业务语义的行实体（5 个：4 单仓库 + 1 双仓库）；ErpInvStockMoveLine 行级无仓库列（仅库位列）按 Non-Goals 排除。(c) 替代方案：同时解析 uomName（否决——uom 识别度低且可经物料带出，先覆盖第二高价值 warehouseName）。(d) Explore：逐实体核实 ORM `_app.orm.xml` 中 `<to-one name="warehouse">` 关系名与 `ErpMdWarehouse.getName()` 可达性（已初步确认 4 单仓库实体均可达，Phase 1 落地前复核）。全部 4 行实体均已有定制 Line BizModel（批次 3 materialName `@BizLoader` 已就绪），无需创建新 BizModel。
  - Skill: `nop-backend-dev`
- [x] `Add`: 4 行实体 xmeta 各增 `warehouseName` 派生 prop（`queryable="false" sortable="false"` + `<schema type="java.lang.String"/>`），镜像 `ErpPurOrder.xmeta` 范式。
  - Skill: `nop-frontend-dev`
- [x] `Add`: 4 行实体已有定制 Line BizModel 在既有 `materialName` `@BizLoader(forType=...)` 方法内追加 warehouse 批量加载（`orm().batchLoadProps(lines, Collections.singleton("warehouse"))` + 逐行 `getWarehouse().getName()`），镜像 `ErpPurOrderBizModel.java:107-145` 范式。
  - Skill: `nop-backend-dev`
- [x] `Add`: 4 行实体 view.xml grid 既有 `<cols x:override="bounded-merge">`（批次 3 已建，含 materialName 列）内追加 `warehouseName` 列替换原始 `warehouseId` 列（保留 `id`/`lineNo`/`materialName`/`quantity`/`unitPrice` 等业务列）。
  - Skill: `nop-frontend-dev`
- [x] `Proof`: 4 行实体 grid 列本地化验证（xmeta `warehouseName` prop 存在 + view.xml grid 含 `warehouseName` 列 + `_gen` 原始 `warehouseId` 列被 bounded-merge 覆盖）+ 涉及模块编译通过 + 代表行实体（purchase + sales 各 ≥1）BizLoader 批量加载测试（在批次 3 既有测试类 `TestErpPurFkNameLoader` / `TestErpSalFkNameLoader` 新增 warehouseName 解析测试方法，经 `IGraphQLEngine` findList 触发 `@BizLoader`，验证 warehouseName 对齐 master-data）。
  - Skill: `nop-testing`

Exit Criteria:

- [x] 4 行实体 xmeta + Line BizModel + view grid 三层 `warehouseName` 落地
- [x] 代表行实体 BizLoader 批量加载测试全绿（purchase + sales 各 ≥1 行实体，warehouseName 对齐 master-data）
...
- [x] `Decision | Explore`: 裁决双仓库命名——沿用头实体 ErpInvStockMove 批次 2 范式 `sourceWarehouseName` / `destWarehouseName`（双关系保留语义区分）。核实 ORM `_app.orm.xml` 中 ErpMfgWorkOrderLine 的 `sourceWarehouse` / `destWarehouse` to-one 关系名与 `ErpMdWarehouse.getName()` 可达性（已初步确认，Phase 2 落地前复核）。
  - Skill: `nop-backend-dev`
- [x] `Add`: ErpMfgWorkOrderLine xmeta 增 `sourceWarehouseName` + `destWarehouseName` 派生 prop；Line BizModel 在既有 materialName `@BizLoader` 方法内追加双仓库批量加载（`orm().batchLoadProps(lines, CollectionHelper.buildSet("sourceWarehouse","destWarehouse"))` + 逐行 `getSourceWarehouse().getName()` / `getDestWarehouse().getName()`）；view.xml grid 既有 `<cols>` 内追加 `sourceWarehouseName` / `destWarehouseName` 列替换原始 `sourceWarehouseId` / `destWarehouseId` 列。
  - Skill: `nop-frontend-dev`、`nop-backend-dev`
- [x] `Proof`: ErpMfgWorkOrderLine grid 列本地化验证（xmeta 双 prop 存在 + view grid 含双 `*Name` 列 + `_gen` 原始双 `*Id` 列被覆盖）+ 涉及模块编译通过 + manufacturing 域代表行实体 BizLoader 批量加载测试（**新建** `TestErpMfgFkNameLoader` 测试类——经实时仓库核实 manufacturing 域尚无 FK name loader 测试类，故需新建，镜像 `TestErpPurFkNameLoader` 范式：`@NopTestConfig(localDb=true, initDatabaseSchema=TRUE)` + `IGraphQLEngine` findList + seedWarehouse/seedMaterial 等 seed helpers；新增 sourceWarehouseName/destWarehouseName 解析测试方法，经 `ErpMfgWorkOrderLine__findList` 触发 `@BizLoader` 验证双仓库名称对齐 master-data）。
  - Skill: `nop-testing`

Exit Criteria:

- [x] ErpMfgWorkOrderLine xmeta + Line BizModel + view grid 三层 `sourceWarehouseName` / `destWarehouseName` 落地
- [x] manufacturing 域 BizLoader 批量加载测试全绿（新建 `TestErpMfgFkNameLoader`，sourceWarehouseName/destWarehouseName 对齐 master-data）
- [x] 涉及模块编译通过

## Draft Review Record

- Independent draft review iteration 1: accept (draft-review-2026-07-12) — 格式合规（必需章节齐备、字段名正确、Phase 结构有效、Item Types/Skill/Prereqs 齐全）；范围清晰（5 行实体 warehouseName 解析，Non-Goals 明确排除 uomId/skuId/全量行实体/ErpInvStockMoveLine）；退出标准可测试；结束门控含验证命令与证据要求。实时仓库核实修正 1 项 Major：Phase 2 Proof 原文"在 `TestErpMfgFkNameLoader` 或既有测试类新增"隐含已存在测试类，但 manufacturing 域尚无 FK name loader 测试类（`TestErpMfgFkNameLoader` 不存在），已修正为"新建 `TestErpMfgFkNameLoader` 测试类"并补 Phase 2 退出标准测试门控。已核实 5 行实体 Line BizModel 均存在且含 materialName `@BizLoader`（追加 warehouse 加载可行）；`TestErpPurFkNameLoader`/`TestErpSalFkNameLoader` 既有且含 seedWarehouse helper。无 Blocker，计划可进入 active。

## Closure Gates

> 仅在所有项目和每个阶段退出标准都勾选 `[x]` 后关闭。完整仓库验证：`mvn clean install -DskipTests`（154 模块）+ 涉及 service `mvn test` 一次。

- [x] 范围内行为完成（5 行实体 warehouseName 解析）
- [x] 相关文档对齐（`view-and-page-strategy.md` 行实体 FK 名称解析范式已由批次 1/2/3 确立，本批次推广至仓库 FK，无新约定）
...
Status Note: 实现完成。5 交易行实体 warehouseName 三层落地（Phase 1：4 单仓库实体 ErpPurOrderLine/ErpPurReceiveLine/ErpSalOrderLine/ErpSalDeliveryLine 的 `warehouseName`；Phase 2：1 双仓库实体 ErpMfgWorkOrderLine 的 `sourceWarehouseName`/`destWarehouseName`），经机制 D（xmeta 派生 prop + Line BizModel `@BizLoader(forType=...)` 批量加载防 N+1 + view.xml grid `bounded-merge` 列替换原始 `*Id`）。全量 154 模块 `mvn clean install -DskipTests` BUILD SUCCESS；代表行实体 BizLoader 测试全绿（purchase `TestErpPurFkNameLoader` 5 tests 含新增 `testPurOrderLineWarehouseNameResolution` / sales `TestErpSalFkNameLoader` 5 tests 含新增 `testSalOrderLineWarehouseNameResolution` / manufacturing 新建 `TestErpMfgFkNameLoader` 1 test 验证双仓库名称解析）。Phase 1+2 全部 [x]，Phase Status 均 completed。Closure Gates 全部 8/8 [x]，独立结束审计由独立子代理新会话完成（closure-audit-2026-07-12），audit 结论 approved。

Closure Audit Evidence:

- 执行者自验（非独立审计）：
  - 全量构建：`mvn clean install -DskipTests` → BUILD SUCCESS（154 模块，2026-07-12T22:07:32+08:00）
  - purchase 测试：`mvn test -pl module-purchase/erp-pur-service -Dtest=TestErpPurFkNameLoader` → Tests run: 5, Failures: 0, Errors: 0
  - sales 测试：`mvn test -pl module-sales/erp-sal-service -Dtest=TestErpSalFkNameLoader` → Tests run: 5, Failures: 0, Errors: 0
  - manufacturing 测试：`mvn test -pl module-manufacturing/erp-mfg-service -Dtest=TestErpMfgFkNameLoader` → Tests run: 1, Failures: 0, Errors: 0
  - 4 单仓库 xmeta `warehouseName` prop 落地：`ErpPurOrderLine/ErpPurReceiveLine/ErpSalOrderLine/ErpSalDeliveryLine.xmeta`（queryable="false" sortable="false" + schema type=java.lang.String）
  - 4 单仓库 Line BizModel `warehouseName` @BizLoader 方法落地（orm().batchLoadProps + 逐行 getWarehouse().getName()）
  - 4 单仓库定制层 view.xml grid 含 `<col id="warehouseName" .../>`（bounded-merge 覆盖 _gen 层 warehouseId 列）
  - ErpMfgWorkOrderLine xmeta 双 prop（sourceWarehouseName + destWarehouseName）+ Line BizModel 双 @BizLoader 方法 + view.xml grid 双 col 落地
- 独立结束审计（closure-audit-2026-07-12，独立子代理新会话）：复核全部 5 行实体 xmeta/BizModel/view.xml 三层落地为实时代码（非签名），逐项核实 BizModel `@BizLoader` 含 `orm().batchLoadProps` + `getWarehouse().getName()` 真实实现（非空体/非 return null 占位）；重跑代表行实体测试复现：purchase `TestErpPurFkNameLoader` Tests run: 5, Failures: 0, Errors: 0；sales `TestErpSalFkNameLoader` Tests run: 5, Failures: 0, Errors: 0；manufacturing `TestErpMfgFkNameLoader` Tests run: 1, Failures: 0, Errors: 0（均 BUILD SUCCESS）；warehouseName 断言对齐 master-data（"原料仓"/"成品仓"）。无 hollow code、无吞异常、无已确认缺陷降级 Deferred。审计结论：approved，计划可关闭。
