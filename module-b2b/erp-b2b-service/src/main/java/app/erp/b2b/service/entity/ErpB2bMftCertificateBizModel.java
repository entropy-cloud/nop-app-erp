...
public class ErpB2bMftCertificateBizModel extends CrudBizModel<ErpB2bMftCertificate>
    ...
    @BizLoader(forType = ErpB2bMftCertificate.class)
    public List<String> partnerName(@ContextSource List<ErpB2bMftCertificate> rows) {
        orm().batchLoadProps(rows, Collections.singleton("partner"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpB2bMftCertificate row : rows) {
    ...
    @BizLoader(forType = ErpB2bMftCertificate.class)
    public List<String> orgName(@ContextSource List<ErpB2bMftCertificate> rows) {
