...
public class ErpMdUoMConversionBizModel extends CrudBizModel<ErpMdUoMConversion> implements IErpMdUoMConversionBiz {
    ...
    @BizLoader(forType = ErpMdUoMConversion.class)
    public List<String> materialName(@ContextSource List<ErpMdUoMConversion> rows) {
        orm().batchLoadProps(rows, Collections.singleton("material"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpMdUoMConversion row : rows) {
    ...
    @BizLoader(forType = ErpMdUoMConversion.class)
    public List<String> fromUoMName(@ContextSource List<ErpMdUoMConversion> rows) {
        orm().batchLoadProps(rows, Collections.singleton("fromUoM"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpMdUoMConversion row : rows) {
            result.add(row.orm_attached() && row.getFromUoM() != null ? row.getFromUoM().getName() : null);
        }
        return result;
    }

    @BizLoader(forType = ErpMdUoMConversion.class)
    public List<String> toUoMName(@ContextSource List<ErpMdUoMConversion> rows) {
        orm().batchLoadProps(rows, Collections.singleton("toUoM"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpMdUoMConversion row : rows) {
            result.add(row.orm_attached() && row.getToUoM() != null ? row.getToUoM().getName() : null);
        }
        return result;
    }
