...
public class ErpMdEmployeeBizModel extends CrudBizModel<ErpMdEmployee> implements IErpMdEmployeeBiz{
    ...
    @BizLoader(forType = ErpMdEmployee.class)
    public List<String> orgName(@ContextSource List<ErpMdEmployee> rows) {
        orm().batchLoadProps(rows, Collections.singleton("organization"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpMdEmployee row : rows) {
    ...
    @BizLoader(forType = ErpMdEmployee.class)
    public List<String> partnerName(@ContextSource List<ErpMdEmployee> rows) {
