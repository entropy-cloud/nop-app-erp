...
public class ErpMdMaterialCategoryBizModel extends CrudBizModel<ErpMdMaterialCategory> implements IErpMdMaterialCategoryBiz{
    ...
    @BizLoader(forType = ErpMdMaterialCategory.class)
    public List<String> parentName(@ContextSource List<ErpMdMaterialCategory> rows) {
        orm().batchLoadProps(rows, Collections.singleton("parent"));
        List<String> result = new ArrayList<>(rows.size());
        for (ErpMdMaterialCategory row : rows) {
