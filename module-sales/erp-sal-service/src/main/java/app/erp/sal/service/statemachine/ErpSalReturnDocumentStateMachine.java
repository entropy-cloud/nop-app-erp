...
/**
 * 销售退货单（{@code ErpSalReturn}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 * {@code ErpSalReturnApprovalStateMachine} 预留命名空间）。
 *
 ...
 * 不编码入边）。
 *
 * <p>接线方 {@code ErpSalReturnCancelProcessor} 将 common 码映射为领域码
 * {@code ERR_RETURN_ILLEGAL_DOC_STATUS_TRANSITION}。
 */
public class ErpSalReturnDocumentStateMachine {
     ...
     * 接线方 {@code ErpSalReturnCancelProcessor} 映射为领域码 {@code ERR_RETURN_ILLEGAL_DOC_STATUS_TRANSITION}。
     */
