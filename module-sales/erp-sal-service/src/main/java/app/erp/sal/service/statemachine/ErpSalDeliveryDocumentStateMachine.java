...
/**
 * 销售出库单（{@code ErpSalDelivery}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 * {@code ErpSalDeliveryApprovalStateMachine} 预留命名空间）。
 *
 * <p>非法边直抛领域码 {@link ErpSalErrors#ERR_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code action}/
 * {@code currentDocStatus}/{@code expectedDocStatus}；plan 2026-09-07-2200-1）；实体元数据（{@code deliveryCode}）
 ...
 * 不编码入边）。领域码 {@code ERR_ILLEGAL_DOC_STATUS_TRANSITION} 为泛型命名漂移，路线图 Non-Goal 不重命名。
 */
public class ErpSalDeliveryDocumentStateMachine {
     ...
     *
     * <p>对 CANCELLED 直抛领域码 {@code ERR_ILLEGAL_DOC_STATUS_TRANSITION}
