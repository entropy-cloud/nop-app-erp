...
/**
 * 销售发票（{@code ErpSalInvoice}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 * {@code ErpSalInvoiceApprovalStateMachine} 预留命名空间）。
 *
 * <p>非法边直抛领域码 {@link ErpSalErrors#ERR_INVOICE_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code action}/
 * {@code currentDocStatus}/{@code expectedDocStatus}；plan 2026-09-07-2200-1）；实体元数据（{@code invoiceCode}）
 ...
 * 不编码入边）。
 */
public class ErpSalInvoiceDocumentStateMachine {
     ...
     *
     * <p>对 CANCELLED 直抛领域码 {@code ERR_INVOICE_ILLEGAL_DOC_STATUS_TRANSITION}
