...
/**
 * 销售发票（{@code ErpSalInvoice}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@code ErpSalInvoiceDocumentStateMachine} docStatus 轴分离）。
 *
 * <p><b>M4 业财过账副作用边界（§11.2 M4）</b>：本 Bean 仅集中固定迁移矩阵。approve 触发的 AR 发票过账
 * （{@code SalInvoicePostingDispatcher}→AR_INVOICE 凭证）+ commitment-release hook 保留在
 * {@code ErpSalInvoiceApproveProcessor} 原位（副作用不入轴，契约 §11.2 M4 (ii)/(iv)）。
 * {@code SalReversalListener} 跨域红冲回写（AR_INVOICE→posted=false + APPROVED→REJECTED）保留原位不改（§11.2 M4 (v)）。
 *
 * <p>非法边直抛领域码 {@link ErpSalErrors#ERR_INVOICE_ILLEGAL_STATUS_TRANSITION}（参数 {@code action}/
 * {@code currentStatus}/{@code expectedStatus}；plan 2026-09-07-2200-1）；实体元数据（{@code invoiceCode}）
 * 经调用点 Processor 同码补参补齐。
 *
 * <p><b>reverseApprove 目标态</b>：实仓核实 {@code ErpSalInvoiceReverseApproveProcessor.reverseApprove} 已设
 * REJECTED（已合规 {@code domain-design-guidelines.md §16.4}）。故本 Bean {@code reverseApproveTargetStatus()}=REJECTED。
 */
public class ErpSalInvoiceApprovalStateMachine {
