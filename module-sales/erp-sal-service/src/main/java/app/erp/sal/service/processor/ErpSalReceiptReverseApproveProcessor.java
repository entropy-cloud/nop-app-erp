...
/**
 * ErpSalReceipt reverseApprove per-mutation Processor (plan 2026-07-30-1433-2 R5.2).
 * reverseApprove 在已过账时红冲收款过账（postingDispatcher.reverse）后 reload 设 REJECTED + 清空审计字段，
 * 需 custom public override（红冲后实体引用变更）。对齐 R5.1 ErpPurReceiveReverseApproveProcessor 模式 B。
 */
public class ErpSalReceiptReverseApproveProcessor extends AbstractReverseApproveProcessor<ErpSalReceipt> {
    ...
    public ErpSalReceipt reverseApprove(String id, IServiceContext context) {
        ...
        setApproveStatus(receipt, ErpSalConstants.APPROVE_STATUS_REJECTED);
