...
/**
 * 收款单（{@code ErpSalReceipt}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code docStatus} 业务生命周期轴）。
 *
 ...
 * {@code ErpSalReceiptApprovalStateMachine} 预留命名空间）。
 *
 * <p>非法边直抛领域码 {@link ErpSalErrors#ERR_RECEIPT_ILLEGAL_DOC_STATUS_TRANSITION}（参数 {@code action}/
 * {@code currentDocStatus}/{@code expectedDocStatus}；plan 2026-09-07-2200-1）；实体元数据（{@code receiptCode}）
 ...
 * 不编码入边）。本 Bean 只管 docStatus 轴；writtenOffStatus（核销轴）/ settlement 编排不受影响。
 */
public class ErpSalReceiptDocumentStateMachine {
     ...
     *
     * <p>对 CANCELLED 直抛领域码 {@code ERR_RECEIPT_ILLEGAL_DOC_STATUS_TRANSITION}
