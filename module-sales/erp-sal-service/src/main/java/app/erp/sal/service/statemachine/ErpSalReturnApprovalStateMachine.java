...
/**
 * 销售退货单（{@code ErpSalReturn}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 ...
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@code ErpSalReturnDocumentStateMachine} docStatus 轴分离）。
 *
 * <p><b>M4 业财过账副作用边界（§11.2 M4）</b>：本 Bean 仅集中固定迁移矩阵。approve 触发的反向入库移动
 * + SALES_RETURN 凭证过账（{@code SalReturnPostingDispatcher}）+ 退款编排保留在 {@code ErpSalReturnApproveProcessor}
 ...
 */
public class ErpSalReturnApprovalStateMachine {
