...
public class ErpMfgJobCardHoldJobProcessor {
    ...
    public ErpMfgJobCard holdJob(String jobCardId, IServiceContext context) {
        ErpMfgJobCard jc = facade.requireJobCard(jobCardId, context);
        String from = jc.getStatus();
        try {
            stateMachine.assertCanHoldJob(from);
        } catch (NopException e) {
            throw facade.illegalTransition(jc, from, "WORK_IN_PROGRESS", e);
        }
        doHoldJob(jc);
        return jc;
    }
    ...
    protected void doHoldJob(ErpMfgJobCard jc) {
        jc.setStatus(stateMachine.holdJobTargetStatus());
