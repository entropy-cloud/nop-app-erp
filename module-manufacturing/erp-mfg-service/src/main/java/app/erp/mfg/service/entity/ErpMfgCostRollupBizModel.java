...
public class ErpMfgCostRollupBizModel extends CrudBizModel<ErpMfgCostRollup> implements IErpMfgCostRollupBiz {
    ...
    @BizLoader(forType = ErpMfgCostRollup.class)
    public List<String> orgName(@ContextSource List<ErpMfgCostRollup> rollups) {
        orm().batchLoadProps(rollups, Collections.singleton("org"));
        List<String> result = new ArrayList<>(rollups.size());
        for (ErpMfgCostRollup rollup : rollups) {
            result.add(rollup.getOrg() != null ? rollup.getOrg().getName() : null);
        }
        return result;
    }
}
