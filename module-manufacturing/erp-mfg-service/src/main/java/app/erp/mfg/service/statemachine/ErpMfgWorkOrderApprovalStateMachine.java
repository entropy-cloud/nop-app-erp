...
/**
 * 生产工单（{@code ErpMfgWorkOrder}）实体级状态机 Bean —— 一 Bean 对应一实体一轴（{@code approveStatus} 审批轴）。
 *
 * <p>权威契约：{@code docs/architecture/entity-state-machine-bean.md}；
 * 业务语义：{@code docs/design/manufacturing/state-machine.md} §适用对象一。
 *
 ...
 *
 * <p>命名带 {@code Approval} 后缀（契约 §1 双轴约定，与 {@code ErpMfgWorkOrderDocumentStateMachine} docStatus 轴分离）。
 *
 ...
 *
 * <p><b>reverseApprove 目标态裁定（plan Phase 1 Decision）</b>：实仓核实 {@code ErpMfgWorkOrderProcessor.doReverseApprove}
 ...
 */
public class ErpMfgWorkOrderApprovalStateMachine {
     ...
     * 接线方 {@code ErpMfgWorkOrderProcessor.validateTransitionForSubmit} 映射为领域码 {@code ERR_INVALID_STATUS_TRANSITION}。
     */
    ...
    /**
     * 业务终态判定。审批轴业务终态为 {@code APPROVED}（owner doc 审核通过）；其为「可逆终态」——经 reverseApprove 有出边，
